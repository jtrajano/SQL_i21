...
FROM	(
			...
			MERGE	
				...
				USING (
					...
					AND inventory_transaction.intCostingMethod IN (@LIFO)-- dbo.fnGetCostingMethod(inventory_transaction.intItemId,inventory_transaction.intItemLocationId) IN (@LIFO)
...
FROM	dbo.tblICInventoryLIFOStorage lifoStorageBucket INNER JOIN (
			...
			ON LIFOOutGrouped.intRevalueLifoId = lifoStorageBucket.intInventoryLIFOStorageId
;
