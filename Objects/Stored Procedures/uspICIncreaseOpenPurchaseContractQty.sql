CREATE PROCEDURE [dbo].[uspICIncreaseOpenPurchaseContractQty]
	@ItemsToIncreaseOpenPurchaseContract AS ItemsOpenPurchaseContractTableType READONLY
AS
...
BEGIN 
	...
	FROM	@ItemsToIncreaseOpenPurchaseContract ItemsToValidate LEFT JOIN dbo.tblICItem Item
...
USING (
		...
		FROM	@ItemsToIncreaseOpenPurchaseContract ib 
				CROSS APPLY (
					SELECT	TOP 1 
							intItemUOMId
							,dblUnitQty 
...
WHEN MATCHED THEN 
	UPDATE 
	SET		dblOpenPurchaseContract = CASE WHEN ISNULL(ItemStock.dblOpenPurchaseContract, 0) + Source_Query.Aggregrate_Qty < 0 THEN 0 ELSE ISNULL(ItemStock.dblOpenPurchaseContract, 0) + Source_Query.Aggregrate_Qty END 
...
WHEN MATCHED THEN 
	UPDATE 
	SET		dblOpenPurchaseContract = CASE WHEN ISNULL(ItemStockUOM.dblOpenPurchaseContract, 0) + Source_Query.Aggregrate_Qty < 0 THEN 0 ELSE ISNULL(ItemStockUOM.dblOpenPurchaseContract, 0) + Source_Query.Aggregrate_Qty END 
...
BEGIN 
	DECLARE		
		...
		,@stockType_OpenPurchaseContract AS INT = 10
	...
	SELECT 
			intItemStockTypeId	= @stockType_OpenPurchaseContract
			,intItemId			= intItemId
			...
			,dblQty				= dblQty
			,intConcurrencyId	= 1
	FROM	@ItemsToIncreaseOpenPurchaseContract cp 
