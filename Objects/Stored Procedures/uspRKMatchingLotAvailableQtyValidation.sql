...
IF(SELECT ABS(dblOpenContract) FROM vyuRKGetMatchingLotAvailableQty where intFutOptTransactionId=@intLFutOptTransactionId and strType = @strType) < @dblMatchQty
BEGIN
	RAISERROR('Selected lots are no longer available for matching.',16,1)
END
ELSE IF(SELECT ABS(dblOpenContract) FROM vyuRKGetMatchingLotAvailableQty where intFutOptTransactionId=@intSFutOptTransactionId and strType = @strType) < @dblMatchQty
BEGIN
