...
BEGIN TRY
	...
	SELECT DISTINCT intParentDetailId
	FROM tblCTContractDetail
	WHERE intContractHeaderId = @intContractHeaderId
		AND ysnSlice = 1

	DECLARE @intMinDetailRecordId INT

	SELECT @intMinDetailRecordId = MIN(intDetailRecordId)
	FROM @ContractDetail

	WHILE (@intMinDetailRecordId > 0)
	BEGIN
		SET @intParentContractDetailId = NULL

		-- Parent contract sequence. ie., from which sequence it sliced
		SELECT @intParentContractDetailId = intContractDetailId
		FROM @ContractDetail
		WHERE intDetailRecordId = @intMinDetailRecordId
		...
		BEGIN
			...
			BEGIN
				...
				WHERE LD.intLoadDetailId = @intOrgLoadDetaild
				...
				BEGIN
					...
					EXEC uspLGLoadContractCopy @intOldLoadDetailId = @intOrgLoadDetaild
