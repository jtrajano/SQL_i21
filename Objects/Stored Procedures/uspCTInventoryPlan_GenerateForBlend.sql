CREATE PROCEDURE [dbo].[uspCTInventoryPlan_GenerateForBlend] @intInvPlngReportMasterID INT
...
BEGIN TRY
	...
	BEGIN
		...
		IF OBJECT_ID('tempdb.dbo.#ExistingPurchases') IS NOT NULL
			DROP TABLE #ExistingPurchases
		...
		IF OBJECT_ID('tempdb.dbo.#IntransitPurchases') IS NOT NULL
			DROP TABLE #IntransitPurchases
						...
						BEGIN
							...
							SET @SQL_ExistingPurchases_Exec_Ext = 'SELECT @ExistingPurchases = ' + CAST((
										...
										WHERE RI.intItemId = @intItemId
											...
											AND right(convert(CHAR(12), EP.dtmUpdatedAvailabilityDate, 107), 4) = right(convert(CHAR(12), DATEADD(m, (@Cnt - 1), GETDATE()), 107), 4)
										) AS NVARCHAR(50)) + ' '
							...
							SET @SQL_IntransitPurchases_Exec_Ext = 'SELECT @IntransitPurchases = ' + CAST((
										...
										WHERE RI.intItemId = @intItemId
											...
											AND right(convert(CHAR(12), IP.dtmUpdatedAvailabilityDate, 107), 4) = right(convert(CHAR(12), DATEADD(m, (@Cnt - 1), GETDATE()), 107), 4)
										) AS NVARCHAR(50)) + ' '
