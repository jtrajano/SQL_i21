...
SELECT CONVERT(INT,ROW_NUMBER() OVER (ORDER BY intFutureMonthId)) AS intRow
	, *
FROM (
	SELECT TOP 100 PERCENT LEFT(strOptionMonth, 3) COLLATE Latin1_General_CI_AS strOptionMonth
		...
		, op.intFutureMonthId
		, strOptionMonth as strOptionMonthYearWOSymbol
		, op.intFutureMarketId
		, ysnMonthExpired
	...
	ORDER BY CONVERT(DATETIME,'01 '+strOptionMonth) ASC
) t