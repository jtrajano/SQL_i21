...
WITH Pricing AS (
	SELECT  c.strCommodityCode
		...
		, mo.intFutureMonthId
		, strFutureMonth
		, fm.intUnitMeasureId intFutMarketUOM
		, dtmFutureMonthsDate
		, ysnExpired
		, strSymbol
		, dblContractSize
	...
	JOIN vyuCTEntity EY	ON EY.intEntityId = ch.intEntityId AND 1 = (CASE WHEN ch.intContractTypeId = 1 AND EY.strEntityType = 'Vendor' THEN 1
...
SELECT * FROM (
	SELECT strCommodityCode
		...
		, intFutureMonthId
		, strFutureMonth
		, intUnitMeasureId intFutMarketUOM
		, dtmFutureMonthsDate
		, ysnExpired
		, strSymbol
		, dblContractSize
		, intItemUOMId
		, ((dblLotsFixed)* case when isnull(dblYield,0)=0 then 1 else dblYield/100 end) dblNoOfLots
		, isnull(ysnDeltaHedge,0) ysnDeltaHedge
		, strItemOrigin
		, strItemDescription
		, intMultiCompanyId
		, strCompanyName
		, strShipmentPeriod = strShipmentPeriod COLLATE Latin1_General_CI_AS
	FROM Pricing WHERE intPricingTypeId = 1
		
	UNION ALL SELECT c.strCommodityCode
		, c.intCommodityId
		, ch.intContractHeaderId
		...
		, IM.intProductLineId
		, strProductLine
		, PRC.intFutureMonthId
		, strFutureMonth
		, PRC.intUnitMeasureId intFutMarketUOM
		, dtmFutureMonthsDate
		, ysnExpired
		, strSymbol
		, dblContractSize
		, PRC.intItemUOMId
		, ((PRC.dblNoOfLots-PRC.dblLotsFixed)* case when isnull(PRC.dblYield,0)=0 then 1 else PRC.dblYield/100 end) dblNoOfLots
		, isnull(ysnDeltaHedge,0) ysnDeltaHedge
		, strItemOrigin
		, strItemDescription
		, intMultiCompanyId
	...
	UNION ALL SELECT c.strCommodityCode
		...
		, dblContractSize
		, intItemUOMId
		, ((dbo.fnCTConvertQtyToTargetCommodityUOM(ch.intCommodityId, CDT.intUnitMeasureId, fm.intUnitMeasureId, (CDT.dblBalance))/fm.dblContractSize) * case when isnull(dblYield,0)=0 then 1 else dblYield/100 end)
		, isnull(ysnDeltaHedge,0) ysnDeltaHedge
		, CA.strDescription AS strItemOrigin
		, IM.strDescription strItemDescription
		, intMultiCompanyId
	...
	join tblRKFuturesMonth mo on mo.intFutureMonthId=CDT.intFutureMonthId
	LEFT JOIN tblCTBook b on b.intBookId=case when isnull(CDT.intBookId,0)=0 then (SELECT DISTINCT b.intBookId
																				from tblCTBookVsEntity e
																				join tblCTContractHeader ch on ch.intEntityId=e.intEntityId
																				join tblCTBook b on b.intBookId=ch.intBookId) else CDT.intBookId end
	LEFT JOIN tblSMMultiCompany comp on comp.intMultiCompanyId=ch.intCompanyId
	LEFT JOIN tblICCommodityAttribute CA ON CA.intCommodityAttributeId = IM.intOriginId
	LEFT JOIN tblICCommodityAttribute pty on IM.intProductTypeId=pty.intCommodityAttributeId and pty.strType='ProductType'
	LEFT JOIN tblICCommodityProductLine ptl on IM.intProductLineId=ptl.intCommodityProductLineId
	WHERE CDT.intContractDetailId not in(select intContractDetailId from Pricing) and dblBalance <> 0
) t