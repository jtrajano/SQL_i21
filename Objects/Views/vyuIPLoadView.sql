...
SELECT -- Load Header
	...
	,strShipmentStatus = CASE L.intShipmentStatus
		...
		WHEN 3
			THEN CASE 
					WHEN (L.ysnCustomsReleased = 1)
						THEN 'Customs Released'
					WHEN (L.ysnDocumentsApproved = 1)
		...
		WHEN 7
			THEN CASE 
					WHEN (ISNULL(L.strBookingReference, '') <> '')
						THEN 'Booked'
					ELSE 'Shipping Instruction Created'
					END
		WHEN 8
		...
		ELSE ''
		END COLLATE Latin1_General_CI_AS
	,strCalenderInfo = L.[strLoadNumber] + CASE L.intTransportationMode
		WHEN 1
			THEN '(T)'
		WHEN 2
			THEN '(V)'
		...
		ELSE ''
		END + CASE 
		WHEN ISNULL(L.strExternalLoadNumber, '') <> ''
...
OUTER APPLY (
	SELECT TOP 1 strOwner
	FROM tblLGShippingLineServiceContractDetail SLSCD
	...
	WHERE SLSC.intEntityId = L.intShippingLineEntityId
		AND SLSCD.strServiceContractNumber = L.strServiceContractNumber
	) SLSC
