...
StartTest (function (t) {
    ...
    engine.start(t)
        ...
        .openScreen('Patronage Category').wait(200)
        .checkScreenShown ('patronagecategory').wait(200)
        .checkScreenWindow({alias: 'patronagecategory', title: 'Patronage Category', collapse: true, maximize: true, minimize: false, restore: false, close: true }).wait(100)
        .checkToolbarButton({new: false, save: true, search: false, delete: false, undo: true, close: true}).wait(100)
        /*Control checking*/
        .checkControlVisible(['#btnDelete','#btnInsertCriteria','#txtFilterGrid'],true)
        .checkControlVisible(['#colCategoryCode','#colDescription','#colPurchaseSale','#colUnitAmount'], true)
