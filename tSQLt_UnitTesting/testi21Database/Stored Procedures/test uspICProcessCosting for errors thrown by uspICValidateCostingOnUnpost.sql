...
CREATE PROCEDURE [testi21Database].[test uspICProcessCosting for errors thrown by uspICValidateCostingOnUnpost]
AS
BEGIN
	-- Arrange 
	BEGIN 
		...
		SET @errorCommand = 'RAISERROR(''Error raised inside uspICValidateCostingOnUnpost'', 16, 1);';

		EXEC tSQLt.SpyProcedure 'dbo.uspICValidateCostingOnPost';
		EXEC tSQLt.SpyProcedure 'dbo.uspICPostCosting';
		EXEC tSQLt.SpyProcedure 'dbo.uspICValidateCostingOnUnpost', @errorCommand;
