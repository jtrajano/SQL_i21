CREATE PROCEDURE [testi21Database].[test uspICIncreaseStockInLIFO from negative to positive (with partial in)]
AS
BEGIN
	-- Arrange 
	BEGIN 
	...
	BEGIN 
		...
		-- Repeat call on uspICReduceStockInLIFO until @dblQty is completely distributed to all the available LIFO buckets
		...
		BEGIN 		
			...
			BEGIN 
				...
				EXEC tSQLt.AssertEquals 4, @UpdatedLIFOId
			END
		...
		-- Assert the new LIFO id is 5
		EXEC tSQLt.AssertEquals 5, @NewLIFOId
