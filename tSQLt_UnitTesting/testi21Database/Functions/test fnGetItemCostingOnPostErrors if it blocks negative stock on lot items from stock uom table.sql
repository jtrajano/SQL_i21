CREATE PROCEDURE testi21Database.[test fnGetItemCostingOnPostErrors if it blocks negative stock on lot items from stock uom table]
AS 
BEGIN
	-- Arrange
	BEGIN 
		...
		-- Flag all items not to accept negative stocks 
		UPDATE	dbo.tblICItemLocation
		SET		intAllowNegativeInventory = 3 -- Value 3 means "NO", Negative stock is NOT allowed. 

		-- Setup stock quantities in the stock uom table. 
		UPDATE dbo.tblICItemStockUOM
		SET dblOnHand = 99
	...
	BEGIN 
		INSERT INTO actual	
		SELECT * FROM dbo.fnGetItemCostingOnPostErrors(@ManualLotGrains, @ManualLotGrains_DefaultLocation, @ManualLotGrains_BushelUOMId, @SubLocation, @StorageLocation, @dblQty, @LotId)
	END
