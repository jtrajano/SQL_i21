CREATE PROCEDURE testi21Database.[test fnGetItemCostingOnUnpostErrors if it allows negative stock on lot items from stock uom table]
AS 
BEGIN
	-- Arrange
	BEGIN 
	...
	BEGIN 
		INSERT INTO actual	
		SELECT * FROM dbo.fnGetItemCostingOnUnpostErrors(@ManualLotGrains, @ManualLotGrains_DefaultLocation, @ManualLotGrains_BushelUOMId, @SubLocation, @StorageLocation, @dblQty, @LotId)
	END
