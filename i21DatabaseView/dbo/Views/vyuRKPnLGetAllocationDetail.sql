...
SELECT dv.strSequenceNumber
	...
	, dv.intContractTypeId
	, dv.intContractHeaderId intContractHeaderId
	, dv.dblBasis
	, dv.intItemUOMId intPItemUOMId
...
UNION ALL SELECT dvp.strSequenceNumber
	, dvp.intItemId
	, dvp.strItemNo
	, dvp.strEntityName
	, isnull(dv.dblDetailQuantity,0) as dblDetailQuantity
	...
	, dv.dblBasis dblSaleBasis
	, isnull(ad.dblSAllocatedQty,0) dblAllocatedQty
	, (SELECT DISTINCT SUM(ID.dblQtyShipped) FROM tblARInvoiceDetail ID WHERE ID.intContractDetailId =ad.intSContractDetailId) dblInvoiceQty
	, (SELECT sum(isnull(pc.dblAmount,0)) FROM vyuCTContractCostEnquiryCost pc WHERE pc.intContractDetailId=ad.intPContractDetailId) dblPurchaseCost
	, (SELECT sum(isnull(sc.dblAmount,0)) FROM vyuCTContractCostEnquiryCost sc WHERE sc.intContractDetailId=ad.intSContractDetailId) dblSaleCost
	, 0.0 as dblActualProfit
	, dv.intContractDetailId
	, ad.intPContractDetailId
	, dvp.intContractTypeId
	, dvp.intContractHeaderId intContractHeaderId
	, dvp.dblBasis
	, dvp.intItemUOMId intPItemUOMId
...
UNION ALL SELECT dv.strSequenceNumber
	, dv.intItemId
	, dv.strItemNo
	, dv.strEntityName
	, isnull(dv.dblDetailQuantity,0) as dblDetailQuantity
...
UNION ALL SELECT dv.strSequenceNumber
	...
	, (SELECT DISTINCT sum(ID.dblQtyShipped) FROM tblARInvoiceDetail ID where ID.intContractDetailId =LD.intSContractDetailId) dblInvoiceQty
