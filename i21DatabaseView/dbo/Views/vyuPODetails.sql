...
INNER JOIN (tblAPVendor C INNER JOIN tblEMEntity C2 ON C.[intEntityId] = C2.intEntityId) 
	ON A.[intEntityVendorId] = C.[intEntityId]
LEFT JOIN tblICItem D 
	ON B.intItemId = D.intItemId
LEFT JOIN tblICItemUOM E 
	ON B.intUnitOfMeasureId = E.intItemUOMId
LEFT JOIN tblICUnitMeasure H 
	ON E.intUnitMeasureId = H.intUnitMeasureId
LEFT JOIN tblSMCompanyLocationSubLocation F 
	ON B.intSubLocationId = F.intCompanyLocationSubLocationId
LEFT JOIN tblICStorageLocation G 
	ON B.intStorageLocationId = G.intStorageLocationId
INNER JOIN dbo.tblSMCompanyLocation I 
	ON A.intShipToId = I.intCompanyLocationId
LEFT JOIN [tblEMEntityLocation] J 
	ON A.intEntityVendorId = J.intEntityId AND J.intEntityLocationId = A.intShipFromId --Add Filter to avoid multuple PO on add Order
 --WHERE D.strType NOT IN ('Service','Software','Non-Inventory','Other Charge')
LEFT JOIN tblSMCurrencyExchangeRateType K 
	ON B.intForexRateTypeId = K.intCurrencyExchangeRateTypeId
LEFT JOIN tblSMCurrency L 
	ON B.intCurrencyId = L.intCurrencyID
LEFT JOIN tblCTContractDetail M 
	ON B.intContractDetailId = M.intContractDetailId
LEFT JOIN tblCTContractHeader N 
