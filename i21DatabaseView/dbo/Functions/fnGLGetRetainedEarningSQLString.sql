...
(
	...
	SUM(Debit.Unit) dblDebitUnit,
	SUM(Credit.Unit) dblCreditUnit,
	...
	OUTER APPLY (select top 1 intAccountId, strAccountId,strDescription FROM tblGLFiscalYear F join tblGLAccount A on F.intRetainAccount = A.intAccountId) Account
	OUTER APPLY (SELECT dblLbsPerUnit,[strUOMCode] FROM Units WHERE [intAccountId] = Account.intAccountId) U
