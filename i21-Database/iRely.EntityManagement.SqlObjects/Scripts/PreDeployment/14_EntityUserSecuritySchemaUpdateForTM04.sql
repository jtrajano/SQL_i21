...
BEGIN
	...
	BEGIN
		...
		set @cmdevent = 'select @maxevent = max(intSiteID) from tblTMSite'
		...
		begin
			...
			BEGIN				
				...
				SET @cmdevent = ''UPDATE tblTMSite SET intUserID = A.intEntityUserSecurityId
					FROM tblSMUserSecurity A
					WHERE tblTMSite.intUserID = A.intUserSecurityIdOld
						AND A.intUserSecurityIdOld IS NOT NULL 
						AND intSiteID > '' + cast(@minId as nvarchar) + '' and intSiteID <= '' + cast((@minId + @COUNTS ) as nvarchar)
