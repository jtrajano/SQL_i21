...
BEGIN
	...
	BEGIN
		...
		IF NOT EXISTS (SELECT TOP 1 1 FROM [dbo].[tblDBUserPreference] WHERE [intEntityUserSecurityId] = @intUserSecurityId)			
			BEGIN

				INSERT INTO [dbo].[tblDBUserPreference] 
					   ([intAutoRefreshMinute], [intColumns], [intColumnWidth1], [intColumnWidth2], [intColumnWidth3], [intColumnWidth4], [intDefaultTabId], [intEntityUserSecurityId], [ysnAutoPanelWidth], [ysnAutoRefresh], [ysnColumnFiltering], [ysnColumnMoving], [ysnColumnResizing], [ysnColumnSorting], [ysnExportAll], [ysnPrintAll], [ysnRefreshTab], [ysnSaveGridLayout])
