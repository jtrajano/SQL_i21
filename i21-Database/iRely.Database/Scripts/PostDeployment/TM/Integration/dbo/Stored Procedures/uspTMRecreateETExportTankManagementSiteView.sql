GO
	PRINT 'START OF CREATING [uspTMRecreateETExportTankManagementSiteView] SP'
GO

IF EXISTS (SELECT TOP 1 1 FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[uspTMRecreateETExportTankManagementSiteView]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].uspTMRecreateETExportTankManagementSiteView
GO

CREATE PROCEDURE uspTMRecreateETExportTankManagementSiteView 
AS
BEGIN
	IF EXISTS(select top 1 1 from INFORMATION_SCHEMA.VIEWS where TABLE_NAME = 'vyuTMETExportTankManagementSite')
	BEGIN
		DROP VIEW vyuTMETExportTankManagementSite
	END
...
GO
	PRINT 'END OF CREATING [uspTMRecreateETExportTankManagementSiteView] SP'
GO
	
GO
	PRINT 'BEGIN OF EXECUTE uspTMRecreateETExportTankManagementSiteView'
GO 
	EXEC ('uspTMRecreateETExportTankManagementSiteView')
GO 
	PRINT 'END OF EXECUTE uspTMRecreateETExportTankManagementSiteView'
GO
