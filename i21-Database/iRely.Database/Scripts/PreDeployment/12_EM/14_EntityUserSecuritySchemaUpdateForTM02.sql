...
BEGIN
	print 'updating tblTMDeliveryHistory '
	set @eventcount = 100000
	...
	BEGIN
		...
		set @cmdevent = 'select @maxevent = max(intDeliveryHistoryID) from tblTMDeliveryHistory'
		...
		WHERE U.TABLE_NAME = 'tblTMDeliveryHistory' 
		...
		begin
			...
			BEGIN				
				...
				SET @cmdevent = ''UPDATE tblTMDeliveryHistory SET intUserID = A.intEntityUserSecurityId
					...
					WHERE tblTMDeliveryHistory.intUserID = A.intUserSecurityIdOld
						...
						AND intDeliveryHistoryID > '' + cast(@minId as nvarchar) + '' and intDeliveryHistoryID <= '' + cast((@minId + @COUNTS ) as nvarchar)
