CREATE PROCEDURE uspWHMoveContainerForPutBack
...
BEGIN TRY
	...
	DECLARE @intContainerStorageLocationId INT
	...
	SELECT @intContainerStorageLocationId = intStorageLocationId FROM tblWHContainer WHERE intContainerId = @intContainerId

	IF EXISTS(SELECT * FROM tblWHTask WHERE intFromContainerId = @intContainerId)
		BEGIN
			SELECT @intAssigneeId = intAssigneeId, @intAssignedTaskId = intTaskId FROM tblWHTask WHERE intFromContainerId = @intContainerId
			IF ISNULL(@intAssigneeId,0) <> 0 
			BEGIN
				UPDATE tblWHTask
					SET intTaskStateId = 2
				WHERE intFromContainerId = @intContainerId
			END
		
				UPDATE tblWHTask
					SET intFromStorageLocationId = @intContainerStorageLocationId
				WHERE intFromContainerId = @intContainerId
		END                             
