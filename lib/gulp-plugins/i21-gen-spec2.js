...
const PLUGIN_NAME = 'i21-gen-spec2';
...
function generateSpecs(config) {
    return es.map(function (file, cb) {
        ...
        switch(specType) {
            case "model":
                spec = generateModelSpec(className, properties);
                break;
            case "store":
                spec = generateStoreSpec(className, properties);
                break;
...
function generateModelSpec(className, properties) {
    ...
    var spec = "Inventory.TestUtils.testModel({name: '".concat(className)
        .concat("', base: '").concat(base).concat("',")
        .concat(!_.isUndefined(idProperty) && !_.isNull(idProperty) ? "idProperty: '" + idProperty + "'," : "")
        .concat("dependencies: [").concat(dependencies)
        .concat("], fields: ").concat(JSON.stringify(fields))
        .concat(", validators: [").concat(JSON.stringify(validators))
        .concat("]});");
    return spec;
}

function generateStoreSpec(className, properties) {
