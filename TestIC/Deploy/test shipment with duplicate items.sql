if exists (select * from sys.procedures where object_id = object_id('testIC.test shipment with duplicate items'))
	drop procedure [testIC].[test shipment with duplicate items];
GO
CREATE PROCEDURE testIC.[test shipment with duplicate items]
AS
BEGIN
	-- Fake data
	BEGIN 
