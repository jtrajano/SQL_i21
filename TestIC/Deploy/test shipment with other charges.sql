if exists (select * from sys.procedures where object_id = object_id('testIC.test shipment with other charges'))
	drop procedure [testIC].[test shipment with other charges];
GO
CREATE PROCEDURE testIC.[test shipment with other charges]
AS
BEGIN
	-- Fake data
	BEGIN 
		EXEC [testIC].[Fake IC Starting Numbers]; 
		EXEC [testIC].[Fake Vendor];
		EXEC [testIC].[Fake Shipment]; 
	END 
