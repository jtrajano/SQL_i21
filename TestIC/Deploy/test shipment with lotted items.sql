if exists (select * from sys.procedures where object_id = object_id('testIC.test shipment with lotted items'))
	drop procedure [testIC].[test shipment with lotted items];
GO
