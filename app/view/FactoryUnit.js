/*
 * File: app/view/FactoryUnit.js
 *
 * This file was generated by Sencha Architect version 3.1.0.
 * http://www.sencha.com/products/architect/
 *
 * This file requires use of the Ext JS 5.0.x library, under independent license.
 * License of Sencha Architect does not include license for Ext JS 5.0.x. For more
 * details see http://www.sencha.com/license or contact license@sencha.com.
 *
 * This file will be auto-generated each and everytime you save your project.
 *
 * Do NOT hand edit this file.
 */

Ext.define('Inventory.view.FactoryUnit', {
    extend: 'Ext.window.Window',
    alias: 'widget.factoryunit',

    requires: [
        'Inventory.view.FactoryUnitViewModel',
        'Inventory.view.Filter',
        'Inventory.view.StatusbarPaging',
        'Ext.form.Panel',
        'Ext.toolbar.Separator',
        'Ext.tab.Panel',
        'Ext.tab.Tab',
        'Ext.form.field.ComboBox',
        'Ext.form.field.Number',
        'Ext.form.field.Checkbox',
        'Ext.grid.Panel',
        'Ext.grid.column.Check',
        'Ext.grid.View',
        'Ext.toolbar.Paging'
    ],

    viewModel: {
        type: 'factoryunit'
    },
    height: 611,
    hidden: false,
    minHeight: 525,
    minWidth: 425,
    width: 943,
    layout: 'fit',
    collapsible: true,
    iconCls: 'small-icon-i21',
    title: 'Factory Unit',
    maximizable: true,

    items: [
        {
            xtype: 'form',
            autoShow: true,
            height: 350,
            itemId: 'frmFactoryUnit',
            margin: -1,
            width: 450,
            bodyBorder: false,
            bodyPadding: 5,
            header: false,
            trackResetOnLoad: true,
            layout: {
                type: 'vbox',
                align: 'stretch'
            },
            dockedItems: [
                {
                    xtype: 'toolbar',
                    dock: 'top',
                    width: 588,
                    layout: {
                        type: 'hbox',
                        padding: '0 0 0 1'
                    },
                    items: [
                        {
                            xtype: 'button',
                            tabIndex: -1,
                            height: 57,
                            itemId: 'btnNew',
                            width: 45,
                            iconAlign: 'top',
                            iconCls: 'large-new',
                            scale: 'large',
                            text: 'New'
                        },
                        {
                            xtype: 'button',
                            tabIndex: -1,
                            height: 57,
                            itemId: 'btnSave',
                            width: 45,
                            iconAlign: 'top',
                            iconCls: 'large-save',
                            scale: 'large',
                            text: 'Save'
                        },
                        {
                            xtype: 'button',
                            tabIndex: -1,
                            height: 57,
                            itemId: 'btnSearch',
                            width: 45,
                            iconAlign: 'top',
                            iconCls: 'large-search',
                            scale: 'large',
                            text: 'Search'
                        },
                        {
                            xtype: 'button',
                            tabIndex: -1,
                            height: 57,
                            itemId: 'btnDelete',
                            width: 45,
                            iconAlign: 'top',
                            iconCls: 'large-delete',
                            scale: 'large',
                            text: 'Delete'
                        },
                        {
                            xtype: 'button',
                            tabIndex: -1,
                            height: 57,
                            itemId: 'btnUndo',
                            width: 45,
                            iconAlign: 'top',
                            iconCls: 'large-undo',
                            scale: 'large',
                            text: 'Undo'
                        },
                        {
                            xtype: 'tbseparator',
                            height: 30
                        },
                        {
                            xtype: 'button',
                            tabIndex: -1,
                            height: 57,
                            itemId: 'btnClose',
                            width: 45,
                            iconAlign: 'top',
                            iconCls: 'large-close',
                            scale: 'large',
                            text: 'Close'
                        }
                    ]
                },
                {
                    xtype: 'ipagingstatusbar',
                    itemId: 'pagingtoolbar',
                    flex: 1,
                    dock: 'bottom'
                }
            ],
            items: [
                {
                    xtype: 'tabpanel',
                    flex: 1,
                    itemId: 'tabFactoryUnit',
                    activeTab: 0,
                    plain: true,
                    items: [
                        {
                            xtype: 'panel',
                            bodyPadding: 10,
                            title: 'Unit Detail',
                            layout: {
                                type: 'hbox',
                                align: 'stretch'
                            },
                            items: [
                                {
                                    xtype: 'container',
                                    flex: 1,
                                    margin: '0 5 0 0',
                                    layout: {
                                        type: 'vbox',
                                        align: 'stretch'
                                    },
                                    items: [
                                        {
                                            xtype: 'textfield',
                                            itemId: 'txtName',
                                            fieldLabel: 'Name',
                                            labelWidth: 110
                                        },
                                        {
                                            xtype: 'textfield',
                                            itemId: 'txtDescription',
                                            fieldLabel: 'Description',
                                            labelWidth: 110
                                        },
                                        {
                                            xtype: 'combobox',
                                            itemId: 'cboUnitType',
                                            fieldLabel: 'Unit Type',
                                            labelWidth: 110
                                        },
                                        {
                                            xtype: 'combobox',
                                            itemId: 'cboLocation',
                                            fieldLabel: 'Location',
                                            labelWidth: 110
                                        },
                                        {
                                            xtype: 'combobox',
                                            itemId: 'cboParentUnit',
                                            fieldLabel: 'Parent Unit',
                                            labelWidth: 110
                                        },
                                        {
                                            xtype: 'combobox',
                                            itemId: 'cboRestrictionType',
                                            fieldLabel: 'Restriction Type',
                                            labelWidth: 110
                                        },
                                        {
                                            xtype: 'textfield',
                                            itemId: 'txtAsile',
                                            fieldLabel: 'Asile',
                                            labelWidth: 110
                                        },
                                        {
                                            xtype: 'numberfield',
                                            itemId: 'txtMinBatchSize',
                                            fieldLabel: 'Min Batch Size',
                                            labelWidth: 110,
                                            hideTrigger: true
                                        },
                                        {
                                            xtype: 'numberfield',
                                            itemId: 'txtBatchSize',
                                            fieldLabel: 'Batch Size',
                                            labelWidth: 110,
                                            hideTrigger: true
                                        },
                                        {
                                            xtype: 'combobox',
                                            itemId: 'cboBatchSizeUom',
                                            fieldLabel: 'Batch Size UOM',
                                            labelWidth: 110
                                        }
                                    ]
                                },
                                {
                                    xtype: 'container',
                                    flex: 1,
                                    margin: '0 0 0 10',
                                    layout: {
                                        type: 'vbox',
                                        align: 'stretch'
                                    },
                                    items: [
                                        {
                                            xtype: 'checkboxfield',
                                            itemId: 'chkAllowConsume',
                                            fieldLabel: 'Allow Consume',
                                            labelWidth: 180
                                        },
                                        {
                                            xtype: 'checkboxfield',
                                            itemId: 'chkAllowMultipleItems',
                                            fieldLabel: 'Allow Multiple Items',
                                            labelWidth: 180
                                        },
                                        {
                                            xtype: 'checkboxfield',
                                            itemId: 'chkAllowMultipleLots',
                                            fieldLabel: 'Allow Multiple Lots',
                                            labelWidth: 180
                                        },
                                        {
                                            xtype: 'checkboxfield',
                                            itemId: 'chkMergeOnMove',
                                            fieldLabel: 'Merge on Move',
                                            labelWidth: 180
                                        },
                                        {
                                            xtype: 'checkboxfield',
                                            itemId: 'cboCycleCounted',
                                            fieldLabel: 'Cycle Counted',
                                            labelWidth: 180
                                        },
                                        {
                                            xtype: 'checkboxfield',
                                            itemId: 'chkDefaultWarehouseStagingUnit',
                                            fieldLabel: 'Default Warehouse Staging Unit',
                                            labelWidth: 180
                                        }
                                    ]
                                }
                            ]
                        },
                        {
                            xtype: 'panel',
                            layout: 'fit',
                            title: 'Measurement',
                            items: [
                                {
                                    xtype: 'gridpanel',
                                    itemId: 'grdMeasurement',
                                    margin: -1,
                                    dockedItems: [
                                        {
                                            xtype: 'toolbar',
                                            dock: 'top',
                                            itemId: 'tlbGridOptions',
                                            layout: {
                                                type: 'hbox',
                                                padding: '0 0 0 1'
                                            },
                                            items: [
                                                {
                                                    xtype: 'button',
                                                    tabIndex: -1,
                                                    itemId: 'btnAddMeasurement',
                                                    iconCls: 'small-add',
                                                    text: 'Add Detail'
                                                },
                                                {
                                                    xtype: 'button',
                                                    tabIndex: -1,
                                                    itemId: 'btnDeleteMeasurement',
                                                    iconCls: 'small-delete',
                                                    text: 'Delete'
                                                },
                                                {
                                                    xtype: 'tbseparator'
                                                },
                                                {
                                                    xtype: 'filter'
                                                }
                                            ]
                                        }
                                    ],
                                    columns: [
                                        {
                                            xtype: 'gridcolumn',
                                            dataIndex: 'string',
                                            text: 'Measurement Name',
                                            flex: 1
                                        },
                                        {
                                            xtype: 'gridcolumn',
                                            dataIndex: 'string',
                                            text: 'Reading Point',
                                            flex: 1
                                        },
                                        {
                                            xtype: 'checkcolumn',
                                            text: 'Is Selected'
                                        }
                                    ],
                                    viewConfig: {
                                        itemId: 'grvMeasurement'
                                    }
                                }
                            ]
                        },
                        {
                            xtype: 'panel',
                            layout: 'fit',
                            title: 'Packing Type',
                            items: [
                                {
                                    xtype: 'gridpanel',
                                    itemId: 'grdPackingType',
                                    margin: -1,
                                    dockedItems: [
                                        {
                                            xtype: 'toolbar',
                                            dock: 'top',
                                            itemId: 'tlbGridOptions',
                                            layout: {
                                                type: 'hbox',
                                                padding: '0 0 0 1'
                                            },
                                            items: [
                                                {
                                                    xtype: 'button',
                                                    tabIndex: -1,
                                                    itemId: 'btnDeletePackingType',
                                                    iconCls: 'small-delete',
                                                    text: 'Delete'
                                                },
                                                {
                                                    xtype: 'tbseparator'
                                                },
                                                {
                                                    xtype: 'filter'
                                                }
                                            ]
                                        }
                                    ],
                                    columns: [
                                        {
                                            xtype: 'gridcolumn',
                                            dataIndex: 'string',
                                            text: 'PackType Name',
                                            flex: 1
                                        },
                                        {
                                            xtype: 'gridcolumn',
                                            dataIndex: 'string',
                                            text: 'Description',
                                            flex: 1
                                        },
                                        {
                                            xtype: 'gridcolumn',
                                            dataIndex: 'string',
                                            text: 'Machine Capacity',
                                            flex: 1
                                        },
                                        {
                                            xtype: 'gridcolumn',
                                            dataIndex: 'string',
                                            text: 'Machine Capacity UOM',
                                            flex: 1
                                        },
                                        {
                                            xtype: 'gridcolumn',
                                            dataIndex: 'string',
                                            text: 'Capacity Rate UOM',
                                            flex: 1
                                        }
                                    ],
                                    viewConfig: {
                                        itemId: 'grvPackingType'
                                    }
                                }
                            ]
                        },
                        {
                            xtype: 'panel',
                            layout: 'fit',
                            title: 'Material Type Allowed',
                            items: [
                                {
                                    xtype: 'gridpanel',
                                    itemId: 'grdMaterialTypeAllowed',
                                    margin: -1,
                                    dockedItems: [
                                        {
                                            xtype: 'toolbar',
                                            dock: 'top',
                                            itemId: 'tlbGridOptions',
                                            layout: {
                                                type: 'hbox',
                                                padding: '0 0 0 1'
                                            },
                                            items: [
                                                {
                                                    xtype: 'button',
                                                    tabIndex: -1,
                                                    itemId: 'btnDeleteMaterialTypeAllowed',
                                                    iconCls: 'small-delete',
                                                    text: 'Delete'
                                                },
                                                {
                                                    xtype: 'tbseparator'
                                                },
                                                {
                                                    xtype: 'filter'
                                                }
                                            ]
                                        }
                                    ],
                                    columns: [
                                        {
                                            xtype: 'gridcolumn',
                                            dataIndex: 'string',
                                            text: 'Material Type',
                                            flex: 1
                                        }
                                    ],
                                    viewConfig: {
                                        itemId: 'grvMaterialTypeAllowed'
                                    }
                                }
                            ]
                        },
                        {
                            xtype: 'panel',
                            title: 'Position',
                            layout: {
                                type: 'hbox',
                                align: 'stretch',
                                padding: 10
                            },
                            items: [
                                {
                                    xtype: 'container',
                                    flex: 1,
                                    layout: {
                                        type: 'vbox',
                                        align: 'stretch'
                                    },
                                    items: [
                                        {
                                            xtype: 'textfield',
                                            itemId: 'txtSequence',
                                            fieldLabel: 'Sequence'
                                        },
                                        {
                                            xtype: 'checkboxfield',
                                            itemId: 'chkActive',
                                            fieldLabel: 'Active'
                                        },
                                        {
                                            xtype: 'numberfield',
                                            itemId: 'txtXPosition',
                                            fieldLabel: 'X Position',
                                            hideTrigger: true
                                        },
                                        {
                                            xtype: 'numberfield',
                                            itemId: 'txtYPosition',
                                            fieldLabel: 'Y Position',
                                            hideTrigger: true
                                        },
                                        {
                                            xtype: 'numberfield',
                                            itemId: 'txtZPosition',
                                            fieldLabel: 'Z Position',
                                            hideTrigger: true
                                        }
                                    ]
                                },
                                {
                                    xtype: 'container',
                                    flex: 2
                                }
                            ]
                        },
                        {
                            xtype: 'panel',
                            layout: 'fit',
                            title: 'Container',
                            items: [
                                {
                                    xtype: 'gridpanel',
                                    itemId: 'grdContainer',
                                    margin: -1,
                                    dockedItems: [
                                        {
                                            xtype: 'toolbar',
                                            dock: 'top',
                                            itemId: 'tlbGridOptions',
                                            layout: {
                                                type: 'hbox',
                                                padding: '0 0 0 1'
                                            },
                                            items: [
                                                {
                                                    xtype: 'button',
                                                    tabIndex: -1,
                                                    itemId: 'btnDeleteContainer',
                                                    iconCls: 'small-delete',
                                                    text: 'Delete'
                                                },
                                                {
                                                    xtype: 'tbseparator'
                                                },
                                                {
                                                    xtype: 'filter'
                                                }
                                            ]
                                        }
                                    ],
                                    columns: [
                                        {
                                            xtype: 'gridcolumn',
                                            dataIndex: 'string',
                                            text: 'Container ID',
                                            flex: 1
                                        },
                                        {
                                            xtype: 'gridcolumn',
                                            dataIndex: 'string',
                                            text: 'External System ID',
                                            flex: 1
                                        },
                                        {
                                            xtype: 'gridcolumn',
                                            dataIndex: 'string',
                                            text: 'Container Type',
                                            flex: 1
                                        },
                                        {
                                            xtype: 'gridcolumn',
                                            dataIndex: 'string',
                                            text: 'Last Updated By',
                                            flex: 1
                                        },
                                        {
                                            xtype: 'gridcolumn',
                                            dataIndex: 'string',
                                            text: 'Last Updated On',
                                            flex: 1
                                        }
                                    ],
                                    viewConfig: {
                                        itemId: 'grvContainer'
                                    }
                                }
                            ]
                        },
                        {
                            xtype: 'panel',
                            layout: 'fit',
                            title: 'SKU',
                            items: [
                                {
                                    xtype: 'gridpanel',
                                    itemId: 'grdSKU',
                                    margin: -1,
                                    dockedItems: [
                                        {
                                            xtype: 'toolbar',
                                            dock: 'top',
                                            itemId: 'tlbGridOptions',
                                            layout: {
                                                type: 'hbox',
                                                padding: '0 0 0 1'
                                            },
                                            items: [
                                                {
                                                    xtype: 'button',
                                                    tabIndex: -1,
                                                    itemId: 'btnDeleteSKU',
                                                    iconCls: 'small-delete',
                                                    text: 'Delete'
                                                },
                                                {
                                                    xtype: 'tbseparator'
                                                },
                                                {
                                                    xtype: 'filter'
                                                }
                                            ]
                                        }
                                    ],
                                    columns: [
                                        {
                                            xtype: 'gridcolumn',
                                            dataIndex: 'string',
                                            text: 'Unit',
                                            flex: 1
                                        },
                                        {
                                            xtype: 'gridcolumn',
                                            dataIndex: 'string',
                                            text: 'Material Description',
                                            flex: 2
                                        },
                                        {
                                            xtype: 'gridcolumn',
                                            dataIndex: 'string',
                                            text: 'SKU ID',
                                            flex: 1
                                        },
                                        {
                                            xtype: 'gridcolumn',
                                            width: 59,
                                            dataIndex: 'string',
                                            text: 'Qty'
                                        },
                                        {
                                            xtype: 'gridcolumn',
                                            dataIndex: 'string',
                                            text: 'Container',
                                            flex: 1
                                        },
                                        {
                                            xtype: 'gridcolumn',
                                            dataIndex: 'string',
                                            text: 'Lot/Serial',
                                            flex: 1
                                        },
                                        {
                                            xtype: 'gridcolumn',
                                            dataIndex: 'string',
                                            text: 'Expiration',
                                            flex: 1
                                        },
                                        {
                                            xtype: 'gridcolumn',
                                            dataIndex: 'string',
                                            text: 'Status',
                                            flex: 1
                                        },
                                        {
                                            xtype: 'gridcolumn',
                                            dataIndex: 'string',
                                            text: 'Owner',
                                            flex: 1
                                        }
                                    ],
                                    viewConfig: {
                                        itemId: 'grvSKU'
                                    }
                                }
                            ]
                        }
                    ]
                }
            ]
        }
    ]

});