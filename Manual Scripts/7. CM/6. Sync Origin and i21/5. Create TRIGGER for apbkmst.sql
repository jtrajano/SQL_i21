...
SET NOCOUNT ON 
	...
	SELECT 
			strBankName		= i.apcbk_desc COLLATE Latin1_General_CI_AS
			,strContact		= ''
			,strAddress		= ''
			,strZipCode		= ''
			,strCity		= ''
			...
			,intConcurrencyID		= 1
	FROM	inserted i
	WHERE	NOT EXISTS (SELECT TOP 1 1 FROM tblCMBank WHERE strBankName = i.apcbk_desc COLLATE Latin1_General_CI_AS)
...
SET NOCOUNT ON 
	...
	WHERE	NOT EXISTS (SELECT TOP 1 1 FROM tblCMBank WHERE strBankName = d.apcbk_desc COLLATE Latin1_General_CI_AS)
	...
	FROM	inserted i INNER JOIN deleted d
			...
			INNER JOIN tblCMBank
				ON d.apcbk_desc COLLATE Latin1_General_CI_AS = tblCMBank.strBankName
