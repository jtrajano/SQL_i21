		...
		AND 1 = 
			CASE	WHEN	@strSide = 'DEBIT' 
							AND (
								...
								OR (
									( dblAmount < 0 AND intBankTransactionTypeID = @BANK_TRANSFER )
									AND ( dblAmount < 0 AND intBankTransactionTypeID = @BANK_TRANSACTION )									
								)
							) THEN 1 					
					WHEN	@strSide = 'CREDIT' 
							AND (
								intBankTransactionTypeID = @BANK_DEPOSIT
								OR ( dblAmount > 0 AND intBankTransactionTypeID = @BANK_TRANSFER )
