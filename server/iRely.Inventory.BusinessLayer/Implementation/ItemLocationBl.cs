    ...
    public class ItemLocationBl : BusinessLayer<tblICItemLocation>, IItemLocationBl 
    {
        ...
        public async Task<SearchResult> SearchItemLocationViews(GetParameter param)
        {
            var query = (
                ...
                .Filter(param, true);
            var data = await query.ToListAsync(param.cancellationToken);
        ...
        public async Task<GetObjectResult> GetItemsWithNoLocation(GetParameter param)
        {
            var query = _db.GetQuery<vyuICGetItemsWithNoLocation>().Filter(param, true);
            var key = Methods.GetPrimaryKey<vyuICGetItemsWithNoLocation>(_db.ContextManager);
