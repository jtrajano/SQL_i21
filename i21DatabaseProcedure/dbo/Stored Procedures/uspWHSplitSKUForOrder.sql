CREATE PROCEDURE [dbo].uspWHSplitSKUForOrder 
	...
	BEGIN
		...
		SELECT @strSKUNo strSKUNo, 0, CASE 
				...
				WHEN @intRestrictionId = 1
					THEN intSKUStatusId
				ELSE 1
				END, strLotCode, @dblSplitQty dblQty, dtmReceiveDate, dtmProductionDate, intItemId, @intDestContainerId intContainerId, intOwnerId, @intUserId intLastModifiedUserId, GETDATE() dtmLastModified, intLotId, intUOMId, intSKUId, strSKUNo, intUnitsPerLayer, intLayersPerPallet, dblWeightPerUnit, intWeightPerUnitUOMId,'Split For Order',''
		FROM tblWHSKU
		WHERE intSKUId = @intSKUId;
