...
DECLARE @endOfMISC AS TABLE(strEndOfMISC NVARCHAR(1500), intTotalMISC INT)
...
SELECT * FROM dbo.fnAP1099EFileEndOfMISC(@year, @reprint, @corrected, @vendorFrom, @vendorTo)

SET @totalMISC = (SELECT intTotalMISC FROM @endOfMISC)
...
SELECT * FROM @payee
UNION ALL
SELECT strEndOfMISC FROM @endOfMISC
UNION ALL 
