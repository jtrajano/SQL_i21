...
IF  (SELECT TOP 1 ysnUsed FROM ##tblOriginMod WHERE strPrefix = 'AG' and strDBName = db_name()	) = 1
	EXEC ('
		...
		,vwcus_high_past_due = ISNULL(agcus_ar_per2,0.0) +  ISNULL(agcus_ar_per3,0.0) + ISNULL(agcus_ar_per4,0.0)+ ISNULL(agcus_ar_per5,0.0)
...
IF  (SELECT TOP 1 ysnUsed FROM ##tblOriginMod WHERE strPrefix = 'PT' and strDBName = db_name()) = 1
	EXEC ('
		...
		,vwcus_budget_amt  = CAST(ptcus_budget_amt AS DECIMAL(18,6))    
		...
		,vwcus_budget_amt_due = CAST(ptcus_budget_amt AS DECIMAL(18,6)) 
		,vwcus_cred_ppd   = ptcus_cred_ppd  
		,vwcus_ytd_srvchr  = CAST(ptcus_ytd_srvchr AS DECIMAL(18,6))
		,vwcus_last_pymt  = ptcus_last_pay_amt  
		,vwcus_last_pay_rev_dt = ptcus_last_pay_rev_dt  
		,vwcus_last_ivc_rev_dt = ptcus_last_ivc_rev_dt  
		,vwcus_high_cred  = CAST(ptcus_high_cred AS DECIMAL(18,6))   
		,vwcus_high_past_due = ISNULL(ptcus_ar_3160,0.0) + ISNULL(ptcus_ar_6190,0.0) + ISNULL(ptcus_ar_91120,0.0) + ISNULL(ptcus_ar_ov120,0.0)
		,vwcus_avg_days_pay  = CAST(ptcus_avg_days_pay AS SMALLINT) 
		,vwcus_avg_days_no_ivcs = CAST(ptcus_avg_days_no_ivcs AS SMALLINT)  
