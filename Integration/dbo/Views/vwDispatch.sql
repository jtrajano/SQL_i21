...
IF  ((((SELECT TOP 1 ysnUsed FROM #tblOriginMod WHERE strPrefix = 'AG') = 1) OR ((SELECT TOP 1 ysnUsed FROM #tblOriginMod WHERE strPrefix = 'PT') = 1)) and
...
BEGIN
	...
	CREATE VIEW [dbo].[vwDispatch]
		...
		Price = A.dblPrice,
		Total = A.dblTotal,
		DriverName = RTRIM(G.vwsls_name) COLLATE Latin1_General_CI_AS,
		DriverID = G.vwsls_slsmn_id COLLATE Latin1_General_CI_AS,
