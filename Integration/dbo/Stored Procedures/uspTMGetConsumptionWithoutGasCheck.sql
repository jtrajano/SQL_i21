...
GO
PRINT 'START OF CREATING [uspTMGetConsumptionWithoutGasCheck] SP'
GO
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[uspTMGetConsumptionWithoutGasCheck]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[uspTMGetConsumptionWithoutGasCheck]
GO



CREATE PROCEDURE [dbo].[uspTMGetConsumptionWithoutGasCheck](
...
(
	...
	,(CASE WHEN (EXISTS(SELECT TOP 1 1 FROM tblTMEvent WHERE tblTMEvent.intSiteID = C.intSiteID AND tblTMEvent.intEventTypeID = (SELECT intEventTypeID FROM tblTMEventType WHERE strEventType = ''Event-003'' AND ysnDefault = 1)))
...
PRINT 'END OF CREATING [uspTMGetConsumptionWithoutGasCheck] SP'
GO
