	...
	BEGIN
		EXEC ('
			...
			SELECT
				...
				,vwcus_first_name = ISNULL((CASE WHEN Cus.strType = ''Company'' THEN '''' ELSE SUBSTRING(Ent.strName,(CASE WHEN CHARINDEX( '', '', Ent.strName) != 0 THEN CHARINDEX( '', '', Ent.strName)  + 2 ELSE 25 END),50) END),'''')
				,vwcus_mid_init = ''''
				,vwcus_name_suffix = ''''
