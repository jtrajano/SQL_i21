...
IF NOT EXISTS (SELECT TOP 1 1 FROM sys.columns WHERE name  = N'agcus_cus_no' AND object_id IN (SELECT object_id FROM sys.tables WHERE name = 'agcusmst'))
BEGIN 
			...
			BEGIN
				...
				BEGIN TRY
					...
					INSERT [dbo].[tblEMEntityLocation]    
						 ...
						 WHERE agcus_bill_to is not null AND ETYP.strType = ''Customer''
