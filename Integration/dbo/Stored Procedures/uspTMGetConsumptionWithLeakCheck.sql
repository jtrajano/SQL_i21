...
GO
PRINT 'START OF CREATING [uspTMGetConsumptionWithLeakCheck] SP'
GO
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[uspTMGetConsumptionWithLeakCheck]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[uspTMGetConsumptionWithLeakCheck]
GO



CREATE PROCEDURE [dbo].[uspTMGetConsumptionWithLeakCheck](
...
(
	...
	FROM tblTMEvent
	WHERE intEventTypeID = (SELECT intEventTypeID FROM tblTMEventType WHERE strEventType = ''Event-004'' AND ysnDefault = 1)
...
GO
PRINT 'END OF CREATING [uspTMGetConsumptionWithLeakCheck] SP'
GO