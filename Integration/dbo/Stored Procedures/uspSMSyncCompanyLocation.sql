    ...
    BEGIN  
     ...
     WHILE(EXISTS(SELECT TOP 1 1 FROM #Temp))  
      BEGIN  
	   ...
	   SELECT l.agloc_loc_no + 1 AS start INTO #Available FROM aglocmst AS l LEFT OUTER JOIN aglocmst AS r ON l.agloc_loc_no = r.agloc_loc_no AND r.agloc_loc_no NOT LIKE ''%[^0-9]%'' WHERE l.[agloc_loc_no] NOT LIKE ''%[^0-9]%'' --r.agloc_loc_no IS NULL
