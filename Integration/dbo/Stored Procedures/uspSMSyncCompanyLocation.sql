CREATE PROCEDURE uspSMSyncCompanyLocation
	@ToOrigin			bit				= 0
...
IF (@ToOrigin = 1)
	BEGIN
		...
		--WHERE strLocationNumber IS NULL OR RTRIM(LTRIM(strLocationNumber)) = ''		-- Uncomment Once strLocationNumberhas been adde to strLocationNumber
...
IF(LOWER(@LocationNumbers) = 'all')
	BEGIN
		IF (@ToOrigin = 1)
			--INSERT INTO @RecordsToProcess(strName, strNumber) -- Uncomment Once strLocationNumberhas been adde to strLocationNumber	
			--SELECT [strLocationName], [strLocationNumber]
			--FROM tblSMCompanyLocation									
			SET @ToOrigin = 1 -- temp code: delete once strLocationNumberhas been adde to strLocationNumber				
		ELSE
			--INSERT INTO @RecordsToProcess(strName, strNumber) -- Uncomment Once strLocationNumberhas been adde to strLocationNumber	
...
ELSE
	...
	SELECT P.*
	FROM @RecordsToProcess P
	--LEFT OUTER JOIN tblSMCompanyLocation CL ON P.[strNumber] = CL.[strLocationNumber] -- Uncomment Once strLocationNumberhas been adde to strLocationNumber	
	--WHERE CL.[strLocationNumber] 
	--	IS NULL									
...
IF(@ToOrigin = 1) 
	BEGIN
		...
		INSERT INTO [aglocmst]
			--([agloc_loc_no]		-- Uncomment Once strLocationNumberhas been adde to strLocationNumber	
			([agloc_name]
			,[agloc_addr]
			,[agloc_addr2]
			,[agloc_city]
		...
		SELECT 
			--CL.[strLocationNumber]			--[agloc_loc_no] -- Uncomment Once strLocationNumberhas been adde to strLocationNumber	
		...
		--INNER JOIN						-- Uncomment Once strLocationNumberhas been adde to strLocationNumber	
		--	@RecordsToAdd A
		--		ON CL.[strLocationNumber] = A.strNumber 			
		LEFT JOIN
		...
		--INNER JOIN		-- Uncomment Once strLocationNumberhas been adde to strLocationNumber	
		--	@RecordsToUpdate U
		--		ON CL.[strLocationNumber] = U.strNumber 			
		LEFT JOIN
			tblGLCOACrossReference CA
				ON CL.[intCashAccount] = CA.[inti21Id]
		LEFT JOIN
		...
		LEFT OUTER JOIN
			tblSMCountry C
				ON CL.strCountry = C.strCountry 				
		--WHERE -- Uncomment Once strLocationNumberhas been adde to strLocationNumber	
...
ELSE
	BEGIN
		INSERT INTO [tblSMCompanyLocation]
			...
			,[intConcurrencyId])
		SELECT
			--AG.[agloc_loc_no]					--<strLocationNumber, nvarchar(3),>	-- Uncomment Once strLocationNumberhas been adde to strLocationNumber	
		...
		LEFT OUTER JOIN
			tblSMCompanyLocation CL
				ON RTRIM(LTRIM(AG.[agloc_name] COLLATE Latin1_General_CI_AS)) = RTRIM(LTRIM(CL.[strLocationName] COLLATE Latin1_General_CI_AS)) 
					--AND RTRIM(LTRIM(AG.[agloc_loc_no] COLLATE Latin1_General_CI_AS)) = RTRIM(LTRIM(CL.[strLocationNumber] COLLATE Latin1_General_CI_AS)) -- Uncomment Once strLocationNumberhas been adde to strLocationNumber	
		WHERE
