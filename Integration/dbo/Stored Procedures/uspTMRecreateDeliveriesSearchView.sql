			...
			EXEC('
				...
				AS  
					SELECT 
						...
						,dblPercentIdeal = CAST( CASE WHEN ((ISNULL(C.dblTotalCapacity,0.0) * (I.vwitm_deflt_percnt/100)) - ISNULL(C.dblTotalReserve,0.0))  <= 0 THEN 0 ELSE F.dblQuantityDelivered / ((ISNULL(C.dblTotalCapacity,0.0) * (I.vwitm_deflt_percnt/100)) - ISNULL(C.dblTotalReserve,0.0)) END AS NUMERIC(18,6)) * 100
						,intConcurrencyId = 0
