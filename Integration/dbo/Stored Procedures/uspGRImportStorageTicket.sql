...
BEGIN
		--================================================
			...
			BEGIN
				...
				IF NOT EXISTS(SELECT 1 FROM gastrmst WHERE gastr_un_bal >0 AND A4GLIdentity NOT IN (SELECT intCustomerStorageId FROM tblGRCustomerStorage))
					SELECT @Total = 0
				ELSE
					SELECT @Total = COUNT(1) FROM gastrmst WHERE gastr_un_bal >0 AND A4GLIdentity NOT IN (SELECT intCustomerStorageId FROM tblGRCustomerStorage)
			...
			DECLARE @StorageType AS TABLE (
				intId INT IDENTITY(1,1)
				,intOrigId INT
				,strStorageType NVARCHAR(40) COLLATE Latin1_General_CI_AS
			)

			INSERT INTO @StorageType
			SELECT intStorageScheduleTypeId,strStorageTypeDescription FROM tblGRStorageType WHERE intStorageScheduleTypeId > 0
			...
			SET IDENTITY_INSERT [dbo].[tblGRCustomerStorage] ON
				...
				JOIN gacommst B 
					ON B.gacom_com_cd = a.gastr_com_cd
				JOIN tblICCommodity Com 
					ON (
						Com.strCommodityCode = LTRIM(RTRIM(B.gacom_desc)) COLLATE Latin1_General_CS_AS
						OR (CASE WHEN LTRIM(RTRIM(B.gacom_com_cd)) = 'B' THEN ('SOY' + LTRIM(RTRIM(B.gacom_desc))) ELSE LTRIM(RTRIM(B.gacom_desc)) END) COLLATE Latin1_General_CS_AS = Com.strCommodityCode
						)
				JOIN @StorageType St 
					ON St.intId = a.gastr_stor_type
				JOIN tblGRStorageScheduleRule Sr 
					ON Sr.intStorageType = St.intOrigId
						AND Sr.intCommodity = Com.intCommodityId
						AND SUBSTRING(Sr.strScheduleId,0,CHARINDEX('/',Sr.strScheduleId)) = a.gastr_stor_schd_no
				JOIN tblGRStorageScheduleLocationUse SRLOC
					ON SRLOC.intStorageScheduleId = Sr.intStorageScheduleRuleId
						AND SRLOC.intCompanyLocationId = cast(a.gastr_loc_no as int)
				JOIN tblSMCompanyLocation L 
					ON L.strLocationNumber=LTRIM(RTRIM(a.gastr_loc_no)) COLLATE  Latin1_General_CS_AS
				JOIN tblGRDiscountSchedule	DSch 
					ON 
						--DSch.strDiscountDescription collate Latin1_General_CI_AS = (CASE WHEN LTRIM(RTRIM(a.gastr_disc_schd_no)) = '0' THEN Com.strDescription +' Discount' ELSE LTRIM(RTRIM(a.gastr_disc_schd_no)) END)
						--AND 
						DSch.intCommodityId = Com.intCommodityId
				JOIN tblSMCurrency Cur 
					ON Cur.strCurrency = a.gastr_currency COLLATE  Latin1_General_CS_AS
				JOIN gacommst UOM 
					ON UOM.gacom_com_cd = a.gastr_com_cd
				LEFT JOIN tblICStorageLocation bin 
					ON bin.strName = a.gastr_bin_no COLLATE Latin1_General_CS_AS
				...
				SELECT
					...
					,intEntityId		     = t.intEntityId
					,intCommodityId		 = Com.intCommodityId
					,intStorageTypeId		 = St.intOrigId--(SELECT intStorageScheduleTypeId FROM tblGRStorageType WHERE ysnCustomerStorage=1 AND strStorageTypeCode=LTRIM(a.gastr_stor_type)+' O')
					,intStorageScheduleId	 = Sr.intStorageScheduleRuleId
					,intCompanyLocationId	 = L.intCompanyLocationId 
					,intTicketId			 = a.A4GLIdentity 
					,intDiscountScheduleId  = DSch.intDiscountScheduleId--a.gastr_disc_schd_no
				...
				JOIN	tblGRDiscountSchedule DSch 
					ON 
					--DSch.strDiscountDescription collate Latin1_General_CI_AS =  CASE WHEN LTRIM(RTRIM(a.gastr_disc_schd_no))='0' THEN Com.strDescription +' Discount' ELSE LTRIM(RTRIM(a.gastr_disc_schd_no)) END AND 
					DSch.intCommodityId = Com.intCommodityId
			...
			--UPDATE UOM 
			UPDATE CS
			SET intItemUOMId = IUOM.intItemUOMId
			FROM tblGRCustomerStorage CS
			INNER JOIN tblICItemUOM IUOM
				ON IUOM.intItemId = CS.intItemId
					AND IUOM.intUnitMeasureId = CS.intUnitMeasureId
		...
		BEGIN
			...
			,@strCommodityCode = IC.strCommodityCode--LTRIM(RTRIM(gastr_com_cd))
			,@intStorageTypeId=gastr_stor_type
			,@strDiscountDescription = DSC.strDiscountDescription--LTRIM(RTRIM(gastr_disc_schd_no))
			FROM gastrmst a
			INNER JOIN tblGRCustomerStorage CS
				ON CS.intCustomerStorageId = a.A4GLIdentity
			INNER JOIN tblGRDiscountSchedule DSC
				ON DSC.intDiscountScheduleId = CS.intDiscountScheduleId
			INNER JOIN tblICCommodity IC
				ON IC.intCommodityId = CS.intCommodityId
			WHERE A4GLIdentity = @intCustomerStorageId
		
			SELECT @strStorageTypeCode =strStorageTypeCode FROM tblGRStorageType WHERE intStorageScheduleTypeId=@intStorageTypeId
		
			IF @strDiscountCode1 <> ''
			BEGIN
			--SELECT '1', @intCustomerStorageId
				...
				AND strShortName=@strDiscountCode1 
				...
				--AND strStorageTypeCode='C' 
				AND strShortName=@strDiscountCode1 
				AND strDiscountDescription=@strDiscountDescription
		
		IF @intDiscountScheduleCodeId1 IS NOT NULL
			...
			--SELECT '2', @intCustomerStorageId
				...
				AND strCurrency=@strCurrency AND strCommodityCode=@strCommodityCode 
				--AND intStorageTypeId=@intStorageTypeId 
				AND strShortName=@strDiscountCode2 
				AND strDiscountDescription=@strDiscountDescription
		...
		IF @intDiscountScheduleCodeId2 IS NOT NULL
				INSERT INTO tblQMTicketDiscount
				(
					 intConcurrencyId
					,dblGradeReading
			...
			--SELECT '3', @intCustomerStorageId
				...
				AND strCurrency=@strCurrency AND strCommodityCode=@strCommodityCode 
				--AND intStorageTypeId=@intStorageTypeId 
				AND strShortName=@strDiscountCode3 
				AND strDiscountDescription=@strDiscountDescription
		...
		IF @intDiscountScheduleCodeId3 IS NOT NULL
				INSERT INTO tblQMTicketDiscount
				(
					 intConcurrencyId
					,dblGradeReading
			...
			--SELECT '4', @intCustomerStorageId
				...
				AND strCurrency=@strCurrency AND strCommodityCode=@strCommodityCode 
				--AND intStorageTypeId=@intStorageTypeId 
				AND strShortName=@strDiscountCode4 
				AND strDiscountDescription=@strDiscountDescription
		...
		IF @intDiscountScheduleCodeId4 IS NOT NULL
					INSERT INTO tblQMTicketDiscount
					(
						 intConcurrencyId
						,dblGradeReading
			...
			--SELECT '5', @intCustomerStorageId
				...
				AND strCurrency=@strCurrency AND strCommodityCode=@strCommodityCode 
				--AND intStorageTypeId=@intStorageTypeId 
				AND strShortName=@strDiscountCode5 
				AND strDiscountDescription=@strDiscountDescription
		...
		IF @intDiscountScheduleCodeId5 IS NOT NULL
				INSERT INTO tblQMTicketDiscount
				(
				 intConcurrencyId
				,dblGradeReading
			...
			--SELECT '6', @intCustomerStorageId
				...
				AND strCurrency=@strCurrency AND strCommodityCode=@strCommodityCode 
				AND intStorageTypeId=@intStorageTypeId
				AND strShortName=@strDiscountCode6 
				AND strDiscountDescription=@strDiscountDescription
		...
		IF @intDiscountScheduleCodeId6 IS NOT NULL
				INSERT INTO tblQMTicketDiscount
				(
				 intConcurrencyId
				,dblGradeReading
			...
			--SELECT '7', @intCustomerStorageId
				...
				AND strCurrency=@strCurrency AND strCommodityCode=@strCommodityCode 
				--AND intStorageTypeId=@intStorageTypeId 
				AND strShortName=@strDiscountCode7 
				AND strDiscountDescription=@strDiscountDescription
		...
		IF @intDiscountScheduleCodeId7 IS NOT NULL
				INSERT INTO tblQMTicketDiscount
				(
				 intConcurrencyId
				,dblGradeReading
			...
			BEGIN
				...
				AND strCurrency=@strCurrency AND strCommodityCode=@strCommodityCode 
				--AND intStorageTypeId=@intStorageTypeId 
				AND strShortName=@strDiscountCode8 
				AND strDiscountDescription=@strDiscountDescription
		...
		IF @intDiscountScheduleCodeId8 IS NOT NULL
				INSERT INTO tblQMTicketDiscount
				(
				 intConcurrencyId
				,dblGradeReading
			...
			BEGIN
				...
				AND strCurrency=@strCurrency AND strCommodityCode=@strCommodityCode 
				--AND intStorageTypeId=@intStorageTypeId 
				AND strShortName=@strDiscountCode9 
				AND strDiscountDescription=@strDiscountDescription
		...
		IF @intDiscountScheduleCodeId9 IS NOT NULL
				INSERT INTO tblQMTicketDiscount
				(
				 intConcurrencyId
				,dblGradeReading
			...
			BEGIN
			--SELECT '10', @intCustomerStorageId
				...
				AND strCurrency=@strCurrency AND strCommodityCode=@strCommodityCode 
				--AND intStorageTypeId=@intStorageTypeId 
				AND strShortName=@strDiscountCode10 
				AND strDiscountDescription=@strDiscountDescription
		...
		IF @intDiscountScheduleCodeId10 IS NOT NULL
				INSERT INTO tblQMTicketDiscount
				(
				 intConcurrencyId
				,dblGradeReading
			...
			BEGIN
			--SELECT '11', @intCustomerStorageId
				...
				AND strCurrency=@strCurrency AND strCommodityCode=@strCommodityCode 
				--AND intStorageTypeId=@intStorageTypeId 
				AND strShortName=@strDiscountCode11 
				AND strDiscountDescription=@strDiscountDescription
		...
		IF @intDiscountScheduleCodeId11 IS NOT NULL
				INSERT INTO tblQMTicketDiscount
				(
				 intConcurrencyId
				,dblGradeReading
			...
			--SELECT '1', @intCustomerStorageId
				...
				AND strCurrency=@strCurrency AND strCommodityCode=@strCommodityCode 
				--AND intStorageTypeId=@intStorageTypeId 
				AND strShortName=@strDiscountCode12 
				AND strDiscountDescription=@strDiscountDescription
		...
		IF @intDiscountScheduleCodeId12 IS NOT NULL
				INSERT INTO tblQMTicketDiscount
				(
				 intConcurrencyId
				,dblGradeReading
