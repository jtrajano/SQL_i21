GO
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[eftrxmst]') AND type IN (N'U'))
BEGIN 

	EXEC('
		IF EXISTS (SELECT 1 FROM sys.objects WHERE name = ''uspNRGenerateEFTSchedule'' and type = ''P'') 
			DROP PROCEDURE [dbo].[uspNRGenerateEFTSchedule];
	')
