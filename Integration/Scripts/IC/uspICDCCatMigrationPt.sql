...
(
	SELECT
		...
		, strInventoryType			= NULL --CASE WHEN (SELECT COUNT(*) cnt FROM ptitmmst WHERE ptitm_class = ptclsmst.ptcls_class AND ptitm_phys_inv_yno = 'Y') > 0 THEN 'Inventory' ELSE 'Other Charge' END COLLATE Latin1_General_CI_AS
		, intCostingMethod			= '1'
