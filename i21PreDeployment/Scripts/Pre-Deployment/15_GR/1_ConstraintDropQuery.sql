...
IF EXISTS(SELECT * FROM INFORMATION_SCHEMA.CONSTRAINT_TABLE_USAGE WHERE CONSTRAINT_NAME = 'FK_tblSCScaleSetup_tblICUnitMeasure_intUnitMeasureId')
	...
	END	
GO
IF EXISTS(SELECT TOP 1 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE [TABLE_NAME] = 'tblGRDiscountScheduleCode' AND [COLUMN_NAME] IN('strDiscountCode','strDiscountCodeDescription'))
BEGIN
	...
	SELECT 
		...
		,0
		,1
		FROM tblGRDiscountScheduleCode WHERE strDiscountCodeDescription NOT IN(SELECT strItemNo FROM tblICItem)

		IF EXISTS(SELECT TOP 1 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE [TABLE_NAME] = 'tblGRDiscountScheduleCode' AND [COLUMN_NAME]='intItemId')
		BEGIN
			UPDATE SC 
