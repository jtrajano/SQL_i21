...
IF(
	EXISTS(SELECT 1 FROM sys.columns WHERE name = N'intEntityLocationId' and object_id = OBJECT_ID(N'tblEMEntityLocation'))
...
BEGIN
	EXEC('UPDATE tblARCustomerSpecialPrice SET intRackLocationId = NULL 
			where intRackLocationId not in ( select intEntityLocationId from tblEMEntityLocation)') 
END