...
BEGIN
	...
	*/
	...
	insert into @tmp
	select name,parent_object_id from sys.objects where OBJECT_NAME(parent_object_id) like '%tblEntity%' and type = 'D'	
	...
	insert into @tmp
	select name,parent_object_id from sys.objects where OBJECT_NAME(parent_object_id) like '%tblEntity%' and type = 'UQ'
	while exists(select top 1 1 from @tmp)
	BEGIN
		SELECT TOP 1 @Current = 'ALTER TABLE ' + OBJECT_NAME(parent_object_id) + ' DROP CONSTRAINT [' + name + ']', @Name = name FROM @tmp
		EXEC( @Current )
	...
	insert into @tmp
	select name,parent_object_id from sys.objects where OBJECT_NAME(parent_object_id) like '%tblEntity%' and type = 'F'
	while exists(select top 1 1 from @tmp)
	BEGIN
		SELECT TOP 1 @Current = 'ALTER TABLE ' + OBJECT_NAME(parent_object_id) + ' DROP CONSTRAINT [' + name + ']', @Name = name FROM @tmp
		EXEC( @Current )
	...
	FROM INFORMATION_SCHEMA.CONSTRAINT_COLUMN_USAGE U
		...
		WHERE U.TABLE_NAME like '%tblEntity%'
	...
	insert into @tmp
	select name,parent_object_id from sys.objects where OBJECT_NAME(parent_object_id) like '%tblEntity%' and type = 'PK'
	while exists(select top 1 1 from @tmp)
	BEGIN
		SELECT TOP 1 @Current = 'ALTER TABLE ' + OBJECT_NAME(parent_object_id) + ' DROP CONSTRAINT [' + name + ']', @Name = name FROM @tmp
		EXEC( @Current )
	...
	EXEC('DROP TABLE tblEntityTempForDelete')
