...
BEGIN
	...
	BEGIN
		...
		set @cmdevent = 'select @maxevent = max(intWorkOrderID) from tblTMWorkOrder'
		...
		WHERE U.TABLE_NAME = 'tblTMWorkOrder' 
		...
		begin
			...
			BEGIN		
				...
				SET @cmdevent = ''UPDATE tblTMWorkOrder SET intEnteredByID = A.intEntityUserSecurityId
					FROM tblSMUserSecurity A
					WHERE tblTMWorkOrder.intEnteredByID = A.intUserSecurityIdOld
						AND A.intUserSecurityIdOld IS NOT NULL 
						AND intWorkOrderID > '' + cast(@minId as nvarchar) + '' and intWorkOrderID <= '' + cast((@minId + @COUNTS ) as nvarchar)
