...
BEGIN
    EXEC('UPDATE tblPRPaycheckEarning SET intEmployeeDepartmentId = NULL 
...
BEGIN
    EXEC('UPDATE tblPRPayGroupDetail SET intDepartmentId = NULL 
		WHERE intDepartmentId NOT IN (SELECT intDepartmentId FROM tblPRDepartment) AND intDepartmentId IS NOT NULL')
END
GO
