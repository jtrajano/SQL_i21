...
CREATE OR ALTER PROCEDURE [dbo].[uspARAddNonInventoryItemToInvoices]
...
--SELECT
--	 [intId]				= IT.[intId]
--	,[strMessage]		= 'The item was not set up to be available on the specified location!'
--	,[strTransactionType]	= IT.[strTransactionType]
--	,[strType]				= IT.[strType]
--	,[strSourceTransaction]	= IT.[strSourceTransaction]
--	,[intSourceId]			= IT.[intSourceId]
...
IF(OBJECT_ID('tempdb..#InvoiceNonInventoryItem') IS NOT NULL)
BEGIN
    DROP TABLE #InvoiceNonInventoryItem
END

CREATE TABLE #InvoiceNonInventoryItem
