...
CREATE OR ALTER PROCEDURE [dbo].[uspARAddDiscountsCharges]
...
begin try
	...
	from
		...
		INNER JOIN tblICInventoryShipment IIS ON IIS.strShipmentNumber = ID.strDocumentNumber
		INNER JOIN tblICInventoryShipmentItem SI on SI.intInventoryShipmentId= IIS.intInventoryShipmentId
	where
		ID.intInvoiceDetailId = @intInvoiceDetailId
	...
	begin
		...
		begin
			...
			SELECT
				...
				,@ItemPrice                             = (@dblQuantity / ARSI.[dblShipmentUnitPrice] ) * @dblISTotalQuantity
				,@ItemUnitPrice                         = (@dblQuantity /  ARSI.[dblUnitPrice]) * @dblISTotalQuantity 
			...
			FROM
				...
				INNER JOIN tblICInventoryShipmentItem ICISI with (nolock) on ICISI.intInventoryShipmentId = ICIS.intInventoryShipmentId 
	...
	select @intCountCharges=Count(intInvoiceId) FROM tblARInvoiceDetail  WHERE intInvoiceDetailId = @intInvoiceDetailId OR intInvoiceDetailId = @NewInvoiceDetailId

	UPDATE tblARInvoiceDetail
	SET dblPrice = dblPrice/@intCountCharges,
		dblBasePrice = dblBasePrice/@intCountCharges,
		dblBaseUnitPrice = dblBaseUnitPrice/@intCountCharges,
		dblUnitPrice = dblUnitPrice/@intCountCharges,
		dblTotal = dblTotal/@intCountCharges,
		dblBaseTotal = dblTotal/@intCountCharges
	WHERE intInvoiceDetailId = @intInvoiceDetailId OR intInvoiceDetailId = @NewInvoiceDetailId
	...
	EXEC [dbo].[uspARReComputeInvoiceAmounts] @InvoiceId = @InvoiceId, @AvailableDiscountOnly = 0
