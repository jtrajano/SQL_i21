...
CREATE OR ALTER PROCEDURE [dbo].[uspARCalculateServiceCharge]
	...
	@ysnPrintByLocation		BIT = 0,
	@companyLocationIds	    NVARCHAR(MAX) = ''
AS
BEGIN
SET NOCOUNT ON
...
BEGIN
	...
	WHILE EXISTS(SELECT TOP 1 1 FROM @tblCurrency)
	BEGIN
		DECLARE @intCurrencyId INT

		SELECT TOP 1 @intCurrencyId = intId
		FROM @tblCurrency
		...
		BEGIN
			...
			AND INV.intCurrencyId = @intCurrencyId
			AND (INV.intCompanyLocationId = @intCompanyLocationId OR @ysnPrintByLocation = 0)
