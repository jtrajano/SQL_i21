...
CREATE OR ALTER PROCEDURE [dbo].[uspARAddInvoicesTaxDetail]
...
BEGIN TRY
	...
	SELECT
		...
		,intSalesTaxExemptionAccountId	= SMTC.[intSalesTaxExemptionAccountId]
		...
		,ysnAddToCost					= ISNULL(SMTC.[ysnAddToCost], 0)
	...
	INNER JOIN
		(SELECT [intTaxCodeId], [intTaxClassId], [intSalesTaxExemptionAccountId], [strTaxableByOtherTaxes], [intSalesTaxAccountId], [ysnCheckoffTax], [ysnAddToCost] FROM tblSMTaxCode WITH(NOLOCK))  SMTC
