...
-- changeset Von:vyuICGetInventoryReceiptSearchByEntity.sql.1 runOnChange:true splitStatements:false
-- comment: RK-1234

--CREATE OR ALTER VIEW [dbo].[vyuICGetInventoryReceiptSearchByEntity]
--AS
...
FROM 
	...
	CROSS APPLY (
		...
		FROM 
			tblEMEntityToContact ec INNER JOIN tblEMEntity e 
				ON e.intEntityId = ec.intEntityContactId
			INNER JOIN tblEMEntityLocation el 
				ON el.intEntityLocationId = ec.intEntityLocationId
				AND el.intEntityId = ec.intEntityId
		WHERE 
			ec.ysnPortalAccess = 1
	) permission
	CROSS APPLY (
		SELECT TOP 1 
			sl.intCompanyLocationSubLocationId
		FROM 
			tblSMCompanyLocationSubLocation sl INNER JOIN tblICInventoryReceiptItem ri 
				ON ri.intSubLocationId = sl.intCompanyLocationSubLocationId
				AND ri.intInventoryReceiptId = v.intInventoryReceiptId
		WHERE 
			sl.intCompanyLocationId = v.intLocationId
			AND sl.intVendorId = permission.intEntityId
	) accessibleReceipts
WHERE 
	v.strReceiptType = 'Purchase Contract'
	AND v.intSourceType = 2
