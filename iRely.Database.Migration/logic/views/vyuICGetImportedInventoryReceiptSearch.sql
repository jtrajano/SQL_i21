...
-- changeset Von:vyuICGetImportedInventoryReceiptSearch.sql.1 runOnChange:true splitStatements:false
-- comment: RK-1234

CREATE OR ALTER VIEW [dbo].[vyuICGetImportedInventoryReceiptSearch]
	AS
...
SELECT Receipt.intInventoryReceiptId
	...
	, [intStoreManagerId] = StoreManager.intEntityId
FROM 
	...
	INNER JOIN tblSMUserSecurity StoreManager 
		ON StoreManager.ysnStoreManager = 1 
		AND StoreManager.intCompanyLocationId = [Location].intCompanyLocationId
	...
	LEFT JOIN tblSMUserSecurity Receiver 
		ON Receiver.[intEntityId] = Receipt.intReceiverId
	LEFT JOIN vyuEMEntity Entity 
		ON Entity.intEntityId = Receipt.intEntityId AND Entity.strType = 'User'
...
WHERE
	Receipt.strDataSource = 'EdiGenerateReceipt'
	AND (Receipt.ysnPosted = 0 OR Receipt.ysnPosted IS NULL)
