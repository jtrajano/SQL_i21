...
-- changeset Von:fnCTConvertQtyToTargetCommodityUOM.sql.1 runOnChange:true splitStatements:false
-- comment: RK-1234

CREATE OR ALTER FUNCTION [dbo].[fnCTConvertQtyToTargetCommodityUOM]
(
	@intCommodityId INT,
...
BEGIN 
	...
	SET @result = 
		CASE	WHEN @dblUnitQtyFrom = @dblUnitQtyTo THEN 
					@dblQty
				ELSE 
					CASE	WHEN @dblUnitQtyTo <> 0 THEN CAST((@dblQty * @dblUnitQtyFrom) AS  NUMERIC(26,3)) / CAST(@dblUnitQtyTo AS NUMERIC(26,12))							
							ELSE NULL 
					END
		END 
