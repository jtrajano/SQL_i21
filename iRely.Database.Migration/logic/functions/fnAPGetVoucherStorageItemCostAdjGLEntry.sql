...
-- changeset Von:fnAPGetVoucherStorageItemCostAdjGLEntry.sql.1 runOnChange:true splitStatements:false
-- comment: RK-1234

CREATE OR ALTER FUNCTION [dbo].[fnAPGetVoucherStorageItemCostAdjGLEntry]
(
	@billId INT
)
...
(
	SELECT
		...
		,CAST((CASE	WHEN A.intTransactionType IN (1) 
				THEN (B.dblTotal  - round((ISNULL(storageOldCost.dblOldCost, B.dblOldCost) * B.dblQtyReceived), 2)  * ISNULL(NULLIF(B.dblRate,0),1)) 
			ELSE 0 END) AS  DECIMAL(18, 2)) AS dblTotal
		,CAST((CASE	WHEN A.intTransactionType IN (1) 
				THEN (B.dblTotal - round((ISNULL(storageOldCost.dblOldCost, B.dblOldCost) * B.dblQtyReceived), 2) ) 
	...
	OUTER APPLY (
		...
		INNER JOIN tblGRStorageHistory storageHistory ON storageHistory.intCustomerStorageId = customerStorage.intCustomerStorageId 
													...
													AND ISNULL(B.intContractHeaderId,0) = ISNULL(storageHistory.intContractHeaderId,0)
