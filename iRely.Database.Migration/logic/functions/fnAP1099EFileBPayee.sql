...
-- changeset Von:fnAP1099EFileBPayee.sql.1 runOnChange:true splitStatements:false
-- comment: RK-1234

CREATE OR ALTER FUNCTION [dbo].[fnAP1099EFileBPayee]
(
	@year INT,
	@reprint BIT = 0,
	@corrected BIT = 0,
...
BEGIN
	...
	SELECT 
		...
		+ SPACE(4)
		+ '1'
		+ ISNULL(LEFT(REPLACE(A.strFederalTaxId, '-', ''),9),SPACE(9)) --ISNULL(NULLIF(A.strFederalTaxId,''),SPACE(9)) -- 12-20
		...
		+ CASE WHEN ISNULL(A.dbl1099B,0) > @maxAmount 
				THEN REPLICATE('0',10 - LEN(CAST(FLOOR((@maxAmount - CAST(ISNULL(A.dbl1099B,0) AS DECIMAL(18,2)))) AS NVARCHAR(100))))
					+ CAST(FLOOR((@maxAmount - CAST(ISNULL(A.dbl1099B,0) AS DECIMAL(18,2)))) AS NVARCHAR(100))
					+ CAST(PARSENAME(CAST(ISNULL(A.dbl1099B,0) AS DECIMAL(18,2)),1) AS NVARCHAR(2))
				ELSE 
					REPLICATE('0',10 - LEN(CAST(FLOOR((ISNULL(A.dbl1099B,0))) AS NVARCHAR(100))))
					+ CAST(FLOOR((CAST(ISNULL(A.dbl1099B,0) AS DECIMAL(18,2)))) AS NVARCHAR(100))
					+ CAST(PARSENAME(CAST(ISNULL(A.dbl1099B,0) AS DECIMAL(18,2)),1) AS NVARCHAR(2))
			END
		+ REPLICATE('0',144)
		+ ' '
