...
-- changeset Von:fnAPCreatePaymentGLEntries.sql.1 runOnChange:true splitStatements:false
-- comment: RK-1234

CREATE OR ALTER FUNCTION [dbo].[fnAPCreatePaymentGLEntries]
(
	...
	FROM (
		...
		(
			...
			SELECT		--DISTINCT
						...
						[dblCredit]	 					=	--CASE WHEN A.dblExchangeRate != 1 THEN 
															CAST(
															dbo.fnAPGetPaymentAmountFactor((ISNULL(loadingFee.dblLoadingFee,0)
																- (CASE WHEN paymentDetail.dblWithheld > 0 THEN (ISNULL(loadingFee.dblLoadingFee,0) * ISNULL(withHoldData.dblWithholdPercent,1)) ELSE 0 END)), 
																paymentDetail.dblPayment, voucher.dblTotal) * ISNULL(NULLIF(A.dblExchangeRate,0),1) 
															AS DECIMAL(18,6)), 
																--* (CASE WHEN paymentDetail.ysnOffset = 1 THEN -1 ELSE 1 END),
															-- ELSE
			...
			INNER JOIN tblAPBill voucher ON paymentDetail.intBillId = voucher.intBillId
			CROSS APPLY (
				SELECT SUM(dblLoadingFee) dblLoadingFee
				FROM (
					SELECT DISTINCT dblAdjustedTax AS dblLoadingFee
					FROM tblAPBillDetailTax R2
					INNER JOIN tblAPBillDetail R3 ON R2.intBillDetailId = R3.intBillDetailId
					WHERE R3.intBillId = voucher.intBillId
					AND R2.strCalculationMethod = 'Using Texas Fee Matrix'
