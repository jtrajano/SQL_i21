...
-- changeset Von:fnARGetNonInventoryItemGLAccount.sql.1 runOnChange:true splitStatements:false
-- comment: RK-1234

CREATE OR ALTER FUNCTION [dbo].[fnARGetNonInventoryItemGLAccount]
(
	@intItemId INT 
)
RETURNS INT
...
BEGIN 
    ...
    --If the item on the invoice does not have a Sales or General GL account attributed to it, but it is in a category with a General GL account attributed to it, that GL account should be passed to the ledger.
        --Inventory --> Catagory --> GL Acct tab
			...
			(SELECT  strDescription = 'Non-Inventory Accounts') N
			OUTER APPLY
			(SELECT
