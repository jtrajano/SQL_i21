...
-- changeset Von:fnCTGetBasisDelivery.sql.1 runOnChange:true splitStatements:false
-- comment: RK-1234

-- ANY CHANGES APPLIED HERE MUST BE APPLIED ALSO IN fnCTGetBasisDeliveryAboveR2 POST SCRIPT

CREATE OR ALTER FUNCTION [dbo].[fnCTGetBasisDelivery]
(
	@dtmDate DATE = NULL
)
...
BEGIN
	WITH OpenBasisContract	AS (
		...
		WHERE (tbl.intPricingTypeId = 2 or (tbl.intContractStatusId = 6 and 'Destination' not in (g.strWhereFinalized, w.strWhereFinalized)))
			AND tbl.intContractStatusId in (1,4,5) or (tbl.intContractStatusId = 6 and isnull(g.strWhereFinalized,'') <> 'Destination' and isnull(w.strWhereFinalized,'') <> 'Destination'))
	, CBL AS (
		SELECT CBL1.*
