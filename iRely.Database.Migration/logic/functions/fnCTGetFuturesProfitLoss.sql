...
-- changeset Von:fnCTGetFuturesProfitLoss.sql.1 runOnChange:true splitStatements:false
-- comment: RK-1234

CREATE OR ALTER FUNCTION [dbo].[fnCTGetFuturesProfitLoss]
(
	...
	SELECT	@dblProfitLoss	=
			SUM	(
					CASE	WHEN	TN.strBuySell = 'Sell' 
							THEN	(ISNULL(CS.dblHedgedLots,0) + ISNULL(CS.dblAssignedLots,0))*TN.dblPrice*MA.dblContractSize
							ELSE	(ISNULL(CS.dblHedgedLots,0) + ISNULL(CS.dblAssignedLots,0))*TN.dblPrice*MA.dblContractSize*-1
