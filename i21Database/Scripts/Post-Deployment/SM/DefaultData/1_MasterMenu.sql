...
GO
	...
	IF NOT EXISTS(SELECT TOP 1 1 FROM tblSMMasterMenu WHERE strMenuName = 'Cross Reference – Vendor Invoice Import' AND strModuleName = 'Transports' AND intParentMenuID = (SELECT TOP 1 intMenuID FROM tblSMMasterMenu WHERE strMenuName = 'Import' AND strModuleName = 'Transports') AND strCommand = N'Transports.view.CrossReferenceDtn') 
	BEGIN
		EXEC uspSMIncreaseECConcurrency 0
