...
CROSS APPLY (
	...
	WHERE 
		...
		REVERSE(SUBSTRING(REVERSE(SUBSTRING(A.strRoute, 1, (CASE WHEN CHARINDEX('?', A.strRoute) = 0 THEN LEN(A.strRoute) ELSE CHARINDEX('?', A.strRoute) END) - 1)), 1 , CHARINDEX('/', REVERSE(SUBSTRING(A.strRoute, 1, (CASE WHEN CHARINDEX('?', A.strRoute) = 0 THEN LEN(A.strRoute) ELSE CHARINDEX('?', A.strRoute) END) - 1))) - 1)) COLLATE Latin1_General_CI_AS AND ysnSearch = 0
) D
