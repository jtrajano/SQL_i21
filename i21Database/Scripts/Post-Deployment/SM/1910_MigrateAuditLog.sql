...
MERGE INTO tblSMLog USING (
	...
	WHERE ISNULL(B.intRecordId, '') <> '' AND ISNULL(A.strRecordNo, '') <> '' AND (ISNULL(ysnInit,'') = '' OR ysnInit = 0) AND B.intEntityId IS NOT NULL  --AND ISNULL(A.strJsonData, '') = '' AND ISNULL(A.strActionType, '') <> 'Updated'
