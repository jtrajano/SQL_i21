...
IF @planId IS NULL
BEGIN
	EXEC @ReturnCode = dbo.sp_add_maintenance_plan 
			@plan_name = 'i21_ReIndex_Maintenance_Plan' ,   
			@plan_id = @planId OUTPUT 
	IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback

	EXEC @ReturnCode = dbo.sp_add_maintenance_plan_db 
			@planId, 
			@currentDatabaseName
	IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
END
...
BEGIN
	...
	EXEC @ReturnCode = dbo.sp_add_maintenance_plan_job @planId, @jobId
	IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
BEGIN
	...
	IF @maxStepId > 1
	BEGIN
		UPDATE dbo.sysjobsteps set on_success_action = 3
