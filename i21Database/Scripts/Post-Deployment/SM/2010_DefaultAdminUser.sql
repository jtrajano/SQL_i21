...
begin transaction
	begin try
		IF NOT EXISTS(SELECT TOP 1 1 FROM tblEMEntity WHERE strName = 'IRELY ADMIN') AND (SELECT COUNT(*) FROM tblSMBuildNumber) = 1
		BEGIN
			--ENTITY HERE
			...
			--ENTITY LOCATION
			IF NOT EXISTS (SELECT TOP 1 1 FROM tblEMEntityLocation WHERE strLocationName = 'IRELY ADMIN') AND EXISTS (SELECT TOP 1 1 FROM tblEMEntity WHERE intEntityId = @intEntityId)
			...
			VALUES ( @intUserRoleId, @intCompanyLocationId, @intSecurityPolicyId, N'irelyadmin', N'', N'IRELY ADMIN',@intEntityId, 1)
