...
DECLARE @ver numeric(4,2)

SELECT @ver = CAST((left(cast(serverproperty('productversion') as varchar), 4)) AS numeric(4,2))  

--SQL2012 and up (Above R2)
IF @ver > 10.5
BEGIN
	EXEC 
	('
		ALTER FUNCTION [dbo].[fnRKGetBucketBasisDeliveries]
		(
			@dtmDate DATETIME,
			@intCommodityId INT,
			@intVendorId INT
