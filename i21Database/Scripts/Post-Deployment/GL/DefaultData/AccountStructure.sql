...
GO
--	SET IDENTITY_INSERT [dbo].[tblGLAccountStructure] ON
--GO
	IF NOT EXISTS(SELECT TOP 1 1 FROM tblGLAccountStructure WHERE strType = N'Primary')
	BEGIN
	...
	END
	IF NOT EXISTS(SELECT TOP 1 1 FROM tblGLAccountStructure WHERE strType = N'Divider' and strStructureName = N'Hypen/Separator')
	BEGIN
	...
	END
	IF NOT EXISTS(SELECT TOP 1 1 FROM tblGLAccountStructure WHERE strType = N'Segment' and strStructureName = N'Profit Center')
	BEGIN
	...
	END
GO
--	SET IDENTITY_INSERT [dbo].[tblGLAccountStructure] OFF
--GO
