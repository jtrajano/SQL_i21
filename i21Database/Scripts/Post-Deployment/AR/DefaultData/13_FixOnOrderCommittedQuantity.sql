	...
	FROM (
		...
		FROM (
			...
			LEFT JOIN (
				...
				INNER JOIN tblARInvoice I ON ID.intInvoiceId = I.intInvoiceId 
									    AND I.ysnPosted = 1
									    AND ID.intSalesOrderDetailId NOT IN (
			...
			LEFT JOIN (
				...
				INNER JOIN tblICInventoryShipment ISH ON ISHI.intInventoryShipmentId = ISH.intInventoryShipmentId AND ISH.ysnPosted = 1
			) ON SOD.intSalesOrderDetailId = ISHI.intLineNo
			WHERE dbo.fnIsStockTrackingItem(SOD.intItemId) = 1		  
			  AND SO.strTransactionType = 'Order'
