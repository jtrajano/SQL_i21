...
BEGIN
    EXEC ('
ALTER VIEW vyuCMCashProjection AS
WITH  WeekQuery AS(
    SELECT  
    DATEADD(wk, 0, DATEADD(DAY, 1-DATEPART(WEEKDAY, GETDATE()), DATEDIFF(dd, 0, GETDATE()))) FirstDayOfWeek,
    CAST(DATEPART (YEAR, GETDATE())AS NVARCHAR(4)) + RIGHT(''0'' + CAST(DATEPART (WEEK, GETDATE())AS NVARCHAR(3)),2) COLLATE Latin1_General_CI_AS WeekNo
    UNION SELECT 
    DATEADD(wk, -1, DATEADD(DAY, 1-DATEPART(WEEKDAY, GETDATE()), DATEDIFF(dd, 0, GETDATE()))) FirstDayOfWeek,
    CAST(DATEPART (YEAR, DATEADD( DAY, -7, GETDATE()))AS NVARCHAR(4)) + RIGHT(''0''+ CAST(DATEPART (WEEK, DATEADD( DAY, -7, GETDATE()))AS NVARCHAR(3)),2) COLLATE Latin1_General_CI_AS WeekNo
    UNION SELECT 
    DATEADD(wk, -2, DATEADD(DAY, 1-DATEPART(WEEKDAY, GETDATE()), DATEDIFF(dd, 0, GETDATE()))) FirstDayOfWeek,
    CAST(DATEPART (YEAR, DATEADD( DAY, -14, GETDATE()))AS NVARCHAR(4)) + RIGHT(''0'' + CAST(DATEPART (WEEK, DATEADD( DAY, -14, GETDATE()))AS NVARCHAR(3)),2) COLLATE Latin1_General_CI_AS WeekNo
    UNION SELECT 
    DATEADD(wk, -3, DATEADD(DAY, 1-DATEPART(WEEKDAY, GETDATE()), DATEDIFF(dd, 0, GETDATE()))) FirstDayOfWeek,
    CAST(DATEPART (YEAR, DATEADD( DAY, -21, GETDATE()))AS NVARCHAR(4)) + RIGHT(''0'' + CAST(DATEPART (WEEK, DATEADD( DAY, -21, GETDATE()))AS NVARCHAR(3)),2) COLLATE Latin1_General_CI_AS WeekNo
    UNION SELECT 
    DATEADD(wk, -4, DATEADD(DAY, 1-DATEPART(WEEKDAY, GETDATE()), DATEDIFF(dd, 0, GETDATE()))) FirstDayOfWeek,
    CAST(DATEPART (YEAR, DATEADD( DAY, -28, GETDATE()))AS NVARCHAR(4)) + RIGHT(''0'' + CAST(DATEPART (WEEK, DATEADD( DAY, -28, GETDATE()))AS NVARCHAR(3)),2) COLLATE Latin1_General_CI_AS WeekNo
),
BoundaryDate as(
