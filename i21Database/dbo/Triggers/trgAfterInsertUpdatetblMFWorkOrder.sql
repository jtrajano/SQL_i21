...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN
			...
			SET @tableHTML = N'<body><FONT COLOR="#4682B4"> ' + ' Inventory shortage for Work Order ' + @strReferenceNo + '</FONT>'
			SET @tableHTML = @tableHTML + N'<table border="1" BORDERCOLOR="#F0E68C">' + N'<tr bgcolor="#F0E68C"><FONT COLOR="#4682B4"><th>Item</th><th>Item Desc</th><th>Available Qty</th>' + N'<th>Required Qty</th></tr>' + CAST((
						SELECT td = I1.strItemNo
							,''
							,td = I1.strDescription
							,''
							,td = Convert(DECIMAL(38, 2), IsNull(Inv.dblQuantity, 0))
							,''
							,td = Convert(DECIMAL(38, 2), I.dblQuantity)
						FROM @tblMFItem I
