...
BEGIN
	...
	SELECT	
		...
		[dblExchangeRate]				=	ISNULL(NULLIF(Details.dblRate,0),1), -- 1,
	...
	SELECT	
		[dtmDate]						=	DATEADD(dd, DATEDIFF(dd, 0, A.dtmDate), 0),
		[strBatchID]					=	@batchId,
		[intAccountId]					=	CASE WHEN ((B.intInventoryReceiptItemId IS NOT NULL OR ISNULL(receiptItem.intOwnershipType,0) <> 2) OR B.intInventoryReceiptChargeId IS NOT NULL)
