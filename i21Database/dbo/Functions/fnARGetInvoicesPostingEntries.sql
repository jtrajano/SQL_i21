CREATE FUNCTION [dbo].[fnARGetInvoicesPostingEntries]
(
...
DECLARE @TempInvoices AS [dbo].[InvoicePostingTable]
INSERT INTO @TempInvoices
SELECT * FROM @Invoices WHERE ISNULL([intPeriodsToAccrue], 0) > 0

WHILE EXISTS(SELECT NULL FROM @TempInvoices I LEFT OUTER JOIN @GLEntries G ON I.[intInvoiceId] = G.intTransactionId WHERE ISNULL(G.intTransactionId,0) = 0)
BEGIN
	...
	SELECT TOP 1 
		...
		,@AccrueLicense				= I.[ysnAccrueLicense]
		,@AccrualPeriod				= I.[intPeriodsToAccrue]
		,@DeferredRevenueAccountId	= I.[intDeferredRevenueAccountId]
		,@BatchId					= I.[strBatchId]
		,@UserId					= I.[intUserId]
		,@CompanyLocationId			= I.[intCompanyLocationId]
	FROM
		@TempInvoices I 
	LEFT OUTER JOIN 
		@GLEntries G 
			ON I.[intInvoiceId] = G.intTransactionId WHERE ISNULL(G.intTransactionId,0) = 0
	...
	SELECT
		...
		,dblDebitUnit				= CASE WHEN A.strTransactionType  IN ('Invoice', 'Cash')	THEN  
																						(
																						SELECT
																							SUM(ISNULL([dbo].[fnCalculateQtyBetweenUOM](ARID.intItemUOMId, ICIS.intItemUOMId, ARID.dblQtyShipped),ISNULL(ARID.dblQtyShipped, @ZeroDecimal)))
																						FROM
																							tblARInvoiceDetail ARID WITH (NOLOCK)
																						LEFT OUTER JOIN
																							(	SELECT ICIU.[intItemId], ICIU.[intItemUOMId] 
																								FROM
																									tblICItemUOM ICIU WITH (NOLOCK) 
																								INNER JOIN
																									(SELECT [intUnitMeasureId] FROM tblICUnitMeasure WITH (NOLOCK)) ICUM
																										ON ICIU.[intUnitMeasureId] = ICUM.[intUnitMeasureId]
																								WHERE
																									ISNULL(ICIU.[ysnStockUnit],0) = 1
																							) ICIS
																								ON ARID.[intItemId] = ICIS.[intItemId]
																						WHERE
																							ARID.intInvoiceId = A.intInvoiceId
	...
	SELECT
		...
		,[dblForeignRate]			= 0
		,[strRateType]				= ''
		,[intARAccountId]				= @DeferredRevenueAccountId
		,[intCOGSAccountId]				= NULL
		,[intSalesAccountId]			= NULL
		,[intInventoryAccountId]		= NULL
		,[intServiceChargeAccountId]	= NULL
...
SELECT
	...
	,dblUnits				= ISNULL([dbo].[fnCalculateQtyBetweenUOM](ARID.intItemUOMId, ICIS.intItemUOMId, ARID.dblQtyShipped),ISNULL(ARID.dblQtyShipped, @ZeroDecimal))
...
FROM
	tblARInvoiceDetail ARID
INNER JOIN
	 (SELECT [intInvoiceId], [strTransactionType] FROM tblARInvoice WITH (NOLOCK)) ARI
...
BEGIN
	...
	BEGIN
		...
		SELECT
			...
			,[dblForeignRate]			= B.dblCurrencyExchangeRate
			,[strRateType]				= SMCERT.strCurrencyExchangeRateType
			,[intARAccountId]				= IST.intGeneralAccountId
			,[intCOGSAccountId]				= NULL
			,[intSalesAccountId]			= NULL
			,[intInventoryAccountId]		= NULL
			,[intServiceChargeAccountId]	= NULL
		...
		SELECT
			...
			,[dblForeignRate]			= B.dblCurrencyExchangeRate
			,[strRateType]				= SMCERT.strCurrencyExchangeRateType
			,[intARAccountId]				= IST.intMaintenanceSalesAccountId
			,[intCOGSAccountId]				= NULL
			,[intSalesAccountId]			= NULL
			,[intInventoryAccountId]		= NULL
			,[intServiceChargeAccountId]	= NULL
...
END
	...
	SELECT
		...
		,[dblForeignRate]			= D.dblCurrencyExchangeRate
		,[strRateType]				= SMCERT.strCurrencyExchangeRateType
		,[intARAccountId]				= ISNULL(DT.intSalesTaxAccountId,TC.intSalesTaxAccountId)
		,[intCOGSAccountId]				= NULL
		,[intSalesAccountId]			= NULL
		,[intInventoryAccountId]		= NULL
		,[intServiceChargeAccountId]	= NULL
