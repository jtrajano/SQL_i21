CREATE FUNCTION dbo.fnICGetStartingNumber (
...
BEGIN
		SELECT	top 1 @strTransactionNumber = strPrefix + str(CONVERT(float, GETDATE()),16,15)
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				SELECT	@strTransactionNumber = @locationNumber + strPrefix + @parameters + CASE WHEN @digit = 0 THEN CAST(location.intNumber AS NVARCHAR(20)) ELSE RIGHT(@padding + CAST(ISNULL(location.intNumber, 1) AS NVARCHAR(20)), @digit) END --CAST(location.intNumber AS NVARCHAR(20))
			...
			BEGIN
				...
				SELECT	@strTransactionNumber = @locationNumber + strPrefix + @parameters + CASE WHEN @digit = 0 THEN CAST(1 AS NVARCHAR(20)) ELSE RIGHT(@padding + CAST(1 AS NVARCHAR(20)), @digit) END--CAST(1 AS NVARCHAR(20))
	...
	BEGIN
		---- Assemble the string ID. 
		--SELECT	@strTransactionNumber = @locationNumber + strPrefix + @parameters + CASE WHEN @digit = 0 THEN CAST(intNumber AS NVARCHAR(20)) ELSE RIGHT(@padding + CAST(ISNULL(intNumber, 1) AS NVARCHAR(20)), @digit) END--CAST(intNumber AS NVARCHAR(20))
		--FROM	tblSMStartingNumber
		--WHERE	intStartingNumberId = @intStartingNumberId
...
IF @strTransactionNumber IS NULL 
BEGIN 
