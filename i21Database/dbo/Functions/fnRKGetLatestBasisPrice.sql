...
RETURNS @returntable TABLE (dblBasisOrDiscount NUMERIC(18, 6)
...
BEGIN
	...
	WITH cte AS (
		SELECT t.intM2MBasisId
			, t.dtmM2MBasisDate
			, ROW_NUMBER() OVER (ORDER BY ABS(DATEDIFF(dd, t.dtmM2MBasisDate, @dtmCurrentDate))) AS rowNum
	...
	WITH cte AS (
		SELECT t.intFutureMonthId
			...
			, ROW_NUMBER() OVER (ORDER BY ABS(DATEDIFF(dd, t.dtmFutureMonthsDate, @dtmCurrentDate))) AS rowNum
	...
	INSERT INTO @returntable (dblBasisOrDiscount
		, intUnitMeasureId
		, strUnitMeasure
		, intItemUOMId)
