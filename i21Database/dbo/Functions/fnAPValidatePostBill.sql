...
BEGIN
	...
	BEGIN
		...
		(
			...
			WHERE 
				B.intBillId IN (SELECT intBillId FROM @tmpBills) 
			AND B.intTransactionId = A.intBillId AND B.ysnApplied = 1
			AND 
			(
			...
			OR	(C.dblAmountDue = 0 AND C.ysnPosted = 1) --VPRE/DM is fully applied
