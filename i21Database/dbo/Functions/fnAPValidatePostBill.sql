...
BEGIN
	...
	BEGIN
		...
		AND A.ysnFinalVoucher = 0 AND A.intTransactionType != 16 -- Exclude Finalize Voucher and Provisional voucher
		...
		(
			--amount due should be total less payment
			(A.dblAmountDue - dblProvisionalTotal != (A.dblTotal - (A.dblPayment + A.dblProvisionalTotal)))
			OR
			--amount due cannot be greater than the total
			((A.dblAmountDue - A.dblProvisionalTotal) > (A.dblTotal - A.dblProvisionalTotal))
			OR
			--amount due cannot be negative
			(A.dblAmountDue - A.dblProvisionalTotal < 0)
		)
		AND A.ysnFinalVoucher = 1 AND A.intTransactionType = 1
		...
		(
			--amount due should be total less payment
			(A.dblProvisionalAmountDue != (A.dblProvisionalTotal - A.dblPayment))
			OR
			--amount due cannot be greater than the total
			(A.dblProvisionalAmountDue > A.dblProvisionalTotal)
			OR
			--amount due cannot be negative
			(A.dblProvisionalAmountDue < 0)
		)
