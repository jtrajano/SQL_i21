...
BEGIN
	...
	BEGIN
			...
			IF EXISTS(SELECT TOP 1 1 FROM tblGLFiscalYear WHERE dtmDateFrom = @dtmDate) 
			INSERT @tbl
				SELECT @strAccountId, 0, 0
			ELSE
			BEGIN
			    SELECT @dtmDate = DATEADD(SECOND, -1, @dtmDate)
				INSERT  @tbl
				SELECT  
						strAccountId,
						SUM ( 
				...
				FROM tblGLAccount A
					...
					CROSS APPLY (SELECT dtmDateFrom,dtmDateTo from tblGLFiscalYear where @dtmDate between dtmDateFrom AND dtmDateTo) D
