...
FROM vyuRKGetSummaryLog sl
WHERE strBucketType = 'Customer Owned'
AND CAST(sl.dtmTransactionDate AS DATE) <= @dtmDate
AND (
	(@intCommodityId IS NULL)
	OR
	(@intCommodityId IS NOT NULL 
		AND sl.intCommodityId IS NOT NULL 
		AND sl.intCommodityId = @intCommodityId)
)
AND (
	(@intVendorId IS NULL)
	OR
	(@intVendorId IS NOT NULL 
		AND sl.intEntityId IS NOT NULL 
		AND sl.intEntityId = @intVendorId)
)
