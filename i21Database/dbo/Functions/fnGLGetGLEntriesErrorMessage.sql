...
BEGIN
	...
	SELECT 60008 ,'You cannot %s transactions you did not create. Please contact your local administrator.'  UNION ALL
	SELECT 60009 ,'Unable to find an open fiscal year period for %s module to match the transaction date.'  UNION ALL
	SELECT 60010 ,'Unable to recalculate summary. General Ledger Detail has out of balance transactions.'  UNION ALL
	SELECT 60011 ,'Fiscal period for %s is %s'  UNION ALL
	SELECT 60012 ,'Fiscal period of reverse date for %s is %s'  UNION ALL
	SELECT 60013 ,'Foreign debit and credit amounts are not balanced.'  UNION ALL
	SELECT 60014 ,'%s is linked to an Origin Account which is not allowed to be deleted.'  UNION ALL
	SELECT 60015 ,'Unable to post/unpost with an inactive account %s' UNION ALL
