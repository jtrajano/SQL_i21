CREATE FUNCTION dbo.fnICGetItemCostByEffectiveDate(@dtmDate DATETIME, @intItemId INT, @intItemLocationId INT, @ysnGetDefault BIT = 1)
RETURNS TABLE
AS
...
OUTER APPLY (
	...
	HAVING dtmEffectiveCostDate <= @dtmDate
	ORDER BY dtmEffectiveCostDate DESC
) matchedCost
OUTER APPLY (
	SELECT TOP 1 dblLastCost dblCost, @dtmDate dtmEffectiveCostDate
