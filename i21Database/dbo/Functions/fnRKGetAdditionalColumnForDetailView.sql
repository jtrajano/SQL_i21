CREATE FUNCTION [dbo].[fnRKGetAdditionalColumnForDetailView]
(
...
BEGIN
	...
	BEGIN
		...
		SELECT	intSeqCurrencyId			 = @intSeqCurrencyId,
				ysnSeqSubCurrency			 = 0,
				intSeqPriceUOMId			 = @intFXPriceUOMId,
				dblSeqPrice					 = @dblCashPrice, --dbo.fnCTConvertQtyToTargetItemUOM(@intFXPriceUOMId,@intPriceItemUOMId,@dblMainCashPrice) * @dblRate,
				strSeqCurrency				 = @strSeqCurrency,
				strSeqPriceUOM				 = @strFXPriceUOM,
				dblQtyToPriceUOMConvFactor	 = dbo.fnCTConvertQtyToTargetItemUOM(@intItemUOMId,@intFXPriceUOMId,1),
				dblNetWtToPriceUOMConvFactor = dbo.fnCTConvertQtyToTargetItemUOM(@intNetWeightUOMId,@intFXPriceUOMId,1),
				dblCostUnitQty				 = @dblFXCostUnitQty,
				dblSeqBasis					 = dbo.fnCTConvertQtyToTargetItemUOM(@intFXPriceUOMId,@intBasisUOMId,@dblMainBasis) * @dblRate,
				intSeqBasisCurrencyId		 = @intSeqCurrencyId,
				intSeqBasisUOMId			 = @intFXPriceUOMId,
				ysnValidFX					 = 1,
				dblSeqFutures				 = dbo.fnCTConvertQtyToTargetItemUOM(@intFXPriceUOMId,@intPriceItemUOMId,@dblMainFutures) * @dblRate
	END
	ELSE
	BEGIN
		INSERT INTO @returntable
		SELECT	
				intSeqCurrencyId			 = @intCurrencyId,
				ysnSeqSubCurrency			 = @ysnSubCurrency,
				intSeqPriceUOMId			 = @intPriceItemUOMId,
				dblSeqPrice					 = @dblCashPrice,
				strSeqCurrency				 = @strCurrency,
				strSeqPriceUOM				 = @strPriceUOM,
				dblQtyToPriceUOMConvFactor	 = dbo.fnCTConvertQtyToTargetItemUOM(@intItemUOMId,@intPriceItemUOMId,1),
				dblNetWtToPriceUOMConvFactor = dbo.fnCTConvertQtyToTargetItemUOM(@intNetWeightUOMId,@intPriceItemUOMId,1),
				dblCostUnitQty				 = @dblCostUnitQty,
				dblSeqBasis					 = @dblBasis,
				intSeqBasisCurrencyId		 = @intBasisCurrencyId,
				intSeqBasisUOMId			 = @intBasisUOMId,
				ysnValidFX					 = 0,
				dblSeqFutures				 = @dblFutures
	END
