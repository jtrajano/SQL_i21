...
BEGIN
	...
	SELECT 
		...
		+ SPACE(4)
		+ '1'
		+ ISNULL(LEFT(REPLACE(A.strFederalTaxId, '-', ''),9),SPACE(9)) -- 12-20
		+ dbo.fnTrimX(A.strVendorId) + SPACE(20 - LEN(dbo.fnTrimX(A.strVendorId))) -- 21-40
		+ SPACE(4) -- 41-44
		+ SPACE(10) -- 45-54
		+ CASE WHEN ISNULL(A.dbl1099INT,0) > @maxAmount 
				THEN REPLICATE('0',10 - LEN(CAST(FLOOR((@maxAmount - CAST(ISNULL(A.dbl1099INT,0) AS DECIMAL(18,2)))) AS NVARCHAR(100))))
					+ CAST(FLOOR((@maxAmount - CAST(ISNULL(A.dbl1099INT,0) AS DECIMAL(18,2)))) AS NVARCHAR(100))
					+ CAST(PARSENAME(CAST(ISNULL(A.dbl1099INT,0) AS DECIMAL(18,2)),1) AS NVARCHAR(2))
				ELSE 
					REPLICATE('0',10 - LEN(CAST(FLOOR((ISNULL(A.dbl1099INT,0))) AS NVARCHAR(100))))
