...
DECLARE @BANK_DEPOSIT INT = 1
		...
		,@BROKER_SETTLEMENT AS INT = 25
		,@BROKER_COMMISSION AS INT = 26
		,@NSF INT = 124
...
WHERE	A.ysnPosted = 1
		...
		AND A.intBankTransactionTypeId IN (@BANK_TRANSACTION, @BANK_WITHDRAWAL,@NSF, @BANK_INTEREST, @BANK_LOAN, @BROKER_SETTLEMENT, @BROKER_COMMISSION)
...
WHERE	ysnPosted = 1
		...
		AND intBankAccountId = @intBankAccountId
		AND CAST(FLOOR(CAST(dtmDate AS FLOAT)) AS DATETIME) <= CAST(FLOOR(CAST(ISNULL(@dtmDate,dtmDate) AS FLOAT)) AS DATETIME)		
		AND intBankTransactionTypeId NOT IN (@MISC_CHECKS, @BANK_TRANSFER_WD, @BANK_TRANSACTION, @BANK_WITHDRAWAL, @NSF, @BANK_INTEREST,@BANK_LOAN, @ORIGIN_CHECKS, @ORIGIN_EFT, @ORIGIN_WITHDRAWAL, @ORIGIN_WIRE, @AP_PAYMENT, @AP_ECHECK, @PAYCHECK, @DIRECT_DEPOSIT, @ACH, @BROKER_SETTLEMENT, @BROKER_COMMISSION)
