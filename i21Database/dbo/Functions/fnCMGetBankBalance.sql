...
DECLARE @BANK_DEPOSIT INT = 1
		...
		,@BANK_FEES AS INT = 27
		,@TREASURY_STOCK_SALE AS INT = 28
		,@TREASURY_STOCK_PURCHASE AS INT = 29
		,@NSF INT = 124
...
WHERE	ysnPosted = 1
		...
		AND intBankAccountId = @intBankAccountId
		AND CAST(FLOOR(CAST(dtmDate AS FLOAT)) AS DATETIME) <= CAST(FLOOR(CAST(ISNULL(@dtmDate,dtmDate) AS FLOAT)) AS DATETIME)		
		AND intBankTransactionTypeId NOT IN (
			@MISC_CHECKS, @BANK_TRANSFER_WD, @BANK_TRANSACTION, 
			@BANK_WITHDRAWAL, @NSF, @BANK_INTEREST,@BANK_LOAN, 
			@ORIGIN_CHECKS, @ORIGIN_EFT, @ORIGIN_WITHDRAWAL, 
			@ORIGIN_WIRE, @AP_PAYMENT, @AP_ECHECK, @PAYCHECK, 
			@DIRECT_DEPOSIT, @ACH, @BROKER_SETTLEMENT, 
			@BROKER_COMMISSION,@BANK_FEES, @TREASURY_STOCK_PURCHASE)
