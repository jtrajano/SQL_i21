...
Inventory AS (
	SELECT 
		SUM((dbo.fnCTConvertQuantityToTargetCommodityUOM(intFromCommodityUnitMeasureId,intDefaultCommodityUnitMeasureId,isnull(dblTotal ,0)))) dblTotal
		,intCommodityId
		,strCommodityCode
		,intDefaultUOMId
	...
	FROM (
		SELECT 
			sum(isnull(s.dblQuantity ,0)) as dblTotal
...
(
	...
	WHERE CD.intContractStatusId <> 6
...
--In-House
SELECT 
	 strCommodity as strCommodityCode
	,strStockUOM as strUnitMeasure
	,'In-House' as strSeqHeader
	,InHouse as dblTotal 
	,intCommodityId
FROM InHouse A
WHERE InHouse <> 0
...
SELECT 
	strCommodityCode 
	,strUnitMeasure
	,'Company Titled Stock' as strSeqHeader
	,sum(dblTotal) as dblTotal 
	,intCommodityId
FROM (
	SELECT strCommodityCode, strUnitMeasure, CASE WHEN (SELECT ysnIncludeDPPurchasesInCompanyTitled FROM tblRKCompanyPreference) = 0 THEN -SUM(Balance) ELSE 0 END as dblTotal, intCommodityId FROM Storage WHERE ysnDPOwnedType = 1 group by strCommodityCode, strUnitMeasure, intCommodityId
	UNION ALL
	SELECT strCommodityCode,strDefaultUOM, sum(dblTotal) as dblTotal, intCommodityId FROM Inventory group by strCommodityCode,strDefaultUOM,intCommodityId
) t 
