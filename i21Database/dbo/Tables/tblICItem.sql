...
*/
	CREATE TABLE [dbo].[tblICItem] (
		...
		[ysnSupported] BIT NULL DEFAULT((1)),
		[ysnDisplayInHelpdesk] BIT NULL DEFAULT((1)),
		...
		[intCommodityAttributeId1] INT NULL,
		[intCommodityAttributeId2] INT NULL,
		...
		CONSTRAINT [FK_tblICItem_Owner] FOREIGN KEY ([intOwnerId]) REFERENCES [tblARCustomer]([intEntityId]),
		CONSTRAINT [FK_tblICItem_Customer] FOREIGN KEY ([intCustomerId]) REFERENCES [tblARCustomer]([intEntityId]),
	...
	CREATE NONCLUSTERED INDEX [IX_tblICItem_strType]
		...
		INCLUDE ([strItemNo], [strDescription], [intCategoryId], [intCommodityId])
	GO
