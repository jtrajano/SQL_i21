...
CREATE TRIGGER trg_tblGLAccountSegment
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        JOIN inserted i on i.intAccountSegmentId = SM.intAccountSegmentId
        JOIN tblGLAccountStructure STRUC  ON STRUC.intAccountStructureId = i.intAccountStructureId
        WHERE STRUC.strType = 'Primary' 
    )
	BEGIN
        UPDATE p
        SET [strCode] = i.strCode
            ,[strDescription] = i.strDescription
            ,[strChartDesc] = i.strChartDesc
            ,[intAccountStructureId] = i.intAccountStructureId
            ,[intAccountGroupId] = i.intAccountGroupId
            ,[ysnActive] = i.ysnActive
            ,[ysnSelected] = i.ysnSelected
            ,[ysnIsNotExisting] = i.ysnIsNotExisting
            ,[intConcurrencyId] = i.intConcurrencyId
            ,[intAccountCategoryId] = i.intAccountCategoryId
            ,[intEntityIdLastModified] = i.intEntityIdLastModified
        FROM [dbo].[tblGLAccountSegment] p
        INNER JOIN inserted i
			ON i.intAccountSegmentId = p.intAccountSegmentId
	END
	ELSE
    BEGIN
       IF EXISTS(SELECT TOP 1 1 FROM inserted i JOIN tblGLAccountCategory C ON C.intAccountCategoryId = i.intAccountCategoryId AND strAccountCategory <> 'Cash Account' )
       BEGIN
       DECLARE @segment NVARCHAR(20)
       DECLARE @category NVARCHAR(40)
            SELECT TOP 1  @segment = strCode , @category = strAccountCategory FROM tblGLAccountCategory C JOIN inserted i on i.intAccountCategoryId = C.intAccountCategoryId
            RAISERROR('Cannot update category to %s. Segment %s is used for bank account',16,1,@category,@segment)		
        END
    END
