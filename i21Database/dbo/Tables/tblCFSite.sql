CREATE TABLE [dbo].[tblCFSite] (
    ...
    [strAllowExemptionsOnExtAndRetailTrans] NVARCHAR (100) COLLATE Latin1_General_CI_AS NULL,
    ...
    CONSTRAINT [PK_tblCFSiteLocation] PRIMARY KEY CLUSTERED ([intSiteId] ASC) WITH (FILLFACTOR = 70),
...
CREATE NONCLUSTERED INDEX [IX_tblCFSite_intSiteId]
    ON [dbo].[tblCFSite]([intSiteId] ASC) WITH (FILLFACTOR = 70);
GO

CREATE NONCLUSTERED INDEX [tblCFSite_intCardId]
    ON [dbo].[tblCFSite]([intCardId] ASC) WITH (FILLFACTOR = 70);
GO

CREATE NONCLUSTERED INDEX [tblCFSite_intNetworkId]
    ON [dbo].[tblCFSite]([intNetworkId] ASC) WITH (FILLFACTOR = 70);
GO
...
CREATE NONCLUSTERED INDEX [tblCFSite_strType_include_intSiteId_intARLocation]
    ON [dbo].[tblCFSite]([strSiteType] ASC)
    INCLUDE([intSiteId], [intARLocationId]);
GO

CREATE UNIQUE NONCLUSTERED INDEX [tblCFSite_UniqueNetworkSiteNumber]
    ON [dbo].[tblCFSite]([intNetworkId] ASC, [strSiteNumber] ASC) WITH (FILLFACTOR = 70);
GO
