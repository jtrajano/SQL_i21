...
BEGIN TRY
	...
	IF @strLotTracking = 'No'
		BEGIN
			...
			SELECT @dblUpperToleranceQuantity	= CASE WHEN dblCalculatedUpperTolerance = 0 THEN @dblPlannedQuantity
			...
			IF @dblQtyToProduce > @dblUpperToleranceQuantity AND @dblUpperTolerance > 0
				BEGIN
					SET @strQtyToProduce = [dbo].[fnRemoveTrailingZeroes]( @dblQtyToProduce);

					SET @strUpperToleranceQuantity = [dbo].[fnRemoveTrailingZeroes](@dblUpperToleranceQuantity);
			...
			IF @dblLowerToleranceQuantity > @dblQtyToProduce
				BEGIN
					...
					SET @strLowerToleranceQuantity = [dbo].[fnRemoveTrailingZeroes](@dblLowerToleranceQuantity);
	...
	ELSE
		BEGIN
			...
			INSERT INTO @tblQuantityBreakup
			(
				strParentLotNumber
			  , strLotNumber
			  , dblQuantity
