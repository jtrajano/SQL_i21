CREATE PROCEDURE [dbo].[uspMFGetYieldView] 
(
	@strXML NVARCHAR(MAX)
)
AS
BEGIN TRY
	...
	SELECT @strMode						= strMode
		 ...
		 , @ysnDetailResult				= ysnDetailResult
	...
	IF @strMode = 'Run' AND @ysnIncludeIngredientItem = 0
		BEGIN
				...
				BEGIN
					...
					WHILE @intItemId IS NOT NULL
						BEGIN
							...
							WHERE intWorkOrderId = @intWorkOrderId AND strTransactionType = 'Input'
							...
							SET @dblYieldP = @dblTOutput / CASE WHEN ISNULL(@dblTInput, 0) = 0 THEN 1
																ELSE @dblTInput
														   END;
			...
			IF (@ysnDetailResult = 0)
				BEGIN	
					...
					FROM (SELECT @dtmFromDate	AS dtmFromDate
						   , @dtmToDate		AS dtmToDate
						   , @intManufacturingProcessId AS intManufacturingProcessId
						   , @intLocationId AS intLocationId
						   , ROUND(SUM(dblTotalOutput), @dblDecimal)	AS dblTotalOutput
						   , ROUND(MIN(dblTotalInput), @dblDecimal)		AS dblTotalInput
						   , ROUND(ABS(MIN(dblTotalInput) - SUM(dblTotalOutput)), @dblDecimal) dblDifference
						   , ROUND(AVG(dblActualYield), @dblDecimal)	AS dblActualYield
						   , intWorkOrderId
					FROM ##tblMFYield
			...
			ELSE
				BEGIN
					SELECT CONVERT(INT, ROW_NUMBER() OVER (ORDER BY Y.dtmDate )) AS intRowId
					 , @intManufacturingProcessId AS intManufacturingProcessId
					 , I.strItemNo
					 , I.strDescription
	...
	IF @strMode = 'Run' AND @ysnIncludeIngredientItem = 1
		BEGIN
			...
			ELSE
				BEGIN
					SELECT CONVERT(INT, ROW_NUMBER() OVER (ORDER BY IY.intWorkOrderId 
															  , IY.dtmDate
															  , IY.intShiftId
															  , (CASE WHEN intRecipeItemTypeId = 2 THEN 'Output'
																	  ELSE 'Input'
																 END))) AS intRowId
						  , @intManufacturingProcessId AS intManufacturingProcessId
						  , I.strItemNo
	...
	IF @strMode = 'Date' AND @ysnIncludeIngredientItem = 0
		BEGIN
	...
	IF @strMode = 'Date' AND @ysnIncludeIngredientItem = 1
		BEGIN
