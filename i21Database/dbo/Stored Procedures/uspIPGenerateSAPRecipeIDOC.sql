...
BEGIN
	...
	BEGIN
		...
		SELECT @strHeaderXML = '<ROOT><CTRL_POINT>' + '<DOC_NO>' + Ltrim(@intRecipeStageId) + '</DOC_NO>' + '<MSG_TYPE>' + Ltrim(strTransactionType) + '</MSG_TYPE>' + '<SENDER>i21</SENDER>' + '<RECEIVER>ERP</RECEIVER>' + '</CTRL_POINT>' + '<HEADER>' + '<RECIPE_NAME>' + IsNULL(strRecipeName, '') + '</RECIPE_NAME>' + '<LOCATION_NO>' + IsNULL(strLocationName, '') + '</LOCATION_NO>' + '<ITEM_NO>' + strItemNo + '</ITEM_NO>' + '<QUANTITY>' + strQuantity + '</QUANTITY>' + '<QUANTITY_UOM>' + strUOM + '</QUANTITY_UOM>' + '<VERSION>' + strVersionNo + '</VERSION>' + '<VALID_FROM>' + IsNULL(Convert(CHAR, dtmValidFrom, 112), '') + '</VALID_FROM>' + '<VALID_TO>' + IsNULL(Convert(CHAR, dtmValidTo, 112), '') + '</VALID_TO>' + '<PROCESS_NAME>' + IsNULL(strManufacturingProcess, '') + '</PROCESS_NAME>' + '<CREATE_DATE>' + IsNULL(Convert(CHAR, dtmCreated, 112), '') + '</CREATE_DATE>' + '<CREATED_BY>' + IsNULL(strCreatedBy, '') + '</CREATED_BY>' + '<TRACKING_NO>' + IsNULL(strSessionId, '') + '</TRACKING_NO>' + '</HEADER>'
		FROM dbo.tblMFRecipeStage
