...
BEGIN
	...
	SELECT *
	INTO #tempInventoryStock
	FROM @InventoryStock
			
	INSERT INTO #tempInventoryStock(strCommodityCode
		, strItemNo
		, dblTotal
		, strLocationName
		, intCommodityId
	...
	-- SHOULD CREATE INDEX AFTER INSERT OF RECORDS.
	CREATE NONCLUSTERED INDEX IX_tempInventoryStock_strInventoryType
	ON #tempInventoryStock (strInventoryType);
	...
	FROM (
		select
				dtmDate = Inv.dtmDate --  CONVERT(DATETIME, CONVERT(VARCHAR(10),Inv.dtmDate, 110), 110)
	...
	FROM (
		select
				dtmDate = I.dtmPostDate -- CONVERT(DATETIME, CONVERT(VARCHAR(10),I.dtmPostDate, 110), 110)
			,dblQtyShipped = Inv.dblTotal
			,I.strInvoiceNumber
			,I.intInvoiceId
			,Inv.strTransactionType
	...
	SELECT
		...
		,strTransactionType
	FROM (
		SELECT DISTINCT dtmDate = Inv.dtmDate -- CONVERT(DATETIME, CONVERT(VARCHAR(10),Inv.dtmDate, 110), 110)
	...
	BEGIN
		...
		FROM #tempInventoryStock 
		WHERE dtmDate <= @date -- CONVERT(DATETIME, CONVERT(VARCHAR(10), dtmDate, 110), 110) <= CONVERT(DATETIME, @date)
