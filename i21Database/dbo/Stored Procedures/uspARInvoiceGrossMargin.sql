CREATE PROCEDURE [dbo].[uspARInvoiceGrossMargin]
...
WITH Query AS (  
    ...
    FROM (  
        ...
        SELECT strTransactionType	= ARI.strTransactionType  
        ...
        INNER JOIN @InvoiceId II ON ARI.intInvoiceId = II.intHeaderId
...
totalQuery AS (  
    SELECT dtmDate  
		 , intInvoiceId
         , dblExpense	= SUM(dblTotalCost)
		 , dblNet		= SUM(A.dblTotal) - SUM( dblTotalCost)
		 , dblRevenue	= SUM(A.dblTotal)
    FROM Query A   
    GROUP BY dtmDate, intInvoiceId
)  

SELECT intInvoiceId		= MAIN.intInvoiceId
     , strType			= MAIN.strType 
	 , dblAmount		= SUM(MAIN.dblAmount) * CASE WHEN II.ysnPost = 1 THEN 1 ELSE - 1 END
	 , dtmDate			= CASE WHEN II.ysnPost = 1 THEN MAIN.dtmDate ELSE '01-01-1900'END
FROM (
	SELECT intInvoiceId
		 , strType		= 'Expense'
		 , dblAmount	= dblExpense
		 , dtmDate  
	FROM totalQuery
...
INNER JOIN @InvoiceId II ON MAIN.intInvoiceId = II.intHeaderId
GROUP BY MAIN.strType
  	   , MAIN.dtmDate
       , MAIN.intInvoiceId
	   , II.ysnPost