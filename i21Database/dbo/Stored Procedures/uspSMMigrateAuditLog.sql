...
DECLARE @TOPROCESSED INT = (SELECT COUNT(intAuditLogId) FROM tblSMAuditLog WHERE strRecordNo = CAST(@keyValue as nvarchar(10))  
 ...
 (      
  ...
  --LEFT OUTER JOIN  tblSMTransaction B ON A.intScreenId = B.intScreenId AND CAST(A.strRecordNo AS INT) = B.intRecordId    
 WHERE ISNULL(A.ysnProcessed, 0) = 0 AND    
  ISNULL(A.strRecordNo, '') = cast(@keyValue as nvarchar(10)) AND     
  ISNULL(A.strNamespace, '') = @screenName AND     
  ISNULL(A.strJsonData, '') LIKE '%}' AND     
  ISNULL(A.strActionType, '') NOT IN ('Created','Deleted')    
END    
