CREATE PROCEDURE [dbo].[uspCTPreSaveContract]		
	...
	@xmlDWG					NVARCHAR(MAX),
	@xmlDeletedContracts	NVARCHAR(MAX),
	@xmlPContractHeaders	NVARCHAR(MAX),
	@xmlVContractDetails	NVARCHAR(MAX)
...
BEGIN TRY
	...
	BEGIN
		...
		SELECT  ROW_NUMBER() OVER(ORDER BY intContractDetailId) intUniqueId,* 
		INTO	#ProcessDWG
		FROM	OPENXML(@idoc,'modifiedContractDetails/modifiedContractDetail',2)          
		WITH	(intContractHeaderId INT,intContractDetailId INT,intContractStatusId INT)
	
		SELECT @intUniqueId = MIN(intUniqueId) FROM #ProcessDWG

		WHILE ISNULL(@intUniqueId,0) > 0
		BEGIN
			SELECT	@_dwgContractHeaderId	=	intContractHeaderId,
			...
			DECLARE @intShortCloseStatusId INT
			...
			BEGIN
				...
				SELECT TOP 1 @_gradeId = intGradeId, @_weightId = intWeightId FROM tblCTContractHeader WHERE intContractHeaderId = @_dwgContractHeaderId

				IF @_gradeId IS NOT NULL AND @_weightId IS NOT NULL
				BEGIN		
					DECLARE @wgId INT
					SELECT TOP 1 @wgId = intWeightGradeId FROM tblCTWeightGrade WHERE strWhereFinalized = 'Destination'
					IF @wgId > 0 AND (@_gradeId = @wgId OR @_weightId = @wgId)
					BEGIN			
			...
			SELECT @intUniqueId = MIN(intUniqueId) FROM #ProcessDWG WHERE intUniqueId > @intUniqueId
		END	
	...
	-----------------------------------------------------------------------
	----------  P R O C E S S  D E L E T E D  C O N T R A C T S  ----------
	-----------------------------------------------------------------------
	IF @xmlDeletedContracts <> ''
	BEGIN

		DECLARE @_pdcContractHeaderId		INT,
				@_pdcUserId					INT
				
		SET @strProcess = 'Delete Contracts'
		EXEC sp_xml_preparedocument @idoc OUTPUT, @xmlDeletedContracts

		IF OBJECT_ID('tempdb..#ProcessDeleteContracts') IS NOT NULL  	
			DROP TABLE #ProcessDeleteContracts

		SELECT  ROW_NUMBER() OVER(ORDER BY intContractHeaderId) intUniqueId,* 
		INTO	#ProcessDeleteContracts
		FROM	OPENXML(@idoc,'deletedContracts/deletedContract',2)
		WITH	(intContractHeaderId INT,intUserId INT)
	
		SELECT @intUniqueId = MIN(intUniqueId) FROM #ProcessDeleteContracts

		WHILE ISNULL(@intUniqueId,0) > 0
		BEGIN
			SELECT	@_pdcContractHeaderId	=	intContractHeaderId,
					@_pdcUserId				=	intUserId
			FROM	#ProcessDeleteContracts 
			WHERE	intUniqueId = @intUniqueId

			SET @Condition = 'intContractHeaderId = ' + STR(@_pdcContractHeaderId)
			EXEC [dbo].[uspCTGetTableDataInXML] 'tblCTContractDetail', @Condition, @p_strXML OUTPUT,null,'intContractDetailId,''Delete'' AS strRowState'
			...
			BEGIN
				EXEC uspCTManagePayable @_pdcContractHeaderId, 'header', 1
			END		
		
			-- DELETE DERIVATIVES
			EXEC uspCTManageDerivatives @_pdcContractHeaderId, 'header', 1
			
			GOTO PROCESS_DETAIL

			DELETED_CONTRACTS:

			SELECT @intUniqueId = MIN(intUniqueId) FROM #ProcessDeleteContracts WHERE intUniqueId > @intUniqueId
		END	
	...
	-----------------------------------------------------------------------
	IF @xmlPContractHeaders <> ''
	BEGIN
		-- Process Contract Headers
		DECLARE @_pchContractHeaderId		INT,
				@_pchUserId					INT,
		-- Validate Contract Headers
				@_vchEntityId				INT,
				@_vchRowState				NVARCHAR(50)
		
		SET @strProcess = 'Process Contract Headers'
		EXEC sp_xml_preparedocument @idoc OUTPUT, @xmlPContractHeaders

		IF OBJECT_ID('tempdb..#ProcessContractHeaders') IS NOT NULL
			DROP TABLE #ProcessContractHeaders

		SELECT  ROW_NUMBER() OVER(ORDER BY intContractHeaderId) intUniqueId,* 
		INTO	#ProcessContractHeaders
		FROM	OPENXML(@idoc,'contractHeaders/contractHeader',2)
		WITH	
		(
			-- Process Contract Headers
			intContractHeaderId INT,
		...
		SELECT @intUniqueId = MIN(intUniqueId) FROM #ProcessContractHeaders

		WHILE ISNULL(@intUniqueId,0) > 0
		BEGIN
			-- Process Contract Headers
			SELECT	@_pchContractHeaderId	=	intContractHeaderId,
					@_pchUserId				=	intUserId,
					@p_strXML				=	tblCTContractDetails,
			-- Validate Contract Headers
					@v_intStartingNumberId	=	intStartingNumberId,
					@_vchEntityId			=	intEntityId,
					@v_RowState				=	strRowState,
					@v_strXML				=	tblCTContractHeaders	
			FROM	#ProcessContractHeaders 
			WHERE	intUniqueId = @intUniqueId

			-- Process Contract Headers
			SELECT @p_strXML = REPLACE(@p_strXML, '^|', '<')
			SELECT @p_strXML = REPLACE(@p_strXML, '|^', '>')
			SELECT @p_strXML = '<tblCTContractDetails>' + @p_strXML + '</tblCTContractDetails>'
			...
			-- GOTO VALIDATE_CONTRACT_HEADER
			-- V_CONTRACT_HEADER:

			SELECT @intUniqueId = MIN(intUniqueId) FROM #ProcessContractHeaders WHERE intUniqueId > @intUniqueId
		END	
	...
	-----------------------------------------------------------------------
	IF @xmlVContractDetails <> ''
	BEGIN
		
		SET @strProcess = 'Validate Contract Details'
		EXEC sp_xml_preparedocument @idoc OUTPUT, @xmlVContractDetails
		...
		INTO	#ValidateContractDetails
		FROM	OPENXML(@idoc,'contractDetails/contractDetail',2)
		WITH	(tblCTContractDetails NVARCHAR(MAX),strRowState NVARCHAR(50))
	
		SELECT @intUniqueId = MIN(intUniqueId) FROM #ValidateContractDetails

		WHILE ISNULL(@intUniqueId,0) > 0
		BEGIN
			SELECT	@vd_RowState		=	strRowState,
			...
			GOTO VALIDATE_CONTRACT_DETAIL

			V_CONTRACT_DETAIL:
			
			SELECT @intUniqueId = MIN(intUniqueId) FROM #ValidateContractDetails WHERE intUniqueId > @intUniqueId
		END	
...
PROCESS_DETAIL:
	...
	IF @strProcess = 'Deleted Contracts'
		GOTO DELETED_CONTRACTS
	ELSE IF @strProcess = 'Process Contract Headers'
		GOTO P_CONTRACT_HEADERS
	ELSE
		GOTO EXIT_PROCESS
...
VALIDATE_CONTRACT_DETAIL:
	...
	IF EXISTS(
			...
			WHERE	IL.intItemId = @_vcdNewItemId AND CS.intCompanyLocationId = @_vcdNewCompanyLocationId 
	) AND ISNULL(@_vcdNewSubLocationId,0) <> 0
	BEGIN
		IF NOT EXISTS(
			SELECT	* 
			FROM	tblICItemSubLocation	SL
	...
	IF ISNULL(@_vcdSlice,0) = 0
	BEGIN
		IF	@_vcdNewItemId IS NOT NULL AND NOT EXISTS(SELECT * FROM tblICItem WHERE intItemId = @_vcdNewItemId AND strStatus = 'Active')
		BEGIN
