...
DECLARE @log TABLE ([strDescription] NVARCHAR(200) COLLATE Latin1_General_CI_AS NULL)
...
SELECT * FROM (
	...
	UNION ALL SELECT A.intInvoiceId
		, A.intAccountId
		, D.strAccountId
	...
	LEFT JOIN tblGLAccount D ON A.intAccountId = D.intAccountId
	LEFT JOIN tblGLAccountSegmentMapping mapping ON mapping.intAccountId = D.intAccountId
	LEFT JOIN tblGLAccountSegment segment ON segment.intAccountSegmentId = mapping.intAccountSegmentId
	LEFT JOIN tblGLAccountCategory cat ON segment.intAccountCategoryId = cat.intAccountCategoryId
	WHERE cat.strAccountCategory = 'AP Account' --there are old data where cash refund have been posted to non AP account
		AND DATEADD(dd, DATEDIFF(dd, 0,dtmDate), 0) BETWEEN @from AND @to
	GROUP BY A.intInvoiceId, A.intAccountId, D.strAccountId
	
	UNION ALL SELECT A.intBillId
		, A.intAccountId
		, D.strAccountId
		, tmpAgingSummaryTotal.dblAmountDue
	FROM (
...
INSERT INTO @log
SELECT 'Account ' + A.strAccountId + ': ' + CAST(A.dblBalance AS NVARCHAR)
FROM #tmpAPAccountBalance A

INSERT INTO tblAPImportVoucherLog (strDescription
	, intEntityId
	, dtmDate
	, strLogKey)
