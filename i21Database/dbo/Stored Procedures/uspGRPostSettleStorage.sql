			 ...
			 BEGIN
				...
				LEFT JOIN (
						...
						ON SH.intCustomerStorageId = CS.intCustomerStorageId
								AND a.intItemType = 1
								and (a.dblSettleContractUnits is null or (
				...
				begin
					...
					select 
							...
							@intSettleStorageId as [settle storage id],
							@dblSelectedUnits as [selected units]
					...
					,[dblQuantityToBill]			= case when @doPartialHistory = 1 then
															...
															CASE 
																WHEN (a.intPricingTypeId = 2 or a.intPricingTypeId = 1 ) and 
																	availableQtyForVoucher.intContractDetailId is not null and 
																	availableQtyForVoucher.dblAvailableQuantity > 0
																	and intItemType = 1
																	THEN availableQtyForVoucher.dblAvailableQuantity -- @dblQtyFromCt 																		
																WHEN @origdblSpotUnits > 0 
																	THEN ROUND(dbo.fnCalculateQtyBetweenUOM(b.intItemUOMId,@intCashPriceUOMId,a.dblUnits),6) 
				...
				SELECT 
					...
					,[intQtyToBillUOMId]			= CASE 
													...
													END	
					,[dblCost] 						= isnull(CASE
														WHEN isnull(ReceiptCharge.intEntityVendorId,0) = isnull(SS.intEntityId,0) AND ISNULL(ReceiptCharge.ysnAccrue, 0) = 1 AND ISNULL(ReceiptCharge.ysnPrice, 0) = 0 THEN ROUND(dbo.fnCTConvertQuantityToTargetItemUOM(CS.intItemId, CU.intUnitMeasureId, CS.intUnitMeasureId, SC.dblFreightRate),20)
														WHEN isnull(ReceiptCharge.intEntityVendorId,0) = isnull(SS.intEntityId,0) AND ISNULL(ReceiptCharge.ysnAccrue, 0) = 0 AND ISNULL(ReceiptCharge.ysnPrice, 0) = 1 THEN -ROUND(dbo.fnCTConvertQuantityToTargetItemUOM(CS.intItemId, CU.intUnitMeasureId, CS.intUnitMeasureId, SC.dblFreightRate), 20)
														WHEN isnull(ReceiptCharge.intEntityVendorId,0) <> isnull(SS.intEntityId,0) AND ISNULL(ReceiptCharge.ysnAccrue, 0) = 1 AND ISNULL(ReceiptCharge.ysnPrice, 0) = 1 THEN -ROUND(dbo.fnCTConvertQuantityToTargetItemUOM(CS.intItemId, CU.intUnitMeasureId, CS.intUnitMeasureId, SC.dblFreightRate), 20)
														WHEN isnull(ReceiptCharge.intEntityVendorId,0) = isnull(SS.intEntityId,0)  AND  ISNULL(ReceiptCharge.ysnAccrue, 0) = 0 AND ISNULL(SC.ysnFarmerPaysFreight, 0) = 1 THEN	-ROUND(dbo.fnCTConvertQuantityToTargetItemUOM(CS.intItemId, CU.intUnitMeasureId, CS.intUnitMeasureId, SC.dblFreightRate), 20)
														WHEN isnull(ReceiptCharge.intEntityVendorId,0) <> isnull(SS.intEntityId,0) AND  ISNULL(ReceiptCharge.ysnAccrue, 0) = 1 AND ISNULL(SC.ysnFarmerPaysFreight, 0) = 1 THEN	-ROUND(dbo.fnCTConvertQuantityToTargetItemUOM(CS.intItemId, CU.intUnitMeasureId, CS.intUnitMeasureId, SC.dblFreightRate), 20)
