			...
			BEGIN
			 ...
			 BEGIN
				...
				SELECT 
					...
					,[dblWeightUnitQty]				= 1 
					,[intWeightUOMId]				= CASE
														WHEN a.[intContractHeaderId] IS NOT NULL  or @origdblSpotUnits > 0 OR (ISNULL(@origdblSpotUnits,0) > 0 AND a.intItemType = 1) THEN b.intItemUOMId
														ELSE NULL
													END
					,[intPurchaseTaxGroupId]		= 
													CASE 
				...
				left join tblICInventoryReceiptItem IRI on SC.intTicketId = IRI.intSourceId AND IRI.intContractHeaderId IS NOT NULL
