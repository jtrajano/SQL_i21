...
FROM (
	SELECT strReferenceNumber			= CASE WHEN ISNULL(I.ysnImportedFromOrigin, 0) = 0 THEN I.strInvoiceNumber ELSE ISNULL(I.strInvoiceOriginId, I.strInvoiceNumber) END
		 ...
		 , strCustomerNumber			= C.strCustomerNumber
		 , strDisplayName				= CASE WHEN C.strStatementFormat <> ''Running Balance'' THEN C.strName WHEN DATALENGTH(C.strCheckPayeeName) = 0 THEN C.strName ELSE ISNULL(C.strCheckPayeeName,C.strName) END
		 , strName						= C.strName
