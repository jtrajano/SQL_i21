...
FROM (
	...
	LEFT JOIN (
		...
		AND (ysnPaid = 0
			OR (ysnPaid = 1 AND intInvoiceId IN (SELECT PD.intInvoiceId
...
WHERE strReferenceNumber IN (SELECT strInvoiceNumber = CASE WHEN ISNULL(ysnImportedFromOrigin, 0) = 0 THEN strInvoiceNumber ELSE strInvoiceOriginId END 
							 FROM tblARInvoice WHERE strType = 'CF Tran' AND strTransactionType NOT IN ('Debit Memo'))
