...
IF @strTransactionType = 'Invoice'
	BEGIN
		...
		SET dblQtyShipped  = CASE WHEN @ysnForDelete = 0 
								  THEN SOD.dblQtyShipped + CONVERT(NUMERIC(18, 6), dbo.fnCalculateQtyBetweenUOM(SOD.intItemUOMId, UOM.intItemUOMId, ISNULL(INVOICEITEMS.dblQtyShipped, 0)))
								  ELSE SOD.dblQtyShipped - CONVERT(NUMERIC(18, 6), dbo.fnCalculateQtyBetweenUOM(SOD.intItemUOMId, UOM.intItemUOMId, ISNULL(INVOICEITEMS.dblQtyShipped, 0)))
							 END
		...
		CROSS APPLY (
			SELECT dblQtyShipped = ISNULL(dbo.fnCalculateQtyBetweenUOM(UOM.intItemUOMId, ID.intItemUOMId, SUM(ID.dblQtyShipped)), 0)
			FROM tblARInvoiceDetail ID
			INNER JOIN tblICItemUOM UOM ON ID.intItemId = UOM.intItemId AND UOM.ysnStockUnit = 1
...
ELSE IF @strTransactionType = 'Inventory'
	BEGIN
		UPDATE SOD
		SET dblQtyShipped  = CASE WHEN @ysnForDelete = 0 
								  THEN SOD.dblQtyShipped + CONVERT(NUMERIC(18, 6), dbo.fnCalculateQtyBetweenUOM(SOD.intItemUOMId, UOM.intItemUOMId, ISNULL(SHIPPEDITEMS.dblQuantity, 0)))
								  ELSE SOD.dblQtyShipped - CONVERT(NUMERIC(18, 6), dbo.fnCalculateQtyBetweenUOM(SOD.intItemUOMId, UOM.intItemUOMId, ISNULL(SHIPPEDITEMS.dblQuantity, 0)))
							 END
		  , @intSalesOrderId = SOD.intSalesOrderId
		FROM tblSOSalesOrderDetail SOD
		LEFT JOIN tblICItemUOM UOM ON SOD.intItemId = UOM.intItemId AND UOM.ysnStockUnit = 1
		CROSS APPLY (
			SELECT dblQuantity = ISNULL(dbo.fnCalculateQtyBetweenUOM(UOM.intItemUOMId, ISI.intItemUOMId, SUM(ISI.dblQuantity)), 0)
			FROM tblICInventoryShipmentItem ISI
			INNER JOIN tblICItemUOM UOM ON ISI.intItemId = UOM.intItemId AND UOM.ysnStockUnit = 1
...
ELSE
	BEGIN
		UPDATE SOD
		SET dblQtyShipped  = CASE WHEN @ysnForDelete = 0 
								  THEN CONVERT(NUMERIC(18, 6), dbo.fnCalculateQtyBetweenUOM(SOD.intItemUOMId, UOM.intItemUOMId, (ISNULL(INVOICEITEMS.dblQtyShipped, 0) + ISNULL(SHIPPEDITEMS.dblQuantity, 0))))
								  ELSE SOD.dblQtyShipped - CONVERT(NUMERIC(18, 6), dbo.fnCalculateQtyBetweenUOM(SOD.intItemUOMId, UOM.intItemUOMId, (ISNULL(INVOICEITEMS.dblQtyShipped, 0) + ISNULL(SHIPPEDITEMS.dblQuantity, 0))))
							 END		  
		FROM tblSOSalesOrderDetail SOD
		LEFT JOIN tblICItemUOM UOM ON SOD.intItemId = UOM.intItemId AND UOM.ysnStockUnit = 1
		OUTER APPLY (
