...
FROM
	(
		SELECT SOD.intSalesOrderDetailId
			 , dblQuantity			= SUM(ISNULL(CASE WHEN ID.intItemUOMId IS NOT NULL AND SOD.intItemUOMId IS NOT NULL THEN ISNULL(dbo.fnCalculateQtyBetweenUOM(SOD.intItemUOMId, ID.intItemUOMId, ISNULL(ID.dblQtyShipped,0)), ISNULL(ID.dblQtyShipped,0)) ELSE ISNULL(ID.dblQtyShipped,0) END, 0)) + SUM(ISNULL(dbo.fnCalculateQtyBetweenUOM(SOD.intItemUOMId, ISHI.intItemUOMId, ISNULL(ISHI.dblQuantity,0)), 0))			 
