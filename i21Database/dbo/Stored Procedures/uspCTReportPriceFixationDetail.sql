...
BEGIN TRY
	...
	SELECT	DISTINCT 
			...
			lblFX = CASE WHEN PD.dblFX != 1  AND PD.dblFX IS NOT NULL THEN 'FX' ELSE NULL  END,
			strFX = CASE WHEN PD.dblFX != 1  AND PD.dblFX IS NOT NULL THEN LTRIM(CAST( PD.dblFX AS NUMERIC(18, 6)))  ELSE NULL END,
			dblFX = PD.dblFX,
