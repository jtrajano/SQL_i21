...
BEGIN
    ...
    LEFT JOIN tblARTransactionDetail TDD ON I.intInvoiceId = TDD.intTransactionId AND ID.intInvoiceDetailId = TDD.intTransactionDetailId AND I.strTransactionType = TDD.strTransactionType
    INNER JOIN tblICItem ITEM2 ON TD.intItemId = ITEM2.intItemId
    WHERE ID.intItemId IS NOT NULL
      AND I.strTransactionType IN ('Credit Memo', 'Debit Memo', 'Invoice')
      AND ISNULL(II.ysnForDelete, 0) = 0
      AND TDD.intTransactionDetailId IS NULL
      AND ITEM.strType != 'Other Charge'
