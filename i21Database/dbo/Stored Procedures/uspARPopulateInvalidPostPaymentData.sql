...
BEGIN
	...
	FROM #ARPostPaymentDetail P
    OUTER APPLY (
		SELECT strOverrideAccount, bitSameLocation
		FROM dbo.[fnARGetOverrideAccount](3, P.[intARAccountId], P.[intWriteOffAccountDetailId])
	) OVERRIDESEGMENT
    WHERE P.[ysnPost] = @OneBit
    AND P.[intInvoiceId] IS NOT NULL
    AND ISNULL(P.[dblWriteOffAmount], 0) <> @ZeroDecimal
    AND @OverrideLocationSegment = 1
    AND OVERRIDESEGMENT.bitSameLocation = 0
    ...
    OUTER APPLY (
		SELECT strOverrideAccount, bitSameLocation
		FROM dbo.[fnARGetOverrideAccount](3, P.[intARAccountId], P.[intAccountId])
	) OVERRIDESEGMENT
    WHERE P.[ysnPost] = @OneBit
    AND ISNULL(P.[intUndepositedFundsId], 0) = 0
    AND @OverrideLocationSegment = 1
    ...
    OUTER APPLY (
		SELECT strOverrideAccount, bitSameLocation
		FROM dbo.[fnARGetOverrideAccount](3, P.[intARAccountId], P.[intGainLossAccount])
	) OVERRIDESEGMENT
    WHERE P.[ysnPost] = @OneBit
    AND P.[intInvoiceId] IS NOT NULL
    AND ISNULL(P.[intGainLossAccount],0) <> 0
