...
BEGIN
	...
	IF NOT EXISTS(SELECT 1 FROM dbo.tblSTCheckoutDepartmetTotals WHERE intCheckoutId = @intCheckoutId)
		BEGIN
			...
			JOIN dbo.tblICCategoryLocation Cat ON Cat.intRegisterDepartmentId = 
						CASE 
							WHEN LEN(Chk.MerchandiseCode COLLATE Latin1_General_CI_AS) <= 4 
								THEN Chk.MerchandiseCode COLLATE Latin1_General_CI_AS 
							ELSE SUBSTRING(Chk.MerchandiseCode COLLATE Latin1_General_CI_AS, 0, 4) 
						END
	...
	ELSE
		BEGIN
			...
			FROM #tempCheckoutInsert Chk
			JOIN dbo.tblICCategoryLocation Cat ON Cat.intRegisterDepartmentId = 
						CASE WHEN LEN(Chk.MerchandiseCode COLLATE Latin1_General_CI_AS) <= 4 
							THEN Chk.MerchandiseCode COLLATE Latin1_General_CI_AS 
							ELSE SUBSTRING(Chk.MerchandiseCode COLLATE Latin1_General_CI_AS, 0, 4) 
						END
			LEFT JOIN dbo.tblICItem I ON Cat.intGeneralItemId = I.intItemId
