		...
		IF NOT EXISTS(SELECT 1 FROM dbo.tblSTCheckoutDepartmetTotals WHERE intCheckoutId = @intCheckoutId)
			BEGIN
				...
				SELECT @intCheckoutId [intCheckoutId]
						...
						CASE 
							...
							WHEN (S.strReportDepartmentAtGrossOrNet) = 'N' -- Net
								THEN ISNULL(CAST(Chk.dblSalesAmount AS DECIMAL(18,6)),0) + ISNULL(CAST(Chk.dblDiscountAmount AS DECIMAL(18,6)),0) 
																					  ...
																					  + ISNULL(CAST(Chk.dblRefundAmount AS DECIMAL(18,6)),0) --// - (ABS(CAST(ISNULL(Chk.dblDiscountAmount, 0) AS DECIMAL(18,6))) + ABS(CAST(ISNULL(Chk.dblRefundAmount, 0) AS DECIMAL(18,6))) + ABS(CAST(ISNULL(Chk.dblPromotionAmount, 0) AS DECIMAL(18,6))))
					    END
					...
					, CAST(Chk.dblRefundAmount AS DECIMAL(18,6)) [dbldblRefundAmount]
					...
					, 1 [intConcurrencyId]
				FROM @UDT_MCM Chk
				JOIN dbo.tblICCategoryLocation Cat ON CAST(ISNULL(Chk.intMerchandiseCode, '') AS NVARCHAR(50)) COLLATE Latin1_General_CI_AS = CAST(Cat.intRegisterDepartmentId AS NVARCHAR(50))
											...
											CASE 
												...
												WHEN (S.strReportDepartmentAtGrossOrNet) = 'N' -- Net
													THEN ISNULL(CAST(Chk.dblSalesAmount AS DECIMAL(18,6)),0) + ( ISNULL(CAST(Chk.dblDiscountAmount AS DECIMAL(18,6)),0) 
																										  ...
																										  --// - (ABS(CAST(ISNULL(Chk.dblDiscountAmount, 0) AS DECIMAL(18,6))) + ABS(CAST(ISNULL(Chk.dblRefundAmount, 0) AS DECIMAL(18,6))) + ABS(CAST(ISNULL(Chk.dblPromotionAmount, 0) AS DECIMAL(18,6))))
											END
										  )
