			...
			BEGIN
				...
				BEGIN TRY
					...
					SELECT 
							...
							SELECT DISTINCT intScanTransactionId
									...
									, CASE	
										...
										WHEN TR.intTrlDeptNumber IN (SELECT strRegisterCode FROM [dbo].[fnSTRebateDepartment]((CAST(ST.intStoreId AS NVARCHAR(10)))) WHERE ysnTobacco = 1) 
											THEN 'CANS'
					...
					WHERE TR.intStoreId IN (SELECT [intID] FROM [dbo].[fnGetRowsFromDelimitedValues](@strStoreIdList)) 
						...
						AND TR.intTrlDeptNumber IN (SELECT DISTINCT strRegisterCode FROM fnSTRebateDepartment(CAST(ST.intStoreId AS NVARCHAR(10))))
				) as innerQuery
				...
				BEGIN TRY
					...
					SELECT 
						...
						LEFT JOIN vyuSTCigaretteRebatePrograms CRP 
							ON CONVERT(NUMERIC(32, 0),CAST(TR.strTrlUPCwithoutCheckDigit AS FLOAT)) = CRP.intUpcCode ---->   Always compare UPC without check digit since Inventory UPC has no check digit, use IC intUpcCode
								AND (CAST(TR.dtmDate AS DATE) BETWEEN CRP.dtmStartDate AND CRP.dtmEndDate)
						WHERE TR.intTrlDeptNumber IN (SELECT DISTINCT strRegisterCode FROM fnSTRebateDepartment(CAST(ST.intStoreId AS NVARCHAR(10))))
								) as innerQuery
