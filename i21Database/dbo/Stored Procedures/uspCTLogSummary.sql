...
BEGIN TRY
	...
	BEGIN
		...
		BEGIN
			...
			WHERE Row_Num = 1
		END
		ELSE IF @strProcess = 'Create Invoice' OR @strProcess = 'Delete Invoice'
		BEGIN
			INSERT INTO @cbLogCurrent (strBatchId
				, dtmTransactionDate
				, strTransactionType
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				DELETE FROM @cbLogPrev 
				WHERE strTransactionType <> 'Contract Balance'
					AND intId <> (SELECT TOP 1 intId FROM @cbLogPrev WHERE strTransactionType <> 'Contract Balance' ORDER BY intId DESC)
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					--UPDATE  @cbLogSpecific SET dblQty = @qtyDiff * -1, intPricingTypeId = 2, strTransactionType = CASE WHEN @intContractTypeId = 1 THEN 'Purchase Basis Deliveries' ELSE 'Sales Basis Deliveries' END
					--EXEC uspCTLogContractBalance @cbLogSpecific, 0
				END
				ELSE
				BEGIN
