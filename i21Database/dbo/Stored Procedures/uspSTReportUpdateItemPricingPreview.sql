	...
	IF(@strGuid IS NOT NULL AND @strGuid <> '')
		BEGIN
			...
			SELECT	CL.intCompanyLocationId
					...
					END
					,CASE
						WHEN [Changes].oldColumnName = 'strStandardCost_Original' THEN 'DECIMAL'
						WHEN [Changes].oldColumnName = 'strSalePrice_Original' THEN 'DECIMAL'
						WHEN [Changes].oldColumnName = 'strLastCost_Original' THEN 'DECIMAL'
					END
			FROM 
			(
			...
			SELECT	CL.intCompanyLocationId
					...
					END
					,CASE
						WHEN [Changes].oldColumnName = 'strUnitAfterDiscount_Original' THEN 'DECIMAL'
