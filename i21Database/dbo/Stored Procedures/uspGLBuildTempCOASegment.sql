...
4, N'0', 2, 1, 1, 5, NULL, NULL)  
 ...
 END  
   ...
   SELECT @Segments = ISNULL(SUBSTRING((SELECT '],[' + strSegmentType FROM tblGLSegmentType WHERE strSegmentType NOT IN ( 'Segment','Divider') FOR XML PATH('')),3,200000) + ']','[Primary Account]')  
   ...
   'INNER JOIN (  
    ...
    ON C.intAccountStructureId = D.intAccountStructureId  
      INNER JOIN tblGLSegmentType E on E.intSegmentTypeId = D.intStructureType  
     ) AS tempTable  
    PIVOT  
    (  
    MIN(strCode)  
    FOR strSegmentType IN (' + @Segments + ')) AS PVT  
    ) AS DAS  
 ...
 --this will ensure there is [Primary Account] column in tblGLTempCOASegment  
 IF NOT EXISTS(  
  SELECT 1   
   FROM   INFORMATION_SCHEMA.COLUMNS  
   WHERE  TABLE_NAME = 'tblGLTempCOASegment'  
 ...
 ALTER TABLE tblGLTempCOASegment ADD [Location] [nvarchar](20) COLLATE Latin1_General_CI_AS NULL  
  
   
 --standardize the column name via tblGLSegmentType  
 IF EXISTS(  
  SELECT 1   
   FROM   INFORMATION_SCHEMA.COLUMNS  
   WHERE  TABLE_NAME = 'tblGLTempCOASegment'  
     AND COLUMN_NAME = 'Line of business'  
 )  
 EXEC sp_rename 'tblGLTempCOASegment.[Line of business]', 'Line Of Business','Column'  
 ...
 ALTER TABLE tblGLTempCOASegment ADD [Line Of Business] [nvarchar](20) COLLATE Latin1_General_CI_AS NULL  
