...
BEGIN
	...
	BEGIN
		EXEC ('CREATE VIEW [dbo].[vyuGLSummary]
			AS
			SELECT * FROM (
			  SELECT  A.dtmDate, A.dblDebit, A.dblCredit, A.dblDebitUnit, A.dblCreditUnit,A.dblDebitForeign,A.dblCreditForeign, ISNULL(A.strCode,'''') strCode, B.strDescription, C.strAccountGroup, C.strAccountType, D.*, E.strUOMCode, E.dblLbsPerUnit, A.intCurrencyId, F.strCurrency,A.intLedgerId,A.intSubledgerId,B.intUnnaturalAccountId
