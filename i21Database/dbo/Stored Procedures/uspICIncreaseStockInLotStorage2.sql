...
CREATE PROCEDURE dbo.uspICIncreaseStockInLotStorage
	...
	,@NewLotStorageId AS INT OUTPUT 
	,@UpdatedLotStorageId AS INT OUTPUT 
...
SET @CostUsed = NULL;
SET @QtyOffset = NULL;
SET @NewLotStorageId = NULL;
SET @UpdatedLotStorageId = NULL;
...
WHEN MATCHED THEN 
	...
	SET	Lot_bucket.dblStockIn = ISNULL(Lot_bucket.dblStockIn, 0) 
		...
		,@ForexCostUsed = Lot_bucket.dblForexCost
		...
		,@UpdatedLotStorageId = Lot_bucket.intInventoryLotStorageId
...
BEGIN 
	...
	SELECT @NewLotStorageId = SCOPE_IDENTITY() WHERE @UpdatedLotStorageId IS NOT NULL; 
END 

-- If Update was not performed, assume an insert was done. 
SELECT @NewLotStorageId = SCOPE_IDENTITY() WHERE @UpdatedLotStorageId IS NULL; 
