...
FROM tblMFManufacturingProcessAttribute AS ProcessAttribute
JOIN tblMFAttribute AS Attribute ON ProcessAttribute.intAttributeId = Attribute.intAttributeId 
WHERE intManufacturingProcessId = @intManufacturingProcessId AND intLocationId = @intLocationId AND Attribute.strAttributeName = 'Packaging Category';
...
WHERE Recipe.intRecipeId=@intRecipeId AND RecipeItem.intRecipeItemTypeId = 1 
  AND ((RecipeItem.ysnYearValidationRequired = 1 AND @dtmDate BETWEEN RecipeItem.dtmValidFrom AND RecipeItem.dtmValidTo)
   OR (RecipeItem.ysnYearValidationRequired = 0 AND @intDayOfYear BETWEEN DATEPART(dy, RecipeItem.dtmValidFrom) AND DATEPART(dy, RecipeItem.dtmValidTo)))
UNION
...
JOIN tblICUnitMeasure AS UnitOfMeasure ON ItemUOM.intUnitMeasureId=UnitOfMeasure.intUnitMeasureId
WHERE Recipe.intRecipeId = @intRecipeId AND SubstituteItem.intRecipeItemTypeId = 1;
...
JOIN (SELECT value FROM dbo.fnCommaSeparatedValueToTable(@strPackagingCategoryId)) AS PackageCategory ON Item.intCategoryId = PackageCategory.value;
...
IF ISNULL(@strHandAddIngredientXml, '') <> ''
	BEGIN
		...
		INSERT INTO @tblHandAddIngredient (intRecipeItemId
										 , dblQuantity)
		SELECT intRecipeItemId
			 , dblQuantity
		FROM OPENXML(@idoc, 'root', 2) WITH (intRecipeItemId INT
										   , dblQuantity	 NUMERIC(38, 20));
		...
		IF @dblQtyDiff <> 0
			BEGIN
				...
				FROM (SELECT TOP 1 RequiredQty.* 
					  FROM @tblRequiredQty AS RequiredQty 
					  JOIN tblICItem AS Item on RequiredQty.intItemId = Item.intItemId
					  WHERE ISNULL(Item.ysnHandAddIngredient, 0) = 0 
					  ORDER BY dblRequiredQty DESC) AS RequiredQty;
			END
	END
...
SELECT RecipeItem.intItemId
	 , SUM(CASE WHEN ISNULL(Lot.dblWeight, 0) > 0 THEN Lot.dblWeight ELSE dbo.fnMFConvertQuantityToTargetItemUOM(Lot.intItemUOMId, RecipeItem.intItemUOMId, Lot.dblQty) END) AS dblPhysicalQty
	 , CASE WHEN ISNULL(MAX(Lot.dblWeightPerQty), 1) = 0 THEN 1 ELSE ISNULL(MAX(Lot.dblWeightPerQty), 1) END AS dblWeightPerUnit
FROM tblICLot AS Lot 
JOIN tblMFRecipeItem AS RecipeItem ON RecipeItem.intItemId = Lot.intItemId 
JOIN tblICItem AS Item on RecipeItem.intItemId = Item.intItemId AND Item.strType <> 'Other Charge'
WHERE RecipeItem.intRecipeId = @intRecipeId AND Lot.intLocationId = @intLocationId
GROUP BY RecipeItem.intItemId;

/* Create Physical Quantity for Substitute. */
INSERT INTO @tblPhysicalQty
SELECT SubstituteItem.intSubstituteItemId
	 , SUM(CASE WHEN ISNULL(Lot.dblWeight, 0) > 0 THEN Lot.dblWeight ELSE dbo.fnMFConvertQuantityToTargetItemUOM(Lot.intItemUOMId, SubstituteItem.intItemUOMId, Lot.dblQty) END) AS dblPhysicalQty
	 , CASE WHEN ISNULL(MAX(Lot.dblWeightPerQty), 1) = 0 THEN 1 ELSE ISNULL(MAX(Lot.dblWeightPerQty), 1) END AS dblWeightPerUnit 
FROM tblICLot AS Lot 
...
JOIN tblICItem AS Item ON RecipeItem.intItemId = Item.intItemId AND Item.strType <> 'Other Charge'
WHERE RecipeItem.intRecipeId = @intRecipeId AND RecipeItem.intRecipeItemTypeId = 1 AND ISNULL(StockReservation.ysnPosted, 0) = 0
GROUP BY RecipeItem.intItemId;
...
JOIN tblMFRecipeItem AS RecipeItem ON RecipeItem.intItemId = Lot.intItemId 
WHERE RecipeItem.intRecipeId = @intRecipeId AND RecipeItem.intRecipeItemTypeId = 1
GROUP BY RecipeItem.intItemId;
...
WHERE SubstituteItem.intRecipeId = @intRecipeId AND ISNULL(StockReservation.ysnPosted, 0) = 0
GROUP BY SubstituteItem.intSubstituteItemId;

/* Create Confirmed Quantity. */
INSERT INTO @tblConfirmedQty(intItemId
						   , dblConfirmedQty)
SELECT intItemId
	 , SUM(dblQuantity)
