CREATE PROCEDURE [dbo].[uspCTReportContractSequence]
...
BEGIN TRY
	...
	DECLARE @strCompanyName			NVARCHAR(500),
			...
			@sequenceContractDetailId	NVARCHAR(MAX),
	...
	SELECT	TOP 1 @sequenceContractDetailId	= Item FROM dbo.fnSplitString(@strContractDetailId,',')
	SELECT	TOP 1 @intContractHeaderId	= intContractHeaderId FROM tblCTContractDetail where intContractDetailId = @sequenceContractDetailId
	SELECT	TOP 1 @intSequenceNo = intContractSeq FROM tblCTContractDetail where intContractDetailId = @sequenceContractDetailId
	...
	SELECT  @strBrkgCommn	=
			CASE	WHEN strParty = 'Broker' AND @strReportTo = strPaidBy
						THEN	dbo.fnCTChangeNumericScale(CC.dblRate,2) + ' ' + CC.strCurrency + ' / ' + CC.strUOM + ' included to be paid directly to:' + CHAR(13)+CHAR(10) +
								...
								ISNULL(CASE WHEN LTRIM(RTRIM(EY.strEntityCountry)) = '' THEN NULL ELSE LTRIM(RTRIM(dbo.fnCTGetTranslation('SystemManager.view.Country',CY.intCountryID,@intLaguageId,'Country',CY.strCountry))) END,'')
					ELSE	'' 
			END
	...
	SELECT	 intContractHeaderId					= CH.intContractHeaderId
			,intContractDetailId					= @sequenceContractDetailId
			,strCaption								= isnull(dbo.fnCTGetTranslatedExpression(@strExpressionLabelName,@intLaguageId,TP.strContractType), TP.strContractType) + ' '+@rtContract+':- ' + CH.strContractNumber +' / ' + CAST (@intSequenceNo AS VARCHAR(MAX))
			...
			,strArbitration							= @rtStrArbitration1 + ' '+ dbo.fnCTGetTranslation('ContractManagement.view.Associations',AN.intAssociationId,@intLaguageId,'Printable Contract Text',AN.strComment) + '  '+@rtStrArbitration2+'. ' 
														+ CHAR(13)+CHAR(10) +
														@rtStrArbitration3 + ' ' + AB.strState +', '+ dbo.fnCTGetTranslation('SystemManager.view.Country',RY.intCountryID,@intLaguageId,'Country',RY.strCountry)
			,strGABArbitration						= ISNULL(NULLIF(AB.strState, ''), AB.strCity) +', '+ dbo.fnCTGetTranslation('SystemManager.view.Country',RY.intCountryID,@intLaguageId,'Country',RY.strCountry)
			...
			,strBrokerAddress						=   LTRIM(RTRIM(EB.strEntityName)) +' - '+ ISNULL(CASE WHEN LTRIM(RTRIM(ISNULL(VR.strFLOId,CR.strFLOId))) = '' THEN NULL ELSE LTRIM(RTRIM(ISNULL(VR.strFLOId,CR.strFLOId))) END,'')+ CHAR(13)+CHAR(10) +
														...
														ISNULL(CASE WHEN LTRIM(RTRIM(EB.strEntityCountry)) = ''      THEN NULL ELSE LTRIM(RTRIM(dbo.fnCTGetTranslation('SystemManager.view.Country',rtc10.intCountryID,@intLaguageId,'Country',rtc10.strCountry))) END,'') 
			...
			,strAtlasCaller							= CASE
														...
														ELSE
															CASE
																...
																THEN ''
																ELSE ISNULL(SQ.strFixationBy,'')
															END
															+ CASE WHEN LTRIM(RTRIM(SQ.strFixationBy)) = '' THEN '' ELSE '''s Call vs ' END
															+ CASE
																	WHEN CH.ysnMultiplePriceFixation = 1 and ISNULL(SQ.dblNoOfLots,0) <= 0
																	THEN convert(nvarchar(20),convert(numeric(10,2),isnull(CH.dblNoOfLots,0.00)))
																	ELSE convert(nvarchar(20),convert(numeric(10,2),ISNULL(SQ.dblNoOfLots,0)))
															END
															+ ' lot(s) '
															+ SQ.strFutureMonth
															+ ' '
	...
	FROM	tblCTContractHeader				CH
	JOIN	tblICCommodity					CM	WITH (NOLOCK) ON	CM.intCommodityId				=	CH.intCommodityId
												AND	CH.intContractHeaderId		= @intContractHeaderId --IN	(SELECT Item FROM dbo.fnSplitString(@strIds,','))
