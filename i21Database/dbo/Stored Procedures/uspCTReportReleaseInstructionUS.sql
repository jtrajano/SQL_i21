CREATE PROCEDURE [dbo].[uspCTReportReleaseInstructionUS]
...
BEGIN TRY
	DECLARE @ErrMsg NVARCHAR(MAX), @xmlDocumentId INT
		...
		, @intContractReleaseInstructionId INT
		, @intLaguageId INT
    ...
    SELECT @intContractReleaseInstructionId = [from]
    FROM @temp_xml_table
    WHERE [fieldname] = 'intContractReleaseInstructionId'
    ...
    SELECT intContractHeaderId = CH.intContractHeaderId
		...
		, strReportTitle = 'RELEASE INSTRUCTIONS'--(CASE WHEN POS.strPositionType = 'Shipment' THEN 'RELEASE INSTRUCTIONS'
	...
	LEFT JOIN tblICItemUOM CRU ON CRU.intItemUOMId = CRI.intItemUOMId
	LEFT JOIN tblICUnitMeasure CRM ON CRM.intUnitMeasureId = CRU.intUnitMeasureId
	...
	WHERE CRI.intContractReleaseInstructionId = @intContractReleaseInstructionId
END TRY
BEGIN CATCH
