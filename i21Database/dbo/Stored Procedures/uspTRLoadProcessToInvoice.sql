...
BEGIN TRY
	...
	BEGIN
		...
		SELECT
			...
			,[strBOLNumberDetail]					= IE.strBOLNumberDetail 
			,[ysnBlended]							= IE.ysnBlended
			,[intLoadDistributionDetailId]			= CASE WHEN IE.ysnComboFreight = 1 THEN NULL ELSE IE.intLoadDistributionDetailId END    
		FROM #tmpSourceTableFinal IE
		...
		SELECT DISTINCT
			...
			,[strBOLNumberDetail]					= IE.strBOLNumberDetail 
			,[ysnBlended]							= IE.ysnBlended
			,[intLoadDistributionDetailId]			= CASE WHEN IE.ysnComboFreight = 1 THEN NULL ELSE IE.intLoadDistributionDetailId END  --CASE WHEN @ysnComboFreight = 1 THEN NULL ELSE IE.intLoadDistributionDetailId END    
		FROM #tmpSourceTableFinal IE
		INNER JOIN tblICItem Item ON Item.intItemId = @intSurchargeItemId
