	...
	BEGIN
		...
		BEGIN
			...
			FROM (SELECT DISTINCT tblARInvoiceDetail.intInvoiceDetailId
					...
					, CASE WHEN tblARInvoice.strType != 'Tank Delivery' THEN Transporter.strName ELSE (SELECT TOP 1 tblEMEntity.strName from tblSMShipVia INNER JOIN tblEMEntity ON tblEMEntity.intEntityId = tblSMShipVia.intEntityId WHERE tblSMShipVia.ysnCompanyOwnedCarrier = 1 ORDER BY tblEMEntity.intEntityId) END AS strTransporterName
					, CASE WHEN tblARInvoice.strType != 'Tank Delivery' THEN Transporter.strFederalTaxId ELSE (SELECT TOP 1 tblEMEntity.strFederalTaxId from tblSMShipVia INNER JOIN tblEMEntity ON tblEMEntity.intEntityId = tblSMShipVia.intEntityId WHERE tblSMShipVia.ysnCompanyOwnedCarrier = 1 ORDER BY tblEMEntity.intEntityId) END AS strTransporterFEIN
