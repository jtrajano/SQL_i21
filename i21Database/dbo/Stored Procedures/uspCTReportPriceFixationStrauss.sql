...
BEGIN TRY
	...
	SELECT	 DISTINCT 
			...
			strDifferential = dbo.fnCTChangeNumericScale(CAST(dbo.fnCTConvertQuantityToTargetCommodityUOM(PF.intFinalPriceUOMId,PU.intCommodityUnitMeasureId, PF.dblOriginalBasis) AS NUMERIC(18, 6)),2) + ' ' + CC.strDescription + ' ' + @per + ' ' + isnull(rtrt3.strTranslation,BB.strUnitMeasure) ,
	...
	JOIN 	tblICItemUOM				AA  ON  AA.intItemUOMId					=   CD.intBasisUOMId		LEFT
	JOIN	tblICUnitMeasure			BB  ON  BB.intUnitMeasureId				=	AA.intUnitMeasureId		LEFT
	JOIN	tblSMCurrency				CY	ON	CY.intCurrencyID				=	CD.intCurrencyId		LEFT
	JOIN	tblSMCurrency				CC	ON	CC.intCurrencyID				=	CD.intBasisCurrencyId	LEFT
