	...
	BEGIN TRY
		...
		begin
			select
				...
				when isnull(c.strDestinationPort,'') <> '' and t.intDestinationPortId is null then 'Destination Port: "' + c.strDestinationPort + '" does not exists.'
				when isnull(c.strTerms,'') <> '' and t.intTermId is null then 'Terms: "' + c.strTerms + '" does not exists.'
				when isnull(c.strGrade,'') <> '' and t.intGradeId is null then 'Grade: "' + c.strGrade + '" does not exists.'
				when isnull(c.strSampleType,'') <> '' and t.intSampleTypeId is null then 'Sample Type: "' + c.strSampleType + '" does not exists.'
				when isnull(c.strWeights,'') <> '' and t.intWeightId is null then 'Weights: "' + c.strWeights + '" does not exists.'
				when isnull(c.strAssociation,'') <> '' and t.intAssociationId is null then 'Association: "' + c.strAssociation + '" does not exists.'
				when isnull(c.strArbitration,'') <> '' and t.intArbitrationId is null then 'Arbitration: "' + c.strArbitration + '" does not exists.'
				when isnull(c.strProducer,'') <> '' and t.intProducerId is null then 'Producer: "' + c.strProducer + '" does not exists.'
				when isnull(c.strPackingDescription,'') <> '' and t.strPackingDescription is null then 'Packing Description: "' + c.strPackingDescription + '" does not exists.'
				when isnull(c.strContainerType,'') <> '' and t.intContainerTypeId is null then 'Container Type: "' + c.strContainerType + '" does not exists.'
				when isnull(c.strCounterCurrency,'') <> '' and t.intInvoiceCurrencyId is null then 'Counter Currency: "' + c.strCounterCurrency + '" does not exists.'
				when isnull(c.strCashFlowOverride,'') = 'Y' and t.dtmCashFlowDate is null then 'Missing Cash Flow Date.'
				when isnull(c.strMarketZone,'') <> '' and t.intMarketZoneId is null then 'Market Zone: "' + c.strMarketZone + '" does not exists.'
				when isnull(c.strBundleItem,'') <> '' and t.intItemBundleId is null then 'Bundle Item: "' + c.strBundleItem + '" does not exists.'
				when isnull(c.strCostTerm,'') <> '' and t.intCostTermId is null then 'Cost Term: "' + c.strCostTerm + '" does not exists.'
				when isnull(c.strMTMPoint,'') <> '' and t.intMTMPointId is null then 'MTM Point: "' + c.strMTMPoint + '" does not exists.'
