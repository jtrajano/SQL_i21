...
WHILE EXISTS(SELECT NULL FROM @PaymentsForImport)
	BEGIN
		...
		SELECT  @intEntityCustomerId		= (SELECT TOP 1 intEntityId FROM tblEMEntity WHERE strEntityNo = ILD.strCustomerNumber)
			  ...
			  , @intBankAccountId			= (SELECT TOP 1 intBankAccountId FROM vyuCMBankAccount WHERE RTRIM(LTRIM(dbo.fnAESDecryptASym(strBankAccountNo))) = RTRIM(LTRIM(ILD.strBankAccountNo)))
