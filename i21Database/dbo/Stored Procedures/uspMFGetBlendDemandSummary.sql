...
(ISNULL(t.DMTotal,0)-(ISNULL(t.BRTotal,0)+(ISNULL(t.BSTotal,0)+ISNULL(t.StorageQty,0)+ISNULL(t.StagingQty,0)))) [dblNetDemandQty],
t.strCompanyLocationName
FROM (  
SELECT  i.intItemId,i.strItemNo,i.strDescription,bd.intLocationId, CompanyLocation.strLocationName AS strCompanyLocationName,SUM(bd.dblQuantity) as DMTotal,  
...
LEFT JOIN tblSMCompanyLocation AS CompanyLocation ON bd.intLocationId = CompanyLocation.intCompanyLocationId
WHERE bd.intStatusId=1  AND bd.dtmDueDate is not null
AND bd.dtmDueDate BETWEEN @dtmFromDate  AND  @dtmToDate
GROUP BY i.intItemId,i.strItemNo,i.strDescription,bd.intLocationId,CompanyLocation.strLocationName) t
