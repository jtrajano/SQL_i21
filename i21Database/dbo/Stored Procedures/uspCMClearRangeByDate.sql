CREATE PROCEDURE uspCMClearRangeByDate
...
WHERE	ysnPosted = 1
		...
		AND CAST(FLOOR(CAST(dtmDate AS FLOAT)) AS DATETIME) BETWEEN CAST(FLOOR(CAST(@dtmDateFrom AS FLOAT)) AS DATETIME) AND CAST(FLOOR(CAST(@dtmDateTo AS FLOAT)) AS DATETIME)
		AND 1 = 
