...
BEGIN
	...
	SELECT 
		...
		,dblTax=ISNULL((
						...
						WHERE intBillId = BillDtl.intBillId
							AND ISNULL(intContractDetailId, 0) = ISNULL(BillDtl.intContractDetailId, 0)
						), 0) 
		...
		,TotalDiscount=ISNULL((
								...
								WHERE intBillId = BillDtl.intBillId
									AND ISNULL(intContractDetailId, 0) = ISNULL(BillDtl.intContractDetailId, 0)
									AND intInventoryReceiptChargeId IS NOT NULL
								), 0) 
		,NetDue=(
			BillDtl.dblTotal + ISNULL((
										SELECT SUM(dblTax)
										FROM tblAPBillDetail
										WHERE intBillId = BillDtl.intBillId
											AND ISNULL(intContractDetailId, 0) = ISNULL(BillDtl.intContractDetailId, 0)
		...
		,InboundTax = CASE 
					  ...
					  END
		,OutboundTax = 0 
		,InboundDiscount=ISNULL((
								SELECT SUM(dblTotal)
								FROM tblAPBillDetail
								WHERE intBillId = BillDtl.intBillId
									AND intInventoryReceiptItemId = BillDtl.intInventoryReceiptItemId
									AND intInventoryReceiptChargeId IS NOT NULL
								), 0) 
		,OutboundDiscount = 0
		,InboundNetDue = CASE 
							  WHEN BillDtl.intInventoryReceiptItemId IS NULL AND BillDtl.intInventoryReceiptChargeId IS NULL THEN 0
						 ELSE (
								BillDtl.dblTotal + BillDtl.dblTax + ISNULL((
																			SELECT SUM(dblTotal)
																			FROM tblAPBillDetail
																			WHERE intBillId = BillDtl.intBillId
																				AND intInventoryReceiptItemId = BillDtl.intInventoryReceiptItemId
						...
						END 
		,OutboundNetDue = 0 
		,VoucherAdjustment = ISNULL((
									SELECT SUM(dblTotal)
									FROM tblAPBillDetail
									WHERE intBillId = BillDtl.intBillId
										AND (
											intInventoryReceiptItemId IS NULL
	...
	SELECT 
		...
		,dtmDeliveryDate = (
								...
								FROM tblGRCustomerStorage GR
								JOIN tblSCTicket SC ON GR.intTicketId = SC.intTicketId
								WHERE GR.intCustomerStorageId = StrgHstry.intCustomerStorageId
						   )
		,intTicketId = (
		...
		,TotalDiscount = ISNULL((
									...
									WHERE intBillId = BillDtl.intBillId AND B.strType = 'Other Charge'
									), 0)
		,NetDue=(
					BillDtl.dblTotal + ISNULL((
												SELECT SUM(dblTax)
		...
		,VoucherAdjustment = ISNULL((
									...
									WHERE intBillId = BillDtl.intBillId
										AND (
											  strType NOT IN ('Other Charge','Inventory')
											)
									), 0) 
		,SalesAdjustment = 0 
		,CheckAmount = PYMT.dblAmountPaid
...
BEGIN
	...
	SELECT 
		...
		,strFarmField = CASE 
						...
						ELSE (
								SELECT strFarmNumber + '\' + strFieldNumber
								FROM tblEMEntityFarm
								WHERE intEntityId = VENDOR.[intEntityId] AND intFarmFieldId = (
																									SELECT TOP 1 ISNULL(SC.intFarmFieldId, 0)
																									FROM tblSCTicket SC
																									WHERE intTicketId = INVRCPTITEM.intSourceId
																								  )
		...
		,dblTax = ISNULL((	
							SELECT SUM(dblTax)
							FROM tblAPBillDetail
							WHERE intBillId = BillDtl.intBillId AND ISNULL(intContractDetailId, 0) = ISNULL(BillDtl.intContractDetailId, 0)
							), 0) 
		,strContractNumber = CNTRCT.strContractNumber
		,TotalDiscount = ISNULL((
									SELECT SUM(dblTotal)
									FROM tblAPBillDetail
									WHERE intBillId = BillDtl.intBillId AND ISNULL(intContractDetailId, 0) = ISNULL(BillDtl.intContractDetailId, 0) AND intInventoryReceiptChargeId IS NOT NULL
									), 0) 
		,NetDue = (
					BillDtl.dblTotal + ISNULL((
												SELECT SUM(dblTax)
	...
	SELECT 
		...
		,strUnitMeasure = UOM.strUnitMeasure
		,dblTotal = BillDtl.dblTotal
		,dblTax = ISNULL((SELECT SUM(dblTax)
							FROM tblAPBillDetail
							WHERE intBillId = BillDtl.intBillId
							), 0) 
