...
BEGIN TRY  
  ...
  IF NOT EXISTS (SELECT 1 FROM tblAPBill WHERE intBillId = @billId AND intTransactionType = 1 AND ISNULL(ysnFinalVoucher,0) = 1 AND dblAmountDue < 0)   
 BEGIN  
  ...
  dblCost,  
  dblNetWeight,  
  dblProvisionalPayment,  
  dblProvisionalFinalCost,  
  dblProvisionalFinalWeight 
 ...
 BEGIN  
    SELECT  
      @diffTotal = dblTotal - dblProvisionalPayment,  
      @diffCost = dblCost - dblProvisionalFinalCost,  
      @diffWeight = dblNetWeight - dblProvisionalFinalWeight,  
  ...
  BEGIN  
    ...
    UPDATE #tmpBillDetail SET dblTotal = @detailTotal  
  ,dblNetWeight = @weight   
  ,dblCost = @averageCost  
