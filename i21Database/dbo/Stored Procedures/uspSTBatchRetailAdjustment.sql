CREATE PROCEDURE [dbo].[uspSTBatchRetailAdjustment]
...
BEGIN
	...
	--Hard coded @ysnRecap to 0 to always post changes--
	SET @ysnRecap = 0;
	...
	BEGIN TRY
		...
		SET @strMessage = ''
			...
			-- ===========================================================================================================
			-- START Validate if there is any future effective date on RPA
			BEGIN
				IF EXISTS (SELECT TOP 1 1 FROM @tblRetailPriceAdjustmentDetailIds trpd
									INNER JOIN tblSTRetailPriceAdjustment trp
										ON trpd.intRetailPriceAdjustmentId = trp.intRetailPriceAdjustmentId
										AND trp.dtmEffectiveDate > GETDATE())
					BEGIN
						SET @ysnSuccess = CAST(0 AS BIT)
						SET @strMessage = 'Cannot post future effective date'
