...
BEGIN
	...
	SELECT	
		...
		dblDebit				= CASE WHEN A.strTransactionType = 'Invoice' THEN  (CASE WHEN @post = 1 THEN  B.dblTotal ELSE 0 END) ELSE 0 END, --Invoice Detail
		dblCredit				= CASE WHEN A.strTransactionType = 'Invoice' THEN 0 ELSE (CASE WHEN @post = 1 THEN 0 ELSE  B.dblTotal END) END, -- Invoice
		dblDebitUnit			= CASE WHEN A.strTransactionType = 'Invoice' THEN  (CASE WHEN @post = 1 THEN  ISNULL(A.dblInvoiceTotal, 0)  * ISNULL(U.dblLbsPerUnit, 0)  ELSE 0 END) ELSE 0 END,
		dblCreditUnit			= CASE WHEN A.strTransactionType = 'Invoice' THEN  0 ELSE (CASE WHEN @post = 1 THEN 0 ELSE ISNULL(A.dblInvoiceTotal, 0) * ISNULL(U.dblLbsPerUnit, 0)  END) END,
	...
	SELECT	
		...
		strReference = C.strCustomerNumber,
		dtmTransactionDate = A.dtmDate,
		dblDebit				= CASE WHEN A.strTransactionType = 'Invoice' THEN (CASE WHEN @post = 1 THEN 0 ELSE B.dblTotal END) ELSE 0 END, --Invoice Detail
		dblCredit				= CASE WHEN A.strTransactionType = 'Invoice' THEN  0  ELSE (CASE WHEN @post = 1 THEN B.dblTotal ELSE 0 END) END, -- Invoice
		dblDebitUnit			= CASE WHEN A.strTransactionType = 'Invoice' THEN  (CASE WHEN @post = 1 THEN 0 ELSE ISNULL(A.dblInvoiceTotal, 0)  * ISNULL(U.dblLbsPerUnit, 0) END) ELSE 0 END,
		dblCreditUnit			= CASE WHEN A.strTransactionType = 'Invoice' THEN 0 ELSE (CASE WHEN @post = 1 THEN ISNULL(A.dblInvoiceTotal, 0) * ISNULL(U.dblLbsPerUnit, 0) ELSE 0 END) END,
		dtmDate = DATEADD(dd, DATEDIFF(dd, 0, A.dtmDate), 0),
		ysnIsUnposted = CASE WHEN @post = 1 THEN 0 ELSE 1 END,
		intConcurrencyId = 1,
		dblExchangeRate		= 1,
	...
	SELECT	
		...
		strReference = C.strCustomerNumber,
		dtmTransactionDate = A.dtmDate,
		dblDebit				= CASE WHEN A.strTransactionType = 'Invoice' THEN  (CASE WHEN @post = 1 THEN A.dblInvoiceTotal ELSE 0 END) ELSE 0 END,
		dblCredit				= CASE WHEN A.strTransactionType = 'Invoice' THEN  0 ELSE (CASE WHEN @post = 1 THEN 0 ELSE A.dblInvoiceTotal END) END,
		dblDebitUnit			= CASE WHEN A.strTransactionType = 'Invoice' THEN  (CASE WHEN @post = 1 THEN ISNULL(A.dblInvoiceTotal, 0)  * ISNULL(U.dblLbsPerUnit, 0) ELSE 0 END) ELSE 0 END,
		dblCreditUnit			= CASE WHEN A.strTransactionType = 'Invoice' THEN  0 ELSE (CASE WHEN @post = 1 THEN 0 ELSE ISNULL(A.dblInvoiceTotal, 0) * ISNULL(U.dblLbsPerUnit, 0) END) END,
		dtmDate			= DATEADD(dd, DATEDIFF(dd, 0, A.dtmDate), 0),
		ysnIsUnposted = CASE WHEN @post = 1 THEN 0 ELSE 1 END,
...
ELSE
	BEGIN
		...
		SELECT	
			...
			strReference = C.strCustomerNumber,
			dtmTransactionDate = A.dtmDate,
			dblDebit = CASE WHEN A.strTransactionType = 'Invoice' THEN (CASE WHEN @post = 1 THEN B.dblTotal ELSE 0 END) ELSE 0 END,
			dblCredit = CASE WHEN A.strTransactionType = 'Invoice' THEN 0 ELSE (CASE WHEN @post = 1 THEN 0 ELSE  B.dblTotal END) END,
			dblDebitUnit = CASE WHEN A.strTransactionType = 'Invoice' THEN (CASE WHEN @post = 1 THEN ISNULL(A.dblInvoiceTotal, 0)  * ISNULL(U.dblLbsPerUnit, 0) ELSE 0 END) ELSE 0 END,
			dblCreditUnit = CASE WHEN A.strTransactionType = 'Invoice' THEN 0 ELSE (CASE WHEN @post = 1 THEN 0 ELSE ISNULL(A.dblInvoiceTotal, 0) * ISNULL(U.dblLbsPerUnit, 0) END) END,
			dtmDate = A.dtmDate,
			ysnIsUnposted = 0,
		...
		SELECT	
			...
			strReference = C.strCustomerNumber,
			dtmTransactionDate = A.dtmDate,
			dblDebit = CASE WHEN A.strTransactionType = 'Invoice' THEN (CASE WHEN @post = 1 THEN 0 ELSE B.dblTotal END) ELSE 0 END,
			dblCredit = CASE WHEN A.strTransactionType = 'Invoice' THEN 0 ELSE (CASE WHEN @post = 1 THEN B.dblTotal ELSE 0 END) END,
			dblDebitUnit = CASE WHEN A.strTransactionType = 'Invoice' THEN (CASE WHEN @post = 1 THEN 0 ELSE ISNULL(A.dblInvoiceTotal, 0)  * ISNULL(U.dblLbsPerUnit, 0) END) ELSE 0 END,
			dblCreditUnit = CASE WHEN A.strTransactionType = 'Invoice' THEN 0 ELSE (CASE WHEN @post = 1 THEN ISNULL(A.dblInvoiceTotal, 0) * ISNULL(U.dblLbsPerUnit, 0) ELSE 0 END) END,
			dtmDate = A.dtmDate,
			ysnIsUnposted = 0,
