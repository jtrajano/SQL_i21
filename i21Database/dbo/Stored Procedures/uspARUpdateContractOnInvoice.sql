...
BEGIN TRY
	...
	BEGIN
		...
		IF ISNULL(@intTicketId, 0) <> 0
			...
			END		
		IF (ISNULL(@intTicketId, 0) = 0 AND ISNULL(@intTicketTypeId, 0) <> 9 AND (ISNULL(@intTicketType, 0) <> 6 AND ISNULL(@strInOutFlag, '') <> 'O')) AND (ISNULL(@intInventoryShipmentItemId, 0) = 0 OR (@intInventoryShipmentItemId IS NOT NULL AND @strPricing = 'Subsystem - Direct')) AND ISNULL(@intLoadDetailId,0) = 0
			BEGIN
