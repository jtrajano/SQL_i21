...
BEGIN TRY
	...
	BEGIN
		...
		BEGIN
			...
			INSERT INTO tblRKLogger (intRecordId
			, intRecordDetailId
			, strRecord
			, strRecordDetail
			, dblQty1
		...
		SELECT @intContractHeaderId
			, @intContractDetailId
			, 'uspCTCreateDetailHistory'
			, NULL
			, NULL
			, NULL
			, NULL
			, 'contents of distinct table'
			, 'intContractStatusId = ' + CAST(ISNULL(intContractStatusId, 0) AS NVARCHAR)
			+ ', intCompanyLocationId = ' + CAST(ISNULL(intCompanyLocationId, 0) AS NVARCHAR)
			+ ', intItemId = ' + CAST(ISNULL(intItemId, 0) AS NVARCHAR)
			+ ', intPricingTypeId = ' + CAST(ISNULL(intPricingTypeId, 0) AS NVARCHAR)
			+ ', intFutureMarketId = ' + CAST(ISNULL(intFutureMarketId, 0) AS NVARCHAR)
			+ ', intFutureMonthId = ' + CAST(ISNULL(intFutureMonthId, 0) AS NVARCHAR)
			+ ', intDtlQtyInCommodityUOMId = ' + CAST(ISNULL(intDtlQtyInCommodityUOMId, 0) AS NVARCHAR)
			+ ', intDtlQtyUnitMeasureId = ' + CAST(ISNULL(intDtlQtyUnitMeasureId, 0) AS NVARCHAR)
			+ ', intBookId = ' + CAST(ISNULL(intBookId, 0) AS NVARCHAR)
			+ ', intSubBookId = ' + CAST(ISNULL(intSubBookId, 0) AS NVARCHAR)
			+ ', dtmStartDate = ' + CAST(ISNULL(dtmStartDate, 0) AS NVARCHAR)
			+ ', dtmEndDate = ' + CAST(ISNULL(dtmEndDate, 0) AS NVARCHAR)
			+ ', dblFutures = ' + CAST(ISNULL(dblFutures, 0) AS NVARCHAR)
			+ ', dblBasis = ' + CAST(ISNULL(dblBasis, 0) AS NVARCHAR)
			+ ', dblCashPrice = ' + CAST(ISNULL(dblCashPrice, 0) AS NVARCHAR)
			+ ', dblLotsPriced = ' + CAST(ISNULL(dblLotsPriced, 0) AS NVARCHAR)
			+ ', dblLotsUnpriced = ' + CAST(ISNULL(dblLotsUnpriced, 0) AS NVARCHAR)
			+ ', dblQtyPriced = ' + CAST(ISNULL(dblQtyPriced, 0) AS NVARCHAR)
			+ ', dblQtyUnpriced = ' + CAST(ISNULL(dblQtyUnpriced, 0) AS NVARCHAR)
			+ ', dblFinalPrice = ' + CAST(ISNULL(dblFinalPrice, 0) AS NVARCHAR)
			+ ', dblRatio = ' + CAST(ISNULL(dblRatio, 0) AS NVARCHAR)
			+ ', strPricingStatus = ' + CAST(ISNULL(strPricingStatus, 0) AS NVARCHAR)
			+ ', intItemUOMId = ' + CAST(ISNULL(intItemUOMId, 0) AS NVARCHAR)
			+ ', intPriceItemUOMId = ' + CAST(ISNULL(intPriceItemUOMId, 0) AS NVARCHAR)
			, NULL
			FROM (
			SELECT intContractStatusId
					...
					UNION ALL SELECT intContractStatusId
						...
						, intPriceItemUOMId
					FROM tblCTSequenceHistory
					WHERE intSequenceHistoryId = @intSequenceHistoryId) tbl
