...
/****** Object:  StoredProcedure [dbo].[uspAPCreateVendorRefundFromPayment]    Script Date: 02/03/2020 9:41:57 PM ******/
CREATE PROCEDURE [dbo].[uspAPCreateVendorRefundFromPayment]
	@voucherPayment BillPayment Readonly,
...
DECLARE @totalAmountPaid DECIMAL(18,6);

--IF EXISTS (SELECT 1 FROM tempdb..sysobjects WHERE id = OBJECT_ID('tempdb..#tmpBillsId')) DROP TABLE #tmpBillsId
--SELECT [intID] INTO #tmpBillsId 

--FROM [dbo].fnGetRowsFromDelimitedValues(@voucherIds)

SELECT @totalAmountPaid=SUM(dblPayment) from @voucherPayment

IF OBJECT_ID('tempdb..#tmpVouchersForPay') IS NOT NULL DROP TABLE #tmpVouchersForPay
CREATE TABLE #tmpVouchersForPay
(
	intBillId INT, intEntityVendorId INT, dblAmountPaid DECIMAL(18,2)
);

IF @transCount = 0 BEGIN TRANSACTION

UPDATE Bill 
SET dblTempPayment = T.dblPayment
from tblAPBill Bill join @voucherPayment T ON T.intBillId = Bill.intBillId
...
SELECT
	...
	,B.dblTempPayment * -1
FROM
@voucherPayment payVouchers  JOIN tblAPBill B on payVouchers.intBillId = B.intBillId

IF (@totalAmountPaid >= 0)
BEGIN
	RAISERROR('Total Amount should be less than zero.', 16, 1);
	RETURN;
END
...
SELECT
	[intId]								=	RANK() OVER(ORDER BY payVouchers.intBillId, payVouchers.intEntityVendorId)
	...
	,[dblAmountDue]						=	(A.dblTotal - A.dblTempDiscount + A.dblTempInterest)
											- (A.dblTempPayment)
	,[strPaymentOriginalId]				=	'Payment Origin Id ' + CAST(RANK() OVER(ORDER BY payVouchers.intBillId, payVouchers.intEntityVendorId) AS NVARCHAR(100))
