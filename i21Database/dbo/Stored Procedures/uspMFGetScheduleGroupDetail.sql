CREATE PROCEDURE [dbo].[uspMFGetScheduleGroupDetail] 
(
...
IF @intLimit = 1 AND @strGroupDetailValue <> ''
	BEGIN
		SET @sqlCommand = 'SELECT TOP ' + LTRIM(@intLimit) + @strColumnName + ' AS strName
									  , strDescription
									  , ROW_NUMBER() OVER (ORDER BY ' + @strColumnName + ') AS intRow
...
ELSE
	BEGIN
		SET @sqlCommand = 'SELECT TOP ' + LTRIM(@intLimit) + ' *
								, 0 AS intConcurrencyId 
							FROM (SELECT ' + @strColumnName + ' AS strName
									   , strDescription, ROW_NUMBER() OVER (ORDER BY ' + @strColumnName + ') AS intRow
								  FROM ' + @strTableName + '
								  WHERE ' + @strColumnName + ' NOT IN (SELECT strGroupValue 
																	   FROM dbo.tblMFScheduleGroupDetail 
																	   WHERE intScheduleGroupId <>' + LTRIM(@intScheduleGroupId) + ') AND ' + @strColumnName + ' NOT IN (''' + @strFilterCriteria + ''')) AS DT WHERE intRow > ' + LTRIM(@intStart) +' AND strName LIKE '''+@strFilter + '%'''
