...
BEGIN TRY
	...
	BEGIN
		BEGIN TRY
			...
			SELECT @strOrderType = NULL

			SELECT @strOrderType = OrderType
			FROM OPENXML(@idoc, 'root/Header', 2) WITH (OrderType NVARCHAR(50))
			...
			BEGIN
				...
				SELECT DocNo
					...
					,WeightUOM
					,NULL AS strStatus
					,NULL AS strReasonCode
					,PONumber AS strNotes
					,NULL AS strNewLocation
					,NULL AS strNewStorageLocation
					,NULL AS strNewStorageUnit
					,NULL AS strOrderNo
					,NULL AS intOrderCompleted
					,NULL AS dtmExpiryDate
				FROM OPENXML(@idoc, 'root/Header/Line/Detail', 2) WITH (
						DocNo BIGINT '../../../DocNo'
