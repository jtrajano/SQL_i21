...
RETURN -- EXIT WHEN CURRENCIES ARE FUNCTIONAL
 ...
 BEGIN  
   ...
   ,[strBatchId]   = A.strBatchId  
   ,[intAccountId]   = @intRealizedGainAccountId  
   ,[dblDebit]    = case when @intBankTransferTypeId = 5 THEN Debit.GainLoss ELSE Credit.GainLoss END  
   ,[dblCredit]   = case when @intBankTransferTypeId = 5 THEN Credit.GainLoss ELSE Debit.GainLoss END  
   ,[dblDebitForeign]  =case when @intBankTransferTypeId = 5 THEN Debit.GainLoss ELSE Credit.GainLoss END  
   ,[dblCreditForeign]  = case when @intBankTransferTypeId = 5 THEN Credit.GainLoss ELSE Debit.GainLoss END  
   ,[dblDebitUnit]   = 0  
   ,[dblCreditUnit]  = 0  
   ,[strDescription]  = @strDescription --'Gain / Loss on Multicurrency Bank Transfer'  
   ,[strCode]    = A.strCode  
   ...
   CROSS APPLY (SELECT case when BankTo.gainLoss >= 0 then BankTo.gainLoss  else 0 end GainLoss ) Debit  
