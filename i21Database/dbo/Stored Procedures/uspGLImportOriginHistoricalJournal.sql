...
BEGIN TRANSACTION
	...
	SELECT 
		...
		CASE WHEN glhst_amt >= 0 THEN 
			...
			END AS Credit,		
		0 AS CreditRate,		
		CASE WHEN glhst_units >= 0 THEN 
			CASE WHEN glhst_dr_cr_ind = 'D' THEN glhst_units ELSE 0 END
			ELSE CASE WHEN (glhst_dr_cr_ind='C' OR glhst_dr_cr_ind IS NULL) THEN (glhst_units * -1) ELSE 0 END 
			END AS DebitUnits,
		CASE WHEN glhst_units >= 0 THEN 
			CASE WHEN (glhst_dr_cr_ind='C' OR glhst_dr_cr_ind IS NULL) THEN glhst_units ELSE 0 END
			ELSE CASE WHEN glhst_dr_cr_ind = 'D' THEN (glhst_units * -1) ELSE 0 END 
			END AS CreditUnits, -- credit unit rate
