...
BEGIN
	...
	SET intStatus = 
			CASE 
				@ysnPosting WHEN 1 THEN 
					CASE @ysnPosted WHEN 1 THEN 4 ELSE 1 END
			ELSE 
				CASE 
	...
	UPDATE l
	SET 
		l.ysnLockedInventory = @ysnLocked
	FROM 
		tblICLot l INNER JOIN tblICInventoryCountDetail cd 
			ON cd.intLotId = l.intLotId
	WHERE 
		cd.intInventoryCountId = @intTransactionId

	-- Lock Non-Lotted Items by Company Location
	UPDATE il
	SET 
		il.ysnLockedInventory = @ysnLocked
	FROM 
		tblICItemLocation il INNER JOIN tblICInventoryCountDetail cd 
			ON cd.intItemLocationId = il.intItemLocationId
	...
	BEGIN
		...
		INSERT INTO tblICLockedSubLocation(
			intTransactionId
			, strTransactionId
			, intItemId
			, intSubLocationId
