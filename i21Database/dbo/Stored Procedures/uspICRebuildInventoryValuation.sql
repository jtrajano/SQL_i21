...
BEGIN 
	...
	BEGIN 
		...
		FROM	#tmpUnOrderedICTransaction t LEFT JOIN #tmpPriorityTransactions priorityTransaction
					ON t.strTransactionId = priorityTransaction.strTransactionId
		ORDER BY 
			intInventoryTransactionId ASC,  CAST(REPLACE(strBatchId, 'BATCH-', '') AS FLOAT) ASC 
...
BEGIN 
	DECLARE @strBatchId AS NVARCHAR(40)
			...
			,@MissingLotsForPost AS ItemCostingTableType 
	...
	BEGIN 
		...
		BEGIN 
			...
			-- Repost the Cost Adjustment from Settle Storage
			...
			BEGIN 
				...
				BEGIN
					...
					-- Receive Lot that has a valid Gross/Net UOM. 
					INSERT INTO @ItemsToPost (
							intItemId  
							,intItemLocationId 
							,intItemUOMId  
					...
					SELECT 	Detail.intItemId  
							...
							,dblQty = -- -TransferSource.dblQty
								CASE 
									WHEN Detail.intGrossNetUOMId = TransferSource.intItemUOMId AND Detail.intLotId IS NOT NULL THEN 
										Detail.dblNet
									ELSE	
										TransferSourceQty.dblQty 
								END	

							,ISNULL(ItemUOM.dblUnitQty, TransferSource.dblUOMQty)
							,dblCost = --TransferSource.dblCost 
								CASE 
									WHEN Detail.intGrossNetUOMId = TransferSource.intItemUOMId AND Detail.intLotId IS NOT NULL THEN 
										dbo.fnDivide(TransferSourceQty.dblValue, Detail.dblNet) 
									ELSE	
										ISNULL(TransferSourceQty.dblCost, 0)
								END
							,dblForexCost = --TransferSource.dblForexCost  
								CASE 
									WHEN Detail.intGrossNetUOMId = TransferSource.intItemUOMId AND Detail.intLotId IS NOT NULL THEN 
										dbo.fnDivide(TransferSourceQty.dblForexValue, Detail.dblNet) 
									ELSE	
										ISNULL(TransferSourceQty.dblForexCost, 0)
								END
							,dblSalesPrice = 0
							,intCurrencyId = TransferSource.intCurrencyId
							,dblExchangeRate = TransferSourceQty.dblExchangeRate --TransferSource.dblExchangeRate									
							...
							,dblForexRate = TransferSourceQty.dblForexRate --TransferSource.dblForexRate
							,intCostingMethod = 
								CASE 
					...
					WHERE	Header.strTransferNo = @strTransactionId
							...
							AND Detail.intGrossNetUOMId = TransferSource.intItemUOMId
					...
					SELECT 	Detail.intItemId  
							...
							,dblSalesPrice = 0
							,intCurrencyId = TransferSource.intCurrencyId
							,dblExchangeRate = TransferSource.dblExchangeRate
							,TransferSource.intTransactionId  
							,Detail.intInventoryTransferDetailId
							,Header.strTransferNo
							,TransferSource.intTransactionTypeId  
					...
					WHERE	Header.strTransferNo = @strTransactionId
							...
							AND (
								(Detail.intLotId IS NOT NULL AND ISNULL(Detail.intGrossNetUOMId, 0) <> TransferSource.intItemUOMId) 
			...
			ELSE IF @strTransactionType IN ('Inventory Adjustment - Split Lot', 'Inventory Adjustment - Lot Merge', 'Inventory Adjustment - Lot Move')
			BEGIN 
			...
			*/
			...
			ELSE IF EXISTS (SELECT	1 FROM	tblICInventoryTransactionType WHERE	intTransactionTypeId = @intTransactionTypeId AND strName IN ('Invoice', 'Credit Memo')) 
			BEGIN 
			...
			BEGIN 
				...
				BEGIN 
					...
					FROM	
						@ItemsToPost itemsToPost INNER JOIN tblICItem i 
				...
				BEGIN 
					...
					FROM	(
								SELECT DISTINCT 
									...
									,dtp.intTransactionId
									,dtp.intTransactionDetailId									
									,dtp.intItemId
									,dtp.intItemLocationId
								FROM 
									@ItemsToPost dtp 
							) dtp 
							CROSS APPLY (
								...
								FROM 
									@ItemsToPost tp
								WHERE
									tp.strTransactionId = dtp.strTransactionId
									AND tp.intTransactionId = dtp.intTransactionId
									AND tp.intTransactionDetailId = dtp.intTransactionDetailId
									AND tp.intItemId = dtp.intItemId
									AND tp.intItemLocationId = dtp.intItemLocationId
							) tp					
							INNER JOIN tblICItem i 
					...
					WHERE	
						i.strType <> 'Bundle' -- Do not include Bundle items in the in-transit costing. Bundle components are the ones included in the in-transit costing. 
					ORDER BY 
				...
				-------------------------------------------------------
				-- Process Missing Lots for Return
				-------------------------------------------------------
				IF @strTransactionType = 'Inventory Return'
				BEGIN 
					DELETE @MissingLotsForPost WHERE dblQty < 0 
					DELETE @ItemsToPost WHERE strCondition = 'Missing' AND dblQty > 0 
				...
				-------------------------------------------------------
				-- Receive on-hand stock. 				
				-------------------------------------------------------					
				IF	EXISTS (SELECT TOP 1 1 FROM @MissingLotsForPost)
					AND @strTransactionType = 'Inventory Receipt'
				BEGIN 
					-- Remove the missing lots that was auto-reduced in the IR. 
					DELETE @ItemsToPost WHERE strCondition = 'Missing' AND dblQty < 0 
				END 
				...
				-------------------------------------------------------
				-- Process Missing Lots for Receipt
				-------------------------------------------------------
				...
				BEGIN 
					...
					-- Process the GL entries for Missing Stocks. Auto reduce the missing lots. 
					IF EXISTS (SELECT TOP 1 1 FROM @MissingLotsForPost)
					BEGIN 			
						SET @strMissingLotBatchId = @strBatchId + '1'
						INSERT INTO @GLEntries (
			...
			ELSE IF EXISTS (SELECT 1 WHERE @strTransactionType IN ('Outbound Shipment', 'Transfer Shipment')) 
			BEGIN 
				SELECT	@strAccountToCounterInventory = NULL 
			...
			BEGIN 								
				...
				-- Normally, Opening balance does not generate GL entries. 
			...
			-- Inventory Fix code: 001-C will re-add the stock back. 
			ELSE IF	@ysnForceClearTheCostBuckets = 1 
				AND @strTransactionType IN ('Inventory Adjustment - Opening Inventory') 
				AND EXISTS (
					SELECT TOP 1 1 FROM tblICInventoryAdjustment a WHERE a.intInventoryAdjustmentId = @intTransactionId AND a.strDescription LIKE '%Inventory Fix code: 001-C%'
				)
			BEGIN 								
				--PRINT 'Inventory Fix code: 001-C'
