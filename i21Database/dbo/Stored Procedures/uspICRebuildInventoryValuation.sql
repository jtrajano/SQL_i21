...
BEGIN 
	...
	BEGIN 
		...
		BEGIN 
			...
			-- Specific repost routines for the following Inventory adjustments: 
			ELSE IF EXISTS (SELECT	TOP 1 1 FROM dbo.tblICInventoryTransactionType WHERE intTransactionTypeId = @intTransactionTypeId AND strName IN ('Inventory Adjustment - Item Change', 'Inventory Adjustment - Split Lot', 'Inventory Adjustment - Lot Merge', 'Inventory Adjustment - Lot Move'))
			BEGIN 
			...
			-- Re-create the Post g/l entries (except for Cost Adjustments, Inventory Shipment, and Invoice)
			IF EXISTS (
				SELECT	TOP 1 1 
			...
			BEGIN 
				...
				-- Do the Regular GL entries used by uspICPostCosting 
				BEGIN 
					SET @intReturnId = NULL 
					INSERT INTO @GLEntries (
							[dtmDate] 
		...
		BEGIN 
			...
			BEGIN CATCH
				...
				SELECT 't', * FROM tblICInventoryTransaction t WHERE t.strTransactionId = @strTransactionId AND t.intItemId = ISNULL(@intItemId, t.intItemId) 
