...
BEGIN TRY
	...
	SELECT	intContractHeaderId		= CD.intContractHeaderId,
			...
			strWalterPricing		= CASE WHEN CD.intPricingTypeId in (1,6) THEN 'Priced at ' + LTRIM(CAST(CD.dblCashPrice AS NUMERIC(18, 2))) +' ' + OP.strCurrency + '/' + PU.strUnitMeasure ELSE 
									   ...
									   +  CASE WHEN CD.intPricingTypeId in (2) AND CD.dblBasis > 0	  THEN '  +' +  LTRIM(CAST(CD.dblBasis AS NUMERIC(18, 2)))  + ' ' + OP.strCurrency + '/' + PU.strUnitMeasure  ELSE '     ' +  LTRIM(CAST(CD.dblBasis AS NUMERIC(18, 2)))  + ' ' + OP.strCurrency + '/' + PU.strUnitMeasure END + ''	
