CREATE PROCEDURE uspAPAccountReportVSGLSummary
(
...
IF OBJECT_ID(N'tmpAPAccountBalance') IS NOT NULL DROP TABLE tmpAPAccountBalance
CREATE TABLE tmpAPAccountBalance(dblBalance DECIMAL(18,6))
...
DECLARE @deletedForeignQuery NVARCHAR(MAX);
...
--GET THE BALANCE
IF OBJECT_ID(N'tmpAPGLAccountBalance') IS NOT NULL DROP TABLE tmpAPGLAccountBalance
CREATE TABLE tmpAPGLAccountBalance(dblBalance DECIMAL(18,6))

INSERT INTO tmpAPGLAccountBalance
SELECT
	SUM(ISNULL(A.dblCredit,0) - ISNULL(A.dblDebit, 0)) AS dblBalance
FROM tblGLDetail A
