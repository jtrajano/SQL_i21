...
DECLARE @ReturnValuation AS TABLE (
...
(
	...
	SELECT	t.dtmDate
			...
			,dblCost = ISNULL(ri.dblLineTotalFunctional, 0) - ISNULL(valuation.dblTotalFunctional, 0) 
			,dblValue = 0
			,t.intTransactionTypeId
			,ISNULL(t.intCurrencyId, @intFunctionalCurrencyId) intCurrencyId
			,t.dblExchangeRate
			...
			(
				SELECT
					...
					,dblLineTotalForeign = 
						SUM(
							ROUND(
								dbo.fnMultiply(-t.dblQty, rv.dblReturnUnitCost)
								, 2
							)
						)
					,dblLineTotalFunctional = 
						SUM(
							CASE 
								WHEN t.intCurrencyId <> @intFunctionalCurrencyId AND t.dblForexRate <> 0 THEN 
									ROUND(
										dbo.fnMultiply(-t.dblQty, rv.dblReturnUnitCost) * t.dblForexRate
										, 2
									)	
								ELSE 
									ROUND (
				...
				FROM 
					@ReturnValuation rv INNER JOIN tblICInventoryReceipt r 
						ON rv.strReceiptNumber = r.strReceiptNumber
					INNER JOIN tblICInventoryReceiptItem ri
						ON r.intInventoryReceiptId = ri.intInventoryReceiptId
					INNER JOIN tblICItem i 
					...
					INNER JOIN tblICInventoryTransaction t 
						...
						AND t.strTransactionId = r.strReceiptNumber
						AND t.intTransactionDetailId = ri.intInventoryReceiptItemId
						AND t.intInventoryTransactionId = rv.intInventoryTransactionId
				WHERE
					t.strBatchId = @strBatchId						
				GROUP BY
					r.strReceiptNumber
...
SELECT	
		...
		,strDescription				= ISNULL(@strGLDescription, tblGLAccount.strDescription) + ' ' + dbo.[fnICDescribeSoldStock](strItemNo, dblQty, dblReceiptUnitCost, strLotNumber) 
		,strCode					= 'IC' 
		,strReference				= ''
...
SELECT	
		...
		,strDescription				= ISNULL(@strGLDescription, tblGLAccount.strDescription) + ' ' + dbo.[fnICDescribeSoldStock](strItemNo, dblQty, (ISNULL(dblCost, 0) - ISNULL(dblReceiptUnitCost, 0)), strLotNumber) 
		,strCode					= 'IC' 
		,strReference				= ''
		,intCurrencyId				= ForGLEntries_CTE.intCurrencyId
		,dblExchangeRate			= ForGLEntries_CTE.dblExchangeRate
...
SELECT	
		...
		,dblDebitUnit				= DebitUnit.Value
		,dblCreditUnit				= CreditUnit.Value
		,strDescription				= ISNULL(ForGLEntries_CTE.strDescription, tblGLAccount.strDescription) + ' ' + dbo.[fnICDescribeSoldStock](strItemNo, dblQty, dblCost, strLotNumber)
		,strCode					= 'IAN' 
		,strReference				= ''
		,intCurrencyId				= ForGLEntries_CTE.intCurrencyId
		,dblExchangeRate			= ForGLEntries_CTE.dblExchangeRate
