...
SELECT intInvoiceDetailId	= ID.intInvoiceDetailId
	...
	, dblQtyOrdered			= CASE WHEN ID.intSalesOrderDetailId IS NOT NULL OR (ID.intTicketId IS NOT NULL AND ISNULL(intTicketType, 0) <> 6 AND ISNULL(strInOutFlag, '') <> 'O') THEN IDD.[dblQtyOrdered] ELSE 0 END
