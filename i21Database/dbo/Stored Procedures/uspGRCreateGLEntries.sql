...
BEGIN
	...
	SELECT @strTransactionId = strStorageTicket FROM tblGRSettleStorage WHERE intSettleStorageId = @intSettleStorageId
	...
	WHERE intItemId = @InventoryItemId AND intLocationId = @LocationId
	...
	JOIN tblICItem IC ON IC.intItemId = ContractCost.intItemId
	WHERE 
	ContractCost.intItemId != @intFreightItemId AND 
	RE.intContractDetailId IS NOT NULL  AND 
	ContractCost.dblRate != 0 AND 
	SST.intSettleStorageId = @intSettleStorageId
	...
	FROM tblGRCustomerStorage CS 
	JOIN tblGRSettleStorageTicket SST ON  SST.intCustomerStorageId = CS.intCustomerStorageId
	LEFT JOIN tblICItemLocation ItemLocation ON ItemLocation.intItemId = CS.intItemId AND ItemLocation.intLocationId = CS.intCompanyLocationId 
	WHERE SST.intSettleStorageId = @intSettleStorageId
	...
	LEFT JOIN tblICItemLocation ItemLocation ON ItemLocation.intItemId = Dcode.intItemId AND ItemLocation.intLocationId = @LocationId
	...
	LEFT JOIN tblICItemLocation ItemLocation ON ItemLocation.intItemId = OtherCharges.intItemId AND ItemLocation.intLocationId = @LocationId
	...
	FROM @tblItem Query WHERE Query.intItemType = 1	AND EXISTS(SELECT 1  FROM @tblOtherCharges WHERE ysnInventoryCost = 1)	
	...
	SELECT 
	 ...
	 ,[strBatchId]             
	 ,[intAccountId]           
	 ,[dblDebit] = CASE WHEN ABS(t.dblDebit)>0 THEN   ABS(Item.dblCost * t.dblUnits) ELSE 0 END             
	 ,[dblCredit] = CASE WHEN ABS(t.dblCredit)>0 THEN ABS(Item.dblCost * t.dblUnits) ELSE 0 END             
	 ,[dblDebitUnit]           
	 ,[dblCreditUnit]          
	 ,[strDescription]         
	 ,[strCode]                
	...
	INNER JOIN dbo.tblGLAccount GLAccount ON GLAccount.intAccountId = ItemGLAccounts.intInventoryId
	...
	INNER JOIN @ItemGLAccounts ItemGLAccounts ON InventoryCostCharges.intItemId = ItemGLAccounts.intItemId
		AND InventoryCostCharges.intItemLocationId = ItemGLAccounts.intItemLocationId
	INNER JOIN dbo.tblGLAccount GLAccount ON GLAccount.intAccountId = ItemGLAccounts.intInventoryId
	CROSS APPLY dbo.fnGetDebitFunctional(InventoryCostCharges.dblCost, InventoryCostCharges.intCurrencyId, @intFunctionalCurrencyId, InventoryCostCharges.dblForexRate) Debit
	CROSS APPLY dbo.fnGetCreditFunctional(InventoryCostCharges.dblCost, InventoryCostCharges.intCurrencyId, @intFunctionalCurrencyId, InventoryCostCharges.dblForexRate) Credit
	...
	WHERE 
		ISNULL(InventoryCostCharges.ysnAccrue, 0) = 1		 AND 
		ISNULL(InventoryCostCharges.ysnInventoryCost, 0) = 1 AND 
	InventoryCostCharges.strBundleType != 'Kit'
	...
	INNER JOIN @ItemGLAccounts ItemGLAccounts ON InventoryCostCharges.intItemId = ItemGLAccounts.intItemId
		AND InventoryCostCharges.intItemLocationId = ItemGLAccounts.intItemLocationId
	INNER JOIN dbo.tblGLAccount GLAccount ON GLAccount.intAccountId = ItemGLAccounts.intContraInventoryId
	CROSS APPLY dbo.fnGetDebitFunctional(InventoryCostCharges.dblCost, InventoryCostCharges.intCurrencyId, @intFunctionalCurrencyId, InventoryCostCharges.dblForexRate) Debit
	CROSS APPLY dbo.fnGetCreditFunctional(InventoryCostCharges.dblCost, InventoryCostCharges.intCurrencyId, @intFunctionalCurrencyId, InventoryCostCharges.dblForexRate) Credit
	CROSS APPLY dbo.fnGetDebit(InventoryCostCharges.dblCost) DebitForeign
	CROSS APPLY dbo.fnGetCredit(InventoryCostCharges.dblCost) CreditForeign
