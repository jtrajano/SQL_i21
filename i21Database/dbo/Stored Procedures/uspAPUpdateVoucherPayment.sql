...
BEGIN TRY
	...
	BEGIN
		...
		SET
			...
			@ysnInPayment = CASE WHEN ((CASE WHEN B.intTransactionType = 16 THEN B.dblProvisionalTotal ELSE B.dblTotal END) - ISNULL(appliedPrepays.dblPayment, 0)) = @dblPaymentTemp
							THEN 1
							ELSE
								CASE WHEN @dblPaymentTemp < 0 OR @dblPaymentTemp > ((CASE WHEN B.intTransactionType = 16 THEN B.dblProvisionalTotal ELSE B.dblTotal END) - ISNULL(appliedPrepays.dblPayment, 0))
								THEN NULL
								ELSE 0
								END
							END,
