...
SET @Savepoint = SUBSTRING(('ARUpdateCustomerInvoices' + CONVERT(VARCHAR, @InitTranCount)), 1, 32)
...
FROM
	@InvoicesToUpdate ITG --WITH (NOLOCK)
WHERE
	ISNULL(ITG.[strType],'') NOT IN ('Meter Billing', 'Standard', 'Software', 'Tank Delivery', 'Provisional', 'Service Charge', 'Transport Delivery', 'Store', 'Card Fueling', 'CF Tran', 'CF Invoice', 'Store End of Day')
