CREATE PROCEDURE [dbo].[uspCTAllowDPContractClosure]
	@ysnAutoCompleteDPDeliveryDate BIT
AS
BEGIN TRY
	...
	BEGIN
		...
		DECLARE @currentDatabaseName AS NVARCHAR(100) = DB_NAME()


		IF @ysnAutoCompleteDPDeliveryDate = 1
		BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				SELECT @step_id = step_id FROM msdb.dbo.sysjobsteps WHERE (step_name = @stepName)

				EXEC @ReturnCode = msdb.dbo.sp_delete_jobstep @job_id=@jobId,@step_id=@step_id
				IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback

				IF @step_id IS NOT NULL
				BEGIN
					EXEC @ReturnCode = msdb.dbo.sp_delete_jobschedule @job_id=@jobId, @name=@stepNameSchedule	
					IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
				END	

				IF NOT EXISTS (SELECT TOP 1 1 FROM msdb.dbo.sysjobsteps WHERE step_name LIKE 'Invoke Contract Hourly Maintenance Scheduler in%')
				BEGIN
