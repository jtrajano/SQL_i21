...
SELECT strCompanyName		= (SELECT TOP 1 strCompanyName FROM tblSMCompanySetup)
     ...
     , * 
	 , dblARBalance			= ISNULL((SELECT SUM(dblDebit) - SUM(dblCredit) FROM vyuGLDetail WHERE strCode = ''AR'' AND ysnIsUnposted = 0 AND dtmTransactionDate <= @dtmDateTo AND intAccountId = (SELECT TOP 1 intAccountId FROM vyuGLAccountDetail WHERE strAccountCategory IN (''AR Account''))), 0)
	 , strARAccount			= (SELECT TOP 1 strAccountId FROM vyuGLAccountDetail WHERE strAccountCategory IN (''AR Account''))
	 , dblPrepaymentBalance	= ISNULL((SELECT SUM(dblDebit) - SUM(dblCredit) FROM vyuGLDetail WHERE strCode = ''AR'' AND ysnIsUnposted = 0 AND dtmTransactionDate <= @dtmDateTo AND intAccountId = (SELECT TOP 1 intAccountId FROM vyuGLAccountDetail WHERE strAccountCategory IN (''Customer Prepayments''))), 0)
