...
(SELECT intEntityCustomerId	= C.intEntityCustomerId
	  ...
	  , strStatementFooterComment = dbo.fnARGetFooterComment(NULL, TRANSACTIONS.intEntityCustomerId, ''Statement Footer'', 1)
    ...
    BEGIN
        SET @queryBudget = CAST('' AS NVARCHAR(MAX)) + 
            'SELECT intEntityCustomerId         = C.intEntityCustomerId 
				  ...
				  , strFullAddress				= NULL
				  , strStatementFooterComment	= dbo.fnARGetFooterComment(NULL, CB.intEntityCustomerId, ''Statement Footer'', 1)
				  , strCompanyName				= NULL
