...
BEGIN TRY
	...
	WHERE	ri.intRecipeId = @intRecipeId
			...
			AND ((
					...
					AND CONVERT(DATETIME, CONVERT(CHAR, GETDATE(), 101)) BETWEEN ri.dtmValidFrom AND ri.dtmValidTo
				)
				OR (
					ri.ysnYearValidationRequired = 0
					AND DATEPART(dy, GETDATE()) BETWEEN DATEPART(dy, ri.dtmValidFrom) AND DATEPART(dy, ri.dtmValidTo)
				)
			)
	...
	SELECT L.intLotId
			...
			,L.intItemUOMId
	FROM	dbo.tblICLot L JOIN @tblItem I 
				ON L.intItemId = I.intItemId
			JOIN dbo.tblICLotStatus LS 
				on LS.intLotStatusId =L.intLotStatusId 
	WHERE	LS.strSecondaryStatus ='Active'
			AND dtmExpiryDate >= Getdate()
			AND L.intStorageLocationId = (
				CASE	WHEN I.intStorageLocationId IS NULL THEN 
	...
	BEGIN
		...
		BEGIN
			...
			-- There is no sufficient quantity for the item %s.
			RAISERROR (51096,11,1,@strItemNo)
		END
