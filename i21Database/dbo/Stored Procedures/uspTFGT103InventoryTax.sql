...
DECLARE @tblTempTransaction TABLE (
	...
	SET @RCId = (SELECT intReportingComponentId FROM @tblTempReportingComponent WHERE intId = @CountRC)
		...
		ELSE
			...
			SET @QueryInventory1 = 'SELECT DISTINCT 0,tblICInventoryReceiptItem.intInventoryReceiptItemId, tblTFReportingComponent.intTaxAuthorityId, tblTFReportingComponent.strFormCode, 
                         ...
                         tblSMCompanySetup.strStateTaxID, tblSMCompanySetup.strFederalTaxID, tblEMEntityLocation.strState AS strOriginState, tblSMCompanyLocation.strStateProvince AS strDestinationState, 
                         tblTFTerminalControlNumber.strTerminalControlNumber FROM tblTRSupplyPoint INNER JOIN
                         ...
                         tblEMEntity INNER JOIN tblSMTaxCode INNER JOIN tblTFTaxCategory ON tblSMTaxCode.intTaxCategoryId = tblTFTaxCategory.intTaxCategoryId INNER JOIN
                         ...
                         tblEMEntityLocation ON tblICInventoryReceipt.intShipFromId = tblEMEntityLocation.intEntityLocationId ON tblTRSupplyPoint.intEntityLocationId = tblICInventoryReceipt.intShipFromId FULL OUTER JOIN
