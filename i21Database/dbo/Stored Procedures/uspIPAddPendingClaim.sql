CREATE PROCEDURE uspIPAddPendingClaim @intLoadId INT
	,@intPurchaseSale INT
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			WHERE intLoadId = @intLoadId
				...
				AND (
					@intLoadContainerId IS NULL
					OR (
						@intLoadContainerId IS NOT NULL
			...
			IF EXISTS (
					...
					WHERE ISNULL(ysnWeightClaimsByContainer, 0) = 1
					)
			BEGIN
				--Claims By Container
				...
				SELECT [intPurchaseSale] = 1
					...
					,[dblWeightLoss]
					,[dblClaimableWt]
					,[dblClaimableAmount] = ROUND(CASE 
							WHEN (
									(
										(dblClaimableWt * dblSeqPriceInWeightUOM) / CASE 
				...
				FROM (
					SELECT intEntityId = EM.intEntityId
						...
						,dblFranchiseWt = CASE 
							WHEN (ISNULL(CLNW.dblLinkNetWt, 0) * WG.dblFranchise / 100) <> 0.0
								THEN ((ISNULL(CLNW.dblLinkNetWt, 0) - ISNULL(IRN.dblIRNet, 0)) * WG.dblFranchise / 100)
							ELSE 0.0
							END
						,dblWeightLoss = CASE 
							WHEN (RI.dblNet - ISNULL(CLNW.dblLinkNetWt, 0)) < 0.0
								THEN (RI.dblNet - ISNULL(CLNW.dblLinkNetWt, 0))
							ELSE (RI.dblNet - ISNULL(CLNW.dblLinkNetWt, 0))
							END
						,dblClaimableWt = CASE 
							WHEN ((RI.dblNet - ISNULL(CLNW.dblLinkNetWt, 0)) + (ISNULL(CLNW.dblLinkNetWt, 0) * WG.dblFranchise / 100)) < 0.0
					...
					OUTER APPLY (
						SELECT TOP 1 intWeightClaimId = WC.intWeightClaimId
						FROM tblLGWeightClaim WC
						INNER JOIN tblLGWeightClaimDetail WCD ON WC.intWeightClaimId = WCD.intWeightClaimId
						WHERE WCD.intContractDetailId = LD.intPContractDetailId
							AND WC.intLoadId = L.intLoadId
							AND WC.intPurchaseSale = 1
							AND WCD.intLoadContainerId = LC.intLoadContainerId
						) WC
					OUTER APPLY (
						...
						WHERE IU.intItemId = CD.intItemId
							AND IU.intUnitMeasureId = L.intWeightUnitMeasureId
						) WUI
					OUTER APPLY (
					...
					OUTER APPLY (
						SELECT TOP 1 strSubLocation = CLSL.strSubLocationName
					...
					CROSS APPLY (
						SELECT dblNet = SUM(ISNULL(IA.dblNet, 0))
							,dblGross = SUM(ISNULL(IA.dblGross, 0))
						FROM tblIPInventoryAdjustment IA
						WHERE IA.intContractHeaderId = CH.intContractHeaderId
							AND IA.intContractDetailId = CD.intContractDetailId
							AND IA.intLoadDetailId = LD.intLoadDetailId
							AND IA.intLoadContainerId = CLNW.intLoadContainerId
						) RI
					CROSS APPLY (
						SELECT dblIRNet = SUM(ISNULL(IRI.dblNet, 0))
							,dblIRGross = SUM(ISNULL(IRI.dblGross, 0))
					...
					WHERE L.intLoadId = @intLoadId
						...
						AND NOT EXISTS (
							...
							FROM tblLGPendingClaim
							WHERE intLoadId = @intLoadId
								AND intPurchaseSale = @intPurchaseSale
			...
			BEGIN
				...
				FROM (
					SELECT intEntityId = EM.intEntityId
						...
						,dblShippedNetWt = (
							CASE 
								WHEN (CLNW.dblLinkNetWt IS NOT NULL)
					...
					OUTER APPLY (
						SELECT dblLinkNetWt = SUM(ISNULL(dblLinkNetWt, 0))
						FROM tblLGLoadDetailContainerLink
						WHERE intLoadDetailId = LD.intLoadDetailId
						) CLNW
					CROSS APPLY (
		...
		BEGIN
			...
			FROM (
				SELECT intEntityId = EM.intEntityId
					...
					,intWeightUnitMeasureId = L.intWeightUnitMeasureId
					,intWeightId = CH.intWeightId
					,dblShippedNetWt = CASE 
						WHEN (CLCT.intCount) > 0
							THEN (CLNW.dblLinkNetWt)
						ELSE LD.dblNet
						END
					,dblReceivedNetWt = CASE 
						WHEN (CLCT.intCount) > 0
							THEN (CLNW.dblLinkNetWt)
						ELSE LD.dblNet
						END
					,dblReceivedGrossWt = CASE 
						WHEN (CLCT.intCount) > 0
							THEN (CLNW.dblLinkGrossWt)
						ELSE LD.dblGross
						END
					,dblFranchisePercent = WG.dblFranchise
					,dblFranchise = WG.dblFranchise / 100
				...
				OUTER APPLY (
					...
					FROM tblLGWeightClaim WC
					INNER JOIN tblLGWeightClaimDetail WCD ON WC.intWeightClaimId = WCD.intWeightClaimId
					WHERE WCD.intContractDetailId = CD.intContractDetailId
						AND WC.intLoadId = L.intLoadId
						AND WC.intPurchaseSale = 2
					) WC
				OUTER APPLY (
					SELECT TOP 1 intWeightUOMId = IU.intItemUOMId
						,dblUnitQty
				...
				OUTER APPLY (
					SELECT dblLinkNetWt = SUM(dblLinkNetWt)
						,dblLinkGrossWt = SUM(dblLinkGrossWt)
					FROM tblLGLoadDetailContainerLink
					WHERE intLoadDetailId = LD.intLoadDetailId
					) CLNW
				WHERE L.intLoadId = @intLoadId
	...
	BEGIN
		...
		WHERE intLoadId = @intLoadId
			AND (
				@intPurchaseSale = 3
				OR intPurchaseSale = @intPurchaseSale
				)
			AND (
				@intLoadContainerId IS NULL
				OR (
