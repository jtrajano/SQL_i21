...
DECLARE @RecordsForReversal		TABLE (intInvoiceDetailId INT)
...
INNER JOIN tblARTransactionDetail TD ON Detail.intInvoiceDetailId = TD.intTransactionDetailId 
									AND Detail.intInvoiceId = TD.intTransactionId 
									AND TD.strTransactionType = Header.strTransactionType
WHERE Header.intInvoiceId = @InvoiceId
...
INNER JOIN tblARInvoice Header ON TD.intTransactionId = Header.intInvoiceId							
WHERE TD.intTransactionId = @InvoiceId
  AND TD.strTransactionType = Header.strTransactionType
...
IF @InvoiceReversed = 1
	BEGIN
		...
		SELECT intEntityCustomerId		= I.intEntityCustomerId 
			...
			, dblQuantity				= dbo.fnCalculateStockUnitQty(ID.dblQtyShipped, ICIU.dblUnitQty)
			, intItemUOMId				= ID.intItemUOMId
			, intLocationId				= I.intCompanyLocationId
			, intStorageScheduleTypeId	= ID.intStorageScheduleTypeId
		...
		INNER JOIN tblICItemUOM ICIU WITH (NOLOCK) ON ID.intItemId = ICIU.intItemId AND ID.intItemUOMId = ICIU.intItemUOMId				 		
...
ELSE
	BEGIN
		...
		SELECT intEntityCustomerId		= I.intEntityCustomerId 
			...
			, intInvoiceDetailId		= ID.intInvoiceDetailId
			, intItemId					= ID.intItemId
			, dblQuantity				= dbo.fnCalculateStockUnitQty((ID.[dblQtyShipped] - dbo.fnCalculateQtyBetweenUOM(TD.intItemUOMId, ID.intItemUOMId, TD.dblQtyShipped)), ICIU.dblUnitQty)
			, intItemUOMId				= ID.intItemUOMId
			, intLocationId				= I.intCompanyLocationId
			, intStorageScheduleTypeId	= ID.intStorageScheduleTypeId
			, intCustomerStorageId		= ID.intCustomerStorageId 
		...
		INNER JOIN tblARInvoiceDetail ID WITH (NOLOCK) ON I.intInvoiceId = ID.intInvoiceId
		INNER JOIN tblICItemUOM ICIU WITH (NOLOCK) ON ID.intItemId = ICIU.intItemId AND ID.intItemUOMId = ICIU.intItemUOMId
		LEFT JOIN tblGRStorageHistory SH ON I.intInvoiceId = SH.intInvoiceId AND I.strInvoiceNumber = SH.strInvoice
		WHERE I.intInvoiceId = @InvoiceId
		  AND ID.intStorageScheduleTypeId IS NOT NULL
		  AND SH.intCustomerStorageId IS NULL
	END
