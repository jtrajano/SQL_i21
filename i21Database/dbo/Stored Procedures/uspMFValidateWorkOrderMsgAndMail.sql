CREATE PROCEDURE uspMFValidateWorkOrderMsgAndMail @intWorkOrderId INT
	,@intNewStatusId INT
	,@intStatusId INT = NULL
	,@strErrMsg NVARCHAR(MAX) = '' OUTPUT
	,@strSubject NVARCHAR(MAX) = '' OUTPUT
	,@strBody NVARCHAR(MAX) = '' OUTPUT
	,@strToAddress NVARCHAR(MAX) = '' OUTPUT
AS
BEGIN TRY
	...
	SELECT @intUserId = intLastModifiedUserId
		,@strReferenceNo = CASE 
			WHEN strReferenceNo <> ''
				THEN strReferenceNo
			ELSE strWorkOrderNo
	...
	SELECT @intStatusId = @intNewStatusId
	...
	BEGIN
		...
		GROUP BY I.intItemId
			,I.intItemUOMId
		ORDER BY Convert(DECIMAL(38, 2), Min(IsNull(Inv.dblQuantity, 0) / dblRequiredQtyByCase))
		...
		BEGIN TRY
			...
			BEGIN
				...
				SELECT @strSubject = @subject
					,@strBody = @tableHTML
					,@strToAddress = @strEmail
					--IF @strEmail <> ''
					--BEGIN
					--	Begin Try
		...
		BEGIN
			...
			SELECT @strErrMsg = 'Available qty for item ' + @strItemNo + ' is ' + @strAvailableQty + ' which is less than the required qty ' + @strRequiredQty + '. ' + @strMinQtyCanBeProduced + ' can be produced with the available inputs. Do you want to continue ?'
