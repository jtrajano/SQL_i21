...
BEGIN
	...
	SELECT REPLACE(strMonth,'ysnFut' ,'') strMonth
		, CASE WHEN strMonth = 'ysnFutJan' THEN '01'
			...
			WHEN strMonth = 'ysnFutDec' THEN '12' END COLLATE Latin1_General_CI_AS AS strMonthCode
		, CASE WHEN strMonth = 'ysnFutJan' THEN 1
			...
			WHEN strMonth = 'ysnFutOct' THEN 10
			WHEN strMonth = 'ysnFutNov' THEN 11
			WHEN strMonth = 'ysnFutDec' THEN 12 END COLLATE Latin1_General_CI_AS AS intMonthCode
	INTO ##AllowedMonths
	FROM (
		SELECT ysnFutJan
			, ysnFutFeb
	...
	UNPIVOT (ysnSelect FOR strMonth IN (ysnFutJan
										, ysnFutFeb
										, ysnFutMar
										, ysnFutApr
										, ysnFutMay
	...
	SELECT CASE WHEN SUBSTRING(strMonth,LEN(strMonth)-1,LEN(strMonth)-1) = '01' THEN 'Jan'
		...
		WHEN SUBSTRING(strMonth,LEN(strMonth)-1,LEN(strMonth)-1) = '12' THEN 'Dec' END COLLATE Latin1_General_CI_AS AS strMonthName
	INTO #Temp
	FROM (
