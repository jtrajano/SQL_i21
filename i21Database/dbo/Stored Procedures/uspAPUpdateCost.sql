...
IF (SELECT TOP 1 A.intInventoryReceiptItemId FROM tblAPBillDetail A
...
ELSE 
BEGIN
	--IF THERE ARE NO RECEIPT TAXES BUT THERE IS AN EXISTING TAX ON VOUCHER DETAIL, RECREATE TAX ENTRIES BASE ON NEW COST
	IF(EXISTS(SELECT 1 FROM tblAPBillDetailTax WHERE intBillDetailId = @billDetailId))
	BEGIN
		EXEC uspAPUpdateVoucherDetailTax @billDetailIds
	END
