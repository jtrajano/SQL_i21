CREATE PROCEDURE uspMFUndoPallet 
(
	@strXML NVARCHAR(MAX)
)
AS
BEGIN TRY
	DECLARE @idoc									INT
		  ...
		  , @OtherChargesGLAccounts					dbo.ItemOtherChargesGLAccount
		  , @intItemId1								INT
		  , @strItemNo1								NVARCHAR(50)
		  , @intRecipeItemUOMId						INT
		  , @intItemLocationId						INT
		  , @INVENTORY_PRODUCE						INT = 9
	...
	IF EXISTS (SELECT 1 
			   ...
			   WHERE intWorkOrderId = @intWorkOrderId AND intBatchId = @intBatchId
	...
	IF EXISTS (SELECT *
			   FROM tblMFWorkOrderProducedLot
			   WHERE intWorkOrderId = @intWorkOrderId AND intBatchId = @intBatchId AND ysnProductionReversed = 1)
		BEGIN
			RAISERROR 
			(
	...
	IF EXISTS (SELECT *
			   ...
			   WHERE intLotId = @intLotId
				 AND intLotStatusId = 2) AND @ysnForceUndo = 0 AND @strLotTracking <> 'No'
		BEGIN
			RAISERROR 
			(
	...
	IF EXISTS (SELECT *
			   FROM dbo.tblICLot
			   WHERE intLotId = @intLotId AND dblQty = 0) AND @ysnForceUndo = 0 AND @strLotTracking <> 'No'
		BEGIN
			RAISERROR 
			(
	...
	IF NOT EXISTS (SELECT *
				   ...
				   WHERE intLotId				= @intLotId
					 ...
					 AND intTransactionId		= @intBatchId
					 AND intTransactionDetailId	= @intTransactionDetailId
					 AND dblStockAvailable		> 0) AND @strLotTracking <> 'No'
	
	--SELECT *
	--			   FROM dbo.tblICLot
	--			   WHERE strLotNumber = @strLotNumber AND intStorageLocationId = @intStorageLocationId AND dblQty > 0
		BEGIN
			RAISERROR 
			(
	...
	IF NOT EXISTS (SELECT *
				   ...
				   WHERE intTransactionId = @intTransactionId AND strTransactionId = @strTransactionId)
		BEGIN
	...
	SELECT @dblPercentage = CASE WHEN ysnConsumptionRequired = 1 AND dblPercentage IS NULL THEN 100
								 WHEN ysnConsumptionRequired = 0 AND dblPercentage IS NULL THEN 0
								 ELSE dblPercentage
							END
	...
	SELECT @dblOtherCharges = SUM((CASE WHEN intMarginById = 2 THEN ISNULL(P.dblStandardCost, 0) + ISNULL(RI.dblMargin, 0)
										ELSE ISNULL(P.dblStandardCost, 0) + (ISNULL(P.dblStandardCost, 0) * ISNULL(RI.dblMargin, 0) / 100)
								   END) / R.dblQuantity)
	FROM dbo.tblMFWorkOrderRecipeItem RI
	JOIN dbo.tblMFWorkOrderRecipe R ON R.intWorkOrderId = RI.intWorkOrderId AND R.intRecipeId = RI.intRecipeId
	JOIN dbo.tblICItem I ON I.intItemId = RI.intItemId AND RI.intRecipeItemTypeId = 1 
													   AND RI.ysnCostAppliedAtInvoice = 0
													   AND I.strType = 'Other Charge'
	JOIN dbo.tblICItemLocation IL ON IL.intItemId = I.intItemId AND IL.intLocationId = @intLocationId
	JOIN dbo.tblICItemPricing P ON P.intItemId = I.intItemId AND P.intItemLocationId = IL.intItemLocationId
	WHERE RI.intWorkOrderId = @intWorkOrderId

	IF @dblPercentage IS NOT NULL
		BEGIN
			SET @dblOtherCharges = @dblOtherCharges * @dblPercentage / 100;
		END

	IF @dblOtherCharges IS NOT NULL AND @dblOtherCharges > 0 AND @strInstantConsumption='True'
		BEGIN
			...
			INNER JOIN @OtherChargesGLAccounts ChargesGLAccounts ON ChargesGLAccounts.intChargeId = il.intItemId AND ChargesGLAccounts.intItemLocationId = il.intItemLocationId
			...
			SELECT dtmDate					= @dtmDate
				 ...
				 , dblCost = (CASE WHEN @intRecipeItemUOMId = @intItemUOMId THEN @dblOtherCharges * @dblQuantity
								   ELSE @dblOtherCharges * @dblPhysicalCount
							  END)
				 , intTransactionTypeId = @INVENTORY_PRODUCE
				 , intCurrencyId = (SELECT TOP 1 intDefaultReportingCurrencyId FROM tblSMCompanyPreference)
				 , dblExchangeRate = 1
	...
	IF @intSpecialPalletLotId IS NOT NULL
		BEGIN
			...
			WHERE intWorkOrderId = @intWorkOrderId AND intLotId = @intSpecialPalletLotId
		END

	UPDATE tblMFWorkOrder
	SET dblProducedQuantity = ISNULL(dblProducedQuantity, 0) - (CASE WHEN intItemId = @intItemId THEN (CASE WHEN intItemUOMId = @intItemUOMId THEN @dblQuantity
	...
	IF EXISTS (SELECT * FROM @GLEntries)
		BEGIN
			IF EXISTS (SELECT *
					   ...
					   WHERE I.strType = 'Other Charge' AND WRI.intWorkOrderId = @intWorkOrderId)
				BEGIN
