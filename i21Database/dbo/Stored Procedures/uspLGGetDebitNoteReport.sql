...
SELECT DISTINCT WC.intWeightClaimId
	...
	,BA.strBankName
	,B.strRemarks
	,dblTotalClaimAmount = ROUND(SUM(WCD.dblClaimAmount),2) + ISNULL(ROUND((SELECT SUM(dblAmount) FROM tblLGWeightClaimOtherCharges OC WHERE OC.intWeightClaimId = WC.intWeightClaimId),2), 0)
