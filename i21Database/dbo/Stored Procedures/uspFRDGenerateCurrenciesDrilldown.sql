CREATE PROCEDURE [dbo].[uspFRDGenerateCurrenciesDrilldown]                
 @intRowId   AS INT              
AS                
...
BEGIN              
 ...
 WHILE EXISTS(SELECT 1 FROM #TMPFILTER)                   
  BEGIN               
   ...
   SET @queryString = 'SELECT DISTINCT '+@filterID+',intCurrencyId,strCurrency FROM vyuGLSummary WHERE '+@filterString+' AND intCurrencyId = '+@Currency+''            
