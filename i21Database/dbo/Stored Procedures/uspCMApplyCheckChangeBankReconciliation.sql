...
WHERE	ysnPosted = 1
		...
		AND 1 = 
			CASE	WHEN	@strSide = 'DEBIT' 
							AND (
								intBankTransactionTypeId IN (@BANK_WITHDRAWAL,@BANK_INTEREST,@NSF, @MISC_CHECKS, @BANK_TRANSFER_WD, @ORIGIN_CHECKS, @ORIGIN_EFT, @ORIGIN_WITHDRAWAL, @ORIGIN_WIRE, @AP_PAYMENT, @AP_ECHECK, @PAYCHECK,  @ACH, @DIRECT_DEPOSIT)
