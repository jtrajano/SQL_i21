...
BEGIN          
 ...
 BEGIN           
  INSERT INTO #tmp_lotTrackingChange (strItemNo, intId)          
SELECT          
    s.strItemNo          
 ...
 IF @ysnAllowOverwrite = 0    
 BEGIN    
  DECLARE @tblDuplicateItems TABLE(          
   strItemNo NVARCHAR(200) COLLATE Latin1_General_CI_AS    
  )          
      
  INSERT INTO @tblDuplicateItems (          
   strItemNo          
  )               
   SELECT           
    a.strItemNo           
   FROM tblICItem a     
    INNER JOIN tblICImportStagingItem b    
  ON LOWER(a.strItemNo) = LOWER(b.strItemNo)     
 END    
...
WHERE           
 ...
 AND s.strItemNo NOT IN (SELECT strItemNo FROM @tblDuplicateItems)    
...
-- Log error entries --  
 ...
 INSERT INTO tblICImportLogDetail(    
    intImportLogId,        --strUniqueId         
    strType,      --intRecordNo         
    intRecordNo,     --strField          
    strField,      --strAction          
    strValue,      --strValue          
  ...
  @intImportLogId    
    ,'Error' AS strType         
    , NUll as intRecordNo          
    , 'Lot Tracking'  AS strField      , strItemNo AS strValue    
    , 'Lot Tracking change is not allowed for item "' + strItemNo + '"' AS strMessage     
    , 'Import Failed.'  AS strStatus      
   ...
   SELECT           
    ...
    ,'Error' AS strType    
    , NUll AS intRecordNo    
    , 'Item Type' AS strField         
    , strItemNo AS strValue         
    , 'Item Type change is not allowed for item "' + strItemNo + '"'   AS strMessage        
    , 'Import Failed.' AS strStatus        
    , 'Skipped' AS strAction          
    , 1 AS intConcurrencyId         
   FROM           
   ...
   SELECT           
    ...
    ,'Error' AS strType    
    , NUll AS intRecordNo    
    , 'Commodity' AS strField               
    , strItemNo AS strValue         
    , 'Commodity change is not allowed for item "' + strItemNo + '"' AS strMessage    
    , 'Import Failed.' AS strStatus    
    , 'Skipped' AS strAction                
    , 1 AS intConcurrencyId          
   FROM           
   ...
   SELECT           
    ...
    , 'Item No' AS strField             
    , strItemNo AS strValue    
    , 'Item No already exists"' + strItemNo + '"' AS strMessage         
    , 'Import Failed.' AS strStatus           
    , 'Skipped' AS strAction          
    , 1 AS intConcurrencyId          
   FROM           
 ...
 BEGIN      
  ...
  SELECT           
   ...
   --, 'Duplicate Item No "' + strItemNo + '"'          
   , dbo.fnFormatMessage(          
    'Already existing item nos. %i record(s) found.'          
    ,COUNT(1)           
    ,DEFAULT           
    ,DEFAULT           
    ,DEFAULT           
...
IF EXISTS(SELECT * FROM tblICImportLogDetail WHERE intImportLogId = @intImportLogId AND strType = 'Error')      
BEGIN       
 UPDATE tblICImportLog SET strDescription = 'Import Finished with Error' WHERE intImportLogId = @intImportLogId      
 DELETE FROM tblICImportLogDetail WHERE intImportLogId = @intImportLogId AND strType = 'Info'     
END    
