CREATE PROCEDURE [dbo].[uspSTReportCheckoutTotaTaxRecap]
...
BEGIN TRY
   ...
   C.intShiftNo as ShiftNo,A.intCheckoutId,A.dblTotalTax As TotalTax, A.dblTaxableSales as TaxableSales,
   A.dblTaxExemptSales as TaxExemptSales,  B.strAccountId, B.strDescription from tblSTCheckoutSalesTaxTotals 
   A LEFT OUTER JOIN tblGLAccount B ON A.intSalesTaxAccount = B.intAccountId
   JOIN tblSTCheckoutHeader C ON A.intCheckoutId = C.intCheckoutId
   JOIN tblSTStore D ON C.intStoreId = D.intStoreId
