...
BEGIN
	...
	SELECT TOP 1		 	
		...
		,@OriginalInvoiceId				= (CASE WHEN ISNULL([strSourceTransaction],'') = 'Provisional' OR ((ISNULL([strSourceTransaction],'') = 'Invoice' OR ISNULL([strSourceTransaction],'') = 'Direct') AND ISNULL([strTransactionType],'') = 'Credit Memo') THEN ISNULL([intOriginalInvoiceId], [intSourceId]) ELSE NULL END)
