...
BEGIN
	...
	SELECT TOP 1		 	
		...
		,@LoadId						= (CASE WHEN @FromImportTransactionCSV = 1 THEN [intLoadId] WHEN ISNULL([strSourceTransaction],'') IN ('Load Schedule', 'Weight Claim') OR ([strTransactionType] = 'Credit Memo'  AND [strSourceTransaction] != 'POS') THEN ISNULL([intLoadId], CASE WHEN [intSourceId] = 0 THEN NULL ELSE [intSourceId] END ) ELSE NULL END)
