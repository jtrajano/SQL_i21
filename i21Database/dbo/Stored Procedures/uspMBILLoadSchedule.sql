...
,MBDH.intCompanyLocationId = CASE WHEN TMS.ysnCompanySite  = 1 then TMS.intLocationId ELSE DO.intCompanyLocationId END  
...
WHERE MB.intDriverId = @intDriverId AND intStopType = 2 and intDispatchStatus = 3 and exists(Select * from tblMBILDeliveryDetail d Where MBDH.intDeliveryHeaderId = d.intDeliveryHeaderId and ysnDelivered = 0)  
...
Group by MB.intLoadHeaderId  
  ...
  ,DOD.intEntityLocationId  
  ,CASE WHEN TMS.ysnCompanySite  = 1 then TMS.intLocationId ELSE DO.intCompanyLocationId END  
  ,ysnRequirePump  
