...
SET  MBDH.intLoadHeaderId = MB.intLoadHeaderId        
 ...
 ,MBDH.intCompanyLocationId = DO.intCompanyLocationId        
...
WHERE intStopType = 2 and intDispatchStatus = 3 AND         
   NOT EXISTS(SELECT intDeliveryHeaderId      
     ...
     WHERE p.intLoadHeaderId = MB.intLoadHeaderId AND         
        ...
        isnull(p.intCompanyLocationId,0) = isnull(DO.intCompanyLocationId,0))      
...
DELETE FROM tblMBILPickupDetail WHERE intDispatchOrderDetailId NOT IN(SELECT intDispatchOrderDetailId FROM tblLGDispatchOrderDetail) and intLoadHeaderId IN(Select intLoadHeaderId from tblMBILLoadHeader where intDriverId = @intDriverId)  
DELETE FROM tblMBILDeliveryDetail WHERE intDispatchOrderDetailId NOT IN(SELECT intDispatchOrderDetailId FROM tblLGDispatchOrderDetail) and   
     intDeliveryHeaderId IN(  
        Select intDeliveryHeaderId from tblMBILDeliveryHeader d   
        join tblMBILLoadHeader l on d.intLoadHeaderId = d.intLoadHeaderId where l.intDriverId = @intDriverId)  
...
DELETE FROM tblMBILLoadHeader WHERE intDispatchOrderId NOT IN(SELECT intDispatchOrderId FROM tblLGDispatchOrder) AND intDriverId = intDriverId  
END