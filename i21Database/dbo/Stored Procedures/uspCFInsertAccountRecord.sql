...
CREATE PROCEDURE [dbo].[uspCFInsertAccountRecord]  
 ...
 ,@strDepartmentGrouping  NVARCHAR(MAX)  =  'No'
...
IF(ISNULL(@strDepartmentGrouping,'') = '')  
 BEGIN  
  INSERT tblCFImportFromCSVLog (strImportFromCSVId,strNote)  
  VALUES (@strCustomerId,'Department Grouping is required')  
  SET @ysnHasError = 1  
 END  
 ...
 IF(LOWER(@strDepartmentGrouping) NOT IN ('no','yes with page break','yes without page break'))  
 BEGIN  
  INSERT tblCFImportFromCSVLog (strImportFromCSVId,strNote)  
  VALUES (@strCustomerId,'Invalid department grouping value '+ @strDepartmentGrouping +'. Value should be No, Yes with Page Break, Yes without Page Break')  
  SET @ysnHasError = 1  
 END  
