...
BEGIN TRANSACTION    
 ...
 WHERE strNamespace IN (SELECT strNamespace FROM tblSMScreenStage WHERE strChange = 'Deleted') AND strNamespace <> 'ContractManagement.view.ContractAmendment' AND strNamespace <> 'SystemManager.view.EmailHistory' AND (ysnApproval = 0 OR ysnApproval IS NULL)
