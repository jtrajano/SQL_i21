...
SELECT Lot.intLotId
	 ...
	 , CASE WHEN (NULLIF(Item.intUnitPerLayer, '') IS NULL OR Item.intUnitPerLayer = 0)
         ...
         THEN 0 WHEN (CASE WHEN ISNULL(Lot.dblQty, 0) > 0 THEN Lot.dblQty
              ELSE dbo.fnMFConvertQuantityToTargetItemUOM(Lot.intItemUOMId, RecipeItem.intItemUOMId, Lot.dblQty) END) = 0
         THEN 0 WHEN Item.intUnitPerLayer * Item.intLayerPerPallet = 0 
         THEN 0 ELSE CAST(ROUND(Lot.dblQty / (Item.intUnitPerLayer * Item.intLayerPerPallet), 0) AS NUMERIC(18, 0))
