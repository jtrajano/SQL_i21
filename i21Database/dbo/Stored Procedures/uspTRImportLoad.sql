...
BEGIN
    ...
    BEGIN TRY
		...
		BEGIN
			UPDATE D
			SET D.ysnValid = 1, D.strMessage = ''
			FROM tblTRImportLoadDetail D
			INNER JOIN tblTRImportLoad L ON L.intImportLoadId = D.intImportLoadId
		...
		BEGIN
			...
			IF EXISTS(SELECT TOP 1 1 FROM tblTRLoadHeader LH INNER JOIN tblTRLoadReceipt LR ON LR.intLoadHeaderId = LH.intLoadHeaderId
						WHERE LH.dtmLoadDateTime = @dtmPullDate AND LR.strBillOfLading = @strBillOfLading)
			BEGIN
			...
			IF(@strSource = 'API')
			BEGIN
				SELECT TOP 1 @intVendorId = V.intEntityId, @intSupplyPointId = S.intSupplyPointId, @intVendorCompanyLocationId = C.intCompanyLocationId
				FROM tblAPVendor V
			...
			-- CHECK PULLED PRODUCT LOCATION SETUP (AGAINST SUPPLIER's COMPANY LOCATION)
			IF(@intPullProductId IS NOT NULL) 
			BEGIN
				IF NOT EXISTS(SELECT TOP 1 1 FROM tblICItemLocation WHERE intItemId = @intPullProductId AND intLocationId = @intVendorCompanyLocationId)
				BEGIN
					DECLARE @strPulledItemNo NVARCHAR(300) = NULL
					DECLARE @strSupplierCompanyLocation NVARCHAR(MAX) = NULL
					
					SELECT @strPulledItemNo = strItemNo FROM tblICItem WHERE intItemId = @intPullProductId
					SELECT @strSupplierCompanyLocation = strLocationName FROM tblSMCompanyLocation WHERE intCompanyLocationId = @intVendorCompanyLocationId
					SELECT @strMessage = dbo.fnTRMessageConcat(@strMessage, 'Supplier Company Location: ''' + @strSupplierCompanyLocation + ''' is not setup for the Pulled Item: ''' + @strPulledItemNo + '''')
				END
			END
			...
			-- CHECK DROPPED PRODUCT LOCATION SETUP (AGAINST CUSTOMER's COMPANY LOCATION)
			IF(@intDropProductId IS NOT NULL)
			BEGIN
				IF NOT EXISTS(SELECT TOP 1 1 FROM tblICItemLocation WHERE intItemId = @intDropProductId AND intLocationId = @intCustomerCompanyLocationId)
				BEGIN
					DECLARE @strDroppedItemNo NVARCHAR(300) = NULL
					DECLARE @strDestinationCompanyLocation NVARCHAR(MAX) = NULL
					
					SELECT @strDroppedItemNo = strItemNo FROM tblICItem WHERE intItemId = @intDropProductId
					SELECT @strDestinationCompanyLocation = strLocationName FROM tblSMCompanyLocation WHERE intCompanyLocationId = @intCustomerCompanyLocationId
					SELECT @strMessage = dbo.fnTRMessageConcat(@strMessage, 'Destination Company Location: ''' + @strDestinationCompanyLocation + ''' is not setup for the Dropped Item: ''' + @strDroppedItemNo + '''')
				END
			END
