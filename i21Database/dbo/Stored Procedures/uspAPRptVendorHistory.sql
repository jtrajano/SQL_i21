CREATE PROCEDURE [dbo].[uspAPRptVendorHistory]
...
--Add this so that XtraReports have fields to get
	SELECT 
		...
		,0	 AS dblInterest
		,0	 AS dblAmountPaid
		,NULL AS strPaymentInfo
		,NULL AS dtmDatePaid
		,0	 AS ysnPaid
		,NULL AS strVendorOrderNumber
		,NULL AS strReference
		,0	 AS dblAmountDue
...
--CREATE date filter
SELECT @dateFrom = [from], @dateTo = [to] FROM @temp_xml_table WHERE [fieldname] = 'dtmDatePaid';
...
SET @innerQuery = 
		'SELECT 
			...
			,LTRIM(RTRIM(strReference)) as strReference
			,dblAmountDue
			,(SELECT TOP 1 dbo.[fnAPFormatAddress](NULL, NULL, NULL, strAddress, strCity, strState, strZip, strCountry, NULL) FROM tblSMCompanySetup) as strCompanyAddress
			,(SELECT Top 1 strCompanyName FROM dbo.tblSMCompanySetup) as strCompanyName
		FROM dbo.vyuAPRptVendorHistory'

IF @dateFrom IS NOT NULL
BEGIN	
	IF @condition = 'Equal To'
	BEGIN 
		SET @innerQuery = @innerQuery + ' WHERE DATEADD(dd, DATEDIFF(dd, 0,dtmDatePaid), 0) = ''' + CONVERT(VARCHAR(10), @dateFrom, 110) + ''''
	END
    ELSE 
	BEGIN 
		SET @innerQuery = @innerQuery + ' WHERE DATEADD(dd, DATEDIFF(dd, 0,dtmDatePaid), 0) BETWEEN ''' + CONVERT(VARCHAR(10), @dateFrom, 110) + ''' AND '''  + CONVERT(VARCHAR(10), @dateTo, 110) + ''''	
	END  
END
