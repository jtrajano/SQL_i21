...
DECLARE @intTimeOffDescCount AS INT
...
SELECT * INTO #TempEmployeeTimeOff FROM tblApiSchemaEmployeeTimeOff where guiApiUniqueId = @guiApiUniqueId
	...
	BEGIN
		SELECT TOP 1 
			...
			,@strAwardOn			= CASE WHEN strAwardOn <> '' AND strAwardOn 
										IN(
											'Start of Week',
											'End of Week',
											'Start of Month',
		...
		SET @intTimeOffDescCount = NULL
		SELECT @intTimeOffDescCount = intTypeTimeOffId FROM tblPRTypeTimeOff 
		WHERE strTimeOff = @strTimeOffId
		  AND strDescription = @strTimeOffDesc
		
		IF(@intTimeOffDescCount IS NOT NULL)
			BEGIN
		...
		ELSE
			BEGIN
				...
				SELECT TOP 1
					...
					, guiApiImportLogId = @guiLogId
					, strField = 'Employee Time Off'
					, strValue = @strTimeOffDesc
					, strLogLevel = 'Error'
					, strStatus = 'Failed'
					, intRowNo = SE.intRowNumber
					, strMessage = 'Wrong input/format for Time Off Description. Please try again.'
				FROM tblApiSchemaEmployeeTimeOff SE
				LEFT JOIN tblPREmployeeTimeOff E ON E.intEntityEmployeeId = (SELECT TOP 1 intEntityId FROM tblPREmployee WHERE strEmployeeId = SE.intEntityNo)
				WHERE SE.guiApiUniqueId = @guiApiUniqueId
				AND SE.strTimeOffId = @strTimeOffId
