...
IF OBJECT_ID('tempdb..##AllowedMonths') IS NOT NULL  
 ...
 ORDER BY strMonth  
 IF  EXISTS(SELECT * FROM tblRKFuturesMonth WHERE intFutureMarketId = @FutureMarketId and dtmFutureMonthsDate is null)   
 BEGIN   
 UPDATE tblRKFuturesMonth SET dtmFutureMonthsDate = Convert(datetime,'1' + strFutureMonth )  where intFutureMarketId=@FutureMarketId and dtmFutureMonthsDate is null  
 END  
 IF EXISTS(SELECT * FROM tblRKFuturesMonth WHERE intFutureMarketId = @FutureMarketId)  
 BEGIN  
...
SELECT distinct t.intConcurrencyId,  
  ...
  t.ysnExpired   
FROM #Temp t)t WHERE t.strFMonth not in(SELECT strFutureMonth collate Latin1_General_CI_AS from tblRKFuturesMonth where intFutureMarketId=@FutureMarketId)  
