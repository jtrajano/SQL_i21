...
;WITH Query AS(
    SELECT
        ...
        , dblTotal
		, dtmDate
		, intInvoiceId		= SAR.intTransactionId
...
totalQuery as (
    ...
    FROM Query A 
	WHERE @intInvoiceId = A.intInvoiceId
    GROUP BY
	dtmDate
)
INSERT INTO @tblTemp
...
IF @ysnPosted = 1 
BEGIN
	MERGE dbo.tblARInvoiceGrossMarginSummary t
		using @tblTemp Temp
		ON(t.dtmDate = Temp.dtmDate and t.strType = Temp.strType)
	WHEN MATCHED
		THEN
			UPDATE SET t.dblAmount = t.dblAmount + Temp.dblAmount,
			t.intConcurrencyId = isnull(t.intConcurrencyId,0) + 1
	WHEN NOT MATCHED BY TARGET 
		THEN
			INSERT (strType, dblAmount, dtmDate,intConcurrencyId)
			VALUES (Temp.strType, Temp.dblAmount, Temp.dtmDate, 1);
