...
from (
	...
	FROM (
		SELECT (convert(int,isnull((Long1-MatchLong),0)- isnull(Sell1-MatchShort,0)))*dblContractSize/ case when ysnSubCurrency = 'true' then intCent else 1 end  GrossPnL1
		...
		FROM (
			SELECT  intFutOptTransactionId
				...
				, dblFutCommission1 = ISNULL((select TOP 1 (case when bc.intFuturesRateType = 2 then 0
															else  isnull(bc.dblFutCommission,0) / case when cur.ysnSubCurrency = 'true' then cur.intCent else 1 end end) 
