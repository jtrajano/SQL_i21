		...
		FROM (
			...
			WHERE isnull(ot.intCommodityId, 0) = CASE 
				...
				AND ot.strBuySell = CASE 
					...
					ELSE @strBuySell
					END
				AND convert(DATETIME, CONVERT(VARCHAR(10), dtmFilledDate, 110), 110) BETWEEN @dtmFromDate
					AND @dtmToDate
				AND isnull(ysnExpired, 0) = CASE 
					WHEN isnull(@ysnExpired, 'false') = 'true'
						THEN isnull(ysnExpired, 0)
					ELSE @ysnExpired
					END
