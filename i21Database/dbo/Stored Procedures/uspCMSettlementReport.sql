...
BEGIN
	...
	*/
	...
	0 as OutboundTax,
	ISNULL((SELECT CASE WHEN A.dblCost<0 AND A.dblQtyOrdered >0 AND A.dblTotal >0 THEN SUM(-A.dblTotal) ELSE SUM(A.dblTotal) END FROM tblAPBillDetail A INNER JOIN tblICItem B ON A.intItemId = B.intItemId WHERE intBillId = BillDtl.intBillId AND B.strType = 'Other Charge' GROUP BY  A.dblCost,A.dblQtyOrdered,A.dblTotal),0) as InboundDiscount,
	0 as OutboundDiscount,
	(BillDtl.dblTotal + BillDtl.dblTax + ISNULL((SELECT CASE WHEN A.dblCost<0 AND A.dblQtyOrdered >0 AND A.dblTotal >0 THEN SUM(-A.dblTotal) ELSE SUM(A.dblTotal) END FROM tblAPBillDetail A INNER JOIN tblICItem B ON A.intItemId = B.intItemId WHERE intBillId = BillDtl.intBillId AND B.strType = 'Other Charge' GROUP BY  A.dblCost,A.dblQtyOrdered,A.dblTotal),0)) as InboundNetDue,
	0 as OutboundNetDue,
