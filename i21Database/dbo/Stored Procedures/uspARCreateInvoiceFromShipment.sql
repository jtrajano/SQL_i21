...
SET @Currency = ISNULL((SELECT TOP 1 strValue FROM tblSMPreferences WHERE strPreference = 'defaultCurrency'),0)
SET @ARAccountId = ISNULL((SELECT TOP 1 strValue FROM tblSMPreferences WHERE strPreference = 'DefaultARAccount'),0)


IF(@ARAccountId IS NULL OR @ARAccountId = 0 OR RTRIM(LTRIM(@ARAccountId)) = '')  
	BEGIN			
...
SELECT 
	...
	,SOD.[intSalesOrderDetailId]			--[intSalesOrderDetailId]									--[intContractDetailId]
	,1		
FROM
