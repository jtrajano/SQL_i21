...
BEGIN
	...
	IF @dblAmount < 1
	BEGIN
		SELECT @Count = Count(intTransactionId) FROM #tmp_list_of_transactions WHERE ABS(dblAmount) = ABS(@dblAmount) AND intBankTransactionTypeId IN (@BANK_WITHDRAWAL,@NSF,@BANK_TRANSACTION,@ORIGIN_WITHDRAWAL, @BANK_TRANSFER_WD,  @MISC_CHECKS, @ORIGIN_CHECKS, @ORIGIN_EFT,  @ORIGIN_WIRE, @AP_PAYMENT, @AP_ECHECK, @PAYCHECK)
	END
	ELSE
	BEGIN
	...
	BEGIN	
		...
		WHERE	--A.intTransactionId = @intTransactionId
				...
				--		END 
							 --WITHDRAWAL ENTRY
				AND 1 = CASE WHEN @dblAmount < 0 AND A.intBankTransactionTypeId IN (@BANK_WITHDRAWAL,@NSF,@BANK_TRANSACTION,@ORIGIN_WITHDRAWAL, @BANK_TRANSFER_WD,  @MISC_CHECKS, @ORIGIN_CHECKS, @ORIGIN_EFT, @ORIGIN_WIRE, @AP_PAYMENT, @AP_ECHECK, @PAYCHECK)THEN
