...
select *  into #tempCommodities from tblICCommodity where ysnExchangeTraded = 1 
...
DECLARE
 ...
 , @intUOMId INT = NULL  
	...
	SELECT TOP 1 @intCommodityId = intCommodityId from #tempCommodities

	SELECT TOP 1 @intFutureMarketId = intFutureMarketId FROM vyuRKGetMarketCommodity where intCommodityId = @intCommodityId

	SELECT TOP 1 @intFutureMonthId = intFutureMonthId FROM tblRKFuturesMonth 
	WHERE ysnExpired = 0 
	AND convert(DATETIME, CONVERT(VARCHAR(10),dtmSpotDate, 110), 110) <= convert(DATETIME, CONVERT(VARCHAR(10), GETDATE(), 110), 110) 
	AND intFutureMarketId = @intFutureMarketId ORDER BY dtmSpotDate DESC

	SELECT @intUOMId = intUnitMeasureId FROM tblICUnitMeasure WHERE strUnitMeasure = 'Kg'
 ...
 , @dtmPositionAsOf DATETIME  = GETDATE()
 , @strUomType NVARCHAR(100)  = 'By Quantity'
	...
	DECLARE @ListFinal AS TABLE (intRowNumber INT
		...
		, dblOrigQuantity NUMERIC(24, 10) 
		, strOrigUOM NVARCHAR(100) COLLATE Latin1_General_CI_AS
		, dblUnitQty NUMERIC(24, 10) 
		, strOrigFutureMonth NVARCHAR(100) COLLATE Latin1_General_CI_AS
	...
	SELECT fm.strFutureMonth
		...
		, strFutureMarket = ffm.strFutMarketName
		, dblOrigQuantity = CASE WHEN @ysnIncludeInventoryHedge = 0 THEN ISNULL(dblBalance, 0) ELSE dblQuantity END
		, strOrigUOM = uom.strUnitMeasure
	...
	inner JOIN tblICUnitMeasure uom ON uom.intUnitMeasureId = cv.intUnitMeasureId
	...
	FROM (
		...
		SELECT strFutureMonth
			...
			, strCommodity
			, strFutureMarket
			, dblOrigQuantity = CASE WHEN intPricingTypeIdHeader = 8 THEN  dblRatioQty ELSE ISNULL(dblQuantity,dblOrigQuantity) END
			, strOrigUOM
			, dblUnitQty
			, dtmStartDate
			, dtmContractDate
		...
		UNION ALL SELECT strFutureMonth
			...
			, strCommodity
			, strFutureMarket
			, dblOrigQuantity = CASE WHEN intPricingTypeIdHeader = 8 THEN  (dblRatioQty / dblNoOfLot) * dblFixedLots ELSE dblFixedQty END
			, strOrigUOM
			, dblUnitQty
			, dtmStartDate
			, dtmContractDate
		...
		UNION ALL SELECT strFutureMonth
			...
			, strCommodity
			, strFutureMarket
			, dblOrigQuantity = CASE WHEN intPricingTypeIdHeader = 8 THEN ((dblRatioQty / dblNoOfLot) * ISNULL(dblNoOfLot, 0) - (dblRatioQty / dblNoOfLot) * ISNULL(dblFixedLots, 0)) ELSE dblQuantity - dblFixedQty END
			, strOrigUOM
			, dblUnitQty
			, dtmStartDate
			, dtmContractDate
	...
	SELECT 1 intRowNumber
		...
		, strOrigUOM
		, dblUnitQty
		, strOrigFutureMonth = strFutureMonth
		, dtmStartDate
		, dtmContractDate
	...
	FROM (
		...
		FROM (
			SELECT DISTINCT 2 intRowNumber
				...
				, strCommodity = strCommodityCode
				, strFutureMarket
				, dblOrigQuantity = dblOpenContract * ft.dblContractSize
				, strOrigUOM = uom.strUnitMeasure
				, dblUnitQty = um.dblUnitQty
				, strOrigFutureMonth  = strFutureMonth
				, dtmFilledDate
			...
			JOIN tblICUnitMeasure uom on uom.intUnitMeasureId = um.intUnitMeasureId
	...
	select
		   strBusinessType = 'Physicals'
		 ,strTransctionId = strTradeNo
		 ,strTransactionType  = TranType
		 ,strCounterParty = CustVendor
		 ,dtmContractDate
		 ,strLocation
		 ,dblQuantity
		 ,@strUnitMeasure
		 ...
		 ,strOrigin
	FROM @ListFinal
	where PriceStatus IN ('1.Priced / Outright - (Outright position)','1.Unpriced - (Balance to be Priced)')

	
	union all
	select
		  strBusinessType = 'Unpriced'
		 ,strTransctionId = strTradeNo
		 ,strTransactionType  = TranType
		 ,strCounterParty = CustVendor
		 , dtmContractDate
		 ...
		 ,strOrigin = ''
	FROM @ListFinal
	where PriceStatus = '1.Unpriced - (Balance to be Priced)'


	union all

	select
		 strBusinessType = 'Derivatives'
		 ,strTransctionId = strTradeNo
		 ,strTransactionType  = TranType
		 ,strCounterParty = CustVendor
		 , dtmContractDate
		 ...
		 ,strOrigin
	FROM @ListFinal
	where PriceStatus = '2.Terminal Position'
