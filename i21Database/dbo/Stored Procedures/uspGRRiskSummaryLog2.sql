CREATE PROCEDURE [dbo].[uspGRRiskSummaryLog2]
(
	@StorageHistoryIds AS Id READONLY
...
BEGIN TRY
	...
	DECLARE @strDebug NVARCHAR(MAX)
	IF (SELECT TOP 1 1 FROM tblGRStorageHistory A INNER JOIN @StorageHistoryIds b ON A.intStorageHistoryId = b.intId WHERE A.intTransactionTypeId IN (1,3,4,5,8,9)) = 1
	BEGIN
		set @strDebug = 'summary log select start ' + CAST(getdate() as nvarchar(max))
		print @strDebug
		...
		INNER JOIN vyuGRStorageHistory sh
			ON sh.intStorageHistoryId = Ids.intId
		...
		LEFT JOIN tblSMCompanyLocationSubLocation sl 
			ON sl.intCompanyLocationSubLocationId = t.intSubLocationId 
				AND sl.intCompanyLocationId = t.intProcessingLocationId
		WHERE --sh.intStorageHistoryId = @intStorageHistoryId AND
			(sh.intTransactionTypeId = 4 AND st.ysnDPOwnedType = 0)
		UNION ALL
		...
		LEFT JOIN tblCTContractDetail CD
			ON CD.intContractHeaderId = sh.intContractHeaderId
		WHERE --sh.intStorageHistoryId = @intStorageHistoryId AND
			(sh.intTransactionTypeId = 3 AND st.ysnDPOwnedType = 1)

		set @strDebug = 'summary log select end ' + CAST(getdate() as nvarchar(max))
		print @strDebug

		UPDATE @SummaryLogs set strTransactionNumber = '' where strTransactionNumber is null

		set @strDebug = 'summary log SP start ' + CAST(getdate() as nvarchar(max))
		print @strDebug
		EXEC uspRKLogRiskPosition @SummaryLogs
		set @strDebug = 'summary log SP end ' + CAST(getdate() as nvarchar(max))
		print @strDebug
	END
