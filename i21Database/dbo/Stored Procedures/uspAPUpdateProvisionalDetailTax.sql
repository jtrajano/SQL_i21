CREATE PROCEDURE [dbo].[uspAPUpdateProvisionalDetailTax]
...
IF @transCount = 0 BEGIN TRANSACTION
	...
	INNER JOIN tblAPProvisionalDetail B ON A.intBillId = B.intBillId
	...
	INSERT INTO tblAPProvisionalDetailTax(
		[intBillDetailId]		, 
		--[intTaxGroupMasterId]	, 
		[intTaxGroupId]			, 
		[intTaxCodeId]			, 
	...
	CROSS APPLY (
		SELECT 
			...
			,B.intTaxGroupId
		FROM tblAPProvisionalDetailTax B WHERE B.intBillDetailId = A.intBillDetailId
	...
	CROSS APPLY (
		...
		FROM tblAPProvisionalDetail WHERE intBillId = A.intBillId
	) TaxAmount
	WHERE TaxAmount.dblTax IS NOT NULL
