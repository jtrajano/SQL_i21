...
(SELECT strInvoiceNumber FROM tblARInvoice ia WHERE ia.strInvoiceNumber=it.strTransactionId) tranInvoiceNumber,
ROUND((SELECT (dblQty*dblUOMQty) FROM tblARInvoice ia       WHERE ia.strInvoiceNumber=it.strTransactionId ),6) dblInvoiceQty
FROM tblICInventoryTransaction it 
JOIN tblICItem i on i.intItemId=it.intItemId and it.ysnIsUnposted=0 and it.intTransactionTypeId in(4,5,10,23,33)
JOIN tblICItemLocation il on it.intItemId=il.intItemId and isnull(il.strDescription,'') <> 'In-Transit' 
...
and i.intItemId= case when isnull(@intItemId,0)=0 then i.intItemId else @intItemId end and strTransactionId not like'%IS%'
                --group by dtmDate,strTransactionId
union
