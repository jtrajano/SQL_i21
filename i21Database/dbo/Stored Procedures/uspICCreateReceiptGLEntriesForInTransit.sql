CREATE PROCEDURE [dbo].[uspICCreateReceiptGLEntriesForInTransit]
	@strBatchId AS NVARCHAR(40)
	,@AccountCategory_ContraInventory AS NVARCHAR(255) = 'Inventory'
	,@intEntityUserSecurityId AS INT
	,@strGLDescription AS NVARCHAR(255) = NULL 	
	,@intContraInventory_LocationId AS INT = NULL 
...
BEGIN 
	UPDATE glAccounts
	SET intContraInventoryId = NULL 
	FROM
		@GLAccounts glAccounts INNER JOIN tblICItemLocation il 
			ON glAccounts.intItemId = il.intItemId

	UPDATE glAccounts
	SET intContraInventoryId = dbo.fnGetItemGLAccount(il.intItemId, il.intItemLocationId, @AccountCategory_ContraInventory) 
	FROM
		@GLAccounts glAccounts INNER JOIN tblICItemLocation il 
			ON glAccounts.intItemId = il.intItemId
	WHERE
		il.intLocationId = @intContraInventory_LocationId
END 
