	...
	FROM (
		...
		CROSS APPLY (
			SELECT intInvoiceId
				...
				, dblLineItemTotal		= (dblQtyShipped * CASE WHEN intPriceUOMId IS NULL THEN dblPrice ELSE dbo.fnCalculateCostBetweenUOM(intPriceUOMId, intItemUOMId, ISNULL(dblPrice, 0)) END) * dbo.fnARGetInvoiceAmountMultiplier(ARI.strTransactionType)
