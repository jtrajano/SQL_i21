...
BEGIN 
	...
	BEGIN 
		...
		BEGIN
			...
			EXEC uspICRaiseError 80076, @strLotNumber;
			RETURN -1
		END
	END 
...
BEGIN 
	...
	WHERE	Header.intInventoryAdjustmentId = @intTransactionId
			...
			AND NewLot.intOwnershipType = @OWNERSHIP_TYPE_Own -- process only company-owned stocks
			AND Detail.dblAdjustByQuantity != 0
...
BEGIN
	...
	WHERE	Header.intInventoryAdjustmentId = @intTransactionId
			...
			AND FromStock.strBatchId = @strBatchId
			AND NewLot.intLotId IS NOT NULL 
			AND NewLot.intOwnershipType = @OWNERSHIP_TYPE_Storage -- process only storage stocks
			AND Detail.dblAdjustByQuantity != 0
