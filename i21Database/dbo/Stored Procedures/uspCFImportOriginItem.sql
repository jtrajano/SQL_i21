CREATE PROCEDURE [dbo].[uspCFImportOriginItem]
	...
	BEGIN
		...
		,intI21IdentityId = (SELECT TOP 1 intItemId FROM tblCFItem WHERE strProductNumber = cfitm_prod_no COLLATE Latin1_General_CI_AS)
		,strUserId = ''
		FROM cfitmmst 
		WHERE EXISTS (SELECT TOP 1 1 FROM tblCFItem WHERE strProductNumber = cfitm_prod_no COLLATE Latin1_General_CI_AS)
		--------------------------------------------------------------------
		--------------------------------------------------------------------------
