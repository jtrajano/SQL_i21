CREATE PROCEDURE [dbo].[uspRKGetCounterpartyExposure]
...
DECLARE @tblResult TABLE (
	...
	,dblCreditExposure NUMERIC(18,6)
	,dblCreditBalance NUMERIC(18,6)
	,strCurrency NVARCHAR(40)
	,dblOpenTonnageLimit NUMERIC(18,6)
...
FROM #tempCommodity ORDER BY intCommodityId 

--Get the Stock UOM
SELECT @intQuantityUOMId = CUM.intUnitMeasureId
	,@intPriceUOMId = CUM.intUnitMeasureId
	,@strUOM = strUnitMeasure
FROM tblICCommodityUnitMeasure  CUM
INNER JOIN tblICUnitMeasure UOM ON UOM.intUnitMeasureId = CUM.intUnitMeasureId
...
SELECT TOP 1 @intFutureSettlementPriceId = intFutureSettlementPriceId 
FROM vyuRKGetSettlementPriceHeader 
WHERE intCommodityId = @intCommodityId ORDER BY intFutureSettlementPriceId DESC

--Get the latest Basis Entry
SELECT TOP 1  @intM2MBasisId = intM2MBasisId FROM tblRKM2MBasis ORDER BY intM2MBasisId DESC
	...
	BEGIN
		...
		SELECT  strCompanyName  = strEntityName
			, strType = 'Vendor' COLLATE Latin1_General_CI_AS
			, strCountry = Loc.strCountry COLLATE Latin1_General_CI_AS
			, dblCreditLimit  = ISNULL(CPLimit.Credit,0)
			, dblCreditExposure = dblFixedPurchaseVolume + dblUnfixedPurchaseVolume --dblTotalCommittedVolume
			, dblCreditBalance  = (dblFixedPurchaseVolume + dblUnfixedPurchaseVolume) - ISNULL(CPLimit.Credit,0)
		...
		FROM (
			SELECT strEntityName
				...
				, dblDefaultRisk = SUM(dblM2M)
			FROM  @tmpCPEDetail CPE 
		...
		SELECT  strCompanyName  = strEntityName
				, strType = 'Customer' COLLATE Latin1_General_CI_AS
			...
			FROM (
				...
				FROM  @tmpCustomerExposureDetail CPE 
				GROUP BY strEntityName
					, intEntityId
					, intM2MHeaderId
					, strCountry
	...
	DELETE FROM @GetContractDetailView
	DELETE FROM @ListTransaction
	DELETE FROM @tblGetSettlementPrice
	DELETE FROM @tblOpenContractList
	DELETE FROM @tmpM2MSettlementPrice
	DELETE FROM @tmpM2MSummary
	
	DELETE FROM @tmpCustomerExposureDetail
	DELETE FROM @tmpCPEDetail
