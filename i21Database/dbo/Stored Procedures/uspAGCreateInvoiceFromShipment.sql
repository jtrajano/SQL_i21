...
SELECT TOP 1 @strReferenceNumber = strOrderNumber FROM tblAGWorkOrder ORDER BY intWorkOrderId DESC
...
DECLARE
	...
	,@WorkOrderComments		NVARCHAR(MAX)
...
LEFT OUTER JOIN tblAGWorkOrder WO ON WO.strOrderNumber = @strReferenceNumber
...
IF (ISNULL(@WorkOrderId, 0) > 0) AND EXISTS  (SELECT NULL FROM tblAGWorkOrderDetail WHERE intWorkOrderId = @WorkOrderId AND ISNULL(intRecipeId, 0) <> 0)
	BEGIN
		EXEC dbo.uspARGetDefaultComment @CompanyLocationId, @EntityCustomerId, 'Invoice', 'Standard', @InvoiceComments OUT

		SET @Comments = ISNULL(@Comments, '') + ' ' + ISNULL(@InvoiceComments,'') + ' ' + ISNULL(@WorkOrderComments, '')
	END
...
-----****************QUESTIONABLE PART OF SHIPPED ITEMS (vyuARShippedItems), FOR NOW DISREGARD***********--------
SELECT 
	...
	,[ysnInventory]							= 0
	,[strDocumentNumber]					= WO.strOrderNumber 
	,[strItemDescription]					= WOD.strItemDescription
	,[intOrderUOMId]						= NULL
	...
	,[intTaxGroupId]						= NULL
	,[intStorageLocationId]					= WOD.[intStorageLocationId] 
	,[intCompanyLocationSubLocationId]		= WOD.[intSubLocationId] 
	,[ysnRecomputeTax]						= (CASE WHEN ISNULL(WOD.[intWorkOrderDetailId], 0) = 0 THEN 1 ELSE 0 END)
	,[intSCInvoiceId]						= NULL
	...
	,[intRecipeItemId]						= WOD.intRecipeItemId 
	,[intRecipeId]							= WOD.intRecipeId
	,[intSubLocationId]						= NULL
	,[intCostTypeId]						= NULL
	,[intMarginById]						= NULL
	,[intCommentTypeId]						= WOD.intCommentTypeId
	,[dblMargin]							= @ZeroDecimal
	,[dblRecipeQuantity]					= @ZeroDecimal
	,[intSalesOrderDetailId]				= WOD.intWorkOrderDetailId
	,[strSalesOrderNumber]					= WO.strOrderNumber 
	...
	,[intTempDetailIdForTaxes]				= WOD.intWorkOrderDetailId
	,[ysnBlended]							= 0
	,[intStorageScheduleTypeId]				= WOD.intStorageScheduleTypeId
	,[intDestinationGradeId]				= NULL
	,[intDestinationWeightId]				= NULL
	,[intCurrencyExchangeRateTypeId]		= WOD.[intCurrencyExchangeRateTypeId]
	,[intCurrencyExchangeRateId]			= WOD.[intCurrencyExchangeRateId]
	,[dblCurrencyExchangeRate]				= WOD.[dblCurrencyExchangeRate]
	,[intSubCurrencyId]						= WOD.[intSubCurrencyId]
	,[dblSubCurrencyRate]					= WOD.[dblSubCurrencyRate]
FROM tblICInventoryShipment ICIS
INNER JOIN tblAGWorkOrder WO ON WO.strOrderNumber = @strReferenceNumber
							 AND ICIS.intEntityCustomerId = WO.intEntityCustomerId 
INNER JOIN tblAGWorkOrderDetail WOD ON WO.intWorkOrderId = WOD.intWorkOrderId 
								    --AND SOD.intCommentTypeId IN (0,1,3)
									AND WOD.dblQtyOrdered = 0
WHERE ICIS.intInventoryShipmentId = @ShipmentId


UNION ALL

--ADDITIONAL LINE ON SHIPMENT
SELECT 
	 [strSourceTransaction]					= 'Inventory Shipment'
	,[intSourceId]							= @ShipmentId
	,[strSourceId]							= @ShipmentNumber
...
IF EXISTS (SELECT NULL FROM #TempTable WHERE ISNULL(intRecipeId, 0) > 0)
	BEGIN
		DECLARE @tblItemsWithRecipe TABLE(intWorkOrderDetailId INT, intRecipeId INT)
		DECLARE @intCurrentWorkOrderDetailId INT
		      , @intCurrentRecipeId INT
			  , @intMinWorkOrderDetailId INT

		INSERT INTO @tblItemsWithRecipe
		SELECT DISTINCT MIN(intWorkOrderDetailId), intRecipeId FROM #TempTable WHERE intRecipeId > 0 GROUP BY intRecipeId

		WHILE EXISTS (SELECT NULL FROM @tblItemsWithRecipe)
			BEGIN
				SELECT TOP 1 @intMinWorkOrderDetailId = MIN(intWorkOrderDetailId) FROM @tblItemsWithRecipe
				SELECT TOP 1 @intCurrentRecipeId = intRecipeId FROM @tblItemsWithRecipe WHERE intSalesOrderDetailId = @intMinWorkOrderDetailId

				WHILE EXISTS (SELECT NULL FROM #TempTable)
					BEGIN
						SELECT TOP 1 @intCurrentWorkOrderDetailId = MIN(intWorkOrderDetailId) FROM #TempTable WHERE intRecipeId IS NULL

						IF @intMinWorkOrderDetailId > @intCurrentWorkOrderDetailId
							BEGIN
								INSERT INTO @EntriesForInvoice
								SELECT * FROM #TempTable WHERE intWorkOrderDetailId = @intCurrentWorkOrderDetailId

								DELETE FROM #TempTable WHERE intWorkOrderDetailId = @intCurrentWorkOrderDetailId
								CONTINUE
							END
						ELSE
							BEGIN
								...
								SET @intMinWorkOrderDetailId = 0
								BREAK
							END

						SET @intCurrentWorkOrderDetailId = 0						
					END
...
ELSE
	BEGIN
		INSERT INTO @EntriesForInvoice
		SELECT * FROM #TempTable ORDER BY intWorkOrderDetailId
	END
...
--TAX ENTRIES	-- NOT SURE IF WE CALCULATE ON AGWO

--GET DISTINCT SHIP TO FROM CONTRACT DETAIL -- TO CHECK IF NEEDED
