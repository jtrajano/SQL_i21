...
BEGIN TRY
	...
	--IF @strLotTracking != 'Yes - Manual'
		BEGIN
			...
			SELECT @intShipmentOrderId = intOrderId FROM tblICInventoryShipmentItem WHERE intInventoryShipmentId = @InventoryShipmentId
			...
			BEGIN
				...
				IF ISNULL(@intInvoiceId , 0) != 0 AND @dblQtyShipped > 0
				BEGIN
