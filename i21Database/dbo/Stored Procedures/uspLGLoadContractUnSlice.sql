			...
			BEGIN
				...
				BEGIN TRY
					...
					INSERT INTO @SingleAuditLogParam ([Id], [KeyValue], [Action], [Change], [From], [To], [Alias], [Field], [Hidden], [ParentId])
							SELECT 1, '', 'Updated', 'Updated - Record: ' + CAST(@intAuditLogLoadId AS VARCHAR(MAX)), NULL, NULL, NULL, NULL, NULL, NULL
							UNION ALL
							SELECT 2, '', '', 'intContractDetailId', LTRIM(@intParentContractDetailId), LTRIM(@intCContractDetailId), NULL, NULL, NULL, 1

					EXEC uspSMSingleAuditLog 
						@screenName     = 'Logistics.view.ShipmentSchedule',
						@recordId       = @intAuditLogLoadId,
						@entityId       = @intUserId,
