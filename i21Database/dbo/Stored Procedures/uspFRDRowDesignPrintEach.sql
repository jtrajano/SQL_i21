...
IF NOT EXISTS(SELECT TOP 1 1 FROM tblFRRowDesignPrintEach WHERE intRowId = @intRowId AND intConcurrencyId = @ConcurrencyId)            
 ...
 BEGIN            
  ...
  IF(@strAccountsUsed LIKE '%Cluster%')          
  BEGIN           
  print(@intRowDetailId)    
  EXEC uspFRDGetClusterFilter @intRowDetailId,0,0, @strAccountsUsed OUTPUT          
  print(@strAccountsUsed)    
  END          
