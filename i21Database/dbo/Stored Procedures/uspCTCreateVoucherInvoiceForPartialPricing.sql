...
BEGIN TRY
	...
	DECLARE @ErrMsg							NVARCHAR(MAX),
		...
		--Additional variable for CT-4886
		declare @dblAvailablePriceQuantity numeric(18,6);
		declare @dblAvailableReceiptQuantity numeric(18,6);
		declare @intExisitingUnpostedVoucherId int;
		...
		declare @createdVouchersId nvarchar(50);
		declare @intAvailableReceiptLoad int;
		declare @intAvailablePriceLoad int;
		declare @dblPriceQuantity numeric(18,6);
	...
	/*
	--Commented this block of Code for CT-4886 Purchase Contract pricing refactor
	...
	--End of Commented this block of Code for CT-4886 Purchase Contract pricing refactor
    */
    ...
    --Fix for CT-4886
	 ...
	 begin  
		...
		/*Check if there's available inventory receipt quantity*/  
		select top 1  
			@intInventoryReceiptId = intInventoryReceiptId  
			,@intInventoryReceiptItemId = intInventoryReceiptItemId  
			,@dblAvailableReceiptQuantity = dblAvailableQuantity  
			,@strReceiptNumber = strReceiptNumber  
			,@strTicketNumber = strTicketNumber  
			,@intTicketId = intTicketId  
			,@dblReceived = dblReceived  
			,@intAvailableReceiptLoad = intAvailableLoad  
		from  
			vyuCTGetAvailableInventoryItemForVoucher  
		where  
			intContractDetailId = @intContractDetailId  
		order by intInventoryReceiptItemId  

		while (@intPriceFixationDetailId is not null and @intInventoryReceiptItemId is not null)  
		begin  
	  
			/*
			Compare the available priced quantity and available receipt quantity.
			The lower quantity will be the quantity to be vouchered
			If load based, Quantity to bill should be the Receipt quantity
			*/  
			set @dblQtyToBill = @dblAvailableReceiptQuantity;  
			if (@dblAvailablePriceQuantity < @dblAvailableReceiptQuantity and isnull(@ysnLoad,0) = 0)  
			begin  
				set @dblQtyToBill = @dblAvailablePriceQuantity;  
			end  

			/*Chek if there's an exisiting unposted Voucher*/  
			select top 1  
				@intExisitingUnpostedVoucherId = b.intBillId  
			from  
				@tblCreatedTransaction ct  
				join tblAPBill b on b.intBillId = ct.intTransactionId  
				join tblAPBillDetail bd on bd.intBillId = b.intBillId  
			where  
				isnull(b.ysnPosted,0) = 0  
				and b.intTransactionType <> 13   
				and bd.intInventoryReceiptItemId = @intInventoryReceiptItemId  
				and bd.intInventoryReceiptChargeId is null;  

			set @strVendorOrderNumber = isnull(@strTicketNumber,@strReceiptNumber);  

			/*Allow Receipt Item to be Vocuhered*/  
			update  
				tblICInventoryReceiptItem  
			set  
				ysnAllowVoucher = 1  
			...
			/*If no unposted Voucher, create new*/  
			if (isnull(@intExisitingUnpostedVoucherId,0) = 0)  
			begin  
				/*Construct voucher detail*/  
				delete from @voucherPayablesData;
				INSERT INTO @voucherPayablesData(  
					[intEntityVendorId]     
					,[intTransactionType]    
				...
				SELECT   
					...
					,[intOrderUOMId] = GP.[intQtyToBillUOMId]  
					...
					,GP.[dblCostUnitQty]       
					,GP.[intCostUOMId]        
					,[dblNetWeight] = dbo.fnCTConvertQtyToTargetItemUOM(@intItemUOMId, GP.[intWeightUOMId], @dblQtyToBill)   
					,[dblWeightUnitQty]       
					,GP.[intWeightUOMId]       
					...
					,GP.[strBillOfLading]       
					,GP.[ysnReturn]   
					,[dtmVoucherDate] = @dtmFixationDate  
					,[dtmDueDate] = @dtmFixationDate  
				...
				exec uspAPCreateVoucher  
					@voucherPayables = @voucherPayablesData  
				...
				/*Loop through created Vouchers*/  
				select @intNewBillId = min(Item) from fnSplitString(@createdVouchersId,',');
				
				while (@intNewBillId is not null)  
				begin
					/*Collect the created Voucher Id*/  
					insert into @tblCreatedTransaction select intTransactionId = @intNewBillId, intTicketId = @intTicketId;  

					/*Add the 'DP/Basis' other charges into the voucher*/  
					select @intBillDetailId = intBillDetailId from tblAPBillDetail where intBillId = @intNewBillId;  
					...
					select @intNewBillId = min(Item) from fnSplitString(@createdVouchersId,',') where Item > @intNewBillId;  
				end  
			end  
			/*If there's unposted Voucher, add detail*/  
			else  
			begin  
				/*Construct voucher detail*/  
				delete from @voucherPayablesData;
				...
				SELECT   
					[intBillId] = @intExisitingUnpostedVoucherId  
					,GP.[intEntityVendorId]  
					,GP.[intTransactionType]  
					,GP.[intLocationId]   
					,[intShipToId] = GP.intLocationId   
				...
				/*Add detail to unposted Voucher*/  
				EXEC uspAPAddVoucherDetail @voucherDetails = @voucherPayablesData, @voucherPayableTax = @voucherPayableTax, @throwError = 1, @error = @ErrMsg;  

				/*Get the newly created Bill DetailId Id*/  
				select top 1   
					@intBillDetailId = a.intBillDetailId   
				from  
					tblAPBillDetail a  
					,tblAPBill b  
				where
					a.intBillId = @intExisitingUnpostedVoucherId   
					and a.intContractDetailId = @intContractDetailId   
					and a.intInventoryReceiptChargeId IS NULL  
					and b.intBillId = a.intBillId  
					and b.intTransactionType <> 13  
			...
			/*Return Receipt Item not to be Vocuhered*/  
			update  
				tblICInventoryReceiptItem  
			set  
				ysnAllowVoucher = 0  
			...
			/*Check next available priced quantity*/  
			select  
				@intPriceFixationId = null  
				,@intPriceFixationDetailId = null  
				,@dblFinalPrice = null  
				,@dblAvailablePriceQuantity = null  
				,@dtmFixationDate = null  
				,@dblPriceQuantity = null  
				,@intAvailablePriceLoad = null  
			...
			/*Check next available inventory receipt quantity*/  
			select  
				@intInventoryReceiptId = null  
				,@intInventoryReceiptItemId = null  
				,@dblAvailableReceiptQuantity = null  
				,@strReceiptNumber = null  
				,@strTicketNumber = null  
				,@intTicketId = null  
				,@dblReceived = null  
				,@intAvailableReceiptLoad = null  
		...
		/*Loop through created Vouchers*/
		select @intBillId = null;
		select @intBillId = min(intTransactionId) from @tblCreatedTransaction;  
		
		while (@intBillId is not null)  
		begin  
			/*Code here to post all created Vouchers*/  
			EXEC [dbo].[uspAPPostBill] @post = 1,@recap = 0,@isBatch = 0,@param = @intBillId,@userId = @intUserId,@success = @ysnSuccess OUTPUT  
			...
			where
				...
				and bd.intScaleTicketId = (Select intTicketId from @tblCreatedTransaction where intTransactionId = @intBillId)   
				and bl.intTransactionType in (2, 13)  
				and bl.ysnPosted = 1   
				and bl.ysnPaid = 0   
			...
			select @intBillId = min(intTransactionId) from @tblCreatedTransaction where intTransactionId > @intBillId;  
		end  

		ExitCreatingVoucher:  
	 end  

    --End of Fix for CT-4886
