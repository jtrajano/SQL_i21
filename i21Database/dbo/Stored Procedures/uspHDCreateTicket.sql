...
if (isnull(@Import, convert(bit,0)) = convert(bit,0))
begin
	INSERT INTO [dbo].[tblHDTicketForImport]
			   ([strSubject]
			   ,[strCustomerNumber]
			   ,[strProduct]
			   ,[strVersion]
		 ...
		 select
			   ...
			   ,strStatus = @Status
			   ,dtmCreated = getdate()
			   ,ysnImported = convert(bit,0)

	set @message = 'Save for furture importing.';
	set @strTicketNumber = 'HDTN-000000';
end
else
begin
