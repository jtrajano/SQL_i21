...
SELECT DISTINCT  
  ...
  ORDER BY 1 desc),0) as dblDelta  
   ...
   ,CASE WHEN strBuySell ='Buy' Then 'B' else 'S' End strBuySell,intFutOptTransactionId,  
    isnull((Select SUM(intLots) From tblRKOptionsPnSExpired ope where  ope.intFutOptTransactionId= ot.intFutOptTransactionId
				and dtmExpiredDate<=@dtmPositionAsOf),0) intExpiredLots,      
    isnull((Select SUM(intLots) FROM tblRKOptionsPnSExercisedAssigned opa 
			where  opa.intFutOptTransactionId= ot.intFutOptTransactionId and dtmTranDate<=@dtmPositionAsOf),0) intAssignedLots, 
