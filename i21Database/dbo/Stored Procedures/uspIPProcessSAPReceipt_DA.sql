CREATE PROCEDURE uspIPProcessSAPReceipt_DA @strInfo1 NVARCHAR(MAX) = '' OUT
...
BEGIN TRY
	...
	DECLARE @strERPPONumber NVARCHAR(100)
		...
		,@strCleanNetWeightUOM NVARCHAR(50)
		...
		BEGIN TRY
			...
			BEGIN
				SELECT @strERPPONumber = NULL
					...
					,@dblCleanGrossWeight = NULL
					,@dblCleanTareWeight = NULL
					,@dblCleanNetWeight = NULL
					,@strCleanNetWeightUOM = NULL
				...
				SELECT @strERPPONumber = strERPPONumber
					...
					,@dblCleanGrossWeight = dblCleanGrossWeight
					,@dblCleanTareWeight = dblCleanTareWeight
					,@dblCleanNetWeight = dblCleanNetWeight
					,@strCleanNetWeightUOM = strCleanNetWeightUOM
				FROM tblIPInvReceiptItemStage WITH (NOLOCK)
