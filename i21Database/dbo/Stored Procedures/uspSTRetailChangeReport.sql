...
BEGIN 
	BEGIN TRY
		...
		DECLARE @strMessageResult NVARCHAR(50) = 'Success'

		DECLARE @tmpReportPreview AS TABLE (
			strCategoryDescription NVARCHAR(50),
			intCategoryId INT,
			intItemId INT,
			strItemNo NVARCHAR(50),
			strItemDescription NVARCHAR(50),
			strOldData DECIMAL(16, 2),
			strNewData DECIMAL(16, 2)
		) 

		INSERT INTO @tmpReportPreview
		SELECT DISTINCT
		...
		IF EXISTS(SELECT TOP 1 1 FROM @tmpReportPreview)
		BEGIN
			INSERT INTO tblSTMassUpdateAudit (strScreenName,strXML, strStatus,dtmDateGenerated,intCurrentUserId)
			VALUES ('Retail Change Report', @XML, @strMessageResult, GETDATE(), @intUserId);
		END
		
		SELECT * FROM @tmpReportPreview

END TRY

BEGIN CATCH
		SET @strMessageResult = 'Failed - ' + ERROR_MESSAGE() + '. '
