CREATE PROCEDURE [dbo].[uspSTReportCheckoutTotalMOPRecap]
...
BEGIN TRY
   ...
   select @strCompanyName as CompanyName, D.strPaymentOptionId as MOPID, D.strDescription, A.dblAmount, SUM(A.dblAmount) over() as TotalMOP ,
   E.strAccountId AS AccountID, E.strDescription AS Description,C.intStoreNo as Store,CONVERT(VARCHAR(50),B.dtmCheckoutDate,101) as checkoutDate,
   B.intShiftNo as ShiftNo from tblSTCheckoutPaymentOptions A  JOIN tblSTCheckoutHeader
