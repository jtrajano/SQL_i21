	...
	BEGIN
		...
		FROM ( 
			SELECT dblCosts = dbo.fnRKGetCurrencyConvertion(CASE WHEN ISNULL(CU.ysnSubCurrency, 0) = 1 THEN CU.intMainCurrencyId ELSE dc.intCurrencyId END, @intCurrencyId, @intMarkToMarketRateTypeId)
												...
												THEN ABS(CASE WHEN dc.strCostMethod = 'Amount' 
														THEN (SUM(dc.dblRate) 
															/ CASE WHEN ISNULL(ch.ysnLoad, 0) = 1 
																THEN (ISNULL(dbo.fnCTConvertQuantityToTargetCommodityUOM(cu2.intCommodityUnitMeasureId, cu1.intCommodityUnitMeasureId, ISNULL(cd2.dblQuantityPerLoad, 1)), 1) 
																		* CAST(ISNULL(cd2.intNoOfLoad, 1) AS NUMERIC(16, 8)))
																ELSE ISNULL(dbo.fnCTConvertQuantityToTargetCommodityUOM(cu2.intCommodityUnitMeasureId, cu1.intCommodityUnitMeasureId, ISNULL(cd.dblDetailQuantity, 1)), 1)
																END)
				...
				, a = cu2.intCommodityUnitMeasureId
			...
			INNER JOIN tblICCommodityUnitMeasure cu2 ON cu2.intCommodityId = @intCommodityId AND cu2.intUnitMeasureId = @intPriceUOMId
			...
			GROUP BY cu2.intCommodityUnitMeasureId
