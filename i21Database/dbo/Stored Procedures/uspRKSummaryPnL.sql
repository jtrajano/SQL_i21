 ...
 FROM (
  ...
  FROM (
   SELECT GrossPnL
    ...
    ,MatchShort
    ,NetPnL,ysnExpired
   FROM vyuRKUnrealizedPnL_Copy where intCommodityId= case when isnull(@intCommodityId,0)=0 then intCommodityId else @intCommodityId end and ysnExpired=@ysnExpired
   ) t
  ) u
 ...
 FROM (
  ...
  FROM (
   SELECT GrossPnL
    ...
    ,MatchShort
    ,NetPnL,ysnExpired
   FROM vyuRKUnrealizedPnL_Copy where intCommodityId= case when isnull(@intCommodityId,0)=0 then intCommodityId else @intCommodityId end 
   ) t
  ) u
 GROUP BY intFutureMonthId
  ,intFutureMarketId
