CREATE PROCEDURE [dbo].[uspMFGetYieldConfigDetail] 
(
...
BEGIN
	...
	SELECT ISNULL(@intYieldId, 0) AS intYieldId
		 , CASE WHEN ysnProcessRelated = 1 THEN 'Process Related'
				ELSE 'Non-Process Related'
		   END AS strGroupName
		 , strYieldTransactionName AS strTransactionName
		 , CASE WHEN ysnInputTransaction = 1 THEN 'Input'
				ELSE 'Output'
		   END AS strSection
		 , YT.intYieldTransactionId
		 , ISNULL(YD.ysnSelect, 0) AS ysnSelect
		 , CONVERT(BIT, CASE WHEN YT.strYieldTransactionName IN (
					'Empty Out Adj'
					,'Cycle Count Adj'
					,'Queued Qty Adj'
					)
