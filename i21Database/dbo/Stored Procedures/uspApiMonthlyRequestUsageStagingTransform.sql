...
WHEN MATCHED
THEN UPDATE SET TARGET.intCount = SOURCE.intCount, TARGET.dtmDateLastUpdated = GETUTCDATE()/*SOURCE.dtmDateLastUpdated*/, TARGET.strLastStatus = SOURCE.strLastStatus
...
THEN INSERT (guiApiMonthlyRequestUsageId, guiSubscriptionId, strName, strMethod, strPath, intCount, intMonth, strMonth, intYear, strLastStatus, dtmDateLastUpdated) 
	VALUES (NEWID(), SOURCE.guiSubscriptionId, SOURCE.strName, SOURCE.strMethod, SOURCE.strPath, 
		SOURCE.intCount, SOURCE.intMonth, SOURCE.strMonth, SOURCE.intYear, SOURCE.strLastStatus, GETUTCDATE()/*SOURCE.dtmDateLastUpdated*/)
;
