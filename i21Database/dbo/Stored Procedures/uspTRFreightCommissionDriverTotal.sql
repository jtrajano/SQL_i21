    ...
    BEGIN  
        ...
        BEGIN  
            ...
            BEGIN  
                ...
                -- Other Item  
                SELECT @dblTotalCommission += ISNULL(ldd.dblPrice, 0)  
                FROM tblTRLoadDistributionDetail ldd  
                WHERE intLoadDistributionDetailId = @curLoadDistributionDetailId  
                    AND ISNULL(strReceiptLink, '') = ''  
            END  
            ...
            BEGIN  
                SELECT @dblTotalCommission += CONVERT(DECIMAL(18,6),CASE WHEN intItemId = @intFreightItemId THEN ((@dblFreightUnitCommissionPct/CONVERT(DECIMAL(18,6),100)) * dblTotal) ELSE ((@dblOtherUnitCommissionPct/CONVERT(DECIMAL(18,6),100)) * dblTotal) END)  
                ...
                SELECT @dblTotalCommission += CONVERT(DECIMAL(18,6),((@dblOtherUnitCommissionPct/CONVERT(DECIMAL(18,6),100)) * dblTotal))  
                FROM vyuTRGetFreightCommissionFreight fcf  
                WHERE intInvoiceId = @curInvoiceId  
                    AND (  
