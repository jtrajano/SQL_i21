...
BEGIN TRY
	...
	IF (@strOutputLotNumber = '' OR @strOutputLotNumber IS NULL)
	...
	IF EXISTS (
		SELECT	*
		FROM	dbo.tblICContainer C JOIN dbo.tblICContainerType CT 
					ON C.intContainerTypeId = CT.intContainerTypeId
	...
	BEGIN
		...
		SELECT	@intWorkOrderId
				,intLotId
				,CASE	WHEN @dblInputWeight = 0 THEN 
							dblWeight
						ELSE 
							@dblInputWeight
				END
	...
	BEGIN
		...
		EXEC dbo.uspMFConsumeWorkOrder 
			@intWorkOrderId			= @intWorkOrderId
			,@dblProduceQty			= @dblProduceQty
			,@intProduceUOMKey		= @intProduceUnitMeasureId
			,@intUserId				= @intUserId
	...
	EXEC dbo.uspMFValidateCreateLot 
	...
	EXEC dbo.uspMFProduceWorkOrder 
		@intWorkOrderId				= @intWorkOrderId
		,@dblProduceQty				= @dblProduceQty
		,@intProduceUOMKey			= @intProduceUnitMeasureId
		,@strVesselNo				= @strVesselNo
