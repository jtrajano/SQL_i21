CREATE PROCEDURE [dbo].[uspSTCheckoutRadiantISM]
	@intCheckoutId INT,
	@UDT_ISM	StagingRadiantISM		READONLY,
...
BEGIN
	BEGIN TRY
		...
		BEGIN CATCH
			SET @intCountRows = 0
			SET @strStatusMsg = 'Radiant ISM - Insert to temporary table: ' + ERROR_MESSAGE()
		...
		IF NOT EXISTS(SELECT TOP 1 1 FROM @tblTemp)
			BEGIN
					...
					(
						'XML LAYOUT MAPPING'
						, 'Radiant ISM XML file did not match the layout mapping'
						, ''
						, ''
						, @intCheckoutId
						, 1
					...
					SET @intCountRows = 0
					SET @strStatusMsg = 'Radiant ISM XML file did not match the layout mapping'
		...
		(
			SELECT DISTINCT
				CAST(dbo.fnICValidateUPCCode(UOM.intUpcCode) AS BIGINT) AS intUpcCode
			FROM @tblTemp Chk
			INNER JOIN vyuSTItemUOMPosCodeFormat UOM
				ON CAST(Chk.intRegisterUpcCode AS BIGINT) = CAST(dbo.fnICValidateUPCCode(UOM.intUpcCode) AS BIGINT)
		...
		-- ==================================================================================================================
			...
			(
				SELECT DISTINCT
					dbo.fnICValidateUPCCode(UOM.intUpcCode) AS intPOSCode
		...
		-- ==================================================================================================================
			...
			INNER JOIN tblICItemUOM UOM
				ON TempChk.intPOSCode = dbo.fnICValidateUPCCode(UOM.intUpcCode)
			INNER JOIN dbo.tblICItem I 
				ON I.intItemId = UOM.intItemId
			INNER JOIN dbo.tblICItemLocation IL 
				ON IL.intItemId = I.intItemId
