...
USING (
		...
		FROM	@ItemsToIncreaseConsignedPurchase ItemsToIncreaseConsignedPurchase LEFT JOIN dbo.tblICItemUOM 
					ON ItemsToIncreaseConsignedPurchase.intItemUOMId = tblICItemUOM.intItemUOMId
...
WHEN MATCHED THEN 
	UPDATE 
	SET		dblConsignedPurchase = CASE WHEN ISNULL(ItemStock.dblConsignedPurchase, 0) + Source_Query.Aggregrate_ReserveQty < 0 THEN 0 ELSE ISNULL(ItemStock.dblConsignedPurchase, 0) + Source_Query.Aggregrate_ReserveQty END 
...
WHEN NOT MATCHED THEN 
	...
	VALUES (
		...
		,CASE WHEN Source_Query.Aggregrate_ReserveQty < 0 THEN 0 ELSE Source_Query.Aggregrate_ReserveQty END -- dblConsignedPurchase
		,NULL 
		,1	
	)		
;
...
WHEN MATCHED THEN 
	UPDATE 
	SET		dblConsignedPurchase = CASE WHEN ISNULL(ItemStockUOM.dblConsignedPurchase, 0) + Source_Query.Aggregrate_ReserveQty < 0 THEN 0 ELSE ISNULL(ItemStockUOM.dblConsignedPurchase, 0) + Source_Query.Aggregrate_ReserveQty END 
