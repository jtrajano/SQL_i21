...
DECLARE @_dblLoopLoadQuantity NUMERIC(38,20)
			...
			*/
				...
				BEGIN
					...
					BEGIN
						...
						SELECT TOP 1
							...
							,@intTicketContractUOMId = A.intItemUOMId
						FROM tblCTContractDetail A
						INNER JOIN tblCTContractHeader B
							ON A.intContractHeaderId = B.intContractHeaderId
						WHERE A.intContractDetailId = @intTicketContractDetailId 

						---UPDATE CONTRACT BALANCE AND SCHEDULE
						BEGIN
							------CONTRACT ------------------------------------------
							IF EXISTS(SELECT TOP 1 1 FROM tblSCTicketContractUsed WHERE intTicketId = @intTicketId)
							BEGIN
								
								SET @dblScheduleQty = dbo.fnCalculateQtyBetweenUOM(@intTicketItemUOMId,@intTicketContractUOMId, @dblTicketNetUnits)

								DECLARE ContractUsed CURSOR FAST_FORWARD
								FOR
									SELECT 
										C.intContractDetailId
										,dblQuantity = dbo.fnCalculateQtyBetweenUOM(@intTicketItemUOMId,C.intItemUOMId,A.dblScheduleQty)
										,ISNULL(D.ysnLoad,0) 
								...
								OPEN ContractUsed
									...
									BEGIN
										...
										ELSE
										BEGIN
											SET @_dblLoopContractUpdateQuantity = @_dblLoopQuantity * -1
										END
							...
							--LOAD-----------------------------------
							--Update LS contract balances and schedule that is used during distribution
							IF EXISTS(SELECT TOP 1 1 FROM tblSCTicketLoadUsed WHERE intTicketId = @intTicketId)
							BEGIN
								DECLARE LoadScheduledUsed CURSOR FAST_FORWARD
								FOR
									SELECT 
										C.intContractDetailId
										,dblQuantity = dbo.fnCalculateQtyBetweenUOM(@intTicketItemUOMId,C.intItemUOMId,A.dblQty)
										,ISNULL(D.ysnLoad,0) 
										,dblLoadQuantity = dbo.fnCalculateQtyBetweenUOM(B.intItemUOMId,C.intItemUOMId,B.dblQuantity)
										,B.intLoadDetailId
								...
								OPEN LoadScheduledUsed
									...
									INTO  @_intLoopContractDetailId, @_dblLoopQuantity,@_ysnLoopContractLoadBase,@_dblLoopLoadQuantity, @_intLoopLoadDetailId
			...
			BEGIN
				...
				BEGIN 
					...
					BEGIN
						UPDATE tblSCTicket
							SET ysnDestinationWeightGradePost = 0
						WHERE intTicketId = @intTicketId
