...
BEGIN
	...
	select convert(int,row_number() OVER(ORDER BY strVendorName)) intRowNum, strVendorName ,intEntityId intVendorId,strRating,
       ...
       SELECT strEntityName strVendorName,intEntityId,strRiskIndicator strRating,
					 ...
					 ,dblResult as dblMToM,dblCompanyExposurePercentage,dblSupplierSalesPercentage
       FROM (
       SELECT strEntityName,intEntityId,CONVERT(NUMERIC(16,2),dblFixedPurchaseVolume) as dblFixedPurchaseVolume,
       ...
       FROM (
              ...
              ,strRiskIndicator,dblRiskTotalBusinessVolume,intRiskUnitOfMeasureId,dblCompanyExposurePercentage,dblSupplierSalesPercentage
                     FROM (
                                  ...
                                  sum(dblResult) dblResult,strRiskIndicator,dblRiskTotalBusinessVolume,intRiskUnitOfMeasureId,dblCompanyExposurePercentage,dblSupplierSalesPercentage 
                                  FROM(
                                  SELECT isnull(strProducer,strEntityName) strEntityName,isnull(fd.intEntityId,fd.intProducerId)  intEntityId,dblOpenQty as dblOpenQty,
