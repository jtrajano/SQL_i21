...
BEGIN
	select convert(int,row_number() OVER(ORDER BY strVendorName)) intRowNum, strVendorName,intEntityId intVendorId,strRating,
			  ...
			  from (
       SELECT strVendorName,intEntityId,strRating,
       ...
       FROM(
       SELECT strEntityName strVendorName,intEntityId,strRiskIndicator strRating,
                     dblFixedPurchaseVolume,dblUnfixedPurchaseVolume,dblTotalCommittedVolume,dblFixedPurchaseValue,dblUnfixedPurchaseValue,dblTotalCommittedValue
       ...
       FROM (
       SELECT strEntityName,intEntityId,dblFixedPurchaseVolume as dblFixedPurchaseVolume,
       ...
       FROM (
                     SELECT strEntityName,intEntityId,
              ...
              ,strRiskIndicator,dblRiskTotalBusinessVolume,intRiskUnitOfMeasureId,dblCompanyExposurePercentage,dblSupplierSalesPercentage
                     FROM (
                                  SELECT strEntityName,intEntityId,sum(dblOpenQty) as dblOpenQty,sum(dblQtyPrice) dblQtyPrice,sum(dblQtyUnFixedPrice) dblQtyUnFixedPrice,strPriOrNotPriOrParPriced,
                                  ...
                                  SELECT fd.strEntityName,fd.intEntityId, fd.dblOpenQty as dblOpenQty,
       ...
       GROUP BY strEntityName,strPriOrNotPriOrParPriced,strRiskIndicator,dblRiskTotalBusinessVolume,intRiskUnitOfMeasureId,dblCompanyExposurePercentage,dblSupplierSalesPercentage,intEntityId)t1
       GROUP BY strEntityName,strRiskIndicator,dblRiskTotalBusinessVolume,intRiskUnitOfMeasureId,dblCompanyExposurePercentage,dblSupplierSalesPercentage,intEntityId)t2)t2 
       )t3)t4
END
...
BEGIN
	...
	select convert(int,row_number() OVER(ORDER BY strVendorName)) intRowNum
		...
		, strRating
		, CONVERT(NUMERIC(16,2),dblFixedPurchaseVolume) dblFixedPurchaseVolume
		, CONVERT(NUMERIC(16,2),dblUnfixedPurchaseVolume) dblUnfixedPurchaseVolume
		, CONVERT(NUMERIC(16,2),dblTotalCommittedVolume) dblTotalCommittedVolume
		...
		, CONVERT(NUMERIC(16,2),dblTotalCommittedValue) dblTotalCommittedValue
		, CONVERT(NUMERIC(16,2),dblTotalSpend) dblTotalSpend
		, dblShareWithSupplier
		, dblMToM
		, dblCompanyExposurePercentage
		, case when (isnull(dblPotentialAdditionalVolume,0)-isnull(dblTotalCommittedVolume,0)) < 0 then 0 else (isnull(dblPotentialAdditionalVolume,0)-isnull(dblTotalCommittedVolume,0)) end  dblPotentialAdditionalVolume
	...
	from (
		SELECT strVendorName
			...
			,CONVERT(NUMERIC(16,2),dblTotalSpend) as dblTotalSpend
			,CONVERT(NUMERIC(16,2),dblShareWithSupplier) as dblShareWithSupplier
			,dblMToM
			,dblCompanyExposurePercentage
			,(dblTotalCommittedVolume / CASE WHEN ISNULL(dblTotalSpend,0) = 0 then 1 else dblTotalSpend end) * dblCompanyExposurePercentage a
			,(dblTotalCommittedVolume / CASE WHEN ISNULL(dblShareWithSupplier,0) = 0 then 1 else dblShareWithSupplier end) *dblSupplierSalesPercentage b
			...
			, 0 as intConcurrencyId
		FROM(
			SELECT strEntityName strVendorName
				,intEntityId
				,strRiskIndicator strRating
				,dblFixedPurchaseVolume
				,dblUnfixedPurchaseVolume
				,dblTotalCommittedVolume
				,dblFixedPurchaseValue
			...
			FROM (
				...
				FROM (
					...
					FROM (
						SELECT strEntityName
							,intEntityId
							,sum(dblOpenQty) as dblOpenQty
							,sum(dblQtyPrice) dblQtyPrice
							,sum(dblQtyUnFixedPrice) dblQtyUnFixedPrice
							,strPriOrNotPriOrParPriced
							,sum(dblResult) dblResult
							,strRiskIndicator
							,dblRiskTotalBusinessVolume
							,intRiskUnitOfMeasureId
							,dblCompanyExposurePercentage
							,dblSupplierSalesPercentage
						FROM(
							SELECT isnull(strProducer,strEntityName) COLLATE Latin1_General_CI_AS strEntityName
								,isnull(fd.intEntityId,fd.intProducerId) intEntityId
								,dblOpenQty as dblOpenQty
								,dbo.fnCTConvertQuantityToTargetCommodityUOM(case when isnull(intQuantityUOMId,0)=0 then fd.intCommodityUnitMeasureId else intQuantityUOMId end
								,fd.intCommodityUnitMeasureId,dbo.fnCTConvertQuantityToTargetCommodityUOM(fd.intCommodityUnitMeasureId,isnull(intPriceUOMId,fd.intCommodityUnitMeasureId)
								,fd.dblOpenQty*((isnull(fd.dblContractBasis,0))+(isnull(fd.dblFutures,0))))) dblQtyPrice
								,dbo.fnCTConvertQuantityToTargetCommodityUOM(case when isnull(intQuantityUOMId,0)=0 then fd.intCommodityUnitMeasureId else intQuantityUOMId end
								,fd.intCommodityUnitMeasureId,dbo.fnCTConvertQuantityToTargetCommodityUOM(fd.intCommodityUnitMeasureId,isnull(intPriceUOMId,fd.intCommodityUnitMeasureId)
								,fd.dblOpenQty*((isnull(fd.dblContractBasis,0))+(isnull(fd.dblFuturePrice,0))))) dblQtyUnFixedPrice
								,CASE WHEN strPriOrNotPriOrParPriced = 'Partially Priced' THEN 'Unpriced'
										WHEN  ISNULL(strPriOrNotPriOrParPriced,'') = '' THEN 'Priced'
										when  strPriOrNotPriOrParPriced = 'Fully Priced' THEN 'Priced' ELSE strPriOrNotPriOrParPriced END COLLATE Latin1_General_CI_AS strPriOrNotPriOrParPriced
								,round(isnull(dblResult,0),2) dblResult
								,CASE WHEN isnull(strProducer,'')='' THEN pf1.strRiskIndicator ELSE pf.strRiskIndicator END COLLATE Latin1_General_CI_AS strRiskIndicator
