...
FROM
	@TaxDetailItems TD
WHERE
	EXISTS(SELECT NULL FROM tblSMTaxCode SMTC WITH (NOLOCK) WHERE SMTC.[intTaxCodeId] = TD.[intTaxCodeId] AND UPPER(LTRIM(RTRIM(ISNULL(TD.[strCalculationMethod],'')))) NOT IN (UPPER('Unit'),UPPER('Percentage'),UPPER('Percentage of Tax Only')))
