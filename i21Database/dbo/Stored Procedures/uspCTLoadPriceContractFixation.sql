...
BEGIN TRY
	...
	DECLARE	@ErrMsg	NVARCHAR(MAX)
		...
		update
			ctcd
		set
			ctcd.dblBalance = ctcd.dblQuantity - isnull(tbl.dblDestinationQuantity, tbl.dblQuantity)
		from
			(
				...
				WHERE Shipment.ysnPosted = 1 AND ShipmentItem.intOrderId = (select intContractHeaderId from tblCTPriceFixation where intPriceContractId = @intPriceContractId)
				GROUP BY ShipmentItem.intOrderId,ShipmentItem.intLineNo 
			)tbl
			, tblCTContractDetail ctcd
		where
			ctcd.intContractDetailId = tbl.intContractDetailId
