...
SELECT cv.strFutureMonth,  
  ...
  dblHeaderNoOfLots as dblNoOfLot,
  dbo.fnCTConvertQuantityToTargetCommodityUOM(um.intCommodityUnitMeasureId,@intUOMId,case when @ysnIncludeInventoryHedge = 0 
  ...
  JOIN tblRKFutureMarket ffm on ffm.intFutureMarketId=cv.intFutureMarketId and cv.intCommodityId=@intCommodityId and cv.intFutureMarketId=@intFutureMarketId and  cv.intContractStatusId <> 3 and cv.intPricingTypeId=1
  ...
  WHERE cv.intCommodityId=@intCommodityId and cv.intFutureMarketId=@intFutureMarketId and  cv.intContractStatusId <> 3 and cv.intPricingTypeId=1
union
--Parcial Priced
...
SELECT cv.strFutureMonth,  
  ...
  strContractType as TranType, 
  strEntityName  as CustVendor,  
  case when isnull(ysnMultiplePriceFixation,0)=0 then dblDetailNoOfLots else dblHeaderNoOfLots end  as dblNoOfLot,
  dbo.fnCTConvertQuantityToTargetCommodityUOM(um.intCommodityUnitMeasureId,@intUOMId,case when @ysnIncludeInventoryHedge = 0 
											then isnull(dblBalance,0) else isnull(dblDetailQuantity,0) end) as dblQuantity,
  cv.intContractHeaderId,null as intFutOptTransactionHeaderId  
  ,1 intPricingTypeId
...
intRowNumber,Selection,PriceStatus,strFutureMonth,strAccountNumber,  
	...
	strTradeNo,TransactionDate,TranType,CustVendor,dblNoOfLot, 
