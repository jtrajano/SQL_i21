	...
	BEGIN TRY
		...
		ELSE IF(@strRegisterClass = 'RADIANT')
			BEGIN
				...
				DECLARE @tblTempRadiantItemLists TABLE 
				(
				...
				INSERT INTO @tblTempRadiantItemLists
				(
					[intItemLocationId],
					[strStoreNo], 
					[strVendorName], 
				...
				IF EXISTS(SELECT TOP 1 1 FROM @tblTempRadiantItemLists)
					BEGIN
						
						DECLARE @xmlRadiant XML = N''
										...
										(	
											SELECT
												...
												(
													...
													FROM @tblTempRadiantItemLists IL
						...
						DECLARE @strXmlnsRadiant AS NVARCHAR(50) = 'http://www.naxml.org/POSBO/Vocabulary/2003-10-16'
								, @strVersionRadiant NVARCHAR(50) = '3.4'

						DECLARE @strNamesSpaceRadiant AS NVARCHAR(150) = 'domain:PLU xmlns:domain="urn:vfi-sapphire:np.domain.2001-07-01" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"'
								, @strDomainPluRadiant AS NVARCHAR(50) = 'domain:PLU'

						-- INSERT Attributes 'page' and 'ofpages' to Root header
						SET @xmlRadiant.modify('insert 
									   (
											attribute version { 
						...
						DECLARE @strXMLRadiant AS NVARCHAR(MAX) = CAST(@xmlRadiant AS NVARCHAR(MAX))
						SET @strXMLRadiant = REPLACE(@strXMLRadiant, '<NAXML-MaintenanceRequest', '<NAXML-MaintenanceRequest release="3.4.0" xmlns="http://www.naxml.org/POSBO/Vocabulary/2003-10-16" xmlns:radiant="http://www.radiantsystems.com/NAXML-Extension" xsi:schemaLocation="NAXML-PBIDictionaryRadiantExtension.xsd" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"')
						
						SET @strGeneratedXML = REPLACE(@strXMLRadiant, '><', '>' + CHAR(13) + '<')
					END
				ELSE 
					BEGIN
						SET @ysnSuccessResult = CAST(0 AS BIT)
