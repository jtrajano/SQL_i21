...
SELECT DISTINCT WC.intWeightClaimId
	...
	,@strCity + ', '+ DATENAME(dd,getdate()) + ' ' + isnull(dbo.fnCTGetTranslatedExpression(@strMonthLabelName,@intLaguageId,convert(nvarchar(3), getdate(), 107)),convert(nvarchar(3), getdate(), 107)) + ' ' + DATENAME(yyyy,getdate()) AS strCityAndDate
	,L.intLoadId
	,L.strLoadNumber
	,E.intEntityId
	...
	,E.strName + CHAR(13) + EL.strAddress + CHAR(13) + EL.strZipCode + ' ' + EL.strCity + CHAR(13) + EL.strState + ' ' + isnull(rtELTranslation.strTranslation,EL.strCountry) AS strVendorAddress
	,B.strBillId
