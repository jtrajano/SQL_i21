...
BEGIN 
	SELECT  @strCustomerName = LEFT(strName, LEN(strName) - 1) 
	FROM (
		SELECT 
			 REPLACE(strName, '''', '''''')  +  ', '
		FROM 
...
FROM   
   (
     SELECT CAST ('<M>' + REPLACE(@strCustomerName, ', ', '</M><M>') + '</M>' AS XML) AS String
   ) AS A
     CROSS APPLY String.nodes('/M') AS Split ( a ); 

-- show what's in the temp table
INSERT INTO @SelectedCustomer (strName)
	...
	WHERE I.ysnPosted = 1
		AND I.intAccountId IN (SELECT 
							 ...
							 WHERE 
								strAccountCategory IN ('AR Account', 'Customer Prepayments', 'Undeposited Funds'))
	AND 
		strName IN (SELECT strName FROM @SelectedCustomer)
		
	ORDER BY 
		intEntityCustomerId, strRecordNumber, intInvoiceId, intItemId
END

DROP TABLE #SelectedCustomer