	...
	BEGIN
		...
		BEGIN
			...
			SELECT @strBatchXML = @strBatchXML
				...
				+ '<Plant>' + CASE WHEN ISNULL(@strPlant, '') = '3321' Then ISNULL(B.strStorageLocation, '') When ISNULL(@strBuyingCountry, '') = ISNULL(@strMixingUnitCountry, '') OR @ysnTrackMFTActivity=1 THEN '' ELSE ISNULL(CL.strVendorRefNoPrefix, '') END + '</Plant>'
