...
IF(OBJECT_ID('tempdb..#CANCELLEDINVOICE') IS NOT NULL) DROP TABLE #CANCELLEDINVOICE
IF(OBJECT_ID('tempdb..#CANCELLEDCMINVOICE') IS NOT NULL) DROP TABLE #CANCELLEDCMINVOICE
...
IF @ysnIncludeCreditsLocal = 0
	DELETE FROM #AGINGPOSTEDINVOICES WHERE strTransactionType NOT IN ('Invoice', 'Debit Memo', 'Cash Refund')
