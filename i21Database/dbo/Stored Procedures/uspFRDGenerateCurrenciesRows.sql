...
DECLARE @filterAccount NVARCHAR(MAX)                      
...
BEGIN                      
 ...
 WHILE EXISTS(SELECT 1 FROM #TMPFILTER)                           
  BEGIN                       
   ...
   IF @filterString LIKE '%Cluster%'                    
   BEGIN       
  EXEC uspFRDGetClusterFilter @filterID,0,0, @filterString OUTPUT       
  SET @filterString  = REPLACE(REPLACE(REPLACE(REPLACE(@filterString ,'[ID]','strAccountId'),'[Group]','strAccountGroup'),'[Type]','strAccountType'),'[Description]','strDescription')      
   END      
