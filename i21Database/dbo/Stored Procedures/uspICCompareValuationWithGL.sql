CREATE PROCEDURE uspICCompareValuationWithGL
...
BEGIN 
	...
	IF @return < 0 AND @emailProfileName IS NULL 
	BEGIN 		
		RAISERROR('Valuation does not match with GL.', 11, 1)
		-- Show the Result 
		SELECT * FROM @icVsGLResult
		RETURN @return
	END 

	IF @return < 0 AND @emailProfileName IS NOT NULL AND @emailRecipient IS NOT NULL 
	BEGIN 
