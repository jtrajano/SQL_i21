...
-- If matched, update the Reserved Qty
WHEN MATCHED THEN 
	UPDATE 
...
-- If separate UOMs is not enabled, convert the non-lot qty to stock unit. 
MERGE	
...
-- If separate UOMs is enabled, don't convert the non-lot qty. Track it using the same uom. 
MERGE	
INTO	dbo.tblICItemStockUOM
WITH	(HOLDLOCK) 
AS		ItemStockUOM
...
-- Convert the lot qty or weight to stock uom. Update the reserve as stock uom. 
MERGE	
INTO	dbo.tblICItemStockUOM 
WITH	(HOLDLOCK) 
AS		ItemStockUOM	
