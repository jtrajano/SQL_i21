...
BEGIN
       ...
       WHERE Convert(DATETIME, Convert(CHAR, WB.dtmTransactionDate, 101)) = @dtmDate
              ...
              AND WB.intStageStockId IN (
                     SELECT Max(WB1.intStageStockId)
                     FROM tblIPStockArchive WB1
                     WHERE Convert(DATETIME, Convert(CHAR, WB1.dtmTransactionDate, 101)) = @dtmDate
                           AND WB1.strItemNo = WB.strItemNo
                           AND WB1.strSubLocation = WB.strSubLocation
                           AND WB1.strStockType = 'WB'
                     )
       GROUP BY WB.strItemNo

          SELECT WB.intStageStockId
INTO #StageStock
FROM tblIPStockArchive WB
WHERE Convert(DATETIME, Convert(CHAR, WB.dtmTransactionDate, 101)) = @dtmDate
       AND WB.strStockType = 'WB'
...
GROUP BY WB.strItemNo
       ...
       SELECT @strDetail = @strDetail + '<tr>
                 <td>&nbsp;' + ISNULL(I.strItemNo, '') + '</td>' + 
                 '<td>&nbsp;' + ISNULL(I.strDescription, '') + '</td>' + 
                 '<td>&nbsp;' + ISNULL(CONVERT(VARCHAR, I.dblQty), '') + '</td>' + 
                 '<td>&nbsp;' + ISNULL(I.strUnitMeasure, '') + '</td>' + 
                 '<td>&nbsp;' + ISNULL(CONVERT(VARCHAR, i21I.dblQty), '') + '</td>' + 
                 '<td>&nbsp;' + ISNULL(CONVERT(VARCHAR, IsNULL(IR.dblNet, 0)), '') + '</td>' + 
                 '<td>&nbsp;' + ISNULL(CONVERT(VARCHAR, (I.dblQty - (IsNULL(i21I.dblQty, 0) + IsNULL(IR.dblNet, 0)))), '') + '</td>
       </tr>'
       FROM #i21Inventory I
       LEFT JOIN #i21FinalIntegration i21I ON i21I.strItemNo = I.strItemNo
       LEFT JOIN #i21InventoryReceipt IR ON IR.strItemNo = I.strItemNo
