CREATE PROCEDURE [dbo].[uspGLCorrigaCOAiPayables]
...
WHERE (name = N'COA to IPayables')   
IF (@_JobID IS NOT NULL)  
/****** Object:  Job [COA to IPayables]    Script Date: 4/18/2023 6:24:07 PM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'COA to IPayables', 
		@enabled=1, 
...
N'sqlcmd -S '+ @@SERVERNAME +' -d ' + DB_NAME() + '  -Q "SET NOCOUNT ON; select * from vyuGLCOAiPayables;" -s "|" -o "' + @folderPath +'\C0000549_CODE_MERGED_'+ @strDate +'.txt" -W -w 1024 -h-1'
...
EXEC msdb.dbo.sp_start_job N'COA to IPayables' ;  
