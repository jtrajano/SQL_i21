CREATE PROCEDURE [dbo].[uspWHCancelOrder] 
...
BEGIN TRY
	...
	SELECT @strBOLNo = strBOLNo, @intShipToAddressId = intShipToAddressId, @intShipFromAddressId = intShipFromAddressId, @intOrderDirectionId = intOrderDirectionId
	FROM tblWHOrderHeader
	WHERE intOrderHeaderId = @intOrderHeaderId
	...
	IF @TaskCount > 0
	BEGIN
		RAISERROR ('There are tasks associated with this order and cannot be cancelled. Please delete the tasks before cancelling the order.', 16, 1)
	END

	--Update the ship date and status in the order header          
	UPDATE tblWHOrderHeader
	SET dtmShipDate = '1/1/1900', intOrderStatusId = 4
	WHERE intOrderHeaderId = @intOrderHeaderId
	...
	BEGIN
		RAISERROR ('The lot(s) are already created for this order. You cannot cancel it.', 16, 1)
	...
	BEGIN
		...
		SET intOrderHeaderId = NULL, strBOLNo = NULL
		WHERE intOrderHeaderId = @intOrderHeaderId
	END
	ELSE IF EXISTS (
			SELECT *
	...
	BEGIN
		--UPDATE dbo.tblMFWorkOrder SET intOrderHeaderId=NULL,strBOLNo=NULL WHERE intOrderHeaderId=@intOrderHeaderId  
