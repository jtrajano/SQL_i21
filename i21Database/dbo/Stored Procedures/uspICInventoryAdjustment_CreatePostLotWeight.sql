CREATE PROCEDURE [dbo].[uspICInventoryAdjustment_CreatePostLotWeight]
	...
	,@dblNewLotWeight AS NUMERIC(38,20)
	,@dblNewWeightPerQty AS NUMERIC(38, 20) 
...
DECLARE @ADJUSTMENT_TYPE_QuantityChange AS INT = 1
		...
		,@ADJUSTMENT_TYPE_LotWeight AS INT = 11
...
BEGIN 
	...
	SELECT	intLocationId				= @intLocationId
			...
			,intAdjustmentType			= @ADJUSTMENT_TYPE_LotWeight
...
BEGIN 
	...
	SELECT 
			...
			,intNewWeightUOMId			= Lot.intWeightUOMId 
			,dblWeight					= Lot.dblWeight
			,dblNewWeight				= dbo.fnMultiply(Lot.dblQty, @dblNewWeightPerQty)
			,dblWeightPerQty			= Lot.dblWeightPerQty
			,dblNewWeightPerQty			= @dblNewWeightPerQty 
			,dblCost					= dbo.fnCalculateCostBetweenUOM(StockUnit.intItemUOMId, Lot.intItemUOMId, ISNULL(Lot.dblLastCost, ISNULL(ItemPricing.dblLastCost, 0)))
			,dblNewCost					= NULL 
