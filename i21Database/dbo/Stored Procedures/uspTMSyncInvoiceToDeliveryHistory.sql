	...
	BEGIN
		...
		BEGIN
			...
			SET intLastDeliveryDegreeDay = @dblAccumulatedDegreeDay
				...
				,dblBurnRate = (CASE WHEN ysnAdjustBurnRate = 1 THEN dbo.[fnTMComputeNewBurnRate](@intSiteId,@intInvoiceDetailId,@intClockReadingId,@intLastClockReadingId) ELSE dblBurnRate END)
			WHERE intSiteID = @intSiteId
		...
		----None Service Starts here	
			...
			BEGIN
				...
				---------GET New Burn rate 
				SET @dblNewBurnRate = dbo.[fnTMComputeNewBurnRate](@intSiteId,@intInvoiceDetailId,@intClockReadingId,@intLastClockReadingId) 
				...
				BEGIN
					...
					SELECT
						...
						,intDegreeDayOnDeliveryDate = @dblAccumulatedDegreeDay
						,intDegreeDayOnLastDeliveryDate = @dblLastAccumulatedDegreeDay
						,dblBurnRateAfterDelivery = dbo.[fnTMComputeNewBurnRate](A.intSiteID,B.intInvoiceDetailId,@intClockReadingId,@intLastClockReadingId)
						,dblCalculatedBurnRate = dbo.[fnTMGetCalculatedBurnRate](A.intSiteID,B.intInvoiceDetailId,@intClockReadingId)
					...
					----Update Next Julian Calendar Date
					UPDATE tblTMSite
