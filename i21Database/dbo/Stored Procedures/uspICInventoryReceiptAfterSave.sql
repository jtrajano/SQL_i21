...
BEGIN
	...
	FROM	tblICInventoryReceiptItem ReceiptItem LEFT JOIN tblICInventoryReceipt Receipt 
				ON Receipt.intInventoryReceiptId = ReceiptItem.intInventoryReceiptId
			LEFT JOIN (
				tblCTContractDetail CD INNER JOIN tblCTContractHeader CH
	...
	SELECT	intInventoryReceiptId = intTransactionId
	...
	SELECT	currentSnapshot.intInventoryReceiptItemId,
			currentSnapshot.intLineNo,
			currentSnapshot.intItemUOMId,
			dblQty =	CASE	WHEN (ISNULL(currentSnapshot.ysnLoad, 0) = 0) THEN 
									dbo.fnCalculateQtyBetweenUOM (
										currentSnapshot.intItemUOMId, 
										ContractDetail.intItemUOMId, 
										(	CASE	WHEN @ForDelete = 1 THEN currentSnapshot.dblOpenReceive 
						...
						END 
	FROM	#tmpReceiptItems currentSnapshot INNER JOIN #tmpLogReceiptItems previousSnapshot
	...
	SELECT	currentSnapshot.intInventoryReceiptItemId
			...
			,previousSnapshot.intItemUOMId
			,dblQty =	CASE	WHEN (ISNULL(previousSnapshot.ysnLoad, 0) = 0) THEN 
									dbo.fnCalculateQtyBetweenUOM(
										previousSnapshot.intItemUOMId, 
										ContractDetail.intItemUOMId, 
	...
	--Deleted Item
	SELECT	previousSnapshot.intInventoryReceiptItemId
			,previousSnapshot.intLineNo
			,previousSnapshot.intItemUOMId
			,dblQty =		CASE	WHEN (ISNULL(previousSnapshot.ysnLoad, 0) = 0) THEN 
										dbo.fnCalculateQtyBetweenUOM(
									...
									ELSE 
										previousSnapshot.intLoadReceive 
							END
	FROM	#tmpLogReceiptItems previousSnapshot INNER JOIN tblCTContractDetail ContractDetail
	...
	SELECT	currentSnapshot.intInventoryReceiptItemId
			...
			,dblQty =		CASE	WHEN (ISNULL(currentSnapshot.ysnLoad, 0) = 0) THEN 
									...
									ELSE 
										currentSnapshot.intLoadReceive 
							END
	FROM	#tmpReceiptItems currentSnapshot INNER JOIN tblCTContractDetail ContractDetail
				ON ContractDetail.intContractDetailId = currentSnapshot.intLineNo
	WHERE	currentSnapshot.intLineNo IS NOT NULL
