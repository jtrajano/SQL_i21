CREATE PROCEDURE [dbo].[uspFADisposeMultipleBooks]
	@Id					AS Id READONLY,	
	@ysnPost			AS BIT				= 0,
	@ysnRecap			AS BIT				= 0,
...
SELECT TOP 1 -- // Remove TOP 1 if multiple disposition for multiple GAAP is going to be implemented
...
OUTER APPLY(
	...
	WHERE FAD.intAssetId = B.intAssetId and FAD.intBookDepreciationId = BD.intBookDepreciationId AND strTransaction = 'Depreciation'
) Dep
OUTER APPLY(
	SELECT TOP 1 DATEADD(DAY, 1, dtmDepreciationToDate) dtmDepreciationToDate 
	FROM tblFAFixedAssetDepreciation WHERE intAssetId = A.intId AND intBookDepreciationId = BD.intBookDepreciationId
...
OUTER APPLY(
	SELECT ISNULL(ysnOpen, 0) & ISNULL(ysnFAOpen,0) ysnOpenPeriod 
	FROM tblGLFiscalYearPeriod 
	WHERE D.dtmDepreciationToDate BETWEEN dtmStartDate AND dtmEndDate
) F
OUTER APPLY(
...
WHERE ISNULL(ysnAcquired, 0) = 1 AND ISNULL(ysnDisposed, 0) = 0 AND ISNULL(ysnDepreciated, 0) = 1 AND BD.intBookDepreciationId = 1
...
BEGIN
	...
	BEGIN
		DECLARE @intRowId INT, @intBookDepreciation INT = NULL, @intAssetId INT
		
		SELECT TOP 1
				@intRowId = A.intRowId,
				@intBookDepreciation = A.intBookDepreciationId,
		...
		BEGIN
			...
			WHERE T.intRowId = @intRowId AND BD.intBookDepreciationId = @intBookDepreciation
			...
			BEGIN
				...
				FROM tblFAFixedAssetDepreciation FAD
				JOIN @tblAsset T ON T.intAssetId = FAD.intAssetId AND FAD.intBookDepreciationId = T.intBookDepreciationId
				OUTER APPLY (
				...
				WHERE FAD.strTransaction = 'Depreciation'
					...
					AND FAD.intBookDepreciationId = @intBookDepreciation
			END
			...
			BEGIN
				...
				JOIN @tblAsset T ON T.intAssetId = B.intAssetId AND B.intBookDepreciationId = T.intDepreciationMethodId
				...
				AND T.intRowId = @intRowId
				AND B.intBookDepreciationId = @intBookDepreciation
			END
			...
			BEGIN
				...
				BEGIN
					...
					EXEC [dbo].[uspFADepreciateSingleBook]
						 @dtmDispositionDate	-- dtmDepreciationDate
						,@intBookDepreciation	-- intBookDepreciationId
						,1						-- ysnPost
						,0						-- ysnRecap
					...
					BEGIN
						SELECT @dblAccumulatedDepreciation = ISNULL(SUM(dblDepreciation), 0)
						FROM tblFAFixedAssetDepreciation B
						JOIN @tblAsset T ON T.intAssetId = B.intAssetId AND B.intBookDepreciationId = T.intBookDepreciationId
						WHERE B.intAssetId = T.intAssetId and ISNULL(intBookId,1) = 1
							AND B.intBookDepreciationId = T.intBookDepreciationId
							AND strTransaction = 'Depreciation'
							AND YEAR(B.dtmDepreciationToDate) = YEAR(@dtmDispositionDate)
							AND T.intRowId = @intRowId
							AND B.intBookDepreciationId = @intBookDepreciation
			...
			BEGIN
				...
				EXEC [dbo].[uspFAManualDepreciateSingleBook]
					@intAssetId,
					@intBookDepreciation,
					@dtmDispositionDate,
					@dblDepreciationTake,
		...
		OUTER APPLY (
			...
			WHERE intAssetId = A.intAssetId AND intBookId = 1 AND strTransaction = 'Depreciation'
				AND intBookDepreciationId = BD.intBookDepreciationId
			ORDER BY dtmDepreciationToDate DESC
		) LastDepreciation
        WHERE BD.intBookId = 1 AND intRowId = @intRowId AND BD.intBookDepreciationId = @intBookDepreciation
