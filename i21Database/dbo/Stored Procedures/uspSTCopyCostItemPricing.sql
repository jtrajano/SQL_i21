CREATE PROCEDURE [dbo].[uspSTCopyCostItemPricing]
	@intItemId							INT		
	, @intPrimaryId						INT	
	, @strType							NVARCHAR(10)	
...
BEGIN
	...
	DECLARE @Savepoint NVARCHAR(32) = SUBSTRING(('uspSTCopyCostItemPricing' + CONVERT(VARCHAR, @InitTranCount)), 1, 32)
	...
	BEGIN TRY
		...
		BEGIN
			...
			DECLARE @dtmEffectiveDate DATETIME = (SELECT TOP 1 dtmEffectiveCostDate FROM tblICEffectiveItemCost WHERE intEffectiveItemCostId = @intPrimaryId)
			...
			DECLARE @dblStandardCost_FROM NUMERIC(38,20)

			-- INSERT [FROM]
			SET @dblStandardCost_FROM = (SELECT TOP 1 dblCost FROM tblICEffectiveItemCost WHERE intEffectiveItemCostId = @intPrimaryId)
			...
			BEGIN
				IF(@ysnDebug = 1)
					BEGIN
						SELECT 'Item Pricing Copy From' + @dblStandardCost_FROM
					END
			END
			-- ===============================================
			...
			IF EXISTS(SELECT TOP 1 1 FROM tblICEffectiveItemCost WHERE dblCost = @dblStandardCost_FROM)
				BEGIN
		...
		BEGIN
			...
			-- INSERT [FROM]
			DECLARE @dblCost_FROM AS NUMERIC(38, 20)  = (SELECT TOP 1 dblCost FROM tblICItemSpecialPricing WHERE intItemSpecialPricingId = @intPrimaryId)
