...
--SET @FormCodeParam = (SELECT TOP 1 strFormCode FROM tblTFTransaction WHERE uniqTransactionGuid = @Guid)
SELECT TOP 1 
...
WHERE uniqTransactionGuid = @Guid AND strFormCode = @FormCodeParam

SELECT TOP 1 @TaxID = strConfiguration
FROM tblTFTaxReportTemplate
WHERE strFormCode = @FormCodeParam AND strTemplateItemId = 'GT-103-TID'
...
BEGIN
	...
	SET @TemplateItemId = (SELECT strTemplateItemId FROM tblTFTaxReportTemplate WHERE intReportTemplateId = @ReportTemplateId AND strFormCode = @FormCodeParam)
						...
						IF(@TemplateSection = 'Disbursements - Schedule 2')
							BEGIN
								IF(@Type = 'Total Gallons of Fuel Sold')
									BEGIN
										SET @TotalGallonsSold = (SELECT ISNULL(SUM(dblQtyShipped), 0) FROM tblTFTransaction WHERE strScheduleCode = @paramTempScheduleCode AND uniqTransactionGuid = @Guid AND strFormCode = @FormCodeParam)
										SET @TotalExemptGallonsSold = (SELECT ISNULL(SUM(dblTaxExempt), 0) FROM tblTFTransaction WHERE strScheduleCode = @paramTempScheduleCode AND uniqTransactionGuid = @Guid AND strFormCode = @FormCodeParam)
										SET @GasolineUseTaxCollected = (SELECT ISNULL(SUM(dblTax), 0) FROM tblTFTransaction WHERE strScheduleCode = @paramTempScheduleCode AND uniqTransactionGuid = @Guid AND strFormCode = @FormCodeParam)

									END
								ELSE
									BEGIN
										SET @TotalGallonsSold = (SELECT ISNULL(SUM(dblQtyShipped), 0) FROM tblTFTransaction WHERE strScheduleCode = @paramTempScheduleCode AND strType = @Type AND uniqTransactionGuid = @Guid AND strFormCode = @FormCodeParam)
										SET @TotalExemptGallonsSold = (SELECT ISNULL(SUM(dblTaxExempt), 0) FROM tblTFTransaction WHERE strScheduleCode = @paramTempScheduleCode AND strType = @Type AND uniqTransactionGuid = @Guid AND strFormCode = @FormCodeParam)
										SET @GasolineUseTaxCollected = (SELECT ISNULL(SUM(dblTax), 0) FROM tblTFTransaction WHERE strScheduleCode = @paramTempScheduleCode AND strType = @Type AND uniqTransactionGuid = @Guid AND strFormCode = @FormCodeParam)
									END
					...
					IF(@TemplateSection = 'Receipts - Schedule 1')
						BEGIN
							...
							IF(@Type = 'Total Gallons of Fuel Purchased')
								BEGIN
									SET @ReceiptTotalGallsPurchased = (SELECT ISNULL(SUM(dblGross), 0) FROM tblTFTransaction WHERE strScheduleCode = @paramTempScheduleCode AND uniqTransactionGuid = @Guid AND strFormCode = @FormCodeParam)
									SET @GasolineUseTaxPaid = (SELECT ISNULL(SUM(dblTax), 0) FROM tblTFTransaction WHERE strScheduleCode = @paramTempScheduleCode AND uniqTransactionGuid = @Guid AND strFormCode = @FormCodeParam)
								END
							ELSE
								BEGIN
									SET @ReceiptTotalGallsPurchased = (SELECT ISNULL(SUM(dblGross), 0) FROM tblTFTransaction WHERE strScheduleCode = @paramTempScheduleCode AND strType = @Type AND uniqTransactionGuid = @Guid AND strFormCode = @FormCodeParam)
									SET @GasolineUseTaxPaid = (SELECT ISNULL(SUM(dblTax), 0) FROM tblTFTransaction WHERE strScheduleCode = @paramTempScheduleCode AND strType = @Type AND uniqTransactionGuid = @Guid AND strFormCode = @FormCodeParam)
								END
