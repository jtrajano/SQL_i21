...
BEGIN TRY
	...
	SELECT	 DISTINCT 
			...
			strDifferential = dbo.fnCTChangeNumericScale(dbo.fnCTConvertToSubCurrency(pc.intFinalCurrencyId,CAST(dbo.fnCTConvertQuantityToTargetCommodityUOM(PF.intFinalPriceUOMId,PU.intCommodityUnitMeasureId, PF.dblOriginalBasis) AS NUMERIC(18, 6))),2) + ' ' + CY.strDescription + ' ' + @per + ' ' + isnull(rtrt3.strTranslation,CM.strUnitMeasure) ,
