...
BEGIN TRY
	...
	SELECT	 DISTINCT 
			...
			strLogoType			= CASE WHEN dbo.[fnCTGetCompanyLocationCount](@intContractHeaderId) > 1 THEN 'Attachment' 
								  ...
								  ELSE ISNULL(@strLogoType,'Attachment') END,
			strCurrencyExchangeRate = CASE WHEN PF.dblFX != 1  AND PF.dblFX IS NOT NULL THEN isnull((FY.strCurrency + '/' + TY.strCurrency), @strFinalCurrency) ELSE NULL END,
			dblRate =  CASE WHEN PF.dblFX != 1  AND PF.dblFX IS NOT NULL THEN (case when isnull(@ysnEnableFXFieldInContractPricing,0) = 1 then PF.dblFX else CD.dblRate end) ELSE NULL END,
