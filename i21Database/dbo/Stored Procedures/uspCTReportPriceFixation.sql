...
BEGIN TRY
	...
	SELECT	 DISTINCT 
			...
			strTotal = dbo.fnCTChangeNumericScale(dbo.fnConvertToBaseCurrency(pc.intFinalCurrencyId,dbo.fnCTConvertQuantityToTargetCommodityUOM(pc.intFinalPriceUOMId,PF.intFinalPriceUOMId,PF.dblPriceWORollArb)),2) + ' ' + CY.strDescription + ' ' + @per + ' ' + isnull(rtrt3.strTranslation,CM.strUnitMeasure),			
			...
			strFinalPrice =	dbo.fnCTChangeNumericScale(dbo.fnConvertToBaseCurrency(pc.intFinalCurrencyId,dbo.fnCTConvertQuantityToTargetCommodityUOM(pc.intFinalPriceUOMId,PF.intFinalPriceUOMId,PF.dblFinalPrice)),2) + ' ' + CY.strDescription + ' ' + @per + ' ' + isnull(rtrt3.strTranslation,CM.strUnitMeasure) ,
