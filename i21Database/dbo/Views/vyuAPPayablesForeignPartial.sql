CREATE VIEW [dbo].[vyuAPPayablesForeignPartial]
AS
...
FROM (  
 SELECT       
  ...
  ELSE (B.dblTotal) * B.dblRate      
   END , 2) AS dblTotal      
  , ROUND(CASE WHEN A.intTransactionType NOT IN (1,14) THEN A.dblAmountDue * -1 ELSE A.dblAmountDue      
   END * B.dblRate, 2) AS dblAmountDue       
  , dblWithheld = 0      
  , dblDiscount = 0       
  , dblInterest = 0       
  , dblPrepaidAmount = 0       
 ...
 SELECT       
  ...
  , ROUND(CASE WHEN A.intTransactionType NOT IN (1,14) THEN A.dblAmountDue * -1 ELSE A.dblAmountDue END, 2) AS dblAmountDue       
  , dblWithheld = 0      
  , dblDiscount = 0       
  , dblInterest = 0       
  , dblPrepaidAmount = 0       
 ...
 -- UNION ALL  
 -- SELECT DT.dtmDatePaid, B.intBillId, B.strBillId, DT.dblDifference, 0, 0, 0, 0, 0, 0, V.strVendorId, ISNULL(V.strVendorId, '') + ' - ' + ISNULL(E.strName, ''), B.dtmDueDate, B.ysnPosted, B.ysnPaid, A.intAccountId, A.strAccountId, EC.strClass, DT.intCurrencyId  
 -- FROM tblAPBill B  
 ...
 -- OUTER APPLY (  
 --  SELECT MAX(IT.dtmDatePaid) dtmDatePaid, IT.intCurrencyId, ROUND(SUM(IT.dblRoundedTotal), 2) dblRoundedTotal, ROUND(SUM(IT.dblUnroundedTotal), 2) dblUnroundedTotal, CAST(ROUND(SUM(IT.dblUnroundedTotal), 2) - ROUND(SUM(IT.dblRoundedTotal), 2) AS DECIMAL(18, 2)) dblDifference  
 --  FROM (  
 --   SELECT P.dtmDatePaid, P.intCurrencyId,  
 --   ROUND(dbo.fnAPGetPaymentAmountFactor(BD.dblTotal, PD.dblPayment + (CASE WHEN (PD.dblPayment + PD.dblDiscount = PD.dblAmountDue) THEN PD.dblDiscount ELSE 0 END) - PD.dblInterest, B.dblTotal) * ISNULL((NULLIF(BD.dblRate, 0)), 1), 2) dblRoundedTotal,  
 --   dbo.fnAPGetPaymentAmountFactor(BD.dblTotal, PD.dblPayment + (CASE WHEN (PD.dblPayment + PD.dblDiscount = PD.dblAmountDue) THEN PD.dblDiscount ELSE 0 END) - PD.dblInterest, B.dblTotal) * ISNULL(NULLIF(BD.dblRate, 0), 1) dblUnroundedTotal  
 --   FROM tblAPBillDetail BD  
 --   INNER JOIN tblAPPaymentDetail PD ON PD.intBillId = BD.intBillId  
 --   INNER JOIN tblAPPayment P ON P.intPaymentId = PD.intPaymentId  
 --   WHERE BD.intBillId = B.intBillId AND P.ysnPosted = 1 AND P.ysnPrepay = 0 AND BD.dblTotal <> 0  
 --   UNION ALL  
 --   SELECT P.dtmDatePaid, P.intCurrencyId,  
 --    ROUND(dbo.fnAPGetPaymentAmountFactor(DT.dblAdjustedTax, PD.dblPayment + (CASE WHEN (PD.dblPayment + PD.dblDiscount = PD.dblAmountDue) THEN PD.dblDiscount ELSE 0 END) - PD.dblInterest, B.dblTotal) * ISNULL(NULLIF(BD.dblRate, 0), 1), 2) dblRoundedTotal,  
 --    dbo.fnAPGetPaymentAmountFactor(DT.dblAdjustedTax, PD.dblPayment + (CASE WHEN (PD.dblPayment + PD.dblDiscount = PD.dblAmountDue) THEN PD.dblDiscount ELSE 0 END) - PD.dblInterest, B.dblTotal) * ISNULL(NULLIF(BD.dblRate, 0), 1) dblUnroundedTotal  
 --   FROM tblAPBillDetail BD  
 --   INNER JOIN tblAPBillDetailTax DT ON DT.intBillDetailId = BD.intBillDetailId  
 --   INNER JOIN tblAPPaymentDetail PD ON PD.intBillId = BD.intBillId  
 --   INNER JOIN tblAPPayment P ON P.intPaymentId = PD.intPaymentId  
 --   WHERE BD.intBillId = B.intBillId AND P.ysnPosted = 1 AND P.ysnPrepay = 0 AND DT.dblAdjustedTax <> 0  
 --  ) IT  
 --  GROUP BY IT.intCurrencyId  
 -- ) DT  
 -- OUTER APPLY (  
 --  SELECT COUNT(DISTINCT intPaymentId) AS intCount FROM tblAPPaymentDetail WHERE intBillId = B.intBillId  
 -- ) PC  
 -- WHERE (DT.dblDifference <> 0 AND DT.dblDifference < 1 AND DT.dblDifference > -1) AND PC.intCount > 1  
 UNION ALL      
 ...
 SELECT --OVERPAYMENT      
  ...
  , A.strBillId       
  , 0 AS dblAmountPaid       
  , ROUND(CASE WHEN A.intTransactionType NOT IN (1,14) AND A.dblTotal > 0 THEN A.dblTotal * -1 ELSE A.dblTotal END, 2) AS dblTotal      
  , ROUND(CASE WHEN A.intTransactionType NOT IN (1,14) AND A.dblAmountDue > 0 THEN A.dblAmountDue * -1 ELSE A.dblAmountDue END, 2) AS dblAmountDue       
  , dblWithheld = 0      
  , dblDiscount = 0       
  , dblInterest = 0       
  , dblPrepaidAmount = 0       
