			...
			SELECT intFutOptTransactionId
				...
				, ISNULL((SELECT SUM(dblMatchQty) from tblRKMatchFuturesPSDetail psd
						...
						WHERE psd.intLFutOptTransactionId=ot.intFutOptTransactionId
							AND convert(datetime,CONVERT(VARCHAR(10),h.dtmMatchDate,110),110) <= (SELECT TOP 1 convert(datetime,CONVERT(VARCHAR(10),dtmToDate,110),110) from tblRKDateFilterFor360 order by 1 desc)),0) as MatchLong
				, ISNULL((SELECT sum(dblMatchQty) from tblRKMatchFuturesPSDetail psd
						JOIN tblRKMatchFuturesPSHeader h on psd.intMatchFuturesPSHeaderId=h.intMatchFuturesPSHeaderId
						WHERE psd.intSFutOptTransactionId=ot.intFutOptTransactionId
							AND convert(datetime,CONVERT(VARCHAR(10),h.dtmMatchDate,110),110) <= (SELECT TOP 1 convert(datetime,CONVERT(VARCHAR(10),dtmToDate,110),110) from tblRKDateFilterFor360 order by 1 desc)),0) as MatchShort
