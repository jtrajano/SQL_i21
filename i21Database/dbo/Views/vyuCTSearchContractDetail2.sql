...
(
	select
		...
		,hhh.intShipmentType
		,intId = convert(int,ROW_NUMBER() over (partition by ggg.intPContractDetailId ORDER BY hhh.dtmScheduledDate ,ggg.intLoadDetailId DESC))
		,strShipmentStatus = CASE hhh.intShipmentStatus  
						 WHEN 1 THEN 'Scheduled'  
						 WHEN 2 THEN 'Dispatched'  
						 WHEN 3 THEN   
									CASE WHEN (hhh.ysnCustomsReleased = 1) THEN 'Customs Released'  
									WHEN (hhh.ysnDocumentsApproved = 1) THEN 'Documents Approved'  
									WHEN (hhh.ysnArrivedInPort = 1) THEN 'Arrived in Port'  
						 ...
						 WHEN 6 THEN   
									...
									ELSE 'Delivered' END  
						 WHEN 7 THEN   
									CASE WHEN (ISNULL(hhh.strBookingReference, '') <> '') THEN 'Booked'  
						 ...
						 ELSE 'Open' END COLLATE Latin1_General_CI_AS
	from
		tblLGLoadDetail ggg
		,tblLGLoad hhh
	where
...
where intId = 1 and ((intShipmentType = 2 AND strShipmentStatus <> 'Scheduled') OR intShipmentType = 1)
),
fixation as
(
select
...
(
	select
		ccc.intContractDetailId, dblRepresentingQty = (case
													   when isnull(eee.dblUnitQty,0) = 0 or isnull(fff.dblUnitQty,0) = 0
													   then null
													   when isnull(eee.dblUnitQty,0) = isnull(fff.dblUnitQty,0)
													   then ddd.dblRepresentingQty
	...
	from
		...
		,tblICItemUOM fff
	where
		ddd.intProductValueId = ccc.intContractDetailId and intProductTypeId = 8  and intSampleStatusId = 3
		and eee.intUnitMeasureId = ddd.intRepresentingUOMId and eee.intItemId = ccc.intItemId
		and fff.intUnitMeasureId = ccc.intUnitMeasureId and fff.intItemId = ccc.intItemId
) as qasample
group by
	intContractDetailId
),
...
select
intContractHeaderId
,strStatuses = CASE WHEN strStatus LIKE '%Incomplete%'
			   THEN 'Incomplete'
			   WHEN strStatus LIKE '%Open%'
			   THEN 'Open'
			   WHEN strStatus LIKE '%Complete%'
			   THEN 'Complete'
			   WHEN strStatus is null
			   THEN 'Incomplete'
			   ELSE strStatus END  COLLATE Latin1_General_CI_AS
from
(
 SELECT lll.intContractHeaderId,strStatus = STUFF(                 
           (  
          SELECT DISTINCT              
          ', ' + nnn.strContractStatus             
          FROM tblCTContractDetail mmm
          JOIN tblCTContractStatus nnn ON nnn.intContractStatusId = mmm.intContractStatusId  
          WHERE mmm.intContractHeaderId=lll.intContractHeaderId                     
          FOR XML PATH('')  
           )             
         ,1,2, ''               
        )                
 FROM tblCTContractHeader lll
 )as ss
),
reserved as
(
...
,strFinancialStatus = CASE WHEN b.intContractTypeId = 1
					  THEN  
							CASE   
							WHEN a.ysnFinalPNL = 1 THEN 'Final P&L Created'  
							WHEN a.ysnProvisionalPNL = 1 THEN 'Provisional P&L Created'  
							ELSE CASE
								WHEN s.intContractDetailId IS NOT NULL
								THEN 'Purchase Invoice Received'
								ELSE null END  
							END  
					  ELSE a.strFinancialStatus END
,dblPricedQty = case when u.strPricingType = 'Basis'
				then isnull(t.dblPricedQuantity,0)
				else null end
,dblUnPricedQty = case when u.strPricingType = 'Basis'
				  then a.dblQuantity - isnull(t.dblPricedQuantity,0)
				  else null end
,dblActualLots = (case
				 when isnull(v.dblUnitQty,0) = 0 or isnull(w.dblUnitQty,0) = 0
				 then null
				 when isnull(v.dblUnitQty,0) = isnull(w.dblUnitQty,0)
				 then a.dblQuantity
...
,dblBalQtyToHedge = (case
				 when isnull(ad.dblUnitQty,0) = 0 or isnull(ae.dblUnitQty,0) = 0
				 then null
				 when isnull(ad.dblUnitQty,0) = isnull(ae.dblUnitQty,0)
				 then i.dblContractSize * (a.dblNoOfLots - ISNULL(ab.dblHedgedLots,0))
...
,dblTotalAppliedQty = (case
					   when isnull(ah.dblUnitQty,0) = 0 or isnull(ai.dblUnitQty,0) = 0
					   then null
					   when isnull(ah.dblUnitQty,0) = isnull(ai.dblUnitQty,0)
					   then (a.dblQuantity - a.dblBalance)
					   else ((a.dblQuantity - a.dblBalance) * isnull(ah.dblUnitQty,0)) / isnull(ai.dblUnitQty,0)
					   end)
,dblTotalBallance = (case
					 when isnull(ah.dblUnitQty,0) = 0 or isnull(ai.dblUnitQty,0) = 0
					 then null
					 when isnull(ah.dblUnitQty,0) = isnull(ai.dblUnitQty,0)
					 then a.dblBalance
					 else (a.dblBalance * isnull(ah.dblUnitQty,0)) / isnull(ai.dblUnitQty,0)
					 end)
...
,dblHedgeQty = (case
				 ...
				 then null
				 when isnull(ad.dblUnitQty,0) = isnull(ae.dblUnitQty,0)
				 then i.dblContractSize * ISNULL(ab.dblHedgedLots,0)
				 else (i.dblContractSize * ISNULL(ab.dblHedgedLots,0)) * (isnull(ad.dblUnitQty,0) / isnull(ae.dblUnitQty,0)) end)
,ab.dblHedgedLots
,strINCOLocation = CASE
				   WHEN av.strINCOLocationType = 'City'
				   THEN aw.strCity
...
,dblQtyInCommodityDefaultUOM = (case
				 when isnull(v.dblUnitQty,0) = 0 or isnull(bt.dblUnitQty,0) = 0
				 then null
				 when isnull(v.dblUnitQty,0) = isnull(bt.dblUnitQty,0)
				 then a.dblQuantity
...
,dblQtyInCommodityStockUOM = (case
				 when isnull(v.dblUnitQty,0) = 0 or isnull(bu.dblUnitQty,0) = 0
				 then null
				 when isnull(v.dblUnitQty,0) = isnull(bu.dblUnitQty,0)
				 then a.dblQuantity
...
left join tblICItemUOM v on v.intItemId = a.intItemId and v.intUnitMeasureId = g.intUnitMeasureId
left join tblICItemUOM w on w.intItemId = a.intItemId and w.intUnitMeasureId = i.intUnitMeasureId
...
left join tblICCommodityUnitMeasure ac on ac.intCommodityId = b.intCommodityId and ac.ysnStockUnit = 1
left join tblICItemUOM ad on ad.intItemId = a.intItemId and ad.intUnitMeasureId = i.intUnitMeasureId
left join tblICItemUOM ae on ae.intItemId = a.intItemId and ae.intUnitMeasureId = ac.intUnitMeasureId
...
left join tblICItemUOM ah on ah.intItemId = a.intItemId and ah.intUnitMeasureId = a.intUnitMeasureId
left join tblICItemUOM ai on ai.intItemId = a.intItemId and ai.intUnitMeasureId = ag.intUnitMeasureId
...
left join tblICCommodityAttribute bq on bq.intCommodityAttributeId = c.intProductTypeId and bq.strType = 'ProductType'
left join tblSMPurchasingGroup br on br.intPurchasingGroupId = a.intPurchasingGroupId

left join tblICCommodityUnitMeasure bs on bs.intCommodityId = b.intCommodityId and bs.ysnDefault = 1
left join tblICItemUOM bt on bt.intItemId = a.intItemId and bt.intUnitMeasureId = bs.intUnitMeasureId

left join tblICItemUOM bu on bu.intItemId = a.intItemId and bu.intUnitMeasureId = ac.intUnitMeasureId
