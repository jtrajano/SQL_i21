...
(
	...
	UNION ALL
	SELECT 'Voucher', intBillId, strBillId, dblTotal, intEntityId, dtmDate, strComment as strReference FROM tblAPBill WHERE intTransactionType = 1 AND intTransactionType != 6 AND ysnForApproval != 1 AND ISNULL(ysnPosted, 0) = 0 OR (dtmApprovalDate IS NOT NULL AND ysnApproved = 1)
	UNION ALL
	...
	UNION ALL
	SELECT 'Debit Memo', intBillId, strBillId, dblTotal, intEntityId, dtmDate, strComment as strReference FROM tblAPBill WHERE intTransactionType = 3 AND ISNULL(ysnPosted, 0) = 0 AND intTransactionType != 6 
	UNION ALL
