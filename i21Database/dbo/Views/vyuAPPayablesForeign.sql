...
SELECT 
	...
	, A.strBillId 
	, 0 AS dblAmountPaid 
	, CAST(CASE WHEN A.intTransactionType NOT IN (1,14) THEN (B.dblTotal) * -1 
				ELSE (B.dblTotal)
...
SELECT 
	...
	, A.strBillId 
	, 0 AS dblAmountPaid 
	, CAST(CASE WHEN A.intTransactionType NOT IN (1,14) THEN ISNULL(B.dblTax, 0) * -1 
...
SELECT  A.dtmDatePaid AS dtmDate,    
	...
	-- 			ELSE B.dblPayment END) AS DECIMAL(18,2)) AS dblAmountPaid,    
...
SELECT A.dtmDatePaid AS dtmDate,   
	 ...
	 CAST(CASE WHEN C.intTransactionType NOT IN (1,2, 14) AND B.dblPayment != 0
			...
			WHEN C.intTransactionType NOT IN (1,2, 14) AND B.dblPayment < 0 AND (E.intBankTransactionTypeId = 116 OR E.intBankTransactionTypeId = 19)
				THEN B.dblPayment * -1 --MAKE THE REVERSAL DEBIT MEMO TRANSACTION POSITIVE
			ELSE ABS(B.dblPayment) END AS DECIMAL(18,2)) AS dblAmountPaid, --ALWAYS CONVERT TO POSSITIVE TO OFFSET THE PAYMENT
	 dblTotal = 0 
	, dblAmountDue = 0 
	, dblWithheld = 0
