...
SELECT r.intInventoryReceiptId,
	dblTotalCharge = SUM(CASE WHEN COALESCE(rc.intCurrencyId, r.intCurrencyId) = r.intCurrencyId THEN
		...
		CASE rc.strChargeEntity 
			WHEN 'Reduce' THEN -ISNULL(rc.dblAmount, 0.00)
			WHEN 'Add' THEN ISNULL(rc.dblAmount, 0.00)
			ELSE 
				CASE WHEN r.intEntityVendorId = COALESCE(rc.intEntityVendorId, r.intEntityVendorId) THEN ISNULL(rc.dblAmount, 0.00) ELSE 0.00 END 
		END
	ELSE 0.00
	END)
	, dblTotalChargeTax = SUM(CASE WHEN COALESCE(rc.intCurrencyId, r.intCurrencyId) = r.intCurrencyId THEN
		...
		CASE rc.strChargeEntity
			WHEN 'Reduce' THEN -ISNULL(rc.dblTax, 0.00)
			WHEN 'Add' THEN ISNULL(rc.dblTax, 0.00)
			ELSE
				CASE WHEN r.intEntityVendorId = COALESCE(rc.intEntityVendorId, r.intEntityVendorId) THEN ISNULL(rc.dblTax, 0.00) ELSE 0.00 END 
		END
	ELSE 0.00 
	END)
