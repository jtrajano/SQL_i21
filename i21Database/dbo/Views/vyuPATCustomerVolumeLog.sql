...
---===voucher details=====
	SELECT  
		...
		intCustomerId = B.intEntityVendorId,
		strCustomerNo = V.strVendorId, 
		strCustomer = E.strName,
		strStockStatus = C.strStockStatus,
		strCategoryCode =  PC.strCategoryCode,  
		strCategorySource = CASE WHEN vl.intBillId IS NOT NULL THEN 'Purchase' 
 	   					    ELSE (CASE WHEN vl.ysnDirectSale = 1 THEN 'Direct' ELSE 'Sale' END) END,
	...
	select 
		id = NEWID(),
		strFiscalYear = F.strFiscalYear,
		intCustomerId = IH.intEntityCustomerId,
	...
	SELECT  
		id = NEWID(),
		strFiscalYear = f.strFiscalYear, 
		intCustomerId = av.intCustomerId,
		strCustomerNo = c.strCustomerNumber,
		strCustomer =  e.strName,
		strStockStatus = c.strStockStatus, 
		strCategoryCode = pc.strCategoryCode, 
		strCategorySource = 'Direct',
		strUnitAmount = pc.strUnitAmount,
		intTransactionId = av.intAdjustmentId,
