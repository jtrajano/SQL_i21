CREATE VIEW [dbo].[vyuCRMCampaignSearch]
	AS
	with result as (
		select z.intApprovalId, z.intTransactionId, z.strStatus from tblSMApproval z
	)
	select
		a.intCampaignId
		,a.strCampaignName
		...
		,d.strType
		,e.strStatus
		,strApprovalStatus = (select i.strStatus from result i where i.intApprovalId = max(h.intApprovalId))
	from
		tblCRMCampaign a
