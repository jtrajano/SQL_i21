...
AS  
	SELECT 
		...
		,dblAverageBurnRate =CAST( CASE WHEN ISNULL(C.dblPreviousBurnRate, 0.0) = 0 THEN ((ISNULL(F.dblCalculatedBurnRate,0.0) + ISNULL(C.dblBurnRate,0.0))/2) ELSE (ISNULL(F.dblCalculatedBurnRate,0.0) + ISNULL(C.dblBurnRate,0.0) + C.dblPreviousBurnRate)/3 END AS NUMERIC(18,6))
		...
		,dblPercentIdeal = CAST( CASE WHEN ((ISNULL(C.dblTotalCapacity,0.0) * (I.dblDefaultFull/100)) - ISNULL(C.dblTotalReserve,0.0))  <= 0 THEN 0 ELSE F.dblQuantityDelivered / ((ISNULL(C.dblTotalCapacity,0.0) * (I.dblDefaultFull/100)) - ISNULL(C.dblTotalReserve,0.0)) END AS NUMERIC(18,6)) * 100
		,intConcurrencyId = 0
