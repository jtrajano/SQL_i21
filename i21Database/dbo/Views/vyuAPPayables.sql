...
SELECT  A.dtmDatePaid AS dtmDate,    
	 ...
	 CAST(
		 	(CASE WHEN C.intTransactionType NOT IN (1,2, 14) AND B.dblPayment > 0
				...
				ELSE B.dblPayment END) * avgRate.dblExchangeRate AS DECIMAL(18,2)) AS dblAmountPaid,     
	 dblTotal = 0 
	, dblAmountDue = 0 
	...
	, CAST(CASE 
			...
			END * avgRate.dblExchangeRate AS DECIMAL(18,2)) AS dblDiscount
	, CAST(CASE 
			WHEN C.intTransactionType NOT IN (1,2,14) AND ABS(B.dblInterest) > 0 
			THEN B.dblInterest * -1 
			ELSE B.dblInterest
			END * avgRate.dblExchangeRate AS DECIMAL(18,2)) AS dblInterest 
	, dblPrepaidAmount = 0 
	, D.strVendorId 
...
FROM dbo.tblAPPayment  A
 ...
 INNER JOIN dbo.fnAPGetVoucherAverageRate() avgRate ON C.intBillId = avgRate.intBillId
