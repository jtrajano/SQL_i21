...
SELECT CD.intContractDetailId
	...
	,SK.intStockUOMId
	,SK.strStockUnitMeasure
	,DC.strCity AS strDestinationCity
	,SK.intStockUnitMeasureId
	...
	,SU.strStockUOM
	,SU.strStockUOMType
	,ISNULL(IU.dblUnitQty, 0) AS dblItemUOMCF
	,SB.intCompanyLocationSubLocationId
	,SB.strSubLocationName
	,ISNULL(SU.intStockUOM, 0) AS intStockUOM
	,CS.strContractStatus
	,ISNULL(SU.dblStockUOMCF, 0) AS dblStockUOMCF
	,IX.strIndex
	,VR.strVendorId
	,SP.intSupplyPointId
	,SP.intEntityVendorId AS intTerminalId
	...
	,CA1.intCountryID AS intOriginCountryId
	,CA1.strDescription AS strItemOriginCountry
	...
	,CAST(CASE 
			WHEN DATEADD(d, 0, DATEDIFF(d, 0, GETDATE())) >= DATEADD(dd, - ISNULL(CP.intEarlyDaysPurchase, 0), CD.dtmStartDate)
				AND CH.intContractTypeId = 1
				THEN 1
			WHEN DATEADD(d, 0, DATEDIFF(d, 0, GETDATE())) >= DATEADD(dd, - ISNULL(CP.intEarlyDaysSales, 0), CD.dtmStartDate)
				AND CH.intContractTypeId = 2
				THEN 1
			ELSE 0
			END AS BIT) AS ysnEarlyDayPassed
	,CAST(CASE 
	...
	,AD.dblSeqBasis
	,AD.intSeqBasisCurrencyId
	,AD.intSeqBasisUOMId
	,AD.ysnValidFX
	,AD.dblSeqPrice * AD.dblQtyToPriceUOMConvFactor AS dblPricePerUnit
	,ISNULL(WU.dblUnitQty, 1) AS dblWeightUnitQty
	,ISNULL(IU.dblUnitQty, 1) AS dblUnitQty
	,RT.strCurrencyExchangeRateType
	,RT.strDescription AS strCurrencyExchangeRateTypeDesc
...
LEFT JOIN tblICItemUOM AU ON AU.intItemUOMId = CD.intAdjItemUOMId
LEFT JOIN tblICUnitMeasure U3 ON U3.intUnitMeasureId = AU.intUnitMeasureId
LEFT JOIN tblICItemUOM SM ON SM.intItemId = CD.intItemId
	AND SM.ysnStockUnit = 1
LEFT JOIN tblICUnitMeasure U4 ON U4.intUnitMeasureId = SM.intUnitMeasureId
...
LEFT JOIN tblICCommodityAttribute CA1 ON CA1.intCommodityAttributeId = IM.intOriginId
...
LEFT JOIN tblSMCity DP ON DP.intCityId = CD.intLoadingPortId
...
LEFT JOIN tblEMEntitySplit ST ON ST.intSplitId = CD.intSplitId
...
LEFT JOIN (
	SELECT intItemUOMId AS intStockUOM
		,strUnitMeasure AS strStockUOM
		,strUnitType AS strStockUOMType
		,dblUnitQty AS dblStockUOMCF
	FROM tblICItemUOM IU
	LEFT JOIN tblICUnitMeasure UM ON UM.intUnitMeasureId = IU.intUnitMeasureId
	WHERE ysnStockUnit = 1
	) SU ON SU.intStockUOM = IU.intItemUOMId
LEFT JOIN (
	SELECT intItemUOMId AS intStockUOMId
		,strUnitMeasure AS strStockUnitMeasure
		,IU.intItemId
		,IU.intUnitMeasureId AS intStockUnitMeasureId
	FROM tblICItemUOM IU
	JOIN tblICUnitMeasure UM ON UM.intUnitMeasureId = IU.intUnitMeasureId
	WHERE IU.ysnStockUnit = 1
	) SK ON SK.intItemId = CD.intItemId
LEFT JOIN tblSMCompanyLocationSubLocation SB ON SB.intCompanyLocationSubLocationId = IL.intSubLocationId
LEFT JOIN tblTRSupplyPoint SP ON SP.intEntityVendorId = IX.intVendorId
	AND SP.intEntityLocationId = IX.intVendorLocationId
LEFT JOIN (
	SELECT intContractDetailId
		,ISNULL(SUM(dblReservedQuantity), 0) AS dblReservedQuantity
	FROM tblLGReservation
	GROUP BY intContractDetailId
	) RV ON RV.intContractDetailId = CD.intContractDetailId
LEFT JOIN (
	SELECT intPContractDetailId
		,ISNULL(SUM(dblPAllocatedQty), 0) AS dblAllocatedQty
		,MIN(intPUnitMeasureId) intAllocationUOMId
...
LEFT JOIN (
	SELECT intSContractDetailId
		,ISNULL(SUM(dblSAllocatedQty), 0) AS dblAllocatedQty
		,MIN(intSUnitMeasureId) intAllocationUOMId
	FROM tblLGAllocationDetail
	GROUP BY intSContractDetailId
	) SA ON SA.intSContractDetailId = CD.intContractDetailId
LEFT JOIN tblICUnitMeasure U5 ON U5.intUnitMeasureId = PA.intAllocationUOMId
LEFT JOIN tblICUnitMeasure U6 ON U6.intUnitMeasureId = SA.intAllocationUOMId
