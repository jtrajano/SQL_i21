...
AS
SELECT strBatchId				= AR.strBatchId
	 ...
	 , strLocationName			= AR.strLocationName	
	 , intCurrencyId			= AR.intCurrencyID
	 , strCurrency				= AR.strCurrency
	 , strCurrencyDescription	= AR.strCurrencyDescription
FROM (
	...
	) LOC ON INV.intCompanyLocationId  = LOC.intCompanyLocationId 
	LEFT OUTER JOIN (
		SELECT intCurrencyID
			 , strCurrency 
			 , strDescription 
		FROM tblSMCurrency
	) SMC ON INV.intCurrencyId = SMC.intCurrencyID	
	...
	LEFT OUTER JOIN (
		SELECT intCompanyLocationId
			 , strLocationName
		FROM dbo.tblSMCompanyLocation
	) LOC ON AR.intLocationId  = LOC.intCompanyLocationId 			
	LEFT OUTER JOIN (
		SELECT intCurrencyID
			 , strCurrency
			 , strDescription 
