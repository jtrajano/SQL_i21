...
(
			...
			,dblVendorPrepayment		  	= CASE WHEN ISNULL(VendorPrepayment.dblVendorPrepayment, 0) <> 0 THEN VendorPrepayment.dblVendorPrepayment			     ELSE 0 END
			,lblVendorPrepayment			= 'Vendor Prepay' COLLATE Latin1_General_CI_AS
			,dblCustomerPrepayment		  	= CASE WHEN ISNULL(Invoice.dblPayment, 0) <> 0				   THEN Invoice.dblPayment								     ELSE 0 END
			,lblCustomerPrepayment			= 'Customer Prepay' COLLATE Latin1_General_CI_AS
			,dblGradeFactorTax			  	= CASE WHEN ISNULL(ScaleDiscountTax.dblGradeFactorTax, 0) <> 0   THEN ScaleDiscountTax.dblGradeFactorTax				     ELSE 0 END
			,lblFactorTax					= 'Factor Tax' COLLATE Latin1_General_CI_AS
			,dblPartialPrepaymentSubTotal = CASE WHEN ISNULL(PartialPayment.dblPayment, 0) <> 0		       THEN PartialPayment.dblTotals							 ELSE 0 END
			,lblPartialPrepayment			= 'Partial Payment Adj' COLLATE Latin1_General_CI_AS
			,dblPartialPrepayment		  	= CASE WHEN ISNULL(PartialPayment.dblPayment, 0) <> 0		       THEN PartialPayment.dblPayment - PartialPayment.dblTotals ELSE 0 END
