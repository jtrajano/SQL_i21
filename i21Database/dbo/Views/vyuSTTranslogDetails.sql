...
(   SELECT --DISTINCT  -- update 09/08
        MIN(intTranslogId) AS intTranslogId
       ...
       , TR.strTrlDesc
	   , SUM(TR.dblTrlLineTot) AS dblTrlLineTot
	   , SUM(TR.dblTrlMatchLineTrlPromoAmount) AS dblTrlMatchLineTrlPromoAmount
	   , SUM(TR.dblTrlMatchLineTrlMatchQuantity) AS dblTrlMatchLineTrlMatchQuantity
       ...
       , CAST(TR.intTrTickNumPosNum AS INT)
       , CAST(TR.intTrTickNumTrSeq AS INT) 
       , TR.strTransType 
       , CAST(TR.intCashierSysId AS INT) 
       , TR.strCashier
       , TR.intTrTickNumTrSeq -- Added 09/08/2021
	   , TR.dblTrlPrcOvrd 
       , RIGHT('0' + CONVERT(VARCHAR(2), DATEPART(HOUR, TR.dtmDate)), 2)
       , CAST(RIGHT(intCashierPosNum , 1) AS INT)
       , CAST((CASE WHEN LEN(strTrlFuelBasePrice) > 0
                           THEN 1
                           ELSE 0
                           END) AS BIT)  
        ,CASE WHEN dblTrlPrcOvrd IS NOT NULL
		 ...
		 ,Cast(CASE WHEN dblTrlMatchLineTrlMatchQuantity=1 THEN 1 ELSE ROUND((dblTrlQty-.5)/dblTrlMatchLineTrlMatchQuantity,0) END AS INT) 
