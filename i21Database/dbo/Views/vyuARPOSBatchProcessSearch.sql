 ...
 FROM (
	...
	FROM
		(SELECT intPOSLogId			= POSLOG.intPOSLogId
			 ...
			 , strInvoiceNumber		= INVNO.strInvoiceNumber --ISNULL(I.strInvoiceNumber, CM.strInvoiceNumber)
		...
		OUTER APPLY (
				...
				FROM (
					SELECT DISTINCT CAST(INV.strInvoiceNumber AS VARCHAR(200))  + ', '
					FROM  tblARInvoice INV 
					INNER JOIN (
						SELECT tblARPOS.intInvoiceId, tblARPOS.intCreditMemoId from tblARPOS
						WHERE tblARPOS.intInvoiceId = POS.intInvoiceId OR tblARPOS.intCreditMemoId = POS.intCreditMemoId 
					) ARPOS
					 ON INV.intInvoiceId = ARPOS.intInvoiceId
					  OR INV.intInvoiceId = ARPOS.intCreditMemoId
			
					FOR XML PATH ('')
				) MERGE_INVNO (strInvoiceNumber)
		)INVNO
		LEFT JOIN (
