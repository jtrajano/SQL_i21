...
AS     
 ...
 FROM (    
    SELECT     
      ...
      ysnIsUsed = CONVERT(BIT, 
					CASE WHEN (SELECT TOP 1 1 FROM tblARInvoiceDetail WHERE intItemContractHeaderId = CH.intItemContractHeaderId) = 1
						 OR (
								select TOP 1 1 
								FROM tblSCTicket a
								INNER JOIN tblCTItemContractDetail b on a.intItemContractDetailId = b.intItemContractDetailId
								WHERE b.intItemContractHeaderId = CH.intItemContractHeaderId 
								) = 1
					THEN 1 ELSE 0 END),    
   ...
   LEFT JOIN tblEMEntitySplit ES ON ES.intSplitId = CH.intSplitId  
   LEFT JOIN (    
