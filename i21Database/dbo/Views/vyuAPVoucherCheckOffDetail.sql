...
SELECT --DISTINCT     
              ...
              ,APBDT.dblTax * (CASE WHEN APBD.intInventoryReceiptChargeId IS NOT NULL AND APBDT.dblAdjustedTax < 0 THEN -1 ELSE 1 END) AS dblTaxAmount  
              ,APBD.dblQtyReceived  
              ,C.strCommodityCode   
              ...
              --Handle data issue on tax sign if check off and charges
              ,ISNULL(APBDT.dblAdjustedTax, ISNULL(APBDT.dblTax,0)) * (CASE WHEN APBD.intInventoryReceiptChargeId IS NOT NULL AND APBDT.dblAdjustedTax < 0 THEN -1 ELSE 1 END)
              AS dblTax  
