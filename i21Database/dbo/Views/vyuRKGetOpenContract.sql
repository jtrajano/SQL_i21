...
SELECT ot.intFutOptTransactionId,sum(isnull(ot.dblNoOfContract,0)) dblNoOfContract,
	   (SELECT SUM(CONVERT(int,mf.intMatchQty)) FROM tblRKOptionsMatchPnS mf where ot.intFutOptTransactionId=mf.intLFutOptTransactionId) dblOpenContract
...
SELECT distinct intFutOptTransactionId,-(dblNoOfContract-isnull(dblOpenContract,0)) dblOpenContract from (
SELECT ot.intFutOptTransactionId,sum(ot.dblNoOfContract) dblNoOfContract,
	   (SELECT SUM(CONVERT(int,mf.intMatchQty)) FROM tblRKOptionsMatchPnS mf where ot.intFutOptTransactionId=mf.intSFutOptTransactionId) dblOpenContract
