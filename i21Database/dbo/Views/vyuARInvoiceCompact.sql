...
SELECT     
	...
	,ysnExcludeForPayment			= CASE WHEN ARI.strTransactionType = 'Customer Prepayment' AND EXISTS(SELECT NULL FROM tblARInvoiceDetail WHERE intInvoiceId = ARI.intInvoiceId AND (ISNULL(ysnRestricted, 0) = 1 OR ISNULL(intContractDetailId, 0) <> 0)) THEN CONVERT(BIT, 1) ELSE CONVERT(BIT, 0) END
FROM         
