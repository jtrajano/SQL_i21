...
FROM tblPREmployee [EMP] 
	...
	LEFT JOIN (SELECT intYear = YEAR(dtmPayDate), intEntityEmployeeId, dblTotal = SUM(dblTotal)
				FROM vyuPRPaycheckTax WHERE ysnVoid = 0 AND strPaidBy = 'Employee' GROUP BY YEAR(dtmPayDate), intEntityEmployeeId) AS [EMPTAX_Y]
		ON EMPTAX_Y.intEntityEmployeeId = EMP.intEntityId AND EMPTAX_Y.intYear = YR.intYear
	LEFT JOIN (SELECT intYear = YEAR(dtmPayDate), intEntityEmployeeId, dblTotal = SUM(dblTotal)
				FROM vyuPRPaycheckDeduction WHERE ysnVoid = 0 AND strPaidBy = 'Employee' GROUP BY YEAR(dtmPayDate), intEntityEmployeeId) AS [EMPDED_Y]
		ON EMPDED_Y.intEntityEmployeeId = EMP.intEntityId AND EMPDED_Y.intYear = YR.intYear
	LEFT JOIN (SELECT intYear = YEAR(dtmPayDate), intEntityEmployeeId, dblTotal = SUM(dblTotal) 
				FROM vyuPRPaycheckDeduction WHERE ysnVoid = 0 AND strPaidBy = 'Company' GROUP BY YEAR(dtmPayDate), intEntityEmployeeId) AS [COMDED_Y]
	...
	LEFT JOIN (SELECT intYear = YEAR(dtmPayDate), intEntityEmployeeId, dblTotal = SUM(dblTotal) 
				FROM vyuPRPaycheckTax WHERE ysnVoid = 0 AND strPaidBy = 'Employee' AND DATEPART(QQ, dtmPayDate) = 1 GROUP BY YEAR(dtmPayDate), intEntityEmployeeId) AS [EMPTAX_Q1]
	...
	LEFT JOIN (SELECT intYear = YEAR(dtmPayDate), intEntityEmployeeId, dblTotal = SUM(dblTotal) FROM vyuPRPaycheckDeduction
				WHERE ysnVoid = 0 AND strPaidBy = 'Employee' AND DATEPART(QQ, dtmPayDate) = 1 GROUP BY YEAR(dtmPayDate), intEntityEmployeeId) AS [EMPDED_Q1]
		ON EMPDED_Q1.intEntityEmployeeId = EMP.intEntityId AND EMPDED_Q1.intYear = YR.intYear
	LEFT JOIN (SELECT intYear = YEAR(dtmPayDate), intEntityEmployeeId, dblTotal = SUM(dblTotal) FROM vyuPRPaycheckDeduction
				WHERE ysnVoid = 0 AND strPaidBy = 'Company' AND DATEPART(QQ, dtmPayDate) = 1 GROUP BY YEAR(dtmPayDate), intEntityEmployeeId) AS [COMDED_Q1]
	...
	LEFT JOIN (SELECT intYear = YEAR(dtmPayDate), intEntityEmployeeId, dblTotal = SUM(dblTotal) 
				FROM vyuPRPaycheckTax WHERE ysnVoid = 0 AND strPaidBy = 'Employee' AND DATEPART(QQ, dtmPayDate) = 2 GROUP BY YEAR(dtmPayDate), intEntityEmployeeId) AS [EMPTAX_Q2]
		ON EMPTAX_Q2.intEntityEmployeeId = EMP.intEntityId AND EMPTAX_Q2.intYear = YR.intYear
	LEFT JOIN (SELECT intYear = YEAR(dtmPayDate), intEntityEmployeeId, dblTotal = SUM(dblTotal) 
				FROM vyuPRPaycheckDeduction WHERE ysnVoid = 0 AND strPaidBy = 'Employee' AND DATEPART(QQ, dtmPayDate) = 2 GROUP BY YEAR(dtmPayDate), intEntityEmployeeId) AS [EMPDED_Q2]
		ON EMPDED_Q2.intEntityEmployeeId = EMP.intEntityId AND EMPDED_Q2.intYear = YR.intYear
	LEFT JOIN (SELECT intYear = YEAR(dtmPayDate), intEntityEmployeeId, dblTotal = SUM(dblTotal) 
				FROM vyuPRPaycheckDeduction WHERE ysnVoid = 0 AND strPaidBy = 'Company' AND DATEPART(QQ, dtmPayDate) = 2 GROUP BY YEAR(dtmPayDate), intEntityEmployeeId) AS [COMDED_Q2]
	...
	LEFT JOIN (SELECT intYear = YEAR(dtmPayDate), intEntityEmployeeId, dblTotal = SUM(dblTotal) 
				FROM vyuPRPaycheckTax WHERE ysnVoid = 0 AND strPaidBy = 'Employee' AND DATEPART(QQ, dtmPayDate) = 3 GROUP BY YEAR(dtmPayDate), intEntityEmployeeId) AS [EMPTAX_Q3]
		ON EMPTAX_Q3.intEntityEmployeeId = EMP.intEntityId AND EMPTAX_Q3.intYear = YR.intYear
	LEFT JOIN (SELECT intYear = YEAR(dtmPayDate), intEntityEmployeeId, dblTotal = SUM(dblTotal) 
				FROM vyuPRPaycheckDeduction WHERE ysnVoid = 0 AND strPaidBy = 'Employee' AND DATEPART(QQ, dtmPayDate) = 3 GROUP BY YEAR(dtmPayDate), intEntityEmployeeId) AS [EMPDED_Q3]
		ON EMPDED_Q3.intEntityEmployeeId = EMP.intEntityId AND EMPDED_Q3.intYear = YR.intYear
	LEFT JOIN (SELECT intYear = YEAR(dtmPayDate), intEntityEmployeeId, dblTotal = SUM(dblTotal) 
				FROM vyuPRPaycheckDeduction WHERE ysnVoid = 0 AND strPaidBy = 'Company' AND DATEPART(QQ, dtmPayDate) = 3 GROUP BY YEAR(dtmPayDate), intEntityEmployeeId) AS [COMDED_Q3]
	...
	LEFT JOIN (SELECT intYear = YEAR(dtmPayDate), intEntityEmployeeId, dblTotal = SUM(dblTotal) 
				FROM vyuPRPaycheckTax WHERE ysnVoid = 0 AND strPaidBy = 'Employee' AND DATEPART(QQ, dtmPayDate) = 4 GROUP BY YEAR(dtmPayDate), intEntityEmployeeId) AS [EMPTAX_Q4]
		ON EMPTAX_Q4.intEntityEmployeeId = EMP.intEntityId AND EMPTAX_Q4.intYear = YR.intYear
	LEFT JOIN (SELECT intYear = YEAR(dtmPayDate), intEntityEmployeeId, dblTotal = SUM(dblTotal) 
				FROM vyuPRPaycheckDeduction WHERE ysnVoid = 0 AND strPaidBy = 'Employee' AND DATEPART(QQ, dtmPayDate) = 4 GROUP BY YEAR(dtmPayDate), intEntityEmployeeId) AS [EMPDED_Q4]
		ON EMPDED_Q4.intEntityEmployeeId = EMP.intEntityId AND EMPDED_Q4.intYear = YR.intYear
	LEFT JOIN (SELECT intYear = YEAR(dtmPayDate), intEntityEmployeeId, dblTotal = SUM(dblTotal) 
				FROM vyuPRPaycheckDeduction WHERE ysnVoid = 0 AND strPaidBy = 'Company' AND DATEPART(QQ, dtmPayDate) = 4 GROUP BY YEAR(dtmPayDate), intEntityEmployeeId) AS [COMDED_Q4]
