...
SELECT OH.intOrderHeaderId
	  ...
	  ,TP.strTaskPriority
	  ,T.dtmReleaseDate
	  ,ISNULL(L.dblQty, SUOM.dblOnHand) AS dblLotQty
...
LEFT JOIN tblICItemUOM LIU ON LIU.intItemUOMId = T.intItemUOMId
LEFT JOIN tblICUnitMeasure LIUM ON LIUM.intUnitMeasureId = LIU.intUnitMeasureId
LEFT JOIN tblICItemUOM LWU ON LWU.intItemUOMId = T.intWeightUOMId
...
LEFT JOIN tblICItemUOM IUOM1 ON IUOM1.intItemId = I.intItemId
	AND IUOM1.ysnStockUnit = 1
LEFT JOIN tblICItemStockUOM SUOM ON SUOM.intItemId = I.intItemId
	AND SUOM.intStorageLocationId = T.intFromStorageLocationId
	AND SUOM.intItemUOMId = IUOM1.intItemUOMId
