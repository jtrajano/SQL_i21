...
SELECT        INV.intEntityCustomerId, INV.intEntityId, C.strCustomerNumber, dbo.fnARFormatCustomerAddress(NULL, NULL, INV.strShipToLocationName, INV.strShipToAddress, INV.strShipToCity, INV.strShipToState, 
                         INV.strShipToZipCode, INV.strShipToCountry, NULL, 0) COLLATE Latin1_General_CI_AS AS strShipTo, dbo.fnARFormatCustomerAddress(NULL, NULL, INV.strBillToLocationName, INV.strBillToAddress, INV.strBillToCity, INV.strBillToState, 
                         INV.strBillToZipCode, INV.strBillToCountry, E.strName, 0)  COLLATE Latin1_General_CI_AS AS strBillTo, CASE WHEN L.strUseLocationAddress = 'Letterhead' THEN '' ELSE
                         ...
                         L.strCountry, NULL, NULL) WHEN L.strUseLocationAddress = 'Letterhead' THEN '' END COLLATE Latin1_General_CI_AS AS strCompanyAddress, ISNULL(INV.strType, 'Standard') AS strType, E.strName AS strCustomerName, L.strLocationName, 
