...
SELECT DISTINCT intItemId				= ITEMS.intItemId
	 ...
	 , dblSalePrice						= [dbo].[fnICConvertUOMtoStockUnit](ITEMS.intItemId,ITEMS.intItemUOMId,1) * ISNULL(IP.dblSalePrice, 0)
	 , dblUnitOnHand					= [dbo].[fnICConvertUOMtoStockUnit](ITEMS.intItemId,ITEMS.intItemUOMId,1) * ISNULL(ISTOCK.dblUnitOnHand, 0)
	 , dblOnOrder						= ISNULL(ISTOCK.dblOnOrder, 0)
	 , dblOrderCommitted				= ISNULL(ISTOCK.dblOrderCommitted, 0)
	 , dblAvailable						= ([dbo].[fnICConvertUOMtoStockUnit](ITEMS.intItemId,ITEMS.intItemUOMId,1) * ISNULL(ISTOCK.dblUnitOnHand, 0)) - (ISNULL(ISTOCK.dblUnitReserved, 0) + ISNULL(ISTOCK.dblConsignedSale, 0))	 
