...
(
	...
	FROM (
			...
			JOIN tblICItem Item 
				ON BillDtl.intItemId = Item.intItemId AND Item.strType = 'Inventory'
	...
	LEFT JOIN (
				 ...
				 JOIN tblICItem B ON A.intItemId = B.intItemId AND B.strType = 'Inventory'
		...
		(
			...
			LEFT JOIN (
			 ...
			 ON ScaleDiscount.intTicketId = BillDtl.intScaleTicketId 
				AND ScaleDiscount.intItemId = BillDtl.intItemId
			...
			WHERE Item.strType = 'Other Charge' AND ISNULL(BillDtl.intContractDetailId, 0) = 0 AND BillDtl.intConcurrencyId = 0
		) tbl 
	 GROUP BY 
		 strId
		,intBillId
	...
	WHERE  Item.strType = 'Other Charge'
		...
		AND BillDtl.intConcurrencyId = 0
	) tbl 
	GROUP BY 
		strId
		,intBillId
		...
		,dblTax
		,dblNetTotal
   )t3 ON t3.intBillId = t2.intBillId 
   			AND t3.intBillId = t1.intBillId
	WHERE t3.intItemId IS NOT NULL
  )t
GROUP BY 
