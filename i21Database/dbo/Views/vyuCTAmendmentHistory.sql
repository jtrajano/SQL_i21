...
(
		SELECT 
			...
			,strReasonCode					=  RC.strReasonCode
		...
		LEFT JOIN tblCTAmendmentApproval        AMP ON AMP.intAmendmentApprovalId = SAL.intAmendmentApprovalId  AND ISNULL(AMP.ysnAmendment,0) = ( case when SAL.strItemChanged in ('Partial Price Qty','Full Price Qty','Partial Price Fixation','Full Price Fixation') then ISNULL(AMP.ysnAmendment,0) else 1 end)
