...
AS
SELECT       CustomerEntity.strDocumentDelivery ,Entity.intEntityId, Entity.strName, CASE WHEN Cus.strCustomerNumber = '' THEN Entity.strEntityNo ELSE Cus.strCustomerNumber END AS strCustomerNumber, Cus.strType, 
                         Con.strPhone, Entity.strAddress, Entity.strCity, Entity.strState, Entity.strZipCode, Cus.ysnActive, Cus.intSalespersonId, Cus.intCurrencyId, Cus.intTermsId, Loc.intShipViaId, 
...
FROM            dbo.vyuEMSearch AS Entity INNER JOIN
                         ...
                         dbo.[tblEMEntityLocation] AS BillToLoc ON Cus.intBillToId = BillToLoc.intEntityLocationId LEFT OUTER JOIN
						 dbo.[tblEMEntity] AS CustomerEntity ON CustomerEntity.intEntityId = Entity.intEntityId
