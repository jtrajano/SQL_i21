...
(CASE WHEN ft.intInstrumentTypeId = 1 then 'Futures'
	 ...
	 WHEN ft.intInstrumentTypeId = 3 then 'Currency Contract' end) COLLATE Latin1_General_CI_AS strInstrumentType,
...
strUnitMeasure,strCommodityCode,strLocationName,bc.strCurrency,(substring(fm.strFutureMonth,0,4) + '(' +fm.strSymbol+')'+convert(nvarchar,fm.intYear)) COLLATE Latin1_General_CI_AS strFutureMonthYear,
fm.strFutureMonth strFutureMonthYearWOSymbol,
 (substring(om.strOptionMonth,0,4) + '(' +fom.strOptSymbol+')'+convert(nvarchar,om.intYear)) COLLATE Latin1_General_CI_AS strOptionMonthYear,
		...
		(CASE WHEN ISNULL(intSelectedInstrumentTypeId,1) =1  then 'Exchange Traded' else 'OTC' end) COLLATE Latin1_General_CI_AS as strSelectedInstrumentType,intAssignedLots as intAssignedLots
