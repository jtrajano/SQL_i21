...
SELECT	intKey = CAST(ROW_NUMBER() OVER(ORDER BY BundleComponent.intItemId, ItemLocation.intLocationId) AS INT),
		...
		intBundleUOMId = BundleComponentUOM.intItemUOMId,
		strBundleUOM = bcUOM.strUnitMeasure,
		strBundleUOMType = bcUOM.strUnitType,
		ItemBundle.dblMarkUpOrDown,
		ItemBundle.dtmBeginDate,
		ItemBundle.dtmEndDate,
		
		BundleComponent.strItemNo,
		BundleComponent.strType,
		BundleComponent.strDescription,
