CREATE VIEW [dbo].[vyuARCollectionOverdueDetailReport]
AS
SELECT * FROM 
(
	...
	SELECT dtmDate					= ISNULL(P.dtmDatePaid, I.dtmPostDate)
		 ...
		 , strAge = CASE WHEN DATEDIFF(DAYOFYEAR, ISNULL(P.dtmDatePaid, I.dtmDueDate), GETDATE()) <= 0 THEN 'Current'
						 ...
						 WHEN DATEDIFF(DAYOFYEAR, ISNULL(P.dtmDatePaid, I.dtmDueDate), GETDATE()) > 120 THEN 'Over 120 Days' END
		 , I.ysnPosted
...
WHERE intInvoiceId NOT IN (SELECT intInvoiceId FROM tblARInvoice WHERE strTransactionType IN ('Credit Memo', 'Overpayment', 'Credit', 'Customer Prepayment'))
