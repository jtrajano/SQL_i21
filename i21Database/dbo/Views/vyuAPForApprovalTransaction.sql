...
CROSS APPLY (
 ...
 AND (B.strApprovalStatus IS NOT NULL AND B.strApprovalStatus NOT IN ('No Need for Approval','Approved','Approved with Modifications','Waiting for Submit'))
) APTransaction
WHERE A.strScreenName IN ('Purchase Order','Voucher','Debit Memo')
-- AND EXISTS (
...
--  UNION ALL
--  SELECT TOP 1 1 FROM tblSMUserSecurityRequireApprovalFor usr WHERE usr.intScreenId = A.intScreenId AND usr.intEntityUserSecurityId IN (
-- 	SELECT intSubmittedById FROM tblSMApproval WHERE strTransactionNumber = APTransaction.strTransactionNo)
-- )
