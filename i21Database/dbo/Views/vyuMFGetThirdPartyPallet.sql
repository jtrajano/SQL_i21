...
JOIN dbo.tblICItem AS I ON I.intItemId = L.intItemId and L.dblQty >0
...
LEFT JOIN dbo.tblSMUserSecurity AS US ON US.intEntityUserSecurityId = L.intCreatedUserId
...
LEFT JOIN dbo.tblMFWorkOrderProducedLot WP ON WP.intSpecialPalletLotId = L.intLotId and WP.ysnProductionReversed =0
...
LEFT JOIN dbo.tblICItemOwner AS O ON O.intItemOwnerId = LI.intItemOwnerId
LEFT JOIN dbo.tblEMEntity AS E ON E.intEntityId = O.intOwnerId
JOIN dbo.tblMFWorkOrderProducedLot WP ON WP.intSpecialPalletLotId = L.intLotId and WP.ysnProductionReversed =0
LEFT JOIN dbo.tblICLot AS L1 ON L1.intLotId = WP.intLotId
LEFT JOIN dbo.tblICInventoryShipmentItemLot IL ON IL.intLotId = L1.intLotId
WHERE I.intItemId IN (
		SELECT strAttributeValue
