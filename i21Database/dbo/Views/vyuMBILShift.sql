...
LEFT JOIN (
			...
			FROM (
					SELECT 
						intShiftId, SUM(dblQuantity) as dblFuelGallonsDelievered,  (ISNULL(dblItemTotal,0) + ISNULL(dblTaxTotal,0)) as dblFuelSales
					FROM 
						...
						GROUP BY intShiftId, dblQuantity, dblItemTotal, dblTaxTotal
				) tblTotalItem
				GROUP BY intShiftId
