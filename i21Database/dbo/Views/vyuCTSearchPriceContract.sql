...
AS 
	...
	(
		SELECT 		CAST (NULL AS INT) AS intPriceContractId,
					...
					(SELECT SUM(dblQuantity) FROM tblCTContractDetail WHERE intContractDetailId = CD.intContractDetailId OR (intSplitFromId IS NOT NULL AND intSplitFromId = CD.intContractDetailId)) AS dblQuantity,
					strItemUOM AS strUOM,
					(SELECT SUM(dblNoOfLots) FROM tblCTContractDetail WHERE intContractDetailId = CD.intContractDetailId OR (intSplitFromId IS NOT NULL AND intSplitFromId = CD.intContractDetailId)) dblNoOfLots,
					strLocationName,
					CD.intItemId,
					CD.intItemUOMId,
					intFutureMarketId,
