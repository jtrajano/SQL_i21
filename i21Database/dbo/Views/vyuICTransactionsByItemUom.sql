CREATE VIEW [dbo].[vyuICTransactionsByItemUom]
AS
...
FROM tblICInventoryReceiptItem ri
	...
	INNER JOIN vyuICItemUOM uom ON uom.intItemUOMId = ri.intUnitMeasureId
...
FROM tblICInventoryShipment s
	...
	INNER JOIN tblSMCompanyLocation loc ON loc.intCompanyLocationId = il.intLocationId
	INNER JOIN vyuICItemUOM uom ON uom.intItemUOMId = si.intItemUOMId
...
FROM tblICInventoryTransfer t
	...
	INNER JOIN tblSMCompanyLocation loc ON loc.intCompanyLocationId = il.intLocationId
	LEFT OUTER JOIN vyuICItemUOM uom ON uom.intItemUOMId = td.intItemUOMId
...
FROM tblICInventoryAdjustment a
	...
	INNER JOIN tblSMCompanyLocation loc ON loc.intCompanyLocationId = il.intLocationId
	LEFT OUTER JOIN vyuICItemUOM uom ON uom.intItemUOMId = ad.intItemUOMId
...
FROM tblICInventoryCount c
	...
	INNER JOIN tblSMCompanyLocation loc ON loc.intCompanyLocationId = il.intLocationId
	LEFT OUTER JOIN vyuICItemUOM uom ON uom.intItemUOMId = cd.intItemUOMId