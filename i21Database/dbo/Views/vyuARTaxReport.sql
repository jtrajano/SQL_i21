...
SELECT I.intEntityCustomerId
	...
	, TAXDETAIL.strTaxAgency
	, TAXDETAIL.strTaxCode				 
	, TAXDETAIL.strTaxCodeDescription
	, TAXDETAIL.intTaxClassId
	, TAXDETAIL.strTaxClass
	, TAXDETAIL.strCountry
	, TAXDETAIL.strState
	, TAXDETAIL.strCounty
	, TAXDETAIL.strCity
	, TAXDETAIL.intSalesTaxAccountId
	, TAXDETAIL.SalesTaxAccount
	, TAXDETAIL.intPurchaseTaxAccountId
	, TAXDETAIL.PurchaseTaxAccount
...
INNER JOIN
	...
	SELECT
		...
		, dblRate					= 0.000000--ARIDT.dblRate
		, ARID.intInvoiceId				 
		, intItemId					= ITEMDETAIL.intItemId
		, strItemNo					= ITEMDETAIL.strItemNo
		, dblUnitPrice				= 0.000000 -- ID.dblPrice
		, dblQtyShipped				= ARID.dblQtyShipped
		, intCategoryId				= ITEMDETAIL.intCategoryId
		, strItemCategory			= ITEMDETAIL.strCategoryCode
		, dblAdjustedTax			= 0.000000 --ARIDT.dblAdjustedTax	 				 				 
		, dblTax					= 0.000000 --ARIDT.dblTax
		, ARID.intInvoiceDetailId
		, dblTotalAdjustedTax		= 0.000000
	...
	INNER JOIN 
		...
		FROM  
			...
			ON TC.intTaxCodeId = ARIDT.intTaxCodeId			
	...
	LEFT OUTER JOIN 
		...
		FROM dbo.tblSMTaxClass WITH (NOLOCK)
			...
			ON TC.intTaxClassId = CL.intTaxClassId
	LEFT OUTER JOIN 
		(
		SELECT intAccountId
			, strAccountId 
		FROM dbo.tblGLAccount WITH (NOLOCK)
			...
			ON TC.intSalesTaxAccountId = SA.intAccountId 
	LEFT OUTER JOIN 
		(
		SELECT intAccountId
			, strAccountId 
		FROM dbo.tblGLAccount WITH (NOLOCK)
			...
			ON TC.intPurchaseTaxAccountId = PA.intAccountId
	OUTER APPLY 
		...
		WHERE ARID.intItemId = ICI.intItemId) ITEMDETAIL				
	) TAXDETAIL
		ON TAXDETAIL.[intInvoiceDetailTaxId] = IMIN.[intInvoiceDetailTaxId]
LEFT OUTER JOIN (SELECT ENTITY.intEntityId 
					  , strCustomerNumber= CASE WHEN CUS.strCustomerNumber = '' THEN ENTITY.strEntityNo ELSE CUS.strCustomerNumber END
					  , strName  
				 FROM dbo.tblEMEntity ENTITY WITH (NOLOCK) 
