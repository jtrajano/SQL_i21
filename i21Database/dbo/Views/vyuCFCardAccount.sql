...
SELECT   A.intAccountId, A.intCustomerId, A.intDiscountDays, A.intDiscountScheduleId, A.intInvoiceCycle, A.intSalesPersonId, A.dtmBonusCommissionDate, 
						 ...
						 (select top 1 strEmailDistributionOption from vyuARCustomerContacts where intEntityCustomerId = A.intCustomerId AND strEmailDistributionOption LIKE '%CF Invoice%' AND ISNULL(strEmail,'') != '' AND ysnActive = 1 )  AS strEmailDistributionOption, 
						  (select top 1 strEmail from vyuARCustomerContacts where intEntityCustomerId = A.intCustomerId AND strEmailDistributionOption LIKE '%CF Invoice%' AND ISNULL(strEmail,'') != ''AND ysnActive = 1 ) AS strEmail 
