...
SELECT DISTINCT TOP 100 PERCENT
	...
	,intTransferCustomerStorageId		= NULL
...
LEFT JOIN (
   ...
   END 
	=1
WHERE SH.intTransactionTypeId <> 3
...
SELECT DISTINCT TOP 100 PERCENT
	 intStorageHistoryId				= SH.intStorageHistoryId
	 									-- CASE 
										-- 	WHEN SH.strType = 'Transfer' THEN (SELECT top 1 intStorageHistoryId FROM tblGRStorageHistory WHERE intCustomerStorageId = TSplit.intTransferToCustomerStorageId AND strType = 'From Transfer')
										-- END
	,intEntityId						= CASE 
											WHEN SH.strType = 'Transfer' THEN TSplit.intEntityId
											WHEN SH.strType = 'From Transfer' THEN TSource.intEntityId
											ELSE CS.intEntityId
										END
	,strName							= CASE 
											WHEN SH.strType = 'Transfer' THEN TSplit.strEntityName
											WHEN SH.strType = 'From Transfer' THEN TSource.strName
											ELSE E.strName
										END COLLATE Latin1_General_CI_AS
	,intCompanyLocationId				= CASE 
											WHEN SH.strType = 'Transfer' THEN TSplit.intCompanyLocationId
											WHEN SH.strType = 'From Transfer' THEN TSource.intCompanyLocationId
											ELSE CS.intCompanyLocationId
										END
	,strLocationName					= CASE 
											WHEN SH.strType = 'Transfer' THEN TSplit.strLocationName
											WHEN SH.strType = 'From Transfer' THEN TSource.strLocationName
											ELSE LOC.strLocationName
										END
	,intCustomerStorageId				= SH.intCustomerStorageId
	,intTransferCustomerStorageId		= CASE 
											WHEN SH.strType = 'Transfer' THEN CASE WHEN CSTO.intCustomerStorageId IS NOT NULL THEN CSTO.intCustomerStorageId ELSE  TSplit.intTransferToCustomerStorageId END
											WHEN SH.strType = 'From Transfer' THEN CASE WHEN CSFRM.intCustomerStorageId IS NOT NULL THEN CSFRM.intCustomerStorageId ELSE TSource.intSourceCustomerStorageId END
											WHEN SH.strType = 'Reversed Transfer' THEN CSTO.intCustomerStorageId
											ELSE NULL
										END
	,strStorageTicket					= CASE 
											WHEN SH.strType = 'Transfer' THEN CASE WHEN CSTO.intCustomerStorageId IS NOT NULL THEN CSTO.strStorageTicketNumber ELSE TSplit.strStorageTicketNumber END
											WHEN SH.strType = 'From Transfer' THEN CASE WHEN CSFRM.intCustomerStorageId IS NOT NULL THEN CSFRM.strStorageTicketNumber ELSE TSource.strStorageTicketNumber END
											ELSE CS.strStorageTicketNumber
										END
	,intTicketId						= CASE 
											WHEN SH.strType = 'Transfer' THEN NULL
											WHEN SH.strType = 'From Transfer' THEN CS.intTicketId
											ELSE SH.intTicketId
										END
	,strScaleTicket						= CASE 
											WHEN SH.strType = 'Transfer' THEN NULL
											WHEN SH.strType = 'From Transfer' THEN (SELECT strTicketNumber FROM tblSCTicket WHERE intTicketId = CS.intTicketId)
	...
	,intDeliverySheetId					= CASE 
											WHEN SH.strType = 'Transfer' THEN NULL
											WHEN SH.strType = 'From Transfer' THEN CS.intDeliverySheetId
											ELSE SH.intDeliverySheetId
										END
	,strDeliverySheetNumber				= CASE 
											WHEN SH.strType = 'Transfer' THEN NULL
											WHEN SH.strType = 'From Transfer' THEN (SELECT strDeliverySheetNumber FROM tblSCDeliverySheet WHERE intDeliverySheetId = CS.intDeliverySheetId)
...
UNION ALL
SELECT DISTINCT TOP 100 PERCENT
	 intStorageHistoryId				= SH2.intStorageHistoryId
	 									-- CASE 
										-- 	WHEN SH.strType = 'Transfer' THEN (SELECT top 1 intStorageHistoryId FROM tblGRStorageHistory WHERE intCustomerStorageId = TSplit.intTransferToCustomerStorageId AND strType = 'From Transfer')
										-- END
	,intEntityId						= CASE 
