...
SELECT TOP 100 PERCENT dblGrossPL1 a
	, dblFutCommission1 b
	, convert(int,DENSE_RANK() OVER(ORDER BY CONVERT(DATETIME,'01 '+strFutureMonth))) RowNum
	...
	FROM (
		...
		FROM (
			SELECT psh.intMatchFuturesPSHeaderId
				...
				, convert(datetime,CONVERT(VARCHAR(10),ot.dtmFilledDate,110),110) dtmLTransDate
				, convert(datetime,CONVERT(VARCHAR(10),ot1.dtmFilledDate,110),110) dtmSTransDate
