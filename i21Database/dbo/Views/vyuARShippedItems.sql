...
FROM (
	...
	SELECT strTransactionType				= 'Inventory Shipment Contract Pricing' COLLATE Latin1_General_CI_AS
	     , strTransactionNumber				= ICIS.strShipmentNumber
	     , strShippedItemId					= 'icis:' + CAST(ICIS.intInventoryShipmentId AS NVARCHAR(250)) COLLATE Latin1_General_CI_AS
	     , intEntityCustomerId				= ICIS.intEntityCustomerId
	     , intCurrencyId					= ISNULL(ICIS.intCurrencyId, ARCC.intCurrencyId)
	...
	INNER JOIN (
		SELECT intContractHeaderId				= CONTRACTS.intContractHeaderId
			 ...
			 , dblDetailQuantity				= ISNULL(ISNULL(CPFD.dblQuantityAppliedAndPriced, 0), CONTRACTS.dblDetailQuantity)
			 , intFreightTermId	
			 , dblShipQuantity					= ISNULL(ISNULL(CPFD.dblQuantityAppliedAndPriced, 0), CONTRACTS.dblDetailQuantity)
			 , dblOrderQuantity					= ISNULL(ISNULL(CPFD.dblQuantityAppliedAndPriced, 0), CONTRACTS.dblDetailQuantity)
			 , dblSubCurrencyRate
			 , intCurrencyExchangeRateTypeId
			 , strCurrencyExchangeRateType
			 , intCurrencyExchangeRateId
