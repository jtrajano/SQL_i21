CREATE VIEW [dbo].[vyuCTEntityContainer]
AS
...
OUTER APPLY (		
	SELECT intEntityId,intContainerTypeId,dblTotalCostPerContainer
	FROM
	(
		SELECT	ROW_NUMBER() OVER (PARTITION BY intContainerTypeId ORDER BY dtmDate) AS intRowNum,EY.intEntityId,FRM.intContainerTypeId,dblTotalCostPerContainer = ISNULL(FRM.dblTotalCostPerContainer,0)
