...
(
	...
	OUTER APPLY 
	(
		SELECT intEntityVendorId,SUM(dblQtyReceived) AS dblQtyReceived FROM tblAPBillDetail BD
	...
	--) Qty
	WHERE  
		(A.[intEntityVendorId] NOT IN (Billed.intEntityVendorId) AND A.dblOrderQty != ISNULL(Billed.dblQtyReceived,0))
	UNION ALL
	SELECT
	DISTINCT  
