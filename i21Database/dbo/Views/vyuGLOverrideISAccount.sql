CREATE VIEW vyuGLOverrideISAccount
AS
WITH q as(
SELECT intAccountId, [Primary Account] ,  [Location],  [LOB], [Company] 
	FROM (
		...
		LEFT JOIN tblGLAccountUnit U on D.intAccountUnitId = U.intAccountUnitId
			OUTER APPLY(
			SELECT top 1 strOverrideISArray From tblGLCompanyPreferenceOption
		)g
	) AS S
	PIVOT
	(
...
CASE WHEN [Location] IS NULL THEN '' ELSE '-'+ CASE  when CHARINDEX('1', O.strOverrideISArray) = 0 THEN   [Location] ELSE REPLICATE ('X' , LEN( [Location] )) END  END +
CASE WHEN [LOB] IS NULL THEN '' ELSE '-'+ CASE  when CHARINDEX('2', O.strOverrideISArray) = 0 THEN   [LOB] ELSE REPLICATE ('X' , LEN( [LOB] )) END  END +
CASE WHEN [Company] IS NULL THEN '' ELSE '-'+ CASE  when CHARINDEX('3', O.strOverrideISArray) = 0 THEN   [Company] ELSE REPLICATE ('X' , LEN( [Company] )) END  END
strOverrideAccount
from q
OUTER APPLY (
	SELECT TOP 1 strOverrideISArray FROM tblGLCompanyPreferenceOption 
