...
SELECT     
	...
	,dblBaseInvoiceTotal			= CASE WHEN (I.strTransactionType  IN ('Invoice','Debit Memo', 'Cash', 'Proforma Invoice')) THEN ROUND(I.dblInvoiceTotal * I.dblCurrencyExchangeRate, dbo.fnARGetDefaultDecimal())
										   WHEN (I.strTransactionType  IN ('Customer Prepayment')) THEN CASE WHEN I.ysnRefundProcessed = 1 THEN ROUND(I.dblInvoiceTotal * I.dblCurrencyExchangeRate, dbo.fnARGetDefaultDecimal()) * -1 ELSE 0 END
										   ELSE ROUND(I.dblInvoiceTotal * I.dblCurrencyExchangeRate, dbo.fnARGetDefaultDecimal()) * -1 END
