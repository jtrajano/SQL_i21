...
SELECT TOP 100 PERCENT CONVERT(INT,ROW_NUMBER() OVER(ORDER BY CONVERT(DATETIME,'01 '+t.strOptionMonth))) as intRowNumber
	, strOptionMonth
	, sum(dblBuy) dblBuy
	, sum(dblSell) dblSell
FROM (
	...
	GROUP BY f.strBuySell
		, fm.intOptionMonthId
		, fm.strOptionMonth
		, f.intInstrumentTypeId
) t group by strOptionMonth
