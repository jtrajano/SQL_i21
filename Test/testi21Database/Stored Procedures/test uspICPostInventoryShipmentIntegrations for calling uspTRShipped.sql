CREATE PROCEDURE [testi21Database].[test uspICPostInventoryShipmentIntegrations for calling uspTRShipped]
AS
BEGIN
	-- Setup the fake data
	BEGIN 
		...
		SELECT	intOrderType = @INT_ORDER_TYPE_SALES_CONTRACT
				,intSourceType = @INT_SOURCE_TYPE_TRANSPORT
				,strShipmentNumber = 'INVSHIP-1XXXX1'

		-- Add a spy for uspTRShipped
		EXEC tSQLt.SpyProcedure 'dbo.uspTRShipped';		
	END
	...
	-- Assert 
	BEGIN 
		EXEC tSQLt.AssertObjectExists 'uspTRShipped_SpyProcedureLog'
		
		DECLARE @expectedCount AS INT = 1 
				,@actualCount AS INT

		SELECT @actualCount = COUNT(*) FROM uspTRShipped_SpyProcedureLog
