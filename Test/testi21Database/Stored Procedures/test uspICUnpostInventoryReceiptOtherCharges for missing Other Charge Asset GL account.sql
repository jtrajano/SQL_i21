CREATE PROCEDURE [testi21Database].[test uspICUnpostInventoryReceiptOtherCharges for missing Other Charge Asset GL account]
AS
...
BEGIN
	...
	-- Assert
	BEGIN 
		EXEC tSQLt.ExpectException @ExpectedMessage = 'Other Charges is missing a GL account setup for Other Charge (Asset) account category.'
	END 
	...
	BEGIN 
		...
		WHERE	intItemId = @OtherCharges
				AND intAccountCategoryId = (SELECT intAccountCategoryId FROM dbo.tblGLAccountCategory WHERE strAccountCategory = 'Other Charge (Asset)')
