CREATE PROCEDURE testi21Database.[test fnGetItemCostingOnUnpostErrors if it blocks negative stock on non-lot items]
AS 
BEGIN
	-- Arrange
	BEGIN 
		...
		SELECT	intItemId = @WetGrains
				...
				,strText = 'Negative stock quantity is not allowed for WET GRAINS in DEFAULT.'
				,intErrorCode = 80003	
	...
	BEGIN 
		INSERT INTO actual	
		SELECT * FROM dbo.fnGetItemCostingOnUnpostErrors(@WetGrains, @WetGrains_DefaultLocation, @WetGrains_BushelUOMId, @SubLocation, @StorageLocation, @dblQty, @LotId)
	END
