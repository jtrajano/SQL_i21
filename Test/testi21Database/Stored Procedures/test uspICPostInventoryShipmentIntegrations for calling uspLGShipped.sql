CREATE PROCEDURE [testi21Database].[test uspICPostInventoryShipmentIntegrations for calling uspLGShipped]
AS
BEGIN
	-- Setup the fake data
	BEGIN 
		...
		SELECT	intOrderType = @INT_ORDER_TYPE_SALES_ORDER
				,intSourceType = @INT_SOURCE_TYPE_INBOUND_SHIPMENT
				,strShipmentNumber = 'INVSHIP-1XXXX1'

		-- Add a spy for uspLGShipped
		EXEC tSQLt.SpyProcedure 'dbo.uspLGShipped';		
	END
	...
	-- Assert 
	BEGIN 
		EXEC tSQLt.AssertObjectExists 'uspLGShipped_SpyProcedureLog'
		
		DECLARE @expectedCount AS INT = 1 
				,@actualCount AS INT

		SELECT @actualCount = COUNT(*) FROM uspLGShipped_SpyProcedureLog
