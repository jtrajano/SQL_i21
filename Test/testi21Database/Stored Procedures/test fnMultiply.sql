...
BEGIN
	...
	BEGIN 
		...
		SET @y = 0.00900000000000000000 
		SET @expected = -0.00565627558260000000 -- -0.00565627558257816300 
		SELECT @result = dbo.fnMultiply(@x, @y)
	...
	BEGIN 
		...
		SET @y = 50.00000000000000000000 
		SET @expected = 28.34949254408350000000 -- 28.34949254408345000000 
		SELECT @result = dbo.fnMultiply(@x, @y)
	...
	BEGIN 
		...
		SET @expected = 12564.36607142857130000000 -- 12564.36607142857127825000 
		SELECT @result = dbo.fnMultiply(@x, @y)
	...
	BEGIN 
		SET @x = 0.01785714285714285714 
		SET @y = 0.01785714285714285714 
		SET @expected = 0.00031887755100000000 --0.00031887755102040816 
		SELECT @result = dbo.fnMultiply(@x, @y)
	...
	BEGIN 
		...
		SET @expected = 11.00000000000000000000 --10.99999999999999978040 
		SELECT @result = dbo.fnMultiply(@x, @y)
	...
	BEGIN 
		...
		SET @y = 55.11560000000000000000 
		SET @expected = 11.00 --11.00000000000000033156 
		SELECT @result = dbo.fnMultiply(@x, @y)
