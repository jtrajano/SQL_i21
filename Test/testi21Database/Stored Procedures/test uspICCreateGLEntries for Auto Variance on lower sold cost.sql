CREATE PROCEDURE [testi21Database].[test uspICCreateGLEntries for Auto Variance on lower sold cost]
AS
BEGIN
	-- Arrange 
	BEGIN 
		...
		DECLARE @Inventory_Auto_Variance_On_Negatively_Sold_Or_Used_Stock AS INT = 35;
		...
		DECLARE @Inventory_AutoVarianceOnNegativelySoldOrUsedStock_Name AS NVARCHAR(200) = 'Inventory Auto Variance on Negatively Sold or Used Stock' 
		...
		SELECT 	intItemId = @WetGrains
				...
				,intTransactionTypeId = @Inventory_Auto_Variance_On_Negatively_Sold_Or_Used_Stock
				,intLotId = NULL 
		...
		SELECT	
			...
			,strTransactionId			= 'SALE-00001'
			,intTransactionId			= 1
			,strTransactionType			= @Inventory_AutoVarianceOnNegativelySoldOrUsedStock_Name
			,strTransactionForm			= 'Inventory Shipment'
	...
	-- Assert
	BEGIN 
		EXEC tSQLt.AssertEqualsTable 'expected', 'actual', 'Failed to generated the expected GL Detail records.';
	END
