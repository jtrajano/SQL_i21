CREATE PROCEDURE [testi21Database].[test uspICPostInventoryReceiptIntegrations for calling uspLGReceived]
AS
BEGIN
	-- Setup the fake data
	BEGIN 
		...
		SELECT	strReceiptType = @RECEIPT_TYPE_PURCHASE_CONTRACT
				,intSourceType = @SOURCE_TYPE_INBOUND_SHIPMENT
				,strReceiptNumber = 'INVRCT-1XXXX1'

		-- Add a spy for uspLGReceived
		EXEC tSQLt.SpyProcedure 'dbo.uspLGReceived';		
	END
	...
	-- Assert 
	BEGIN 
		EXEC tSQLt.AssertObjectExists 'uspLGReceived_SpyProcedureLog'
		
		DECLARE @expectedCount AS INT = 1 
				,@actualCount AS INT

		SELECT @actualCount = COUNT(*) FROM uspLGReceived_SpyProcedureLog
