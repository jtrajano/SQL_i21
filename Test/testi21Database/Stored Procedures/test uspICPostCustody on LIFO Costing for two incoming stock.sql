CREATE PROCEDURE [testi21Database].[test uspICPostCustody on LIFO Costing for two incoming stock]
AS
...
BEGIN
	...
	BEGIN 
		...
		SELECT intItemId, intItemLocationId, intItemUOMId, dtmDate, dblStockIn, dblStockOut, dblCost INTO expected FROM dbo.tblICInventoryLIFOInCustody WHERE 1 = 0		
		SELECT intItemId, intItemLocationId, intItemUOMId, dtmDate, dblStockIn, dblStockOut, dblCost INTO actual FROM dbo.tblICInventoryLIFOInCustody WHERE 1 = 0
	...
	BEGIN 
		...
		WHERE	intItemId = @WetGrains
				AND intItemLocationId = @WetGrains_BetterHaven
