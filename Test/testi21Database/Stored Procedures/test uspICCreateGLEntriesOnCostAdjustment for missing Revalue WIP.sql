CREATE PROCEDURE [testi21Database].[test uspICCreateGLEntriesOnCostAdjustment for missing Revalue WIP]
AS
BEGIN
	-- Arrange 
	BEGIN 
	...
	DECLARE @INV_TRANS_TYPE_Auto_Negative AS INT = 1
		...
		-- Delete Revalue WIP from the G/L account setup to simulate a missing account id
		DELETE	ItemAccount
		...
		WHERE	GLAccountCategory.strAccountCategory = @AccountCategory_Revalue_WIP
	END 
