CREATE PROCEDURE [testi21Database].[test uspICIncreaseStockInFIFOCustody from zero to positive]
AS
BEGIN
	-- Arrange 
	BEGIN 
		...
		-- Create the variables used by uspICIncreaseStockInFIFOCustody
	...
	BEGIN 
		EXEC dbo.uspICIncreaseStockInFIFOCustody
			...
			,@strTransactionId 
			,@intTransactionId 
			,@NewFifoInCustodyId OUTPUT 
	END 

	-- Assert
	BEGIN 
		-- Assert that it created the correct fifo id in custody id. 
		EXEC tSQLt.AssertEquals 1, @NewFifoInCustodyId;
