CREATE PROCEDURE [testi21Database].[test uspICPostInventoryAdjustmentLotMove for validating the company location]
AS
BEGIN
	-- Item Ids
	...
	BEGIN 
		...
		WHERE	intAdjustmentType = @ADJUSTMENT_TYPE_SplitLot
				AND intInventoryAdjustmentId = @intTransactionId

		-- Give the detail an invalid company location id 
		UPDATE	dbo.tblICInventoryAdjustmentDetail
		SET		intNewLocationId = -1
	...
	-- Assert 
	BEGIN 
		EXEC tSQLt.ExpectException @ExpectedMessage = 'The new Item Location is invalid or missing for MANUAL LOT GRAINS.'			 
	END 	
