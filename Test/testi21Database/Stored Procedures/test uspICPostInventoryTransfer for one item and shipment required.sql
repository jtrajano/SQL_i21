CREATE PROCEDURE [testi21Database].[test uspICPostInventoryTransfer for one item and shipment required]
AS
BEGIN
	...
	BEGIN 
		...
		SELECT @ysnPosted = ysnPosted FROM dbo.tblICInventoryTransfer WHERE strTransferNo = 'INVTRN-3'
		...
		SELECT 
			...
			,strTransactionId	= 'INVTRN-3'
			,dblDebit			= 0
			,dblCredit			= ROUND((75 * @25KgBagUnitQty * ROUND(2.50 / @25KgBagUnitQty, 6) ), 2)
		UNION ALL 
		SELECT 
			intAccountId		= @InventoryInTransit_Default
			,strBatchId			= 'BATCH-1'
			,strTransactionId	= 'INVTRN-3'
			,dblDebit			= ROUND((75 * @25KgBagUnitQty * ROUND(2.50 / @25KgBagUnitQty, 6)), 2) 
			,dblCredit			= 0
		...
		WHERE	strTransactionId = 'INVTRN-3'
