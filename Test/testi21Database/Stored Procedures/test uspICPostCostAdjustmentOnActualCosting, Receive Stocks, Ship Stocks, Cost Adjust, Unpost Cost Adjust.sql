CREATE PROCEDURE [testi21Database].[test uspICPostCostAdjustmentOnActualCosting, Receive Stocks, Ship Stocks, Cost Adjust, Unpost Cost Adjust]
AS
BEGIN
	...
	BEGIN 
		...
		INSERT INTO actualInventoryActualCostAdjustmentLog (
	...
	BEGIN 
		...
		SELECT	[intInventoryTransactionId] = 8
				...
				,[strTransactionId]			= 'BILL-10001'
				,[strBatchId]				= 'BATCH-10293'
				,[intTransactionTypeId]		= @RevalueSold
				,[intLotId]					= NULL 
				,[intCostingMethod]			= @ACTUALCOST
				,[ysnIsUnposted]			= 1

		-- Unpost Cost Adustment
		UNION ALL 
		...
		INSERT INTO expectedInventoryActualCostAdjustmentLog (
				[intInventoryActualCostId]
				,[intInventoryCostAdjustmentTypeId]
				,[dblQty] 
				,[dblCost]
	...
	BEGIN
		...
		EXEC tSQLt.AssertEqualsTable 'expected', 'actual', 'Failed to meet expected data on tblICInventoryTransaction';
		
		-- Assert the expected data for tblICInventoryActualCostAdjustmentLog is built correctly. 
		EXEC tSQLt.AssertEqualsTable 'expectedInventoryActualCostAdjustmentLog', 'actualInventoryActualCostAdjustmentLog', 'Failed to meet expected data on tblICInventoryActualCostAdjustmentLog'
	END 
