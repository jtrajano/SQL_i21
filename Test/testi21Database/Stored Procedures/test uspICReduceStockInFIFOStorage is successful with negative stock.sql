CREATE PROCEDURE [testi21Database].[test uspICReduceStockInFIFOStorage is successful with negative stock]
AS
BEGIN
	-- Arrange 
	BEGIN 
	...
	BEGIN 
		...
		BEGIN
			...
			BEGIN 
				...
				EXEC tSQLt.AssertEquals -550, @RemainingQty, 'Failed on remaining Qty.'; 

				-- the cost used is 10.00 
				EXEC tSQLt.AssertEquals 10.00, @CostUsed, 'Failed on Cost used.'; 

				-- qty offset is -100 
				EXEC tSQLt.AssertEquals -100, @QtyOffset, 'Failed on Qty Offset.'; 
			...
			BEGIN 
				...
				EXEC tSQLt.AssertEquals -450, @RemainingQty, 'Failed on remaining Qty.'; 

				-- the cost used is 11.00
				EXEC tSQLt.AssertEquals 11.00, @CostUsed, 'Failed on cost used.'; 
			...
			BEGIN 
				...
				EXEC tSQLt.AssertEquals -350, @RemainingQty, 'Failed on remaining Qty.'; 

				-- the cost used is 12.00
				EXEC tSQLt.AssertEquals 12.00, @CostUsed, 'Failed on cost used.'; 
			...
			BEGIN 
				...
				EXEC tSQLt.AssertEquals -250, @RemainingQty, 'Failed on remaining Qty.'; 

				-- the cost used is 13.00
				EXEC tSQLt.AssertEquals 13.00, @CostUsed, 'Failed on cost used.'; 
			...
			BEGIN 
				...
				EXEC tSQLt.AssertEquals -150, @RemainingQty, 'Failed on remaining Qty.'; 

				-- the cost used is 14.00
				EXEC tSQLt.AssertEquals 14.00, @CostUsed, 'Failed on cost used.'; 
			...
			BEGIN 
				-- remaining qty is -50
				EXEC tSQLt.AssertEquals -50, @RemainingQty, 'Failed on remaining Qty.'; 

				-- the cost used is 15.00
				EXEC tSQLt.AssertEquals 15.00, @CostUsed, 'Failed on cost used.'; 
			...
			-- Assert on 7th pass, the cost used is 9.50 and remaining qty is 0
			IF @intIterationCounter = 7
			BEGIN 
				-- remaining qty is NULL
				EXEC tSQLt.AssertEquals NULL, @RemainingQty, 'Failed on remaining Qty.'; 

				-- the cost used is 9.50
				EXEC tSQLt.AssertEquals NULL, @CostUsed, 'Failed on cost used.'; 

				-- qty offset is -50
				EXEC tSQLt.AssertEquals -50, @QtyOffset, 'Failed on Qty Offset.'; 
			END
	...
	BEGIN 
		...
		new 	1           1                 2014-01-15 00:00:00.000   0.000000                               50.000000                               9.500000                               1                1
				***************************************************************************************************************************************************************************************************************/
