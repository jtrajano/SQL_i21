CREATE PROCEDURE [testi21Database].[test uspICPostInventoryReceiptOtherCharges for ysnPrice scenario 1]
AS
...
BEGIN
	...
	-- Assert
	BEGIN 
		-- The %s is both a payable and deductible to the bill of the same vendor. Please correct the Accrue or Price checkbox.
		EXEC tSQLt.ExpectException @ExpectedMessage = 'The Other Charges is both a payable and deductible to the bill of the same vendor. Please correct the Accrue or Price checkbox.'
	END 
	...
	BEGIN 
		...
		SET		ysnAccrue = 1
				...
				,strAllocateCostBy = @ALLOCATE_COST_BY_Stock_Unit
