CREATE PROCEDURE [testi21Database].[test uspICCreateGLEntriesOnCostAdjustment for missing Inventory]
AS
BEGIN
	-- Arrange 
	BEGIN 
	...
	DECLARE @INV_TRANS_TYPE_Auto_Negative AS INT = 1
		...
		-- Delete Inventory category from the Item > G/L account setup to simulate a missing account id
		DELETE	ItemAccount
		...
		WHERE	GLAccountCategory.strAccountCategory = @AccountCategory_Inventory
	END 
	...
	BEGIN 
		...
		EXEC tSQLt.ExpectException 
			@ExpectedMessagePattern = 'STICKY GRAINS is missing a GL account setup for Inventory account category.'
