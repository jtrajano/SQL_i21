CREATE TABLE [dbo].[tblAPVendors] (
    ...
    CONSTRAINT [PK_dbo.tblAPVendors] PRIMARY KEY CLUSTERED ([intEntityId] ASC),
    CONSTRAINT [FK_dbo.tblAPVendors_dbo.tblEntities_intEntityId] FOREIGN KEY ([intEntityId]) REFERENCES [dbo].[tblEntities] ([intEntityId]),
    CONSTRAINT [APVendorId_Unique] UNIQUE NONCLUSTERED ([strVendorId] ASC)
);


GO
ALTER TABLE [dbo].[tblAPVendors] NOCHECK CONSTRAINT [FK_dbo.tblAPVendors_dbo.tblEntities_intEntityId];
...
CREATE NONCLUSTERED INDEX [IX_intEntityId]
    ON [dbo].[tblAPVendors]([intEntityId] ASC);
